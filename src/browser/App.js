...
import { buildCoreSources } from "./utils/commands/sources";
import { THINKING_LEVELS } from "@/common/types/thinking";
import { CUSTOM_EVENTS } from "@/common/constants/events";
import { isWorkspaceForkSwitchEvent } from "./utils/workspaceEvents";
import { getAgentIdKey, getAgentsInitNudgeKey, getModelKey, getNotifyOnResponseKey, getThinkingLevelByModelKey, getThinkingLevelKey, getWorkspaceAISettingsByAgentKey, EXPANDED_PROJECTS_KEY, LEFT_SIDEBAR_COLLAPSED_KEY, LEFT_SIDEBAR_WIDTH_KEY, } from "@/common/constants/storage";
...
function AppInner() {
    ...
    const { workspaceMetadata, loading, setWorkspaceMetadata, removeWorkspace, renameWorkspace, refreshWorkspaceMetadata, selectedWorkspace, setSelectedWorkspace, pendingNewWorkspaceProject, pendingNewWorkspaceSectionId, pendingNewWorkspaceDraftId, beginWorkspaceCreation, } = useWorkspaceContext();
    ...
    const { projects, refreshProjects, removeProject, openProjectCreateModal, isProjectCreateModalOpen, closeProjectCreateModal, addProject, } = useProjectContext();
    // Auto-collapse sidebar on mobile by default
    const isMobile = typeof window !== "undefined" && window.innerWidth <= 768;
    const [sidebarCollapsed, setSidebarCollapsed] = usePersistedState(LEFT_SIDEBAR_COLLAPSED_KEY, isMobile, {
        listener: true,
    });
    ...
    const creationProjectPath = !selectedWorkspace && !currentWorkspaceId
    ...
    useEffect(() => {
        if (selectedWorkspace) {
            ...
            if (!metadata) {
                ...
                console.warn(`Workspace ${selectedWorkspace.workspaceId} no longer exists, clearing selection`);
    ...
    const sortedWorkspacesByProject = useStableReference(() => buildSortedWorkspacesByProject(projects, workspaceMetadata, workspaceRecency), (prev, next) => compareMaps(prev, next, (a, b) => {
    ...
    const handleNavigateWorkspace = useCallback((direction) => {
        ...
        if (targetMeta)
            setSelectedWorkspace(toWorkspaceSelection(targetMeta));
    }, [selectedWorkspace, workspaceMetadata, setSelectedWorkspace]);
    // Register command sources with registry
    const { registerSource, isOpen: isCommandPaletteOpen, open: openCommandPalette, close: closeCommandPalette, } = useCommandRegistry();
    /**
     * Get model for a workspace, returning canonical format.
     */
    const getModelForWorkspace = useCallback((workspaceId) => {
    ...
    const getThinkingLevelForWorkspace = useCallback((workspaceId) => {
        if (!workspaceId) {
            return "off";
        }
        const scopedKey = getThinkingLevelKey(workspaceId);
        const scoped = readPersistedState(scopedKey, undefined);
        if (scoped !== undefined) {
        ...
        const legacy = readPersistedState(getThinkingLevelByModelKey(model), undefined);
    ...
    const setThinkingLevelFromPalette = useCallback((workspaceId, level) => {
        if (!workspaceId) {
            return;
        }
        ...
        const normalizedAgentId = readPersistedState(getAgentIdKey(workspaceId), "exec").trim().toLowerCase() ||
            "exec";
        ...
        if (typeof window !== "undefined") {
            window.dispatchEvent(new CustomEvent(CUSTOM_EVENTS.THINKING_LEVEL_TOAST, {
                detail: { workspaceId, level: normalized },
            }));
        }
    }, [api, getModelForWorkspace]);
    const registerParamsRef = useRef(null);
    const openNewWorkspaceFromPalette = useCallback((projectPath) => {
        startWorkspaceCreation(projectPath);
    }, [startWorkspaceCreation]);
    const archiveMergedWorkspacesInProjectFromPalette = useCallback(async (projectPath) => {
        ...
        try {
            ...
            if (errorCount > 0) {
                ...
                if (typeof window !== "undefined") {
                    window.alert(`Archived merged workspaces with some errors.\n\nArchived: ${archivedCount}\nSkipped: ${skippedCount}\nErrors: ${errorCount}\n\nErrors:\n${shownErrors}${remainingSuffix}`);
                }
            }
        }
        catch (error) {
    ...
    const getBranchesForProject = useCallback(async (projectPath) => {
        ...
        const sanitizedBranches = branchResult.branches.filter((branch) => typeof branch === "string");
        const recommended = branchResult.recommendedTrunk && sanitizedBranches.includes(branchResult.recommendedTrunk)
    ...
    const selectWorkspaceFromPalette = useCallback((selection) => {
        setSelectedWorkspace(selection);
    }, [setSelectedWorkspace]);
    const removeWorkspaceFromPalette = useCallback(async (workspaceId) => removeWorkspace(workspaceId), [removeWorkspace]);
    const renameWorkspaceFromPalette = useCallback(async (workspaceId, newName) => renameWorkspace(workspaceId, newName), [renameWorkspace]);
    const addProjectFromPalette = useCallback(() => {
        openProjectCreateModal();
    }, [openProjectCreateModal]);
    const removeProjectFromPalette = useCallback((path) => {
        void handleRemoveProject(path);
    }, [handleRemoveProject]);
    const toggleSidebarFromPalette = useCallback(() => {
        setSidebarCollapsed((prev) => !prev);
    }, [setSidebarCollapsed]);
    const navigateWorkspaceFromPalette = useCallback((dir) => {
    ...
    registerParamsRef.current = {
        ...
        onClearTimingStats: (workspaceId) => workspaceStore.clearTimingStats(workspaceId),
        api,
    };
    useEffect(() => {
    ...
    const handleMouseNavigation = useCallback((e) => {
        if (e.button === 3) {
            e.preventDefault();
            void navigate(-1);
        }
    ...
    useEffect(() => {
        const handleForkSwitch = (e) => {
            if (!isWorkspaceForkSwitchEvent(e))
                return;
            ...
            if (!project) {
                console.warn(`[Frontend] Project not found for forked workspace path: ${workspaceInfo.projectPath} (will refresh)`);
                void refreshProjects();
            }
            // DEFENSIVE: Ensure createdAt exists
            if (!workspaceInfo.createdAt) {
                console.warn(`[Frontend] Workspace ${workspaceInfo.id} missing createdAt in fork switch - using default (2025-01-01)`);
    ...
    useEffect(() => {
        ...
        const handleResponseComplete = (workspaceId, _messageId, isFinal, finalText, compaction) => {
            ...
            if (compaction?.hasContinueMessage)
                return;
            // Skip notification if workspace is focused (like Slack behavior)
            const isWorkspaceFocused = document.hasFocus() && selectedWorkspaceRef.current?.workspaceId === workspaceId;
    ...
    if (status === "auth_required") {
        return _jsx(AuthTokenModal, { isOpen: true, onSubmit: authenticate, error: error });
    }
    return (_jsx(_Fragment, { children: _jsxs("div", { className: "bg-bg-dark mobile-layout flex h-full overflow-hidden pt-[env(safe-area-inset-top)] pr-[env(safe-area-inset-right)] pb-[min(env(safe-area-inset-bottom,0px),40px)] pl-[env(safe-area-inset-left)]", onMouseUp: handleMouseNavigation, children: [_jsx(LeftSidebar, { collapsed: sidebarCollapsed, onToggleCollapsed: handleToggleSidebar, widthPx: leftSidebar.width, isResizing: leftSidebar.isResizing, onStartResize: leftSidebar.startResize, sortedWorkspacesByProject: sortedWorkspacesByProject, workspaceRecency: workspaceRecency, muxChatProjectPath: muxChatProjectPath }), _jsxs("div", { className: "mobile-main-content flex min-w-0 flex-1 flex-col overflow-hidden", children: [_jsx(WindowsToolchainBanner, {}), _jsx(RosettaBanner, {}), _jsx("div", { className: "mobile-layout flex flex-1 overflow-hidden", children: currentSettingsSection ? (_jsx(SettingsPage, { leftSidebarCollapsed: sidebarCollapsed, onToggleLeftSidebarCollapsed: handleToggleSidebar })) : selectedWorkspace ? ((() => {
                                ...
                                const workspaceName = currentMetadata.name ??
                                ...
                                const workspacePath = currentMetadata.namedWorkspacePath ?? selectedWorkspace.namedWorkspacePath ?? "";
                                return (_jsx(ErrorBoundary, { workspaceInfo: `${selectedWorkspace.projectName}/${workspaceName}`, children: _jsx(AIView, { workspaceId: selectedWorkspace.workspaceId, projectPath: selectedWorkspace.projectPath, projectName: selectedWorkspace.projectName, leftSidebarCollapsed: sidebarCollapsed, onToggleLeftSidebarCollapsed: handleToggleSidebar, workspaceName: workspaceName, namedWorkspacePath: workspacePath, runtimeConfig: currentMetadata.runtimeConfig, incompatibleRuntime: currentMetadata.incompatibleRuntime, isInitializing: currentMetadata.isInitializing === true }) }));
                            })()) : creationProjectPath ? ((() => {
                                const projectPath = creationProjectPath;
                                const projectName = projectPath.split("/").pop() ?? projectPath.split("\\").pop() ?? "Project";
                                return (_jsx(ProjectPage, { projectPath: projectPath, projectName: projectName, leftSidebarCollapsed: sidebarCollapsed, onToggleLeftSidebarCollapsed: handleToggleSidebar, pendingSectionId: pendingNewWorkspaceSectionId, pendingDraftId: pendingNewWorkspaceDraftId, onWorkspaceCreated: (metadata, options) => {
                            ...
                            })()) : (_jsxs("div", { className: "bg-dark flex flex-1 flex-col overflow-hidden", children: [_jsx("div", { className: cn("bg-sidebar border-border-light flex shrink-0 items-center border-b px-[15px] [@media(max-width:768px)]:h-auto [@media(max-width:768px)]:py-2", isDesktopMode() ? "h-10 titlebar-drag" : "h-8"), children: sidebarCollapsed && (_jsx(Button, { variant: "ghost", size: "icon", onClick: handleToggleSidebar, title: "Open sidebar", "aria-label": "Open sidebar menu", className: cn("mobile-menu-btn text-muted hover:text-foreground hidden h-6 w-6 shrink-0", isDesktopMode() && "titlebar-no-drag"), children: _jsx(Menu, { className: "h-4 w-4" }) })) }), _jsxs("div", { className: "[&_p]:text-muted [&_h2]:text-foreground mx-auto w-full max-w-3xl flex-1 text-center [&_h2]:mb-4 [&_h2]:font-bold [&_h2]:tracking-tight [&_p]:leading-[1.6]", style: {
                                            padding: "clamp(40px, 10vh, 100px) 20px",
                                            fontSize: "clamp(14px, 2vw, 16px)",
                                        }, children: [_jsx("h2", { style: { fontSize: "clamp(24px, 5vw, 36px)", letterSpacing: "-1px" }, children: currentWorkspaceId ? "Opening workspace…" : "Welcome to Mux" }), _jsx("p", { children: currentWorkspaceId
                                                    ? loading
                                                        ? "Loading workspace metadata…"
                                                        : "Workspace not found."
                                                    : "Add a project from the sidebar to get started." })] })] })) })] }), _jsx(CommandPalette, { getSlashContext: () => ({ workspaceId: selectedWorkspace?.workspaceId }) }), _jsx(ProjectCreateModal, { isOpen: isProjectCreateModalOpen, onClose: closeProjectCreateModal, onSuccess: (normalizedPath, projectConfig) => {
                        ...
                        updatePersistedState(EXPANDED_PROJECTS_KEY, (prev) => [...(Array.isArray(prev) ? prev : []), normalizedPath], []);
                        beginWorkspaceCreation(normalizedPath);
                    } }), _jsx(AboutDialog, {}), _jsx(MuxGatewaySessionExpiredDialog, {})] }) }));
}
function App() {
    return (_jsx(ExperimentsProvider, { children: _jsx(FeatureFlagsProvider, { children: _jsx(UILayoutsProvider, { children: _jsx(TooltipProvider, { delayDuration: 200, children: _jsx(SettingsProvider, { children: _jsx(AboutDialogProvider, { children: _jsx(ProviderOptionsProvider, { children: _jsx(SplashScreenProvider, { children: _jsx(TutorialProvider, { children: _jsx(CommandRegistryProvider, { children: _jsx(PowerModeProvider, { children: _jsx(AppInner, {}) }) }) }) }) }) }) }) }) }) }) }));
}
export default App;
