...
export function AppLoader(props) {
    return (_jsx(ThemeProvider, { children: _jsx(APIProvider, { client: props.client, children: _jsx(PolicyProvider, { children: _jsx(RouterProvider, { children: _jsx(ProjectProvider, { children: _jsx(WorkspaceProvider, { children: _jsx(AppLoaderInner, {}) }) }) }) }) }) }));
}
/**
...
function AppLoaderInner() {
    ...
    useEffect(() => {
        ...
        if (!workspaceContext.loading) {
            ...
            gitStatusStore.subscribeToFileModifications((listener) => workspaceStore.subscribeFileModifyingTool(listener));
    ...
    if (policyState.status.state === "blocked") {
        return _jsx(PolicyBlockedScreen, { reason: policyState.status.reason });
    }
    ...
    if (apiState.status === "auth_required") {
        return _jsx(AuthTokenModal, { isOpen: true, onSubmit: apiState.authenticate, error: apiState.error });
    }
    ...
    if (!initialLoadComplete) {
        if (apiState.status === "error") {
            return _jsx(StartupConnectionError, { error: apiState.error, onRetry: apiState.retry });
        }
        const statusText = apiState.status === "reconnecting"
            ? `Reconnecting to backend (attempt ${apiState.attempt})...`
            : "Loading workspaces...";
        return _jsx(LoadingScreen, { statusText: statusText });
    }
    // Render App - all state available via contexts
    return (_jsx(TelemetryEnabledProvider, { children: _jsx(TerminalRouterProvider, { children: _jsx(App, {}) }) }));
}
