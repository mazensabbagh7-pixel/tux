...
import { estimatePersistedChatAttachmentsChars, parsePersistedChatAttachments, } from "./draftAttachmentsStorage";
describe("draftAttachmentsStorage", () => {
    ...
    test("parsePersistedChatAttachments returns [] for invalid array items", () => {
        expect(parsePersistedChatAttachments([{}])).toEqual([]);
        expect(parsePersistedChatAttachments([{ id: "img", url: 123, mediaType: "image/png" }])).toEqual([]);
    });
    ...
    test("estimatePersistedChatAttachmentsChars matches JSON length", () => {
        ...
        expect(estimatePersistedChatAttachmentsChars(attachments)).toBe(JSON.stringify(attachments).length);
    });
});
//# sourceMappingURL=draftAttachmentsStorage.test.js.map