...
import { CommandSuggestions, COMMAND_SUGGESTION_KEYS, FILE_SUGGESTION_KEYS, } from "../CommandSuggestions";
...
import { readPersistedState, usePersistedState, updatePersistedState, } from "@/browser/hooks/usePersistedState";
import { useSettings } from "@/browser/contexts/SettingsContext";
import { useWorkspaceContext } from "@/browser/contexts/WorkspaceContext";
import { useProjectContext } from "@/browser/contexts/ProjectContext";
import { useAgent } from "@/browser/contexts/AgentContext";
import { ThinkingSliderComponent } from "../ThinkingSlider";
import { getAllowedRuntimeModesForUi, isParsedRuntimeAllowedByPolicy, } from "@/browser/utils/policyUi";
...
import { getModelKey, getThinkingLevelKey, getWorkspaceAISettingsByAgentKey, getInputKey, getInputAttachmentsKey, AGENT_AI_DEFAULTS_KEY, VIM_ENABLED_KEY, getProjectScopeId, getPendingScopeId, getDraftScopeId, getPendingWorkspaceSendErrorKey, getWorkspaceLastReadKey, } from "@/common/constants/storage";
import { executeCompaction, prepareCompactionMessage, processSlashCommand, } from "@/browser/utils/chatCommands";
...
import { extractAttachmentsFromClipboard, extractAttachmentsFromDrop, chatAttachmentsToFileParts, processAttachmentFiles, } from "@/browser/utils/attachmentsHandling";
...
import { estimatePersistedChatAttachmentsChars, readPersistedChatAttachments, } from "./draftAttachmentsStorage";
import { RecordingOverlay } from "./RecordingOverlay";
import { AttachedReviewsPanel } from "./AttachedReviewsPanel";
import { buildSkillInvocationMetadata, parseCommandWithSkillInvocation, validateCreationRuntime, filePartsToChatAttachments, } from "./utils";
...
const MAX_PERSISTED_ATTACHMENT_DRAFT_CHARS = 4000000;
const ChatInputInner = (props) => {
    const { api } = useAPI();
    const policyState = usePolicy();
    const effectivePolicy = policyState.status.state === "enforced" ? (policyState.policy ?? null) : null;
    const runtimePolicy = useMemo(() => getAllowedRuntimeModesForUi(effectivePolicy), [effectivePolicy]);
    const { variant } = props;
    ...
    const hasQueuedCompaction = variant === "workspace" ? (props.hasQueuedCompaction ?? false) : false;
    const [isMobileTouch, setIsMobileTouch] = useState(() => typeof window !== "undefined" &&
    ...
    const storageKeys = (() => {
        if (variant === "creation") {
            const pendingScopeId = typeof props.pendingDraftId === "string" && props.pendingDraftId.trim().length > 0
    ...
    const [atMentionSuggestions, setAtMentionSuggestions] = useState([]);
    const agentSkillsRequestIdRef = useRef(0);
    const atMentionDebounceRef = useRef(null);
    const atMentionRequestIdRef = useRef(0);
    const lastAtMentionScopeIdRef = useRef(null);
    const lastAtMentionQueryRef = useRef(null);
    const lastAtMentionInputRef = useRef(input);
    const [showCommandSuggestions, setShowCommandSuggestions] = useState(false);
    const [commandSuggestions, setCommandSuggestions] = useState([]);
    const [agentSkillDescriptors, setAgentSkillDescriptors] = useState([]);
    const [toast, setToast] = useState(null);
    // State for destructive command confirmation modal
    const [pendingDestructiveCommand, setPendingDestructiveCommand] = useState(null);
    ...
    const pendingErrorKey = variant === "workspace" && workspaceId ? getPendingWorkspaceSendErrorKey(workspaceId) : null;
    const [pendingError, setPendingError] = usePersistedState(pendingErrorKey ?? "__unused__", null, { listener: true });
    useEffect(() => {
        if (!pendingErrorKey || !pendingError)
            return;
    ...
    const attachmentDraftTooLargeToastKeyRef = useRef(null);
    ...
    const [draftReviews, setDraftReviews] = useState(null);
    const persistAttachments = useCallback((nextAttachments) => {
        if (nextAttachments.length === 0) {
            attachmentDraftTooLargeToastKeyRef.current = null;
            updatePersistedState(storageKeys.attachmentsKey, undefined);
            return;
        }
        ...
        if (estimatedChars > MAX_PERSISTED_ATTACHMENT_DRAFT_CHARS) {
            ...
            if (attachmentDraftTooLargeToastKeyRef.current !== storageKeys.attachmentsKey) {
                ...
                pushToast({
                    type: "error",
                    message: "This draft attachment is too large to save. It will be lost when you switch workspaces or restart.",
                    duration: 5000,
                });
            }
            return;
        ...
        attachmentDraftTooLargeToastKeyRef.current = null;
        updatePersistedState(storageKeys.attachmentsKey, nextAttachments);
    ...
    const setAttachments = useCallback((value) => {
    ...
    const lastAtMentionCursorRef = useRef(null);
    ...
    const handleInputChange = useCallback((next, caretFromEvent) => {
    ...
    const preEditDraftRef = useRef({ text: "", attachments: [] });
    const preEditReviewsRef = useRef(null);
    const { open } = useSettings();
    const { selectedWorkspace, beginWorkspaceCreation, updateWorkspaceDraftSection } = useWorkspaceContext();
    ...
    const { models, hiddenModelsForSelector, ensureModelInSettings, defaultModel, setDefaultModel, codexOauthSet, } = useModelsFromSettings();
    const [agentAiDefaults] = usePersistedState(AGENT_AI_DEFAULTS_KEY, {}, {
        listener: true,
    });
    const atMentionListId = useId();
    const commandListId = useId();
    const telemetry = useTelemetry();
    const [vimEnabled, setVimEnabled] = usePersistedState(VIM_ENABLED_KEY, false, {
        listener: true,
    });
    ...
    const sendMessageOptions = useSendMessageOptions(variant === "workspace" ? props.workspaceId : getProjectScopeId(props.projectPath));
    ...
    const { threshold: autoCompactThreshold, setThreshold: setAutoCompactThreshold } = useAutoCompactionSettings(workspaceIdForUsage, contextDisplayModel);
    const autoCompactionProps = useMemo(() => ({ threshold: autoCompactThreshold, setThreshold: setAutoCompactThreshold }), [autoCompactThreshold, setAutoCompactThreshold]);
    ...
    const setPreferredModel = useCallback((model) => {
        ...
        if (onModelChange) {
            ...
            const scopeId = variant === "creation" ? getProjectScopeId(creationProjectPath) : workspaceId;
            if (scopeId) {
    ...
    useEffect(() => {
        ...
        const stillExists = (creationProject.sections ?? []).some((section) => section.id === selectedSectionId);
        if (!stillExists) {
            setSelectedSectionId(null);
        }
    }, [creationProject, selectedSectionId, variant]);
    const handleCreationSectionChange = useCallback((sectionId) => {
        ...
        beginWorkspaceCreation(creationProjectPath, typeof sectionId === "string" && sectionId.trim().length > 0 ? sectionId : undefined);
    }, [
    ...
    const creationState = useCreationWorkspace(variant === "creation"
        ? {
    ...
    const sendInFlightBlocksInput = variant === "workspace" ? isSendInFlight && !isStreamStarting : isSendInFlight;
    ...
    const creationRuntimeError = variant === "creation"
        ? validateCreationRuntime(creationState.selectedRuntime, coderState.presets.length)
        : null;
    const creationRuntimePolicyError = variant === "creation" &&
        ...
        : null;
    const runtimeFieldError = variant === "creation" && hasAttemptedCreateSend ? (creationRuntimeError?.mode ?? null) : null;
    const creationControlsProps = variant === "creation"
        ? {
            ...
            coderProps: coderState.coderInfo === null ||
                coderState.enabled ||
    ...
    const coderPresetsLoading = coderState.enabled && !coderState.coderConfig?.existingWorkspace && coderState.loadingPresets;
    const canSend = (hasTypedText || hasImages || hasReviews) &&
        !disabled &&
    ...
    const prevCreationAgentIdRef = useRef(null);
    const prevCreationScopeIdRef = useRef(null);
    ...
    useEffect(() => {
        ...
        const normalizedAgentId = typeof agentId === "string" && agentId.trim().length > 0
            ? agentId.trim().toLowerCase()
            : "exec";
        const isExplicitAgentSwitch = prevCreationAgentIdRef.current !== null &&
        ...
        const resolvedModel = typeof candidateModel === "string" && candidateModel.trim().length > 0
            ? candidateModel
            : fallbackModel;
        const existingThinking = readPersistedState(thinkingKey, "off");
        const candidateThinking = agentAiDefaults[normalizedAgentId]?.thinkingLevel ?? existingThinking ?? "off";
    ...
    const setChatInputAutoFocusState = useCallback((state) => {
    ...
    const applyDraftFromPending = useCallback((pending, attachmentKeyPrefix) => {
        setDraft({
    ...
    const restoreDraft = useCallback((pending) => {
    ...
    const restoreText = useCallback((text) => {
        setInput(() => text);
        focusMessageInput();
    ...
    const prependText = useCallback((text) => {
        setInput((prev) => text + prev);
        focusMessageInput();
    }, [focusMessageInput, setInput]);
    const handleSendRef = useRef(() => Promise.resolve());
    ...
    useEffect(() => {
        ...
        const loadAgentSkills = async () => {
            ...
            const discoveryInput = variant === "workspace" && workspaceId
                ? {
                    workspaceId,
    ...
    const commandProjectPath = variant === "creation" ? props.projectPath : (selectedWorkspace?.projectPath ?? null);
    ...
    const executeParsedCommand = async (parsed, restoreInput, options) => {
        if (!parsed) {
            return false;
        }
        ...
        const commandContext = {
            ...
            setInput,
            setAttachments,
            setSendingState: (increment) => setSendingCount((c) => c + (increment ? 1 : -1)),
            setToast,
            setPreferredModel,
            setVimEnabled,
    ...
    const handleDestructiveCommandConfirm = async () => {
        if (!pendingDestructiveCommand || variant !== "workspace")
            return;
        const parsedCommand = pendingDestructiveCommand.type === "clear"
            ? { type: "clear" }
            : {
                type: "truncate",
    ...
    const handleAtMentionSelect = useCallback((suggestion) => {
        const cursor = inputRef.current?.selectionStart ?? input.length;
        const match = findAtMentionAtCursor(input, cursor);
        if (!match) {
            return;
        ...
        const next = input.slice(0, match.startIndex) +
    ...
    const handleCommandSelect = useCallback((suggestion) => {
    ...
    const handleSend = async () => {
        ...
        const skillDiscovery = variant === "creation"
            ? atMentionProjectPath
        ...
        if (variant === "creation") {
            ...
            const runtimeError = validateCreationRuntime(creationState.selectedRuntime, coderState.presets.length);
            if (runtimeError) {
                return;
            }
            // Creation variant: simple message send + workspace creation
            const creationFileParts = chatAttachmentsToFileParts(attachments);
            const creationResult = await creationState.handleSend(creationMessageTextForSend, creationFileParts.length > 0 ? creationFileParts : undefined, creationOptionsOverride);
        ...
        try {
            ...
            const pdfAttachments = attachments.filter((attachment) => getBaseMediaType(attachment.mediaType) === PDF_MEDIA_TYPE);
            if (pdfAttachments.length > 0) {
                ...
                if (caps && !caps.supportsPdfInput) {
                    ...
                    pushToast({
                        ...
                        message: `Model ${policyModel} does not support PDF input.` +
            ...
            if (variant === "workspace" &&
                shouldTriggerAutoCompaction(props.autoCompactionCheck, isCompacting || isStreamStarting, !!editingMessage, hasQueuedCompaction)) {
                ...
                setInput("");
                setDraftReviews(null);
                const compactionSendMessageOptions = {
                    ...sendMessageOptions,
                };
                setAttachments([]);
                ...
                try {
                    ...
                    pushToast({
                        type: "error",
                        title: "Auto-Compaction Failed",
                        message: error instanceof Error ? error.message : "Unexpected error during auto-compaction",
                    });
                }
                finally {
                    setSendingCount((c) => c - 1);
            ...
            try {
                ...
                let muxMetadata = skillMuxMetadata;
                let compactionOptions = {};
                if (editingMessage && actualMessageText.startsWith("/")) {
                    const parsed = parseCommand(messageText);
                    if (parsed?.type === "compact") {
                        const { messageText: regeneratedText, metadata, sendOptions, } = prepareCompactionMessage({
                            api,
                            workspaceId: props.workspaceId,
                            ...
                            followUpContent: parsed.continueMessage || sendFileParts?.length || reviewsData?.length
                                ? {
                ...
                const { finalText: finalMessageText, metadata: reviewMetadata } = prepareUserMessageForSend({ text: actualMessageText, reviews: reviewsData }, muxMetadata);
                ...
                const additionalSystemInstructions = compactionOptions.additionalSystemInstructions ??
                ...
                const thinkingOverride = rawThinkingOverride != null
                ...
                if (!result.success) {
                    ...
                    telemetry.messageSent(props.workspaceId, effectiveModel, sendMessageOptions.agentId ?? agentId ?? "exec", finalMessageText.length, runtimeType, sendMessageOptions.thinkingLevel ?? "off");
                    if (modelOneShot) {
    ...
    const handleKeyDown = (e) => {
        ...
        if ((hasCommandSuggestionMenu && COMMAND_SUGGESTION_KEYS.includes(e.key)) ||
    ...
    const wrapperProps = {};
    return (_jsxs(Wrapper, { ...wrapperProps, children: [variant === "creation" && (_jsx(CreationCenterContent, { projectName: props.projectName, isSending: isSendInFlight, workspaceName: isSendInFlight ? creationState.creatingWithIdentity?.name : undefined, workspaceTitle: isSendInFlight ? creationState.creatingWithIdentity?.title : undefined })), _jsx("div", { ref: chatInputSectionRef, className: cn("relative flex flex-col gap-1", variant === "creation"
                    ? "bg-separator w-full max-w-3xl rounded-lg border border-border-light px-6 py-5 shadow-lg"
                    : "bg-separator border-border-light border-t px-[15px] pt-[5px] pb-[max(8px,min(env(safe-area-inset-bottom,0px),40px))] mb-[calc(-1*min(env(safe-area-inset-bottom,0px),40px))]"), "data-component": "ChatInputSection", "data-autofocus-state": "done", children: _jsxs("div", { className: cn("w-full", variant !== "creation" && "mx-auto max-w-4xl"), children: [_jsxs("div", { className: "pointer-events-none absolute right-[15px] bottom-full left-[15px] z-[1000] mb-2 flex flex-col gap-2 [&>*]:pointer-events-auto", children: [_jsx(ConnectionStatusToast, { wrap: false }), _jsx(ChatInputToast, { toast: activeToast, wrap: false, onDismiss: () => {
                                        handleToastDismiss();
                                        if (variant === "creation") {
                                            creationState.setToast(null);
                                        }
                                    } })] }), variant === "workspace" && !hideReviewsDuringSend && (_jsx(AttachedReviewsPanel, { reviews: reviewPanelItems, onDetachAll: reviewOverrideActive ? undefined : props.onDetachAllReviews, onDetach: reviewOverrideActive ? undefined : props.onDetachReview, onCheck: reviewOverrideActive ? undefined : props.onCheckReview, onDelete: reviewOverrideActive ? undefined : props.onDeleteReview, onUpdateNote: reviewOverrideActive ? undefined : props.onUpdateReviewNote })), creationControlsProps && _jsx(CreationControls, { ...creationControlsProps }), _jsx(CodexOauthWarningBanner, { activeModel: baseModel, codexOauthSet: codexOauthSet, onOpenProviders: () => open("providers", { expandProvider: "openai" }) }), _jsx(CommandSuggestions, { suggestions: atMentionSuggestions, onSelectSuggestion: handleAtMentionSelect, onDismiss: () => setShowAtMentionSuggestions(false), isVisible: showAtMentionSuggestions, ariaLabel: "File path suggestions", listId: atMentionListId, anchorRef: variant === "creation" ? inputRef : undefined, highlightQuery: lastAtMentionQueryRef.current ?? "", isFileSuggestion: true }), _jsx(CommandSuggestions, { suggestions: commandSuggestions, onSelectSuggestion: handleCommandSelect, onDismiss: () => setShowCommandSuggestions(false), isVisible: showCommandSuggestions, ariaLabel: "Slash command suggestions", listId: commandListId, anchorRef: variant === "creation" ? inputRef : undefined }), _jsx("div", { className: "relative flex items-end pb-1", "data-component": "ChatInputControls", children: voiceInput.state !== "idle" ? (_jsx(RecordingOverlay, { state: voiceInput.state, agentColor: focusBorderColor, mediaRecorder: voiceInput.mediaRecorder, onStop: voiceInput.toggle })) : (_jsxs(_Fragment, { children: [_jsx(VimTextArea, { ref: inputRef, "data-escape-interrupts-stream": "true", value: input, isEditing: !!editingMessage, focusBorderColor: focusBorderColor, onChange: handleInputChange, onKeyDown: handleKeyDown, onPaste: handlePaste, onKeyUp: handleAtMentionCursorActivity, onMouseUp: handleAtMentionCursorActivity, onSelect: handleAtMentionCursorActivity, onDragOver: handleDragOver, onDrop: handleDrop, onEscapeInNormalMode: handleEscapeInNormalMode, suppressKeys: showAtMentionSuggestions
                                            ? FILE_SUGGESTION_KEYS
                                            : showCommandSuggestions
                                                ? COMMAND_SUGGESTION_KEYS
                                                : undefined, placeholder: placeholder, disabled: !editingMessage && (disabled || sendInFlightBlocksInput), "aria-label": editingMessage ? "Edit your last message" : "Message Claude", "aria-autocomplete": "list", "aria-controls": showAtMentionSuggestions && atMentionSuggestions.length > 0
                                            ? atMentionListId
                                            : showCommandSuggestions && commandSuggestions.length > 0
                                                ? commandListId
                                                : undefined, "aria-expanded": (showCommandSuggestions && commandSuggestions.length > 0) ||
                                            (showAtMentionSuggestions && atMentionSuggestions.length > 0), className: variant === "creation" ? "min-h-28" : "min-h-16" }), input.trim() === "" && !editingMessage && (_jsxs("div", { className: "mobile-hide-shortcut-hints text-muted pointer-events-none absolute right-12 bottom-3 left-2 flex items-center gap-4 text-[11px]", children: [_jsxs("span", { children: [_jsx("span", { className: "font-mono", children: formatKeybind(KEYBINDS.FOCUS_CHAT) }), _jsx("span", { children: " - focus chat" })] }), _jsxs("span", { children: [_jsx("span", { className: "font-mono", children: formatKeybind(KEYBINDS.CYCLE_MODEL) }), _jsx("span", { children: " - change model" })] }), _jsxs("span", { children: [_jsx("span", { className: "font-mono", children: formatKeybind(KEYBINDS.CYCLE_AGENT) }), _jsx("span", { children: " - change agent mode" })] })] })), _jsx("div", { className: "absolute right-2 bottom-2", children: _jsx(VoiceInputButton, { state: voiceInput.state, isApiKeySet: voiceInput.isApiKeySet, isProviderEnabled: voiceInput.isProviderEnabled, shouldShowUI: voiceInput.shouldShowUI, requiresSecureContext: voiceInput.requiresSecureContext, onToggle: voiceInput.toggle, disabled: disabled || sendInFlightBlocksInput, agentColor: focusBorderColor }) })] })) }), _jsx(ChatAttachments, { attachments: attachments, onRemove: handleRemoveAttachment }), _jsxs("div", { className: "flex flex-col gap-0.5", "data-component": "ChatModeToggles", children: [variant === "workspace" && editingMessage && (_jsxs("div", { className: "text-edit-mode text-[11px] font-medium", children: ["Editing message", " ", _jsxs("span", { className: "mobile-hide-shortcut-hints", children: ["(", formatKeybind(KEYBINDS.CANCEL_EDIT), vimEnabled ? "×2" : "", " to cancel)"] })] })), _jsxs("div", { className: "@container flex min-w-[340px] flex-nowrap items-center gap-1.5", children: [_jsxs("div", { className: "flex min-w-0 flex-1 items-center gap-1.5", children: [_jsxs("div", { className: "flex min-w-0 items-center gap-1.5", "data-component": "ModelSelectorGroup", "data-tutorial": "model-selector", children: [_jsx(ModelSelector, { ref: modelSelectorRef, value: baseModel, onChange: setPreferredModel, models: models, onComplete: () => inputRef.current?.focus(), defaultModel: defaultModel, onSetDefaultModel: setDefaultModel, hiddenModels: hiddenModelsForSelector, onOpenSettings: () => open("models"), className: "w-[clamp(5.5rem,28vw,8rem)] min-w-0" }), _jsx("div", { className: "hidden [@container(min-width:500px)]:[@media(hover:hover)_and_(pointer:fine)]:block", children: _jsxs(Tooltip, { children: [_jsx(TooltipTrigger, { asChild: true, children: _jsx(HelpIndicator, { children: "?" }) }), _jsxs(TooltipContent, { align: "start", className: "max-w-80 whitespace-normal", children: [_jsx("strong", { children: "Click to edit" }), _jsx("br", {}), _jsx("strong", { children: formatKeybind(KEYBINDS.CYCLE_MODEL) }), " to cycle models", _jsx("br", {}), _jsx("br", {}), _jsx("strong", { children: "Abbreviations:" }), MODEL_ABBREVIATION_EXAMPLES.map((ex) => (_jsxs(React.Fragment, { children: [_jsx("br", {}), "\u2022 ", _jsxs("code", { children: ["/model ", ex.abbrev] }), " - ", ex.displayName] }, ex.abbrev))), _jsx("br", {}), _jsx("br", {}), _jsx("strong", { children: "Full format:" }), _jsx("br", {}), _jsx("code", { children: "/model provider:model-name" }), _jsx("br", {}), "(e.g., ", _jsx("code", { children: "/model anthropic:claude-sonnet-4-5" }), ")"] })] }) })] }), _jsx("div", { className: "flex shrink-0 items-center [@container(max-width:420px)]:[&_[data-thinking-paddle]]:hidden", "data-component": "ThinkingSliderGroup", children: _jsx(ThinkingSliderComponent, { modelString: baseModel }) })] }), _jsxs("div", { className: "flex min-w-0 items-center justify-end gap-1.5", "data-component": "ModelControls", "data-tutorial": "mode-selector", children: [variant === "workspace" && (_jsx(ContextUsageIndicatorButton, { data: contextUsageData, autoCompaction: autoCompactionProps, idleCompaction: idleCompactionProps, model: contextDisplayModel })), _jsx("div", { className: "min-w-0 [@container(max-width:340px)]:hidden", children: _jsx(AgentModePicker, { className: "min-w-0", onComplete: () => inputRef.current?.focus() }) }), _jsxs(Tooltip, { children: [_jsx(TooltipTrigger, { asChild: true, children: _jsx(Button, { type: "button", onClick: () => void handleSend(), disabled: !canSend, "aria-label": "Send message", size: "xs", variant: "ghost", className: cn("text-muted hover:text-foreground hover:bg-hover inline-flex items-center justify-center rounded-sm px-1.5 py-0.5 font-medium transition-colors duration-200 disabled:opacity-50", 
                                                                // Touch: wider tap target, keep icon centered.
                                                                "[@media(hover:none)_and_(pointer:coarse)]:h-9 [@media(hover:none)_and_(pointer:coarse)]:w-11 [@media(hover:none)_and_(pointer:coarse)]:px-0 [@media(hover:none)_and_(pointer:coarse)]:py-0 [@media(hover:none)_and_(pointer:coarse)]:text-sm"), children: _jsx(SendHorizontal, { className: "h-3.5 w-3.5 [@media(hover:none)_and_(pointer:coarse)]:h-4 [@media(hover:none)_and_(pointer:coarse)]:w-4", strokeWidth: 2.5 }) }) }), _jsxs(TooltipContent, { align: "center", children: ["Send message", " ", _jsxs("span", { className: "mobile-hide-shortcut-hints", children: ["(", formatKeybind(KEYBINDS.SEND_MESSAGE), ")"] })] })] })] })] })] })] }) }), _jsx(ConfirmationModal, { isOpen: pendingDestructiveCommand !== null, title: pendingDestructiveCommand?.type === "clear"
                    ? "Clear Chat History?"
                    : `Truncate ${Math.round((pendingDestructiveCommand?.percentage ?? 0) * 100)}% of Chat History?`, description: pendingDestructiveCommand?.type === "clear"
                    ? "This will remove all messages from the conversation."
                    : `This will remove approximately ${Math.round((pendingDestructiveCommand?.percentage ?? 0) * 100)}% of the oldest messages.`, warning: "This action cannot be undone.", confirmLabel: pendingDestructiveCommand?.type === "clear" ? "Clear" : "Truncate", onConfirm: handleDestructiveCommandConfirm, onCancel: handleDestructiveCommandCancel })] }));
};
