...
export const DirectoryPickerModal = ({ isOpen, initialPath, onClose, onSelectPath, }) => {
    const { api } = useAPI();
    ...
    const loadDirectory = useCallback(async (path) => {
        ...
        try {
            ...
            if (!result.success) {
                ...
                const isNotFound = errorMessage.includes("ENOENT") || errorMessage.includes("no such file or directory");
                if (isNotFound) {
    ...
    return (_jsx(Dialog, { open: isOpen, onOpenChange: handleOpenChange, children: _jsxs(DialogContent, { showCloseButton: false, children: [_jsxs(DialogHeader, { children: [_jsx(DialogTitle, { children: "Select Project Directory" }), _jsx(DialogDescription, { children: "Navigate to select a directory for your project" })] }), _jsx("div", { className: "mb-3", children: _jsx(Input, { value: pathInput, onChange: (e) => {
                            setPathInput(e.target.value);
                            setCanCreateFolder(false);
                        }, onKeyDown: handlePathInputKeyDown, placeholder: "Enter path...", className: "bg-modal-bg border-border-medium h-9 font-mono text-sm" }) }), _jsx("div", { className: "mb-3 flex items-center", children: _jsxs("label", { className: "text-muted-foreground flex cursor-pointer items-center gap-2 text-xs", children: [_jsx(Checkbox, { checked: showHiddenFiles, onCheckedChange: (checked) => {
                                    ...
                                    setSelectedIndex((prev) => {
                                        ...
                                        if (selectedEntryPath) {
                                            const nextEntryIndex = nextVisibleEntries.findIndex((child) => child.path === selectedEntryPath);
                                            if (nextEntryIndex !== -1) {
                                ...
                                } }), "Show hidden files"] }) }), error && (_jsxs("div", { className: "mb-3 flex items-center gap-2 text-xs", children: [_jsx("span", { className: canCreateFolder ? "text-muted" : "text-error", children: error }), canCreateFolder && (_jsx(Button, { size: "sm", onClick: () => void handleCreateFolder(), disabled: isLoading, className: "h-6 px-2 py-0 text-xs", children: "Create Folder" }))] })), _jsx("div", { ref: treeRef, className: "bg-modal-bg border-border-medium mb-4 h-80 overflow-hidden rounded border", children: _jsx(DirectoryTree, { currentPath: root ? root.path : null, entries: entries, isLoading: isLoading, onNavigateTo: handleNavigateTo, onNavigateParent: handleNavigateParent, onConfirm: () => void handleConfirm(), selectedIndex: selectedIndex, onSelectedIndexChange: setSelectedIndex }) }), _jsxs(DialogFooter, { children: [_jsx(Button, { variant: "secondary", onClick: onClose, disabled: isLoading, children: "Cancel" }), _jsxs(Button, { onClick: () => void handleConfirm(), disabled: isLoading || !root, title: `Open folder (${shortcutLabel})`, children: ["Open (", shortcutLabel, ")"] })] })] }) }));
};
//# sourceMappingURL=DirectoryPickerModal.js.map