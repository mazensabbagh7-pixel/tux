 ...
 */
export const MarkdownCore = React.memo(({ content, children, parseIncompleteMarkdown = false, preserveLineBreaks = false }) => {
    // Memoize the normalized content to avoid recalculating on every render
    const normalizedContent = useMemo(() => normalizeMarkdown(content), [content]);
    return (_jsxs(_Fragment, { children: [_jsx(Streamdown, { components: markdownComponents, remarkPlugins: preserveLineBreaks ? REMARK_PLUGINS_WITH_BREAKS : REMARK_PLUGINS, rehypePlugins: REHYPE_PLUGINS, parseIncompleteMarkdown: parseIncompleteMarkdown, 
                ...
                mode: parseIncompleteMarkdown ? "streaming" : "static", className: "space-y-2" // Reduce from default space-y-4 (16px) to space-y-2 (8px)
                , controls: { table: false, code: true, mermaid: true }, children: normalizedContent }), children] }));
});
