...
import { matchesKeybind, KEYBINDS, formatKeybind, isEditableElement, } from "@/browser/utils/ui/keybinds";
 ...
 */
function preserveHunkReferences(prev, next) {
    if (prev.length === 0)
        return next;
...
const reviewPanelCache = new LRUCache({
...
function getOriginBranchForFetch(diffBase) {
...
async function executeWorkspaceBashAndCache(params) {
...
export const ReviewPanel = ({ workspaceId, workspacePath, projectPath, onReviewNote, focusTrigger, isCreating = false, onStatsChange, onOpenFile, }) => {
    const originFetchRef = useRef(null);
    const { api } = useAPI();
    ...
    const [diffState, setDiffState] = useState({ status: "loading" });
    // Persist selected hunk per workspace so navigation survives tab switches
    const [selectedHunkId, setSelectedHunkId] = usePersistedState(`review-selected-hunk:${workspaceId}`, null);
    ...
    const toggleExpandFnsRef = useRef(new Map());
    ...
    const lastDiffRefreshTriggerRef = useRef(null);
    const lastFileTreeRefreshTriggerRef = useRef(null);
    ...
    const skipCacheOnMountRef = useRef(workspaceStore.getFileModifyingToolMs(workspaceId) !== undefined);
    // Unified search state (per-workspace persistence)
    const [searchState, setSearchState] = usePersistedState(getReviewSearchStateKey(workspaceId), { input: "", useRegex: false, matchCase: false });
    const [debouncedSearchTerm, setDebouncedSearchTerm] = useState("");
    // Persist file filter per workspace
    const [selectedFilePath, setSelectedFilePath] = usePersistedState(`review-file-filter:${workspaceId}`, null);
    ...
    const [defaultBase] = usePersistedState(STORAGE_KEYS.reviewDefaultBase(projectPath), WORKSPACE_DEFAULTS.reviewBase, { listener: true });
    ...
    const [diffBase, setDiffBase] = usePersistedState(STORAGE_KEYS.reviewDiffBase(workspaceId), defaultBase, { listener: true });
    // Persist includeUncommitted flag globally
    const [includeUncommitted, setIncludeUncommitted] = usePersistedState("review-include-uncommitted", false);
    ...
    const [sortOrder, setSortOrder] = usePersistedState(REVIEW_SORT_ORDER_KEY, "last-edit");
    ...
    const { reviews, updateReviewNote, checkReview, uncheckReview, attachReview, detachReview, removeReview, } = useReviews(workspaceId);
    ...
    const hunks = useMemo(() => diffState.status === "loaded" || diffState.status === "refreshing" ? diffState.hunks : [], [diffState]);
    ...
    const lastGitStatusBaseRef = useRef(null);
    useEffect(() => {
    ...
    const composingHunksRef = useRef(new Set());
    const editingReviewIdsRef = useRef(new Set());
    ...
    const handleHunkComposingChange = useCallback((hunkId, isComposing) => {
        if (isComposing) {
    ...
    const handleInlineReviewEditingChange = useCallback((reviewId, isEditing) => {
        if (isEditing) {
    ...
    useEffect(() => {
        ...
        const numstatCommand = buildGitDiffCommand(filters.diffBase, filters.includeUncommitted, "", // No path filter for file tree
        "numstat");
        const nameStatusCommand = buildGitDiffCommand(filters.diffBase, filters.includeUncommitted, "", // No path filter for file tree
        "name-status");
        ...
        if (!isManualRefresh && !skipCacheOnMountRef.current) {
            const cachedTree = reviewPanelCache.get(cacheKey);
            if (cachedTree) {
    ...
    useEffect(() => {
        ...
        const diffCommand = buildGitDiffCommand(filters.diffBase, filters.includeUncommitted, pathFilter, "diff");
        const cacheKey = makeReviewPanelCacheKey({
            workspaceId,
            workspacePath,
        ...
        // or tools completed while we were unmounted).
        if (!isManualRefresh && !skipCacheOnMountRef.current) {
            const cached = reviewPanelCache.get(cacheKey);
            if (cached) {
            ...
            try {
                ...
                const data = await executeWorkspaceBashAndCache({
                    ...
                    parse: (result) => {
                        ...
                        const truncationWarning = truncationInfo && !selectedFilePath
                ...
                setDiffState((prev) => {
                    const prevHunks = prev.status === "loaded" || prev.status === "refreshing" ? prev.hunks : [];
    ...
    const getFileReadStatus = useCallback((filePath) => {
    ...
    const searchConfig = useMemo(() => debouncedSearchTerm
        ? {
            searchTerm: debouncedSearchTerm,
            useRegex: searchState.useRegex,
            matchCase: searchState.matchCase,
        }
        : undefined, [debouncedSearchTerm, searchState.useRegex, searchState.matchCase]);
    ...
    const handleToggleRead = useCallback((hunkId) => {
    ...
    // Uses refs to keep callback stable across state changes - prevents HunkViewer re-renders
    const handleMarkAsRead = useCallback((hunkId) => {
        const wasRead = isReadRef.current(hunkId);
        markAsRead(hunkId);
    ...
    const handleMarkFileAsRead = useCallback((hunkId) => {
        ...
        if (hunkId === selectedHunkId && !filters.showReadHunks) {
            ...
            setSelectedHunkId(findNextHunkIdAfterFileRemoval(filteredHunksRef.current, hunkId, hunk.filePath));
        }
    ...
    const isNonGitWorkspace = diffState.status === "error" &&
    ...
    if (isCreating) {
        return (_jsxs("div", { className: "flex h-full flex-col items-center justify-center p-8 text-center", children: [_jsx(Loader2, { "aria-hidden": "true", className: "text-secondary mb-4 h-6 w-6 animate-spin" }), _jsx("p", { className: "text-secondary text-sm", children: "Setting up workspace..." }), _jsx("p", { className: "text-secondary mt-1 text-xs", children: "Review will be available once ready" })] }));
    }
    return (_jsxs("div", { ref: panelRef, tabIndex: 0, "data-testid": "review-panel", onFocus: () => setIsPanelFocused(true), onBlur: () => setIsPanelFocused(false), className: "bg-dark [container-type:inline-size] flex h-full min-h-0 flex-col outline-none [container-name:review-panel]", children: [_jsx(ReviewControls, { filters: filters, stats: stats, onFiltersChange: setFilters, onRefresh: handleRefresh, isLoading: diffState.status === "loading" || diffState.status === "refreshing" || isLoadingTree, isRefreshBlocked: isRefreshBlocked, projectPath: projectPath, lastRefreshInfo: lastRefreshInfo }), diffState.status === "error" ? (isNonGitWorkspace ? (_jsxs("div", { className: "text-muted flex flex-col items-center justify-start gap-3 px-6 pt-12 pb-6 text-center", children: [_jsx("div", { className: "text-foreground text-base font-medium", children: "Not a git repository" }), _jsxs("div", { className: "text-[13px] leading-[1.5]", children: ["This project is not a git repository, so changes ", "can't", " be computed."] })] })) : (_jsxs("div", { className: "text-danger-soft bg-danger-soft/10 border-danger-soft/30 font-monospace m-3 rounded border p-6 text-xs leading-[1.5] break-words whitespace-pre-wrap", children: [diffState.message, diffState.message.includes("unknown revision") && (_jsxs("div", { className: "text-muted mt-3 flex items-start gap-2 border-t border-current/20 pt-3 font-sans text-[11px]", children: [_jsx(Lightbulb, { "aria-hidden": "true", className: "mt-0.5 h-3 w-3 shrink-0" }), _jsxs("span", { children: ["The ref ", _jsx("code", { className: "text-foreground", children: filters.diffBase }), " does not exist in this repository. Use the dropdown above to select a different base (e.g., HEAD, origin/master)."] })] }))] }))) : diffState.status === "loading" ? (_jsx("div", { className: "text-muted flex h-full items-center justify-center text-sm", children: "Loading diff..." })) : (_jsxs("div", { className: "flex min-h-0 flex-1 flex-col overflow-hidden", children: [diffState.truncationWarning && (_jsxs("div", { className: "bg-warning/10 border-warning/30 text-warning mx-3 my-3 flex items-center gap-1.5 rounded border px-3 py-1.5 text-[10px] leading-[1.3]", children: [_jsx(AlertTriangle, { "aria-hidden": "true", className: "h-3 w-3 shrink-0" }), _jsx("span", { children: diffState.truncationWarning })] })), _jsxs("div", { className: "border-border-light flex items-center gap-1.5 border-b px-2 py-1", children: [_jsx("input", { ref: searchInputRef, type: "text", placeholder: `Search... (${formatKeybind(KEYBINDS.FOCUS_REVIEW_SEARCH)}, ${formatKeybind(KEYBINDS.FOCUS_REVIEW_SEARCH_QUICK)})`, value: searchState.input, onChange: (e) => setSearchState({ ...searchState, input: e.target.value }), className: "bg-dark text-foreground border-border-medium placeholder:text-dim hover:border-accent focus:border-accent min-w-0 flex-1 rounded border px-1.5 py-0.5 font-mono text-[11px] transition-[border-color] duration-150 focus:outline-none" }), _jsxs(Tooltip, { children: [_jsx(TooltipTrigger, { asChild: true, children: _jsx("button", { className: cn("font-monospace cursor-pointer border-none bg-transparent p-0 text-[11px] font-semibold transition-colors duration-150", searchState.useRegex ? "text-accent-light" : "text-muted hover:text-foreground"), onClick: () => setSearchState({ ...searchState, useRegex: !searchState.useRegex }), children: ".*" }) }), _jsx(TooltipContent, { side: "bottom", children: searchState.useRegex ? "Using regex search" : "Using substring search" })] }), _jsxs(Tooltip, { children: [_jsx(TooltipTrigger, { asChild: true, children: _jsx("button", { className: cn("font-monospace cursor-pointer border-none bg-transparent p-0 text-[11px] font-semibold transition-colors duration-150", searchState.matchCase ? "text-accent-light" : "text-muted hover:text-foreground"), onClick: () => setSearchState({ ...searchState, matchCase: !searchState.matchCase }), children: "Aa" }) }), _jsx(TooltipContent, { side: "bottom", children: searchState.matchCase
                                            ? "Match case (case-sensitive)"
                                            : "Ignore case (case-insensitive)" })] })] }), _jsxs("div", { ref: scrollContainerRef, className: "flex min-h-0 flex-1 flex-col overflow-y-auto", children: [(fileTree ?? isLoadingTree) && (_jsx("div", { className: "border-border-light flex w-full flex-[0_0_auto] flex-col overflow-hidden border-b", children: _jsx(FileTree, { root: fileTree, selectedPath: selectedFilePath, onSelectFile: setSelectedFilePath, isLoading: isLoadingTree, getFileReadStatus: getFileReadStatus, workspaceId: workspaceId }) })), _jsx(UntrackedStatus, { workspaceId: workspaceId, workspacePath: workspacePath, refreshTrigger: refreshTrigger, onRefresh: handleRefresh }), _jsx("div", { className: "flex flex-[0_0_auto] flex-col p-3", children: hunks.length === 0 ? (_jsxs("div", { className: "text-muted flex flex-col items-center justify-start gap-3 px-6 pt-12 pb-6 text-center", children: [_jsx("div", { className: "text-foreground text-base font-medium", children: "No changes found" }), _jsxs("div", { className: "text-[13px] leading-[1.5]", children: ["No changes found for the selected diff base.", _jsx("br", {}), "Try selecting a different base or make some changes."] }), diagnosticInfo && (_jsxs("details", { className: "bg-modal-bg border-border-light [&_summary]:text-muted mt-4 w-full max-w-96 cursor-pointer rounded border p-3 [&_summary]:flex [&_summary]:list-none [&_summary]:items-center [&_summary]:gap-1.5 [&_summary]:text-xs [&_summary]:font-medium [&_summary]:select-none [&_summary::-webkit-details-marker]:hidden [&_summary::before]:text-[10px] [&_summary::before]:transition-transform [&_summary::before]:duration-200 [&_summary::before]:content-['\u25B6'] [&[open]_summary::before]:rotate-90", children: [_jsx("summary", { children: "Show diagnostic info" }), _jsxs("div", { className: "font-monospace text-foreground mt-3 text-[11px] leading-[1.6]", children: [_jsxs("div", { className: "[&:not(:last-child)]:border-border-light grid grid-cols-[140px_1fr] gap-3 py-1 [&:not(:last-child)]:border-b", children: [_jsx("div", { className: "text-muted font-medium", children: "Command:" }), _jsx("div", { className: "text-foreground break-all select-all", children: diagnosticInfo.command })] }), _jsxs("div", { className: "[&:not(:last-child)]:border-border-light grid grid-cols-[140px_1fr] gap-3 py-1 [&:not(:last-child)]:border-b", children: [_jsx("div", { className: "text-muted font-medium", children: "Output size:" }), _jsxs("div", { className: "text-foreground break-all select-all", children: [diagnosticInfo.outputLength.toLocaleString(), " bytes"] })] }), _jsxs("div", { className: "[&:not(:last-child)]:border-border-light grid grid-cols-[140px_1fr] gap-3 py-1 [&:not(:last-child)]:border-b", children: [_jsx("div", { className: "text-muted font-medium", children: "Files parsed:" }), _jsx("div", { className: "text-foreground break-all select-all", children: diagnosticInfo.fileDiffCount })] }), _jsxs("div", { className: "grid grid-cols-[140px_1fr] gap-3 py-1", children: [_jsx("div", { className: "text-muted font-medium", children: "Hunks extracted:" }), _jsx("div", { className: "text-foreground break-all select-all", children: diagnosticInfo.hunkCount })] })] })] }))] })) : filteredHunks.length === 0 ? (_jsx("div", { className: "text-muted flex flex-col items-center justify-start gap-3 px-6 pt-12 pb-6 text-center", children: _jsx("div", { className: "text-[13px] leading-[1.5]", children: debouncedSearchTerm.trim()
                                            ...
                                            : selectedFilePath
                                                ...
                                                : "No hunks match the current filters. Try adjusting your filter settings." }) })) : (filteredHunks.map((hunk) => {
                                    ...
                                    return (_jsx(HunkViewer, { hunk: hunk, hunkId: hunk.id, workspaceId: workspaceId, inlineReviews: reviewsByFilePath.get(hunk.filePath), isSelected: isSelected, isRead: hunkIsRead, firstSeenAt: hunkFirstSeenAt, onClick: handleHunkClick, onToggleRead: handleHunkToggleRead, onRegisterToggleExpand: handleRegisterToggleExpand, onReviewNote: onReviewNote, searchConfig: searchConfig, onComposingChange: handleHunkComposingChange, diffBase: filters.diffBase, includeUncommitted: filters.includeUncommitted, reviewActions: reviewActions, onOpenFile: onOpenFile }, hunk.id));
                                })) })] })] }))] }));
};
