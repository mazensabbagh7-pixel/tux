...
export const UntrackedStatus = ({ workspaceId, workspacePath, refreshTrigger, onRefresh, }) => {
    const { api } = useAPI();
    ...
    const handleTrackAll = async () => {
        if (untrackedFiles.length === 0 || isTracking)
            return;
        setIsTracking(true);
        try {
    ...
    return (_jsxs("div", { className: "bg-info-yellow/10 border-info-yellow/30 mx-3 mt-3 rounded border", children: [_jsxs("button", { onClick: () => setIsExpanded(!isExpanded), className: cn("flex w-full cursor-pointer items-center gap-2 px-3 py-2 text-left", "hover:bg-info-yellow/5 transition-colors duration-150"), children: [_jsx("span", { className: cn("text-[10px] transition-transform duration-200", isExpanded && "rotate-90"), children: "\u25B6" }), _jsxs("span", { className: "text-info-yellow text-xs font-medium", children: [count, " untracked ", count === 1 ? "file" : "files"] }), _jsx("span", { className: "text-muted text-[11px]", children: "not included in diff" })] }), isExpanded && (_jsxs("div", { className: "border-info-yellow/20 border-t px-3 py-2", children: [_jsxs("div", { className: "mb-2 max-h-[200px] overflow-y-auto", children: [displayedFiles.map((file) => (_jsx("div", { className: "text-label hover:bg-bg-subtle truncate py-0.5 font-mono text-[11px]", children: file }, file))), hiddenCount > 0 && (_jsxs("div", { className: "text-muted py-0.5 text-[11px] italic", children: ["and ", hiddenCount, " more ", hiddenCount === 1 ? "file" : "files", "..."] }))] }), _jsx("button", { onClick: () => void handleTrackAll(), disabled: isTracking, className: cn("w-full py-1.5 px-2 bg-info-yellow/20 text-info-yellow border border-info-yellow/30 rounded text-[11px] cursor-pointer transition-all duration-200 font-medium", "hover:bg-info-yellow/30 hover:border-info-yellow/50", "active:bg-info-yellow/40", "disabled:opacity-50 disabled:cursor-not-allowed"), children: isTracking ? "Tracking..." : "Track All Files" })] }))] }));
};
//# sourceMappingURL=UntrackedStatus.js.map