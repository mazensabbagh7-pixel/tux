...
import { readFileLines, formatAsContextLines, getOldFileRef, LINES_PER_EXPANSION, } from "@/browser/utils/review/readFileLines";
export function useReadMore(options) {
    ...
    const [readMoreMap, setReadMoreMap] = usePersistedState(getReviewReadMoreKey(workspaceId), {}, { listener: true });
    ...
    const gitRef = useMemo(() => getOldFileRef(diffBase, includeUncommitted), [diffBase, includeUncommitted]);
    ...
    useEffect(() => {
        ...
        void readFileLines(api, workspaceId, hunk.filePath, startLine, endLine, gitRef).then((lines) => {
            if (cancelled)
                return;
            setUpLoading(false);
            if (lines) {
    ...
    const handleExpandUp = useCallback((e) => {
        e.stopPropagation();
        setReadMoreMap((prev) => ({
            ...prev,
    ...
    const handleExpandDown = useCallback((e) => {
        e.stopPropagation();
        setReadMoreMap((prev) => ({
            ...prev,
    ...
    const handleCollapseUp = useCallback((e) => {
        e.stopPropagation();
        setReadMoreMap((prev) => ({
            ...prev,
    ...
    const handleCollapseDown = useCallback((e) => {
        e.stopPropagation();
        setReadMoreMap((prev) => ({
            ...prev,
...
//# sourceMappingURL=useReadMore.js.map