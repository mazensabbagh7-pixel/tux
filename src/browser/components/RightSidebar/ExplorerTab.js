...
import { ChevronDown, ChevronRight, ChevronsDownUp, FolderClosed, FolderOpen, RefreshCw, } from "lucide-react";
import { FileIcon } from "../FileIcon";
import { cn } from "@/common/lib/utils";
import { Tooltip, TooltipContent, TooltipTrigger } from "../ui/tooltip";
import { validateRelativePath, buildListDirScript, buildGitIgnoredScript, buildGitCheckIgnoreScript, parseLsOutput, parseGitStatus, parseGitCheckIgnoreOutput, } from "@/browser/utils/fileExplorer";
const DEBOUNCE_MS = 2000;
const INDENT_PX = 12;
// Cache key builders
const entriesCacheKey = (workspaceId) => `explorer:entries:${workspaceId}`;
const expandedCacheKey = (workspaceId) => `explorer:expanded:${workspaceId}`;
export const ExplorerTab = (props) => {
    const { api } = useAPI();
    // Persist expanded folders across tab switches
    const [expandedPaths, setExpandedPaths] = usePersistedState(expandedCacheKey(props.workspaceId), [], { listener: true });
    const expandedSet = React.useMemo(() => new Set(expandedPaths), [expandedPaths]);
    // Initialize entries from cache
    const [state, setState] = React.useState(() => {
        const cached = readPersistedState(entriesCacheKey(props.workspaceId), {});
        return {
    ...
    const fetchDirectory = React.useCallback(async (relativePath, suppressErrors = false) => {
        if (!api)
            return null;
        ...
        try {
            ...
            setState((prev) => {
                ...
                const checkedDirPaths = isRoot ? new Set() : prev.checkedDirPaths;
    ...
    const renderNode = (node, depth) => {
        ...
        const statusTextHighlightOverrideClass = statusTextClass
            ...
            : undefined;
        return (_jsxs("div", { children: [_jsxs("button", { type: "button", className: cn("group flex w-full cursor-pointer items-center gap-1 px-2 py-0.5 text-left text-sm hover:bg-accent/50", "focus:bg-accent/50 focus:outline-none", isIgnored && "opacity-50"), style: { paddingLeft: `${8 + depth * INDENT_PX}px` }, onClick: () => {
                        if (node.isDirectory) {
                            toggleExpand(node);
                        }
                        else {
                    ...
                    }, children: [node.isDirectory ? (_jsxs(_Fragment, { children: [isLoading ? (_jsx(RefreshCw, { className: "text-muted h-3 w-3 shrink-0 animate-spin" })) : isExpanded ? (_jsx(ChevronDown, { className: "text-muted h-3 w-3 shrink-0" })) : (_jsx(ChevronRight, { className: "text-muted h-3 w-3 shrink-0" })), isExpanded ? (_jsx(FolderOpen, { className: "h-4 w-4 shrink-0 text-[var(--color-folder-icon)]" })) : (_jsx(FolderClosed, { className: "h-4 w-4 shrink-0 text-[var(--color-folder-icon)]" }))] })) : (_jsxs(_Fragment, { children: [_jsx("span", { className: "w-3 shrink-0" }), _jsx(FileIcon, { fileName: node.name, style: { fontSize: 18 }, className: "h-4 w-4" })] })), _jsx("span", { className: cn("truncate", statusTextClass, statusTextHighlightOverrideClass), children: node.name })] }), node.isDirectory && isExpanded && (_jsx("div", { children: children.map((child) => renderNode(child, depth + 1)) }))] }, key));
    };
    ...
    return (_jsxs("div", { className: "flex h-full flex-col", children: [_jsxs("div", { className: "border-border-light flex items-center gap-1 border-b px-2 py-1", children: [_jsx(FolderOpen, { className: "h-4 w-4 shrink-0 text-[var(--color-folder-icon)]" }), _jsxs(Tooltip, { children: [_jsx(TooltipTrigger, { asChild: true, children: _jsx("span", { className: "min-w-0 flex-1 truncate text-xs font-medium", children: displayPath }) }), _jsx(TooltipContent, { side: "bottom", children: props.workspacePath })] }), _jsxs("div", { className: "flex shrink-0 items-center gap-0.5", children: [_jsxs(Tooltip, { children: [_jsx(TooltipTrigger, { asChild: true, children: _jsx("button", { type: "button", className: "text-muted hover:bg-accent/50 hover:text-foreground rounded p-1", onClick: handleRefresh, disabled: isRootLoading, children: _jsx(RefreshCw, { className: cn("h-3.5 w-3.5", isRootLoading && "animate-spin") }) }) }), _jsx(TooltipContent, { side: "bottom", children: "Refresh" })] }), hasExpandedDirs && (_jsxs(Tooltip, { children: [_jsx(TooltipTrigger, { asChild: true, children: _jsx("button", { type: "button", className: "text-muted hover:bg-accent/50 hover:text-foreground rounded p-1", onClick: handleCollapseAll, "aria-label": "Collapse All", children: _jsx(ChevronsDownUp, { className: "h-3.5 w-3.5" }) }) }), _jsx(TooltipContent, { side: "bottom", children: "Collapse All" })] }))] })] }), _jsxs("div", { className: "flex-1 overflow-y-auto py-1", children: [state.error && _jsx("div", { className: "text-destructive px-3 py-2 text-sm", children: state.error }), isRootLoading && rootEntries.length === 0 ? (_jsx("div", { className: "flex items-center justify-center py-4", children: _jsx(RefreshCw, { className: "text-muted h-5 w-5 animate-spin" }) })) : (rootEntries.map((node) => renderNode(node, 0))), !isRootLoading && rootEntries.length === 0 && !state.error && (_jsx("div", { className: "text-muted px-3 py-2 text-sm", children: "No files found" }))] })] }));
};
//# sourceMappingURL=ExplorerTab.js.map