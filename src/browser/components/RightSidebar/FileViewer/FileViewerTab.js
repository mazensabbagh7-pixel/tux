...
import { validateRelativePath, buildReadFileScript, buildFileDiffScript, processFileContents, EXIT_CODE_TOO_LARGE, } from "@/browser/utils/fileExplorer";
import { getCachedFileContent, setCachedFileContent, removeCachedFileContent, cacheToResult, } from "@/browser/utils/fileContentCache";
const DEBOUNCE_MS = 2000;
export const FileViewerTab = (props) => {
    const { api } = useAPI();
    ...
    const [loaded, setLoaded] = React.useState(initialCached);
    ...
    const loadedPathRef = React.useRef(initialCached ? props.relativePath : null);
    ...
    if (isLoading && !hasValidCache) {
        return (_jsx("div", { className: "flex h-full items-center justify-center", children: _jsx(RefreshCw, { className: "text-muted h-5 w-5 animate-spin" }) }));
    }
    // Show error only if we have no content to fall back to
    if (error && !hasValidCache) {
        return (_jsxs("div", { className: "flex h-full flex-col items-center justify-center gap-2 p-4", children: [_jsx(AlertCircle, { className: "text-destructive h-8 w-8" }), _jsx("p", { className: "text-destructive text-center text-sm", children: error })] }));
    }
    // No data at all (shouldn't happen but handle gracefully)
    if (!hasValidCache) {
        return (_jsx("div", { className: "flex h-full items-center justify-center", children: _jsx("p", { className: "text-muted-foreground text-sm", children: "No file loaded" }) }));
    }
    const { data, diff } = loaded;
    // Handle error response from API (file too large, binary, etc.)
    if (data.type === "error") {
        return (_jsxs("div", { className: "flex h-full flex-col items-center justify-center gap-2 p-4", children: [_jsx(AlertCircle, { className: "text-muted h-8 w-8" }), _jsx("p", { className: "text-muted-foreground text-center text-sm", children: data.message })] }));
    }
    const handleRefresh = () => setRefreshCounter((c) => c + 1);
    // Route to appropriate viewer
    if (data.type === "text") {
        return (_jsx(TextFileViewer, { workspaceId: props.workspaceId, content: data.content, filePath: props.relativePath, size: data.size, diff: diff, onRefresh: handleRefresh, isRefreshing: isRefreshing, onReviewNote: props.onReviewNote }));
    }
    if (data.type === "image") {
        return (_jsx(ImageFileViewer, { base64: data.base64, mimeType: data.mimeType, size: data.size, filePath: props.relativePath }));
    }
    // This shouldn't happen, but handle it gracefully
    return (_jsx("div", { className: "flex h-full items-center justify-center", children: _jsx("p", { className: "text-muted-foreground text-sm", children: "Unknown file type" }) }));
};
