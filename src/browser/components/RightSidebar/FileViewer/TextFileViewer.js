...
import { DiffRenderer, SelectableDiffRenderer, } from "@/browser/components/shared/DiffRenderer";
import { useReviews } from "@/browser/hooks/useReviews";
import { getLanguageFromPath, getLanguageDisplayName } from "@/common/utils/git/languageDetector";
const MAX_HIGHLIGHT_CHUNK_BYTES = 30000;
...
const normalizeFileLines = (content) => {
 ...
 */
function buildUnifiedDiffLines(fileLines, diffText) {
    try {
...
function buildChunkedDiffContent(diffLines, oldStart, newStart) {
    const result = [];
...
export const TextFileViewer = (props) => {
    const { reviews, updateReviewNote, checkReview, uncheckReview, attachReview, detachReview, removeReview, } = useReviews(props.workspaceId);
    const inlineReviews = React.useMemo(() => reviews.filter((review) => review.data?.filePath === props.filePath), [reviews, props.filePath]);
    ...
    const diffLines = unifiedLines ?? fileLines.map((content) => ({ type: "context", content }));
    ...
    return (_jsxs("div", { "data-testid": "text-file-viewer", className: "bg-code-bg flex h-full flex-col", children: [_jsx("div", { className: "border-border-light flex flex-col border-b", children: _jsxs("div", { className: "text-muted-foreground flex items-center gap-3 px-2 py-1 text-[11px]", children: [_jsx("span", { className: "min-w-0 truncate", children: props.filePath }), _jsx("span", { className: "shrink-0", children: formatSize(props.size) }), _jsxs("span", { className: "shrink-0", children: [lineCount.toLocaleString(), " lines"] }), (addedCount > 0 || removedCount > 0) && (_jsxs("span", { className: "shrink-0", children: [_jsxs("span", { className: "text-success-light", children: ["+", addedCount] }), _jsx("span", { className: "text-muted-foreground", children: "/" }), _jsxs("span", { className: "text-warning-light", children: ["\u2212", removedCount] })] })), _jsx("span", { className: "ml-auto shrink-0", children: languageDisplayName }), props.onRefresh && (_jsx("button", { type: "button", className: "text-muted hover:bg-accent/50 hover:text-foreground rounded p-0.5", onClick: props.onRefresh, title: "Refresh file", disabled: props.isRefreshing, children: _jsx(RefreshCw, { className: `h-3.5 w-3.5 ${props.isRefreshing ? "animate-spin" : ""}` }) }))] }) }), _jsx("div", { className: "min-h-0 flex-1 overflow-auto", children: shouldUseSelectable ? (_jsx(SelectableDiffRenderer, { ...diffRendererProps, onReviewNote: props.onReviewNote, inlineReviews: inlineReviews, reviewActions: reviewActions })) : (_jsx(DiffRenderer, { ...diffRendererProps })) })] }));
};
//# sourceMappingURL=TextFileViewer.js.map