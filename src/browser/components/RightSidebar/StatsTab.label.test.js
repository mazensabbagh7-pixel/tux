...
describe("formatModelBreakdownLabel", () => {
    test("prefers agentId over mode", () => {
        expect(formatModelBreakdownLabel({ model: "openai:gpt-4o", mode: "exec", agentId: "explore" })).toBe("openai:gpt-4o (explore)");
    });
    test("falls back to mode when agentId is missing", () => {
        expect(formatModelBreakdownLabel({ model: "openai:gpt-4o", mode: "plan" })).toBe("openai:gpt-4o (plan)");
    });
...
//# sourceMappingURL=StatsTab.label.test.js.map