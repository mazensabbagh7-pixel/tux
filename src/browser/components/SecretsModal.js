...
import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogDescription, DialogFooter, DialogInfo, } from "@/browser/components/ui/dialog";
...
const SecretsModal = ({ isOpen, projectPath: _projectPath, projectName, initialSecrets, onClose, onSave, }) => {
    const { api } = useAPI();
    ...
    const updateSecretValue = (index, value) => {
        setSecrets((prev) => {
            const next = [...prev];
            const existing = next[index] ?? { key: "", value: "" };
    ...
    const updateSecretValueKind = (index, kind) => {
        setSecrets((prev) => {
            const next = [...prev];
            const existing = next[index] ?? { key: "", value: "" };
            if (kind === "literal") {
    ...
    return (_jsx(Dialog, { open: isOpen, onOpenChange: handleOpenChange, children: _jsxs(DialogContent, { maxWidth: "600px", maxHeight: "80vh", showCloseButton: false, children: [_jsxs(DialogHeader, { children: [_jsx(DialogTitle, { children: "Manage Secrets" }), _jsxs(DialogDescription, { children: ["Project: ", projectName] })] }), _jsxs(DialogInfo, { children: [_jsxs("p", { children: ["Secrets are stored in ", _jsx("code", { children: "~/.mux/secrets.json" }), " (kept away from source code) but namespaced per project."] }), _jsx("p", { children: "Secrets are injected as environment variables to compute commands (e.g. Bash)" })] }), _jsx("div", { className: "mb-4 min-h-[200px] flex-1 overflow-y-auto", children: secrets.length === 0 ? (_jsx("div", { className: "text-muted px-4 py-8 text-center text-[13px]", children: "No secrets configured" })) : (_jsxs("div", { className: "[&>label]:text-muted grid grid-cols-[1fr_auto_1fr_auto_auto] items-end gap-1 [&>label]:mb-0.5 [&>label]:text-[11px]", children: [_jsx("label", { children: "Key" }), _jsx("label", { children: "Type" }), _jsx("label", { children: "Value" }), _jsx("div", {}), _jsx("div", {}), secrets.map((secret, index) => {
