...
const KEYBIND_DISPLAY_ALTERNATES = {
    OPEN_COMMAND_PALETTE: ["OPEN_COMMAND_PALETTE_ALT"],
};
export function KeybindsSection() {
    return (_jsx("div", { className: "space-y-6", children: KEYBIND_GROUPS.map((group) => (_jsxs("div", { children: [_jsx("h3", { className: "text-foreground mb-3 text-sm font-medium", children: group.label }), _jsx("div", { className: "space-y-1", children: group.keys.map((key) => (_jsxs("div", { className: "flex items-center justify-between rounded px-2 py-1.5 text-sm", children: [_jsx("span", { className: "text-muted", children: KEYBIND_LABELS[key] }), _jsx("kbd", { className: "bg-background-secondary text-foreground border-border-medium rounded border px-2 py-0.5 font-mono text-xs", children: [key, ...(KEYBIND_DISPLAY_ALTERNATES[key] ?? [])]
                                    .map((keybindId) => formatKeybind(KEYBINDS[keybindId]))
                                    .join(" / ") })] }, key))) })] }, group.label))) }));
}
//# sourceMappingURL=KeybindsSection.js.map