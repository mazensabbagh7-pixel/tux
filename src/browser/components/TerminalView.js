...
import { DEFAULT_TERMINAL_FONT_CONFIG, TERMINAL_FONT_CONFIG_KEY, } from "@/common/constants/storage";
import { useTerminalRouter } from "@/browser/terminal/TerminalRouterContext";
import { appendTerminalIconFallback, formatCssFontFamilyList, isFontFamilyAvailableInBrowser, isGenericFontFamily, splitFontFamilyList, stripOuterQuotes, TERMINAL_ICON_FALLBACK_FAMILY, } from "@/browser/terminal/terminalFontFamily";
...
function canLoadFontFamily(primary, fontSize) {
...
function resolveTerminalFontFamily(fontFamily, fontSize) {
...
async function preloadTerminalWebfonts(resolvedFontFamily, fontSize) {
    ...
    try {
        await Promise.all([
            ...
            fontFaceSet.load(`${fontSize}px ${formatCssFontFamilyList(TERMINAL_ICON_FALLBACK_FAMILY)}`, TERMINAL_ICON_LOAD_TEST_STRING),
        ]);
    }
    catch (err) {
...
export function TerminalView({ workspaceId, sessionId, visible, setDocumentTitle = true, onTitleChange, onAutoFocusConsumed, autoFocus = true, onExit, }) {
    ...
    const [rawTerminalFontConfig] = usePersistedState(TERMINAL_FONT_CONFIG_KEY, DEFAULT_TERMINAL_FONT_CONFIG, { listener: true });
    ...
    const disposeOnDataRef = useRef(null);
    const disposeOnTitleChangeRef = useRef(null);
    ...
    useEffect(() => {
        if (!visible)
            return;
        if (termRef.current || initInProgressRef.current)
            return;
        const containerEl = containerRef.current;
        if (!containerEl) {
            return;
        ...
        const initTerminal = async () => {
            try {
                ...
                const resolvedFontFamily = resolveTerminalFontFamily(terminalFontConfig.fontFamily, terminalFontConfig.fontSize);
                ...
                term.attachCustomKeyEventHandler((ev) => {
                    ...
                    // Paste shortcuts
                    const isPaste = (isMac && ev.metaKey && key === "v") ||
    ...
    useEffect(() => {
        ...
        const doResize = async () => {
            ...
            if (!fitAddonRef.current || !activeRouter || activeRouter !== router)
                return;
    ...
    return (_jsxs("div", { className: "terminal-view", style: {
            ...
            position: "relative",
        }, onClick: handleContainerClick, children: [errorMessage && (_jsxs("div", { className: "border-b border-red-900/30 bg-red-900/20 p-2 text-sm text-red-400", children: ["Terminal Error: ", errorMessage] })), _jsx("div", { ref: containerRef, className: "terminal-container", [TERMINAL_CONTAINER_ATTR]: true, style: {
                    flex: 1,
                    minHeight: 0,
                    width: "100%",
                    overflow: "hidden",
                    ...
                    backgroundColor: "var(--color-terminal-bg)",
                }, children: _jsx("span", { className: "text-muted animate-pulse text-sm", children: "Connecting..." }) }))] }));
}
