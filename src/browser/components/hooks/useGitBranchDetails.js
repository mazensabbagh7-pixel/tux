...
import { z } from "zod";
import { parseGitShowBranch, } from "@/common/utils/git/parseGitLog";
...
function extractSection(output, startMarker, endMarker) {
    const startIndex = output.indexOf(startMarker);
    const endIndex = output.indexOf(endMarker);
    if (startIndex === -1 || endIndex === -1 || endIndex < startIndex) {
        debugAssert(false, `Expected script output to contain markers ${startMarker} and ${endMarker}, but it did not.`);
        return null;
    }
...
function parseGitBranchScriptOutput(rawOutput) {
    const normalizedOutput = rawOutput.replace(/\r\n/g, "\n");
    const showBranch = extractSection(normalizedOutput, SECTION_MARKERS.showBranchStart, SECTION_MARKERS.showBranchEnd);
    if (showBranch === null) {
        return { success: false, error: "Missing branch details from git script output." };
    }
    const datesRaw = extractSection(normalizedOutput, SECTION_MARKERS.datesStart, SECTION_MARKERS.datesEnd);
    if (datesRaw === null) {
        return { success: false, error: "Missing commit dates from git script output." };
    }
    const dirtyRaw = extractSection(normalizedOutput, SECTION_MARKERS.dirtyStart, SECTION_MARKERS.dirtyEnd);
    if (dirtyRaw === null) {
 ...
 */
export function useGitBranchDetails(workspaceId, gitStatus, enabled) {
    debugAssert(workspaceId.trim().length > 0, "useGitBranchDetails expects a non-empty workspaceId argument.");
    const { api } = useAPI();
    const [branchHeaders, setBranchHeaders] = useState(null);
...
`;
            ...
            const dateMap = new Map(gitData.dates.map((d) => [d.hash, d.date]));
            ...
            setErrorMessage(`Failed to fetch branch info: ${error instanceof Error ? error.message : String(error)}`);
            setCommits(null);
        }
        finally {
            setIsLoading(false);
...
//# sourceMappingURL=useGitBranchDetails.js.map