...
describe("extractHookOutput", () => {
    ...
    test("extracts hook_output when present and non-empty", () => {
        ...
        expect(extractHookOutput({ success: true, hook_output: "formatter ran" })).toBe("formatter ran");
    });
...
describe("extractHookDuration", () => {
    ...
    test("extracts hook_duration_ms alongside other fields", () => {
        expect(extractHookDuration({ hook_output: "output", hook_duration_ms: 456, success: true })).toBe(456);
    });
});
//# sourceMappingURL=HookOutputDisplay.test.js.map