...
const PowerModeContext = createContext(null);
...
function ensureMirror(mirrorRef) {
...
function syncMirrorStyles(textarea, mirror) {
    ...
    const contentWidthPx = Math.max(0, textarea.clientWidth - mirror.paddingLeftPx - mirror.paddingRightPx);
    const contentHeightPx = Math.max(0, textarea.clientHeight - mirror.paddingTopPx - mirror.paddingBottomPx);
...
function getCaretViewportPosition(textarea, mirror, caretIndex) {
    try {
...
export function PowerModeProvider(props) {
    ...
    useEffect(() => {
        const onKeyDown = (e) => {
            if (e.repeat)
                return;
            if (!matchesKeybind(e, KEYBINDS.TOGGLE_POWER_MODE))
                return;
            e.preventDefault();
    ...
    const burstFromTextarea = useCallback((textarea, intensity = 1, kind = "insert", caretIndex) => {
        if (!enabled)
            return;
        const engine = engineRef.current;
        const rect = textarea.getBoundingClientRect();
    ...
    return (_jsxs(PowerModeContext.Provider, { value: value, children: [props.children, enabled && _jsx(PowerModeOverlay, { engine: engineRef.current })] }));
}
//# sourceMappingURL=PowerModeContext.js.map