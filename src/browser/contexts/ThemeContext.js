...
function applyThemeFavicon(theme) {
    ...
    const favicon = document.querySelector('link[rel="icon"][data-theme-icon]');
    if (!favicon) {
        return;
    }
...
export function ThemeProvider({ children, forcedTheme, }) {
    ...
    const [persistedTheme, setTheme] = usePersistedState(UI_THEME_KEY, resolveSystemTheme(), {
        listener: true,
    });
    ...
    const theme = isNestedUnderForcedProvider && parentContext
        ? parentContext.theme
