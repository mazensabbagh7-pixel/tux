...
import { SELECTED_WORKSPACE_KEY, getModelKey, getThinkingLevelKey, } from "@/common/constants/storage";
// Mock API
...
const createWorkspaceMetadata = (overrides) => ({
    projectPath: "/test",
    projectName: "test",
    name: "main",
...
describe("WorkspaceContext", () => {
    ...
    test("syncs workspace store subscriptions when metadata loads", async () => {
        ...
        await waitFor(() => expect(workspaceApi.onChat.mock.calls.some(([{ workspaceId }]) => workspaceId === "ws-sync-load")).toBe(true));
    });
    ...
    test("switches selection to parent when selected child workspace is deleted", async () => {
        ...
        const { workspace: workspaceApi } = createMockAPI({
            workspace: {
                ...
                onMetadata: () => Promise.resolve((async function* () {
    ...
    test("seeds model + thinking localStorage from backend metadata", async () => {
        ...
        const ctx = await setup();
        await waitFor(() => expect(ctx().workspaceMetadata.size).toBe(1));
        expect(JSON.parse(globalThis.localStorage.getItem(getModelKey("ws-ai")))).toBe("openai:gpt-5.2");
        expect(JSON.parse(globalThis.localStorage.getItem(getThinkingLevelKey("ws-ai")))).toBe("xhigh");
    });
    ...
    test("createWorkspace creates new workspace and reloads data", async () => {
        ...
        workspaceApi.create.mockResolvedValue({ success: true, metadata: newMetadata });
    ...
    test("selectedWorkspace persists to localStorage", async () => {
        ...
        await waitFor(() => expect(localStorage.getItem(SELECTED_WORKSPACE_KEY)).toContain("ws-persist"));
    });
    ...
    test("launch project does not override pending workspace creation", async () => {
        ...
        let resolveLaunchProject;
        const launchProjectPromise = new Promise((resolve) => {
...
async function setup() {
    ...
    render(_jsx(RouterProvider, { children: _jsx(ProjectProvider, { children: _jsx(WorkspaceProvider, { children: _jsx(ContextCapture, {}) }) }) }));
    // Inject client immediately to handle race conditions where effects run before store update
    getWorkspaceStoreRaw().setClient(currentClientMock);
...
function createMockAPI(options = {}) {
    ...
    // Create mocks
    const workspace = {
        create: mock(options.workspace?.create ??
            (() => Promise.resolve({
                success: true,
                metadata: createWorkspaceMetadata({ id: "ws-1" }),
            }))),
        list: mock(options.workspace?.list ?? (() => Promise.resolve([]))),
        remove: mock(options.workspace?.remove ?? (() => Promise.resolve({ success: true }))),
        rename: mock(options.workspace?.rename ??
            (() => Promise.resolve({ success: true, data: { newWorkspaceId: "ws-1" } }))),
        updateTitle: mock(options.workspace?.updateTitle ??
        ...
        onMetadata: mock(options.workspace?.onMetadata ??
            (async () => {
        ...
        onChat: mock(options.workspace?.onChat ??
            (async () => {
                await Promise.resolve();
                return (
                // eslint-disable-next-line require-yield
        ...
        activity: {
            ...
            subscribe: mock(options.workspace?.activity?.subscribe ??
                (async () => {
                    await Promise.resolve();
                    return (
                    // eslint-disable-next-line require-yield
        ...
        truncateHistory: mock(() => Promise.resolve({ success: true, data: undefined })),
        interruptStream: mock(() => Promise.resolve({ success: true, data: undefined })),
    };
    const projects = {
