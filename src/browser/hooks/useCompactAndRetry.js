...
import { getExplicitCompactionSuggestion, getHigherContextCompactionSuggestion, } from "@/browser/utils/compaction/suggestion";
...
function findTriggerUserMessage(messages) {
 ...
 */
function buildFollowUpFromSource(source) {
    return {
        text: source.content,
...
export function useCompactAndRetry(props) {
    ...
    const autoCompactionAttemptRef = useRef(null);
    useEffect(() => {
        return () => {
    ...
    const isCompactionRecoveryFlow = lastMessage?.type === "stream-error" && !!triggerUserMessage?.compactionRequest;
    const isContextExceeded = lastMessage?.type === "stream-error" && lastMessage.errorType === "context_exceeded";
     ...
     */
    const retryWithCompaction = useCallback(async () => {
        const insertIntoChatInput = (text, fileParts) => {
        ...
        try {
            ...
            if (source.compactionRequest) {
                ...
                if (!result.success) {
                    ...
                    insertIntoChatInput(fallbackText + (shouldAppendNewline ? "\n" : ""), nestedFollowUp?.fileParts);
                }
                return;
            }
    ...
    useEffect(() => {
        if (!shouldAutoCompact || !lastMessage)
            return;
        if (autoCompactionAttemptRef.current === lastMessage.id)
            return;
...
//# sourceMappingURL=useCompactAndRetry.js.map