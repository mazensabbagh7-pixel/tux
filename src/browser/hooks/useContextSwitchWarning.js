...
import { checkContextSwitch, findPreviousModel, } from "@/browser/utils/compaction/contextSwitchCheck";
...
const createSwitchState = (model) => ({
    currentModel: model,
    pending: null,
    warning: null,
});
// Avoid re-dispatching identical warnings when policy/config refreshes churn.
const areWarningsEqual = (a, b) => {
    if (a === b)
        return true;
    if (!a || !b)
        return false;
    return (a.currentTokens === b.currentTokens &&
...
export function useContextSwitchWarning(props) {
    const { workspaceId, messages, pendingModel, use1M, workspaceUsage, api, pendingSendOptions } = props;
    ...
    const checkOptions = useMemo(() => ({ providersConfig, policy: effectivePolicy }), [providersConfig, effectivePolicy]);
    const prevCheckOptionsRef = useRef(checkOptions);
    const prevWarningPreviousModelRef = useRef(null);
    ...
    const enhanceWarning = useCallback((w) => {
        if (!w || w.compactionModel)
            return w;
    ...
    const evaluateWarning = useCallback((options) => {
        if (options.tokens === 0) {
            return null;
        }
        const previousModel = options.previousModel ?? findPreviousModel(messages);
        prevWarningPreviousModelRef.current = previousModel;
        const result = checkContextSwitch(options.tokens, options.targetModel, previousModel, use1M, checkOptions, { allowSameModel: options.allowSameModel });
        return enhanceWarning(result);
    }, [checkOptions, enhanceWarning, messages, use1M]);
    const queueExplicitSwitch = useCallback((options) => {
    ...
    const handleCompact = useCallback(() => {
        if (!api || !warning?.compactionModel)
            return;
...
//# sourceMappingURL=useContextSwitchWarning.js.map