...
describe("useGateway", () => {
    ...
    test("toggleModelGateway flips gateway-models membership once per call", async () => {
        ...
        expect(JSON.parse(globalThis.window.localStorage.getItem("gateway-models") ?? "[]")).toContain(modelId);
        act(() => result.current.toggleModelGateway(modelId));
        expect(result.current.modelUsesGateway(modelId)).toBe(false);
        expect(JSON.parse(globalThis.window.localStorage.getItem("gateway-models") ?? "[]")).not.toContain(modelId);
    });
});
//# sourceMappingURL=useGatewayModels.test.js.map