 ...
 */
function evictOldestEntries(firstSeen, maxCount) {
 ...
 */
export function useHunkFirstSeen(workspaceId) {
    const [state, setState] = usePersistedState(getHunkFirstSeenKey(workspaceId), { firstSeen: {} });
    // Track pending updates to batch them
    const pendingUpdatesRef = useRef(new Set());
    const updateScheduledRef = useRef(false);
    const getFirstSeen = useCallback((hunkId) => {
        return state.firstSeen[hunkId];
    }, [state.firstSeen]);
    const recordFirstSeen = useCallback((hunkIds) => {
...
//# sourceMappingURL=useHunkFirstSeen.js.map