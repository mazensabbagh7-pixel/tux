...
import { executeCompaction as executeCompactionDefault, } from "@/browser/utils/chatCommands";
 ...
 */
export function useIdleCompactionHandler(params) {
    const { api, _executeCompaction = executeCompactionDefault, _getSendOptionsFromStorage } = params;
    const readSendOptions = _getSendOptionsFromStorage ?? getSendOptionsFromStorage;
    // Track which workspaces we've triggered compaction for (to prevent duplicates)
    const triggeredWorkspacesRef = useRef(new Set());
    ...
    useEffect(() => {
        ...
        const handleIdleCompactionNeeded = (workspaceId) => {
...
//# sourceMappingURL=useIdleCompactionHandler.js.map