...
import { filterHiddenModels, getSuggestedModels, useModelsFromSettings, } from "./useModelsFromSettings";
import { KNOWN_MODELS } from "@/common/constants/knownModels";
import { GATEWAY_ENABLED_KEY, GATEWAY_MODELS_KEY, HIDDEN_MODELS_KEY, } from "@/common/constants/storage";
...
describe("useModelsFromSettings provider availability gating", () => {
    ...
    test("keeps gateway-opted-in models visible when gateway is active", () => {
        ...
        globalThis.window.localStorage.setItem(GATEWAY_MODELS_KEY, JSON.stringify([KNOWN_MODELS.OPUS.id]));
        const { result } = renderHook(() => useModelsFromSettings());
    ...
    test("keeps persisted hiddenModels separate from provider-hidden models", () => {
        globalThis.window.localStorage.setItem(HIDDEN_MODELS_KEY, JSON.stringify([KNOWN_MODELS.GPT.id]));
        providersConfig = {
            anthropic: { apiKeySet: false, isEnabled: true, isConfigured: false },
            openai: { apiKeySet: true, isEnabled: true, isConfigured: true },
        };
...
//# sourceMappingURL=useModelsFromSettings.test.js.map