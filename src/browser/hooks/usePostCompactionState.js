...
function loadFromCache(wsId) {
    const cached = readPersistedState(getPostCompactionStateKey(wsId), null);
    return {
 ...
 */
export function usePostCompactionState(workspaceId) {
    const { api } = useAPI();
    ...
    useEffect(() => {
        ...
        const fetchState = async () => {
            try {
                ...
                // Cache for next time
                updatePersistedState(getPostCompactionStateKey(workspaceId), {
                    planPath: result.planPath,
                    trackedFilePaths: result.trackedFilePaths,
    ...
    const toggleExclusion = useCallback(async (itemId) => {
        if (!api)
            return;
...
//# sourceMappingURL=usePostCompactionState.js.map