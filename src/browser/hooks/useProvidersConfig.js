...
export function useProvidersConfig() {
     ...
     */
    const updateOptimistically = useCallback((provider, updates) => {
     ...
     */
    const updateModelsOptimistically = useCallback((provider, updater) => {
        // Invalidate any in-flight fetches so they don't overwrite our optimistic update
        fetchVersionRef.current++;
        const prev = configRef.current;
        if (!prev)
...
//# sourceMappingURL=useProvidersConfig.js.map