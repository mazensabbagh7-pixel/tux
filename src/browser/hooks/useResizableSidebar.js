...
export function useResizableSidebar({ enabled, defaultWidth, minWidth, maxWidth, getMaxWidthPx, storageKey, side = "right", }) {
    ...
    const startXRef = useRef(0); // Mouse X position when drag started
    const startWidthRef = useRef(0); // Sidebar width when drag started
    ...
    const resolveMaxWidthPx = useCallback(() => {
        ...
        const resolved = typeof candidate === "number" && Number.isFinite(candidate)
    ...
    useEffect(() => {
        if (!enabled)
            return;
        updatePersistedState(storageKey, width);
