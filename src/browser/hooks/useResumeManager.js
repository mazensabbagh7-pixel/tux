...
import { getInterruptionContext, isNonRetryableSendError, } from "@/browser/utils/messages/retryEligibility";
import { applyCompactionOverrides } from "@/browser/utils/messages/compactionOptions";
import { createFailedRetryState, calculateBackoffDelay, INITIAL_DELAY, } from "@/browser/utils/messages/retryState";
...
export function useResumeManager() {
    ...
    const workspaceStatesRef = useRef(new Map());
    useEffect(() => {
     ...
     */
    const isEligibleForResume = (workspaceId) => {
        ...
        if (state.canInterrupt)
            return false; // Currently streaming
        const { isEligibleForAutoRetry } = getInterruptionContext(state.messages, state.pendingStreamStartTime, state.runtimeStatus, state.lastAbortReason);
        ...
        if (retryingRef.current.has(workspaceId))
            return false;
        // 4. Check if previous error was non-retryable (e.g., api_key_not_found)
        const retryState = readPersistedState(getRetryStateKey(workspaceId), { attempt: 0, retryStartTime: Date.now() - INITIAL_DELAY } // Make immediately eligible on first check
        );
...
//# sourceMappingURL=useResumeManager.js.map