...
import { DEFAULT_MODEL_KEY, getModelKey, PREFERRED_SYSTEM_1_MODEL_KEY, PREFERRED_SYSTEM_1_THINKING_LEVEL_KEY, } from "@/common/constants/storage";
 ...
 */
export function useSendMessageOptions(workspaceId) {
    ...
    const programmaticToolCalling = useExperimentOverrideValue(EXPERIMENT_IDS.PROGRAMMATIC_TOOL_CALLING);
    const programmaticToolCallingExclusive = useExperimentOverrideValue(EXPERIMENT_IDS.PROGRAMMATIC_TOOL_CALLING_EXCLUSIVE);
    const system1 = useExperimentOverrideValue(EXPERIMENT_IDS.SYSTEM_1);
    const execSubagentHardRestart = useExperimentOverrideValue(EXPERIMENT_IDS.EXEC_SUBAGENT_HARD_RESTART);
    const [preferredSystem1Model] = usePersistedState(PREFERRED_SYSTEM_1_MODEL_KEY, "", {
        listener: true,
    });
    const system1Model = normalizeSystem1Model(preferredSystem1Model);
    const [preferredSystem1ThinkingLevel] = usePersistedState(PREFERRED_SYSTEM_1_THINKING_LEVEL_KEY, "off", { listener: true });
...
//# sourceMappingURL=useSendMessageOptions.js.map