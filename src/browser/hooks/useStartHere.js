 ...
 */
export function useStartHere(workspaceId, content, isCompacted = false, options) {
    const { api } = useAPI();
    const [isModalOpen, setIsModalOpen] = useState(false);
    const [isStartingHere, setIsStartingHere] = useState(false);
    // Opens the confirmation modal
    const openModal = () => {
        if (!workspaceId || isCompacted)
            return;
        setIsModalOpen(true);
    };
    // Closes the modal
    ...
    const executeStartHere = async () => {
        if (!workspaceId || isStartingHere || isCompacted || !api)
            return;
        setIsStartingHere(true);
        try {
            const summaryMessage = createMuxMessage(`start-here-${Date.now()}-${Math.random().toString(36).substring(2, 11)}`, "assistant", content, {
                timestamp: Date.now(),
                compacted: "user",
...
//# sourceMappingURL=useStartHere.js.map