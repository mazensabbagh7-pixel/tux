 ...
 */
export function useUnreadTracking(selectedWorkspace) {
    const didMigrateRef = useRef(false);
    useEffect(() => {
        if (didMigrateRef.current)
            return;
        didMigrateRef.current = true;
        const legacy = readPersistedState(LEGACY_LAST_READ_KEY, {});
        ...
        for (const [workspaceId, timestamp] of entries) {
            ...
            const existing = readPersistedState(nextKey, undefined);
    ...
    const markAsRead = useCallback((workspaceId) => {
...
//# sourceMappingURL=useUnreadTracking.js.map