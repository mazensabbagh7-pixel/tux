 ...
 */
function buildCandidates(isGatewayActive, modelUsesGateway, userModel) {
    const candidates = [];
    // 1. Preferred models with gateway prefs applied
    for (const modelId of PREFERRED_MODELS) {
        const transformed = isGatewayActive && modelUsesGateway(modelId) ? formatAsGatewayModel(modelId) : modelId;
 ...
 */
export function getDisplayTitleFromPersistedState(state) {
 ...
 */
export function useWorkspaceName(options) {
    ...
    const candidates = useMemo(() => buildCandidates(isGatewayActive, modelUsesGateway, userModel), [isGatewayActive, modelUsesGateway, userModel]);
    ...
    const [persistedState, setPersistedState] = usePersistedState(persistedKey, DEFAULT_PERSISTED_STATE, { listener: true });
    const [localState, setLocalState] = useState(DEFAULT_PERSISTED_STATE);
    ...
    const generateIdentity = useCallback(async (forMessage) => {
        ...
        try {
            ...
            const errorMsg = result.error.type === "unknown" && "raw" in result.error
                ? result.error.raw
    ...
    const handleSetAutoGenerate = useCallback((enabled) => {
        if (enabled) {
    ...
    const setNameManual = useCallback((newName) => {
...
//# sourceMappingURL=useWorkspaceName.js.map