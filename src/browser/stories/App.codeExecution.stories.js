...
import { STABLE_TIMESTAMP, createUserMessage, createAssistantMessage, createCodeExecutionTool, createPendingCodeExecutionTool, } from "./mockFactory";
    ...
    render: () => (_jsx(AppWithMocks, { setup: () => setupSimpleChatStory({
            messages: [
                ...
                createAssistantMessage("msg-2", "I'll update the config file to enable debug mode.", {
                    ...
                    toolCalls: [
                        createCodeExecutionTool("call-1", SAMPLE_CODE, {
                            success: true,
                            result: "Done!",
                            toolCalls: [
                                {
    ...
    render: () => (_jsx(AppWithMocks, { setup: () => setupSimpleChatStory({
            messages: [
                ...
                createAssistantMessage("msg-2", "I'll run that code.", {
                    ...
                    timestamp: STABLE_TIMESTAMP - 50000,
                    toolCalls: [
                        createCodeExecutionTool("call-1", `const result = await mux.bash({ script: "cat /nonexistent" });`, {
                            success: false,
    ...
    render: () => (_jsx(AppWithMocks, { setup: () => setupSimpleChatStory({
            messages: [
                ...
                createAssistantMessage("msg-2", "Computing fibonacci sequence.", {
                    ...
                    timestamp: STABLE_TIMESTAMP - 50000,
                    toolCalls: [
                        createCodeExecutionTool("call-1", `function fib(n) {
  if (n <= 1) return n;
    ...
    render: () => (_jsx(AppWithMocks, { setup: () => setupSimpleChatStory({
            messages: [
                ...
                createAssistantMessage("msg-2", "I'll try to read that file.", {
                    ...
                    timestamp: STABLE_TIMESTAMP - 50000,
                    toolCalls: [
                        createCodeExecutionTool("call-1", `const result = mux.file_read({ path: "nonexistent.ts" });
return result;`, {
                            success: false,
    ...
    render: () => (_jsx(AppWithMocks, { setup: () => setupSimpleChatStory({
            messages: [
                ...
                createAssistantMessage("msg-2", "Running analysis.", {
                    ...
                    timestamp: STABLE_TIMESTAMP - 50000,
                    toolCalls: [
                        createCodeExecutionTool("call-1", `// Analysis script with various syntax elements
...
return results;`, {
    ...
    play: async ({ canvasElement }) => {
        ...
        await waitFor(() => {
            ...
            if (!showCodeBtn)
                throw new Error("Show Code button not found");
            return showCodeBtn;
        });
        const buttons = canvasElement.querySelectorAll('button[type="button"]');
        const showCodeBtn = Array.from(buttons).find((btn) => {
        ...
        await waitFor(() => {
            ...
            if (!codeContainer)
                throw new Error("Code view not displayed");
        });
    },
};
//# sourceMappingURL=App.codeExecution.stories.js.map