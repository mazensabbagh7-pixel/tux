...
import { STABLE_TIMESTAMP, createWorkspace, createIncompatibleWorkspace, groupWorkspacesByProject, createUserMessage, createAssistantMessage, createFileEditTool, createStaticChatHandler, } from "./mockFactory";
import { disableAutoRetryPreference } from "@/browser/utils/messages/autoRetryPreference";
import { collapseRightSidebar, createOnChatAdapter, expandProjects, selectWorkspace, setupCustomChatStory, setupSimpleChatStory, } from "./storyHelpers";
...
// ═══════════════════════════════════════════════════════════════════════════════
const createDebugLlmRequestSnapshot = (workspaceId) => ({
    ...
    systemMessage: "You are Mux, a focused coding agent. Follow the user’s instructions and keep answers short.",
    messages: [
        {
            role: "user",
 ...
 */
export const ContextExceededSuggestion = {
    render: () => (_jsx(AppWithMocks, { setup: () => {
            ...
            return setupCustomChatStory({
                ...
                chatHandler: (callback) => {
                    setTimeout(() => {
                        callback(createUserMessage("msg-1", "Can you help me with this huge codebase?", {
                            historySequence: 1,
                        ...
                        callback({
                            type: "stream-error",
                            messageId: "assistant-1",
                            error: "Context length exceeded: the conversation is too long to send to this model.",
...
export const DebugLlmRequestModal = {
    render: () => (_jsx(AppWithMocks, { setup: () => {
            ...
            const chatHandlers = new Map([
                [
                    ...
                    (callback) => {
                        setTimeout(() => {
                            callback(createUserMessage("msg-1", "Can you summarize what just happened?", {
                                historySequence: 1,
                                timestamp: STABLE_TIMESTAMP - 100000,
                            }));
                            callback({ type: "caught-up" });
    ...
    play: async ({ canvasElement }) => {
        await waitFor(() => {
            const debugButton = canvasElement.querySelector('button[aria-label="Open last LLM request debug modal"]');
            if (!debugButton)
...
export const StreamError = {
    render: () => (_jsx(AppWithMocks, { setup: () => {
            ...
            return setupCustomChatStory({
                ...
                chatHandler: (callback) => {
                    setTimeout(() => {
                        callback(createUserMessage("msg-1", "Help me refactor the database layer", {
                            historySequence: 1,
                            timestamp: STABLE_TIMESTAMP - 100000,
                        }));
                        callback({ type: "caught-up" });
...
export const AnthropicOverloaded = {
    render: () => (_jsx(AppWithMocks, { setup: () => {
            ...
            return setupCustomChatStory({
                ...
                chatHandler: (callback) => {
                    setTimeout(() => {
                        callback(createUserMessage("msg-1", "Why did my request fail?", {
                            historySequence: 1,
                            timestamp: STABLE_TIMESTAMP - 100000,
                        }));
                        callback({ type: "caught-up" });
...
export const HiddenHistory = {
    render: () => (_jsx(AppWithMocks, { setup: () => {
            ...
            const messages = [
                ...
                createAssistantMessage("msg-2", "Based on our previous conversation, we discussed implementing authentication, adding tests, and refactoring the database layer.", {
                    historySequence: 44,
 ...
 */
export const IncompatibleWorkspace = {
    render: () => (_jsx(AppWithMocks, { setup: () => {
            const workspaces = [
...
export const LargeDiff = {
    render: () => (_jsx(AppWithMocks, { setup: () => setupSimpleChatStory({
            workspaceId: "ws-diff",
            messages: [
                createUserMessage("msg-1", "Refactor the user API with proper auth and error handling", {
                    historySequence: 1,
                    timestamp: STABLE_TIMESTAMP - 100000,
                }),
                createAssistantMessage("msg-2", "I've refactored the user API with authentication, validation, and proper error handling:", {
                    historySequence: 2,
                    timestamp: STABLE_TIMESTAMP - 90000,
 ...
 */
export const ProjectRemovalError = {
    render: () => (_jsx(AppWithMocks, { setup: () => {
            const workspaces = [
            ...
            return createMockORPCClient({
                ...
                onProjectRemove: () => ({
                    success: false,
                    error: "Cannot remove project with active workspaces. Please remove all 2 workspace(s) first.",
                }),
            });
        } })),
    play: async ({ canvasElement }) => {
        ...
        await waitFor(() => {
            ...
            if (!errorPopover)
                throw new Error("Error popover not found");
        });
    },
};
//# sourceMappingURL=App.errors.stories.js.map