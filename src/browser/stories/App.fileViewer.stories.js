...
import { setupSimpleChatStory, expandRightSidebar, setReviews, createReview, } from "./storyHelpers";
 ...
 */
function createFileViewerExecuteBash(fileContents, fileDiffs) {
    return (_workspaceId, script) => {
        const bashResult = (output, exitCode = 0) => Promise.resolve({
            success: true,
            output,
            exitCode,
            wall_duration_ms: 1,
...
function createFileTabLayout(filePath) {
...
export const TextFileNoChanges = {
    render: () => (_jsx(AppWithMocks, { setup: () => {
            const workspaceId = "ws-file-viewer-text";
            const filePath = "src/components/Button.tsx";
            localStorage.setItem(getRightSidebarLayoutKey(workspaceId), JSON.stringify(createFileTabLayout(filePath)));
...
export const TextFileWithDiff = {
    render: () => (_jsx(AppWithMocks, { setup: () => {
            ...
            setReviews(workspaceId, [
                createReview("review-range-1", filePath, "+1-4", "Double-check the updated imports.", "pending", baseTime),
            ]);
 ...
 */
export const BinaryFileError = {
    render: () => (_jsx(AppWithMocks, { setup: () => {
            const workspaceId = "ws-file-viewer-binary";
            const filePath = "build/app.exe";
            localStorage.setItem(getRightSidebarLayoutKey(workspaceId), JSON.stringify(createFileTabLayout(filePath)));
            localStorage.setItem(RIGHT_SIDEBAR_WIDTH_KEY, "500");
            const fileContents = new Map([["build/app.exe", { type: "binary" }]]);
            const client = setupSimpleChatStory({
                workspaceId,
...
//# sourceMappingURL=App.fileViewer.stories.js.map