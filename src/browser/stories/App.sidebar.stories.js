...
import { NOW, STABLE_TIMESTAMP, createWorkspace, createSSHWorkspace, createLocalWorkspace, createUserMessage, createStreamingChatHandler, groupWorkspacesByProject, createGitStatusOutput, } from "./mockFactory";
import { clearWorkspaceSelection, createOnChatAdapter, expandProjects, setWorkspaceDrafts, } from "./storyHelpers";
 ...
 */
function createGitStatusExecutor(gitStatus) {
    const buildBranchDetailsOutput = (status) => {
...
export const ManyWorkspaces = {
    render: () => (_jsx(AppWithMocks, { setup: () => {
            ...
            const workspaces = names.map((name, i) => createWorkspace({ id: `ws-${i}`, name, projectName: "big-app" }));
            return createMockORPCClient({
                projects: groupWorkspacesByProject(workspaces),
                workspaces,
            });
...
export const LongWorkspaceNames = {
    render: () => (_jsx(AppWithMocks, { setup: () => {
            const workspaces = [
                createWorkspace({
                    id: "ws-short",
 ...
 */
export const GitStatusVariations = {
    render: () => (_jsx(AppWithMocks, { setup: () => {
    ...
    play: async ({ canvasElement }) => {
        ...
        await waitFor(() => {
            const row = canvasElement.querySelector('[data-workspace-id="ws-diverged"]');
            if (!row)
                throw new Error("ws-diverged row not found");
        ...
        const getVisibleTooltip = () => document.body.querySelector('.bg-modal-bg[data-state="open"]');
        // Wait for tooltip (portaled) and toggle to commits mode
        await waitFor(() => {
            const tooltip = getVisibleTooltip();
            if (!tooltip)
                throw new Error("git status tooltip not visible");
        ...
        await waitFor(() => {
            const updatedRow = canvasElement.querySelector('[data-workspace-id="ws-diverged"]');
            if (!updatedRow)
                throw new Error("ws-diverged row not found");
 ...
 */
export const RuntimeBadgeVariations = {
    render: () => (_jsx(AppWithMocks, { setup: () => {
 ...
 */
export const WorkspaceTitleHoverCard = {
    render: () => (_jsx(AppWithMocks, { setup: () => {
    ...
    play: async ({ canvasElement }) => {
        ...
        await waitFor(() => {
            const row = canvasElement.querySelector('[data-workspace-id="ws-ssh-hover"]');
            if (!row)
                throw new Error("ws-ssh-hover row not found");
        });
        const row = canvasElement.querySelector('[data-workspace-id="ws-ssh-hover"]');
        ...
        await waitFor(() => {
            const hoverCard = document.body.querySelector("[data-radix-popper-content-wrapper] .bg-modal-bg");
            if (!hoverCard)
                throw new Error("HoverCard not visible");
 ...
 */
export const WorkspaceDraftSelected = {
    render: () => (_jsx(AppWithMocks, { setup: () => {
    ...
    play: async ({ canvasElement }) => {
        ...
        await waitFor(() => {
            const row = canvasElement.querySelector('[data-draft-id="selected-draft"]');
            if (!row)
                throw new Error("selected-draft row not found");
        });
        const row = canvasElement.querySelector('[data-draft-id="selected-draft"]');
 ...
 */
export const ArchivingWorkspaceAlignment = {
    render: () => (_jsx(AppWithMocks, { setup: () => {
    ...
    play: async ({ canvasElement }) => {
        ...
        await waitFor(() => {
            const row = canvasElement.querySelector('[data-workspace-id="ws-to-archive"]');
            if (!row)
                throw new Error("ws-to-archive row not found");
        });
        const row = canvasElement.querySelector('[data-workspace-id="ws-to-archive"]');
        ...
        await waitFor(() => {
            const archiveButton = document.body.querySelector("[data-radix-popper-content-wrapper] button");
            if (!archiveButton)
...
//# sourceMappingURL=App.sidebar.stories.js.map