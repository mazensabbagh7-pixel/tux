...
import { collapseRightSidebar, createOnChatAdapter, selectWorkspace, setupSimpleChatStory, } from "./storyHelpers";
import { waitForScrollStabilization } from "./storyPlayHelpers";
import { STABLE_TIMESTAMP, createUserMessage, createAssistantMessage, createBashTool, createPendingTool, createPendingTaskTool, createStaticChatHandler, createTaskTool, createCompletedTaskTool, createFailedTaskTool, createTaskApplyGitPatchTool, createTaskAwaitTool, createTaskListTool, createTaskTerminateTool, createWorkspace, groupWorkspacesByProject, } from "./mockFactory";
 ...
 */
export const TaskForegroundShowsTaskId = {
 ...
 */
export const TaskAwaitExecuting = {
    render: () => (_jsx(AppWithMocks, { setup: () => {
            const workspaceId = "ws-main";
            const projectName = "my-app";
            const mainWorkspace = createWorkspace({
                ...
                createAssistantMessage("a1", "I'll spawn a sub-agent to explore the test files.", {
                    ...
                    toolCalls: [
                        createCompletedTaskTool("tc1", {
                            subagent_type: "explore",
                            prompt: "Find all test files in this project. Look for patterns like *.test.ts, *.spec.ts, and test directories.",
 ...
 */
export const TaskTranscriptViewer = {
    render: () => (_jsx(AppWithMocks, { setup: () => {
            ...
            const transcriptMessages = [
                ...
                createAssistantMessage("ta1", "Here's what happens during cleanup:", {
                    historySequence: 2,
                    toolCalls: [
                        createBashTool("tcb1", "ls -la", "total 0\n-rw-r--r-- 1 user group 0 Jan 1 00:00 chat.jsonl", 0),
                    ],
                }),
            ];
                    ...
                    createAssistantMessage("a1", "Here's the task result:", {
                        ...
                        toolCalls: [
                            createCompletedTaskTool("tc1", {
                                ...
                                taskId,
                                reportMarkdown: "Report is trimmed for brevity. Click **View transcript** to inspect the full chat.",
    ...
    play: async ({ canvasElement }) => {
        ...
        let viewTranscriptButton = null;
        let searchNode = taskIdButton;
        while (searchNode) {
        ...
        await waitFor(() => {
            const dialog = Array.from(canvasElement.ownerDocument.body.querySelectorAll('[role="dialog"]')).find((el) => el.textContent?.includes(taskId));
            if (!dialog) {
 ...
 */
export const TaskApplyGitPatchStates = {
    render: () => (_jsx(AppWithMocks, { setup: () => setupSimpleChatStory({
...
async function playTaskApplyGitPatchCommitListStory(canvasElement, opts) {
    const getMessageWindow = () => {
        ...
        const candidates = Array.from(canvasElement.ownerDocument.querySelectorAll('[data-testid="message-window"]')).filter((candidate) => candidate instanceof HTMLElement);
        const match = candidates.find((candidate) => candidate.textContent?.includes(opts.expectedAssistantText));
        if (match) {
            return match;
        }
        const debugUrl = window.location.href;
        const debugText = candidates.at(0)?.textContent?.trim().slice(0, 200) ?? "<no text>";
        throw new Error(`Story not loaded yet: missing "${opts.expectedAssistantText}" (url=${debugUrl}, windows=${candidates.length}, firstText=${JSON.stringify(debugText)})`);
    };
    const isExpanded = (header) => header.querySelector("span.rotate-90") !== null;
    const getToolHeader = () => {
        const messageWindow = getMessageWindow();
        const matches = Array.from(messageWindow.querySelectorAll("div.cursor-pointer")).filter((candidate) => candidate.textContent?.includes("Apply patch") &&
 ...
 */
export const TaskApplyGitPatchCommitList = {
    render: () => (_jsx(AppWithMocks, { setup: () => setupSimpleChatStory({
 ...
 * Chromatic note: this story expands the tool card so the commit list is visible.
 */
export const TaskApplyGitPatchDryRunCommitList = {
    render: () => (_jsx(AppWithMocks, { setup: () => setupSimpleChatStory({
...
//# sourceMappingURL=App.task.stories.js.map