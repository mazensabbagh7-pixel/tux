...
function getSupportedMediaType(file) {
 ...
 */
export function chatAttachmentsToFileParts(attachments, options) {
    const validate = options?.validate ?? false;
    return attachments.map((attachment, index) => {
        if (validate) {
            if (!attachment.url || typeof attachment.url !== "string") {
                console.error(`Attachment [${index}] has invalid url:`, typeof attachment.url, attachment.url);
            }
            ...
            if (!attachment.mediaType || typeof attachment.mediaType !== "string") {
                console.error(`Attachment [${index}] has invalid mediaType:`, typeof attachment.mediaType, attachment.mediaType);
            }
        }
        return {
            url: attachment.url,
...
export async function fileToChatAttachment(file) {
    ...
    if (mediaType === SVG_MEDIA_TYPE) {
        ...
        if (svgText.length > MAX_SVG_TEXT_CHARS) {
            throw new Error(`SVG attachments must be ${MAX_SVG_TEXT_CHARS.toLocaleString()} characters or less (this one is ${svgText.length.toLocaleString()}).`);
        }
        return {
 ...
 */
export function extractAttachmentsFromClipboard(items) {
    const files = [];
 ...
 */
export function extractAttachmentsFromDrop(dataTransfer) {
    const files = [];
 ...
 */
export async function processAttachmentFiles(files) {
    return await Promise.all(files.map(fileToChatAttachment));
}
//# sourceMappingURL=attachmentsHandling.js.map