 ...
 */
import { isDefaultSourceContent, pickPreservedSendOptions, } from "@/common/types/message";
import { RUNTIME_MODE, parseRuntimeModeAndHost } from "@/common/types/runtime";
import { CUSTOM_EVENTS, createCustomEvent } from "@/common/constants/events";
import { KNOWN_MODELS } from "@/common/constants/knownModels";
import { WORKSPACE_ONLY_COMMAND_KEYS, WORKSPACE_ONLY_COMMAND_TYPES, } from "@/constants/slashCommands";
import { formatCompactionCommandLine, getFollowUpContentText, } from "@/browser/utils/compaction/format";
...
// Workspace Creation
// ============================================================================
import { createCommandToast, createInvalidCompactModelToast, } from "@/browser/components/ChatInputToasts";
import { trackCommandUsed } from "@/common/telemetry";
import { addEphemeralMessage } from "@/browser/stores/WorkspaceStore";
const BUILT_IN_MODEL_SET = new Set(Object.values(KNOWN_MODELS).map((model) => model.id));
/**
 ...
 */
export async function forkWorkspace(options) {
 ...
 */
export async function processSlashCommand(parsed, context) {
    if (!parsed)
    ...
    const isWorkspaceOnlyCommand = isWorkspaceCommandType ||
...
// Command Handlers
// ============================================================================
async function handleClearCommand(_parsed, context) {
    const { setInput, onTruncateHistory, resetInputHeight, setToast } = context;
    setInput("");
    resetInputHeight();
    if (!onTruncateHistory)
        return { clearInput: true, toastShown: false };
    try {
...
async function handleTruncateCommand(parsed, context) {
    const { setInput, onTruncateHistory, resetInputHeight, setToast } = context;
    setInput("");
    resetInputHeight();
    if (!onTruncateHistory)
...
async function handleForkCommand(parsed, context) {
    const { api: client, workspaceId, sendMessageOptions, setInput, setSendingState, setToast, } = context;
    ...
    try {
        ...
        if (!workspaceId)
            throw new Error("Workspace ID required for fork");
        if (!client)
            throw new Error("Client required for fork");
 ...
 */
export function parseRuntimeString(runtime, _workspaceName) {
    ...
    if (parsed === null) {
        ...
        if (trimmed === RUNTIME_MODE.DEVCONTAINER || trimmed.startsWith("devcontainer")) {
            throw new Error("Dev container runtime requires a config path (e.g., 'devcontainer .devcontainer/devcontainer.json')");
        }
        throw new Error(`Unknown runtime type: '${runtime ?? ""}'. Use 'ssh <host>', 'docker <image>', 'devcontainer <config>', 'worktree', or 'local'`);
    }
 ...
 */
export async function createNewWorkspace(options) {
 ...
 */
export function formatNewCommand(workspaceName, trunkBranch, runtime, startMessage) {
 ...
 */
export function prepareCompactionMessage(options) {
    ...
    if (options.followUpContent) {
        ...
        const existingModel = "model" in options.followUpContent &&
            typeof options.followUpContent.model === "string" &&
            options.followUpContent.model
            ? options.followUpContent.model
            : undefined;
        const existingAgentId = "agentId" in options.followUpContent &&
 ...
 */
export async function handleNewCommand(parsed, context) {
    const { api: client, workspaceId, sendMessageOptions, setInput, setSendingState, setToast, } = context;
 ...
 */
export async function handleCompactCommand(parsed, context) {
    const { api, workspaceId, sendMessageOptions, editMessageId, setInput, setAttachments, setSendingState, setToast, onCancelEdit, } = context;
    ...
    try {
        ...
        const hasContent = parsed.continueMessage ?? context.fileParts?.length ?? context.reviews?.length;
        const followUpContent = hasContent
            ? {
...
// ============================================================================
export async function handlePlanShowCommand(context) {
    ...
    const planMessage = {
        ...
        metadata: {
            ...
            muxMetadata: { type: "plan-display", path: result.data.path },
        },
    };
...
export async function handlePlanOpenCommand(context) {
    const { api, workspaceId, setInput, setToast } = context;
    setInput("");
