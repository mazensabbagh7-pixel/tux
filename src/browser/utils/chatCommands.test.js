...
import { parseRuntimeString, prepareCompactionMessage, handlePlanShowCommand, handlePlanOpenCommand, handleCompactCommand, buildContinueMessage, } from "./chatCommands";
...
describe("parseRuntimeString", () => {
    ...
    test("parses devcontainer runtime with config path", () => {
        const result = parseRuntimeString("devcontainer .devcontainer/devcontainer.json", workspaceName);
        expect(result).toEqual({
            type: "devcontainer",
    ...
    test("throws error for devcontainer without config path", () => {
        expect(() => parseRuntimeString("devcontainer", workspaceName)).toThrow("Dev container runtime requires a config path");
    });
    ...
    test("throws error for docker without image", () => {
        expect(() => parseRuntimeString("docker", workspaceName)).toThrow("Docker runtime requires image");
        expect(() => parseRuntimeString("docker ", workspaceName)).toThrow("Docker runtime requires image");
    });
    test("throws error for unknown runtime type", () => {
        expect(() => parseRuntimeString("remote", workspaceName)).toThrow("Unknown runtime type: 'remote'. Use 'ssh <host>', 'docker <image>', 'devcontainer <config>', 'worktree', or 'local'");
        expect(() => parseRuntimeString("kubernetes", workspaceName)).toThrow("Unknown runtime type: 'kubernetes'. Use 'ssh <host>', 'docker <image>', 'devcontainer <config>', 'worktree', or 'local'");
    });
});
...
describe("handlePlanShowCommand", () => {
    const createMockContext = (getPlanContentResult) => {
    ...
    test("shows error toast when no plan exists", async () => {
        ...
        expect(context.setToast).toHaveBeenCalledWith(expect.objectContaining({
            type: "error",
...
describe("handlePlanOpenCommand", () => {
    const createMockContext = (getPlanContentResult, openInEditorResult) => {
        const setInput = mock(() => undefined);
        const setToast = mock(() => undefined);
        return {
            workspaceId: "test-workspace-id",
            ...
            api: {
                ...
                general: {
                    openInEditor: mock(() => Promise.resolve(openInEditorResult ?? { success: true, data: undefined })),
                },
            },
            // Required fields for CommandHandlerContext
            sendMessageOptions: {
    ...
    test("opens plan in editor when plan exists", async () => {
        const context = createMockContext({ success: true, data: { content: "# My Plan", path: "/path/to/plan.md" } }, { success: true, data: undefined });
        const result = await handlePlanOpenCommand(context);
        expect(result.clearInput).toBe(true);
        expect(context.setInput).toHaveBeenCalledWith("");
        expect(context.api.workspace.getPlanContent).toHaveBeenCalledWith({
...
describe("handleCompactCommand", () => {
    const createMockContext = (sendMessageResult, options) => {
        const setInput = mock(() => undefined);
        const setToast = mock(() => undefined);
    ...
    test("passes reviews to followUpContent when reviews are attached", async () => {
        ...
        const sendMessageMock = context.api.workspace.sendMessage;
...
//# sourceMappingURL=chatCommands.test.js.map