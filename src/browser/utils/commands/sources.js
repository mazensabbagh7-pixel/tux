import { THEME_OPTIONS } from "@/browser/contexts/ThemeContext";
...
import { getRightSidebarLayoutKey, RIGHT_SIDEBAR_COLLAPSED_KEY, RIGHT_SIDEBAR_TAB_KEY, } from "@/common/constants/storage";
...
import { getEffectiveSlotKeybind, getLayoutsConfigOrDefault, getPresetForSlot, } from "@/browser/utils/uiLayouts";
import { addToolToFocusedTabset, getDefaultRightSidebarLayoutState, hasTab, parseRightSidebarLayoutState, selectTabInTabset, setFocusedTabset, splitFocusedTabset, toggleTab, } from "@/browser/utils/rightSidebarLayout";
/**
...
const getRightSidebarTabFallback = () => {
    const raw = readPersistedState(RIGHT_SIDEBAR_TAB_KEY, "costs");
    return isTabType(raw) ? raw : "costs";
};
const readRightSidebarLayout = (workspaceId) => {
    const fallback = getRightSidebarTabFallback();
    const raw = readPersistedState(getRightSidebarLayoutKey(workspaceId), getDefaultRightSidebarLayoutState(fallback));
    return parseRightSidebarLayoutState(raw, fallback);
};
const updateRightSidebarLayout = (workspaceId, updater) => {
    const fallback = getRightSidebarTabFallback();
    const defaultLayout = getDefaultRightSidebarLayoutState(fallback);
    updatePersistedState(getRightSidebarLayoutKey(workspaceId), (prev) => updater(parseRightSidebarLayoutState(prev, fallback)), defaultLayout);
};
...
const findFirstTerminalSessionTab = (node) => {
    if (node.type === "tabset") {
        const tab = node.tabs.find((t) => t.startsWith("terminal:") && t !== "terminal");
        return tab ? { tabsetId: node.id, tab } : null;
    }
    return (findFirstTerminalSessionTab(node.children[0]) ?? findFirstTerminalSessionTab(node.children[1]));
};
export function buildCoreSources(p) {
    const actions = [];
    ...
    const createWorkspaceForSelectedProjectAction = (selected) => {
        return {
    ...
    actions.push(() => {
        ...
        for (const meta of p.workspaceMetadata.values()) {
            ...
            if (isStreaming)
                secondaryParts.push("streaming");
            list.push({
                id: CommandIds.workspaceSwitch(meta.id),
                title: `${isCurrent ? "• " : ""}${primaryLabel}`,
                subtitle: secondaryParts.join(" · "),
                section: section.workspaces,
                keywords: [meta.name, meta.projectName, meta.namedWorkspacePath, meta.title].filter((k) => !!k),
        ...
        if (selected?.namedWorkspacePath) {
            ...
            list.push({
                ...
                section: section.workspaces,
                run: async () => {
                    const branchName = selectedMeta?.name ??
                        selected.namedWorkspacePath.split("/").pop() ??
                        selected.namedWorkspacePath;
                    const ok = confirm(`Remove current workspace? This will delete the worktree and local branch "${branchName}". This cannot be undone.`);
                    if (ok)
            ...
            list.push({
                ...
                prompt: {
                    ...
                    onSubmit: async (vals) => {
                        ...
                        const ok = confirm(`Remove workspace ${workspaceName}? This will delete the worktree and local branch "${branchName}". This cannot be undone.`);
                        if (ok) {
    ...
    actions.push(() => {
        ...
        if (wsId) {
            list.push({
                ...
                section: section.navigation,
                run: () => updateRightSidebarLayout(wsId, (s) => {
                    const found = findFirstTerminalSessionTab(s.root);
                    if (!found)
                        return s;
                    return selectTabInTabset(setFocusedTabset(s, found.tabsetId), found.tabsetId, found.tab);
                }),
            }, {
                    ...
                    fields: [
                        {
                            ...
                            getOptions: () => ["costs", "review", "output", "terminal"].map((tab) => ({
                                id: tab,
                                label: tab === "costs"
                                    ? "Costs"
                                    : tab === "review"
            ...
            list.push({
                ...
                prompt: {
                    ...
                    onSubmit: (vals) => {
                        ...
                        const level = THINKING_LEVELS.includes(rawLevel)
                            ? rawLevel
                            : "off";
