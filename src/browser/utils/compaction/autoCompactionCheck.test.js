...
const createUsageEntry = (contextTokens, model = KNOWN_MODELS.SONNET.id) => {
    ...
    return {
        ...
        output: { tokens: 1000 }, // Some output (doesn't affect context calculation)
        reasoning: { tokens: 0 },
        model,
    };
};
// Helper to create mock WorkspaceUsageState
const createMockUsage = (lastEntryTokens, _historicalTokens, // Kept for backward compat but unused (session-usage.json handles historical)
model = KNOWN_MODELS.SONNET.id, liveUsage) => {
...
describe("checkAutoCompaction", () => {
    const SONNET_MAX_TOKENS = 200000;
    ...
    describe("Basic Functionality", () => {
        ...
        test("returns false when model has no max_input_tokens (unknown model)", () => {
            const usage = createMockUsage(50000);
        ...
        test("returns false when usage is low (10%)", () => {
            const usage = createMockUsage(20000); // 10% of 200k
            const result = checkAutoCompaction(usage, KNOWN_MODELS.SONNET.id, false);
            expect(result.shouldShowWarning).toBe(false);
        ...
        test("returns true above threshold (80%)", () => {
            const usage = createMockUsage(160000); // 80% of 200k
            const result = checkAutoCompaction(usage, KNOWN_MODELS.SONNET.id, false);
            expect(result.shouldShowWarning).toBe(true);
    ...
    describe("Usage Calculation (Critical for infinite loop fix)", () => {
        test("uses last usage entry tokens, not cumulative sum", () => {
            const usage = createMockUsage(10000); // Only 5% of context
            const result = checkAutoCompaction(usage, KNOWN_MODELS.SONNET.id, false);
        ...
        test("handles historical usage correctly - ignores it in calculation", () => {
            ...
            const usage = createMockUsage(5000, 70000);
            const result = checkAutoCompaction(usage, KNOWN_MODELS.SONNET.id, false);
        ...
        test("includes cacheCreate in context calculation (prompt caching)", () => {
            ...
            const usageEntry = {
                input: { tokens: 10000 },
                cached: { tokens: 5000 },
                cacheCreate: { tokens: 2000 },
                output: { tokens: 3000 },
                reasoning: { tokens: 1000 },
        ...
        test("excludes output and reasoning tokens from context calculation (prevents compaction loops)", () => {
            ...
            const usageEntry = {
                input: { tokens: 20000 }, // Low actual context
                cached: { tokens: 0 },
                cacheCreate: { tokens: 0 },
                output: { tokens: 5000 },
                reasoning: { tokens: 50000 }, // High reasoning from Extended Thinking
                model: KNOWN_MODELS.SONNET.id,
            };
            const usage = {
                lastContextUsage: usageEntry,
    ...
    describe("1M Context Mode", () => {
        test("uses 1M tokens when use1M=true and model supports it (Sonnet 4)", () => {
            const usage = createMockUsage(600000); // 60% of 1M
        ...
        test("uses 1M tokens for Sonnet with use1M=true (model is claude-sonnet-4-5)", () => {
            const usage = createMockUsage(700000); // 70% of 1M
            const result = checkAutoCompaction(usage, KNOWN_MODELS.SONNET.id, true);
            expect(result.usagePercentage).toBe(70);
            expect(result.shouldShowWarning).toBe(true);
        });
        test("uses standard max_input_tokens when use1M=false", () => {
            const usage = createMockUsage(140000); // 70% of 200k
            const result = checkAutoCompaction(usage, KNOWN_MODELS.SONNET.id, false);
            expect(result.usagePercentage).toBe(70);
            expect(result.shouldShowWarning).toBe(true);
        });
        test("ignores use1M for models that don't support it (GPT)", () => {
            const usage = createMockUsage(100000, undefined, KNOWN_MODELS.GPT_MINI.id);
    ...
    describe("Edge Cases", () => {
        ...
        test("single context usage entry works correctly", () => {
            const usage = createMockUsage(140000);
            const result = checkAutoCompaction(usage, KNOWN_MODELS.SONNET.id, false);
            expect(result.shouldShowWarning).toBe(true);
            expect(result.usagePercentage).toBe(70);
        });
        test("custom threshold parameter (80%)", () => {
            const usage = createMockUsage(140000); // 70% of context
        ...
        test("custom warning advance (5% instead of 10%)", () => {
            const usage = createMockUsage(130000); // 65% of context
        ...
        test("handles usage beyond 100% of context", () => {
            const usage = createMockUsage(SONNET_MAX_TOKENS + 50000);
            const result = checkAutoCompaction(usage, KNOWN_MODELS.SONNET.id, false);
            expect(result.shouldShowWarning).toBe(true);
    ...
    describe("Percentage Calculation Accuracy", () => {
        test("calculates percentage correctly for various token counts", () => {
            ...
            const testCases = [
                { tokens: 20000, expectedPercent: 10 },
                { tokens: 40000, expectedPercent: 20 },
                { tokens: 100000, expectedPercent: 50 },
                { tokens: 120000, expectedPercent: 60 },
                { tokens: 140000, expectedPercent: 70 },
                { tokens: 160000, expectedPercent: 80 },
                { tokens: 180000, expectedPercent: 90 },
            ];
        ...
        test("handles fractional percentages correctly", () => {
            const usage = createMockUsage(123456); // 61.728%
            const result = checkAutoCompaction(usage, KNOWN_MODELS.SONNET.id, false);
    ...
    describe("Force Compaction (threshold + 5% buffer)", () => {
        ...
        test("shouldForceCompact is true when usage above force threshold", () => {
            ...
            const usage = createMockUsage(160000); // 80%
            const result = checkAutoCompaction(usage, KNOWN_MODELS.SONNET.id, false);
            expect(result.shouldForceCompact).toBe(true);
        });
        test("shouldForceCompact uses liveUsage when available", () => {
            // lastUsage at 50%, liveUsage at 75% - should trigger based on live
            const liveUsage = createUsageEntry(150000); // 75%
            const usage = createMockUsage(100000, undefined, KNOWN_MODELS.SONNET.id, liveUsage);
            const result = checkAutoCompaction(usage, KNOWN_MODELS.SONNET.id, false);
            expect(result.shouldForceCompact).toBe(true);
        ...
        test("shouldForceCompact respects custom threshold", () => {
            ...
            const usage = createMockUsage(110000); // 55%
        ...
        test("shouldForceCompact respects 1M context mode", () => {
            ...
            const liveUsage = createUsageEntry(750000);
            const usage = createMockUsage(50000, undefined, KNOWN_MODELS.SONNET.id, liveUsage);
            const result = checkAutoCompaction(usage, KNOWN_MODELS.SONNET.id, true);
            expect(result.shouldForceCompact).toBe(true);
        });
        ...
        test("shouldShowWarning uses live usage when no lastContextUsage exists", () => {
            ...
            const liveUsage = createUsageEntry(130000); // 65%
            const usage = {
                totalTokens: 0,
                liveUsage,
            };
        ...
        test("shouldShowWarning uses max of last and live usage", () => {
            ...
            const liveUsage = createUsageEntry(144000); // 72%
            const usage = createMockUsage(100000, undefined, KNOWN_MODELS.SONNET.id, liveUsage);
            const result = checkAutoCompaction(usage, KNOWN_MODELS.SONNET.id, false);
        ...
        test("shouldForceCompact is false when auto-compaction disabled", () => {
            const usage = createMockUsage(190000); // 95% - would trigger if enabled
            const result = checkAutoCompaction(usage, KNOWN_MODELS.SONNET.id, false, 1.0); // disabled
            expect(result.shouldForceCompact).toBe(false);
        });
    });
});
//# sourceMappingURL=autoCompactionCheck.test.js.map