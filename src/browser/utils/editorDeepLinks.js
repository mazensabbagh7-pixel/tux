 ...
 */
export function getEditorDeepLink(options) {
...
function normalizeContainerPathForDevcontainerDeepLink(path) {
...
function normalizeLocalPathForEditorDeepLink(path) {
    const trimmed = path.trim();
    const unquoted = (trimmed.startsWith('"') && trimmed.endsWith('"')) ||
        (trimmed.startsWith("'") && trimmed.endsWith("'"))
        ? trimmed.slice(1, -1)
...
//# sourceMappingURL=editorDeepLinks.js.map