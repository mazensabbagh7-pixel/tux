...
import { getCachedFileContent, setCachedFileContent, removeCachedFileContent, cacheToResult, CACHE_CONFIG, } from "./fileContentCache";
describe("fileContentCache", () => {
    ...
    describe("basic operations", () => {
        ...
        test("stores and retrieves text file content", () => {
            const data = { type: "text", content: "hello world", size: 11 };
        ...
        test("handles Unicode text content", () => {
            ...
            const data = { type: "text", content, size: content.length };
        ...
        test("does not cache error results", () => {
            const data = { type: "error", message: "File not found" };
        ...
        test("stores diff alongside file content", () => {
            const data = { type: "text", content: "modified", size: 8 };
            setCachedFileContent("ws1", "file.txt", data, "+modified\n-original");
            const cached = getCachedFileContent("ws1", "file.txt");
            expect(cached.diff).toBe("+modified\n-original");
        });
        test("separates entries by workspace", () => {
            const data1 = { type: "text", content: "ws1 content", size: 11 };
            const data2 = { type: "text", content: "ws2 content", size: 11 };
    ...
    describe("TTL expiration", () => {
        test("returns null and removes entry when TTL expired", () => {
            ...
            const data = { type: "text", content: "will expire", size: 11 };
            setCachedFileContent("ws1", "file.txt", data, null);
        ...
        test("removes expired entry from index", () => {
            CACHE_CONFIG.TTL_MS = 100;
            const data = { type: "text", content: "test", size: 4 };
            setCachedFileContent("ws1", "file.txt", data, null);
            ...
            const index = JSON.parse(globalThis.window.localStorage.getItem("explorer:fileIndex") ?? "[]");
    ...
    describe("LRU eviction", () => {
        test("evicts oldest entries when exceeding max", () => {
            ...
            // Add 3 entries
            for (let i = 1; i <= 3; i++) {
                const data = { type: "text", content: `file${i}`, size: 5 };
            ...
            const data4 = { type: "text", content: "file4", size: 5 };
        ...
        test("re-writing an entry moves it to end of LRU queue", () => {
            ...
            const data1 = { type: "text", content: "file1", size: 5 };
        ...
        test("removes evicted entries from localStorage", () => {
            ...
            const data1 = { type: "text", content: "file1", size: 5 };
            const data2 = { type: "text", content: "file2", size: 5 };
            const data3 = { type: "text", content: "file3", size: 5 };
            setCachedFileContent("ws1", "file1.txt", data1, null);
    ...
    describe("removeCachedFileContent", () => {
        test("removes entry from cache", () => {
            const data = { type: "text", content: "hello", size: 5 };
            setCachedFileContent("ws1", "file.txt", data, null);
            expect(getCachedFileContent("ws1", "file.txt")).not.toBeNull();
...
//# sourceMappingURL=fileContentCache.test.js.map