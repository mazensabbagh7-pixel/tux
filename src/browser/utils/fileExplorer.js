...
export function validateRelativePath(path) {
    if (!path)
        return undefined; // Empty path is valid (root)
    if (path.startsWith("/"))
 ...
 */
export function parseLsOutput(output, basePath) {
 ...
 * - ` D` or `D ` = deleted
 */
export function parseGitStatus(output, basePath) {
    ...
    for (const line of lines) {
        if (line.length < 4)
            continue; // Need at least "XY " + 1 char path
 ...
 */
export function detectSvg(buffer) {
 ...
 */
export function buildListDirScript(relativePath) {
 ...
 */
export function buildGitIgnoredScript(relativePath) {
    const dir = relativePath ? shellEscape(relativePath) : ".";
 ...
 */
export function buildGitCheckIgnoreScript(paths) {
 ...
 */
export function parseGitCheckIgnoreOutput(output) {
    const ignored = new Set();
    if (!output.trim())
        return ignored;
 ...
 */
export function buildReadFileScript(relativePath) {
 ...
 */
export function buildFileDiffScript(relativePath) {
    const file = shellEscape(relativePath);
 ...
 */
export function parseReadFileOutput(output) {
 ...
 */
export function processFileContents(output, exitCode) {
