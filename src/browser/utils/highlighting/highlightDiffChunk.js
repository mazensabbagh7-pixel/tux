 ...
 */
export async function highlightDiffChunk(chunk, language, themeMode = "dark") {
    ...
    try {
        ...
        const hasEmptyExtraction = lines.some((extractedHtml, i) => extractedHtml.length === 0 && chunk.lines[i].length > 0);
 ...
 */
function createFallbackChunk(chunk) {
    return {
        type: chunk.type,
        lines: chunk.lines.map((line, i) => ({
            html: escapeHtml(line),
...
//# sourceMappingURL=highlightDiffChunk.js.map