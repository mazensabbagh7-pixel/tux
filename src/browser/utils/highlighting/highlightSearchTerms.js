...
const getParser = () => {
    parserInstance ?? (parserInstance = new DOMParser());
    return parserInstance;
};
// LRU cache for compiled regex patterns
// Key: search config string, Value: compiled RegExp
const regexCache = new LRUCache({
 ...
 */
export function highlightSearchInText(text, config) {
 ...
 */
export function highlightSearchMatches(html, config) {
    const { searchTerm, useRegex, matchCase } = config;
    // No highlighting if search term is empty
    if (!searchTerm.trim()) {
        return html;
...
//# sourceMappingURL=highlightSearchTerms.js.map