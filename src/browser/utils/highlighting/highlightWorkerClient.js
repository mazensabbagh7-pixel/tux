...
async function highlightMainThread(code, language, theme) {
    if (!warnedMainThread) {
        warnedMainThread = true;
        console.warn("[highlightWorkerClient] Syntax highlighting running on main thread (worker unavailable)");
    }
...
export async function highlightCode(code, language, theme) {
    ...
    try {
        ...
        console.error("[highlightWorkerClient] Worker highlight failed; falling back to main thread:", e);
        workerFailed = true;
        workerAPI = null;
        return highlightMainThread(code, language, theme);
    }
}
//# sourceMappingURL=highlightWorkerClient.js.map