...
function withDebugLlmRequestEnabled(fn) {
    var _a;
    ...
    (_a = globalWithWindow.window).api ?? (_a.api = { platform: process.platform, versions: {} });
...
describe("StreamingMessageAggregator", () => {
    describe("init state reference stability", () => {
        ...
        test("should create new init message object on each state change", async () => {
            ...
            if (initMsg1?.type === "workspace-init" &&
    ...
    describe("display flags", () => {
        ...
        test("should disable displayed message cap when showAllMessages is enabled", () => {
            ...
            for (let i = 0; i < 100; i++) {
                ...
                manyMessages.push(createMuxMessage(`u${i}`, "user", `msg-${i}`, {
                ...
                manyMessages.push(createMuxMessage(`a${i}`, "assistant", `response-${i}`, {
        ...
        test("should not show history-hidden when only user messages exceed cap", () => {
            ...
            const manyMessages = Array.from({ length: 200 }, (_, i) => createMuxMessage(`u${i}`, "user", `msg-${i}`, {
                timestamp: i,
                historySequence: i,
            }));
            const aggregator = new StreamingMessageAggregator(TEST_CREATED_AT);
    ...
    describe("compaction boundary rows", () => {
        ...
        test("omits malformed compaction epoch values instead of crashing transcript rendering", () => {
            ...
            const summaryWithMalformedEpoch = createMuxMessage("summary-malformed", "assistant", "Compacted summary", {
                historySequence: 2,
                timestamp: 2,
                compacted: "user",
                compactionBoundary: true,
    ...
    describe("live compaction boundary pruning", () => {
        ...
        test("does not prune on first compaction (no penultimate boundary exists)", () => {
            ...
            const msg1 = asChatMessage(createMuxMessage("user-1", "user", "First message", {
                historySequence: 0,
                timestamp: 1,
            }));
            const msg2 = asChatMessage(createMuxMessage("assistant-1", "assistant", "Response", {
                historySequence: 1,
                timestamp: 2,
            }));
            ...
            const summary = asChatMessage(createMuxMessage("summary-1", "assistant", "Compacted summary", {
                historySequence: 2,
                compacted: "user",
                compactionBoundary: true,
                compactionEpoch: 1,
        ...
        test("prunes pre-penultimate messages on second compaction", () => {
            ...
            const epoch0Msg = asChatMessage(createMuxMessage("epoch0-user", "user", "Old message", {
                historySequence: 0,
                timestamp: 1,
            }));
            aggregator.handleMessage(epoch0Msg);
            // First compaction boundary (epoch 1)
            const boundary1 = asChatMessage(createMuxMessage("boundary-1", "assistant", "Summary epoch 1", {
                historySequence: 1,
                compacted: "user",
                compactionBoundary: true,
                compactionEpoch: 1,
            ...
            // Epoch 1 messages
            const epoch1Msg = asChatMessage(createMuxMessage("epoch1-user", "user", "Message in epoch 1", {
                historySequence: 2,
                timestamp: 3,
            }));
            ...
            const boundary2 = asChatMessage(createMuxMessage("boundary-2", "assistant", "Summary epoch 2", {
                historySequence: 3,
                compacted: "user",
                compactionBoundary: true,
                compactionEpoch: 2,
        ...
        test("does not prune messages when a non-boundary message arrives", () => {
            const aggregator = new StreamingMessageAggregator(TEST_CREATED_AT);
            const msg1 = asChatMessage(createMuxMessage("user-1", "user", "First message", { historySequence: 0 }));
            const msg2 = asChatMessage(createMuxMessage("assistant-1", "assistant", "Normal response", { historySequence: 1 }));
            aggregator.handleMessage(msg1);
            aggregator.handleMessage(msg2);
    ...
    describe("usage-delta handling", () => {
        ...
        test("multi-step scenario: step usage replaced, cumulative accumulated", () => {
            ...
            expect((aggregator.getActiveStreamCumulativeProviderMetadata("msg-1")?.anthropic).cacheCreationInputTokens).toBe(800);
...
//# sourceMappingURL=StreamingMessageAggregator.test.js.map