 ...
 */
export function filterEmptyAssistantMessages(messages, preserveReasoningOnly = false) {
 ...
 * survives filtering.
 */
export function addInterruptedSentinel(messages) {
    const result = [];
 ...
 */
export function injectAgentTransition(messages, currentAgentId, toolNames, planContent, planFilePath) {
 ...
 */
export function injectFileChangeNotifications(messages, changedFileAttachments) {
 ...
 */
export function injectPostCompactionAttachments(messages, attachments) {
 ...
 */
export function stripOrphanedToolCalls(messages) {
 ...
 */
export function transformModelMessages(messages, provider, options) {
 ...
 */
export function validateAnthropicCompliance(messages) {
 ...
 */
export function getAnthropicThinkingDisableReason(messages) {
    for (let i = 0; i < messages.length; i++) {
        const msg = messages[i];
        if (msg.role !== "assistant") {
            continue;
