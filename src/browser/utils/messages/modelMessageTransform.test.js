...
import { transformModelMessages, validateAnthropicCompliance, getAnthropicThinkingDisableReason, addInterruptedSentinel, injectAgentTransition, filterEmptyAssistantMessages, injectFileChangeNotifications, injectPostCompactionAttachments, stripOrphanedToolCalls, } from "./modelMessageTransform";
...
describe("modelMessageTransform", () => {
    describe("transformModelMessages", () => {
        ...
        it("should interleave multiple tool-call groups with their results", () => {
            ...
            expect(result[1].role).toBe("tool");
            expect(result[1].content[0]).toMatchObject({ toolCallId: "call1" });
            expect(result[2].role).toBe("assistant");
            expect(result[2].content).toEqual([
                { type: "text", text: "Step 2" },
                { type: "tool-call", toolCallId: "call2", toolName: "bash", input: { script: "ls" } },
            ...
            expect(result[3].content[0]).toMatchObject({ toolCallId: "call2" });
        });
        it("preserves signed reasoning in assistant messages that contain tool calls when thinking is enabled", () => {
            ...
            expect(result[0].content[0]).toMatchObject({ type: "reasoning" });
        ...
        it("should keep text-only messages unchanged", () => {
            ...
            const messages = [assistantMsg1, assistantMsg2];
            const result = transformModelMessages(messages, "anthropic");
            expect(result).toEqual(messages);
        });
        it("coalesces 3 consecutive identical no-progress task_await pairs into 1 (keep last pair)", () => {
            ...
            const result = transformModelMessages([assistantMsg1, toolMsg1, assistantMsg2, toolMsg2, assistantMsg3, toolMsg3], "anthropic");
    ...
    describe("validateAnthropicCompliance", () => {
        ...
        it("should detect mismatched tool results", () => {
            ...
            const messages = [assistantMsg, toolMsg];
            const result = validateAnthropicCompliance(messages);
            expect(result.valid).toBe(false);
    ...
    describe("consecutive user messages", () => {
        ...
        it("should not merge user messages separated by assistant message", () => {
            ...
            expect(result[2].content[0].text).toBe("How are you?");
        });
    });
...
describe("injectAgentTransition", () => {
    ...
    it("should include plan content when transitioning from plan to exec", () => {
        ...
        if (textPart.type === "text") {
            expect(textPart.text).toContain("[Agent switched from plan to exec. Follow exec agent instructions.]");
    ...
    it("should include plan content when transitioning from plan to orchestrator", () => {
        ...
        const planContent = "# My Plan\n\n## Step 1\nDo something\n\n## Step 2\nDo more";
        const planFilePath = "~/.mux/plans/demo/ws-123.md";
        const result = injectAgentTransition(messages, "orchestrator", undefined, planContent, planFilePath);
        expect(result.length).toBe(4);
        const transitionMessage = result[2];
        const textPart = transitionMessage.parts[0];
        expect(textPart.type).toBe("text");
        if (textPart.type === "text") {
            expect(textPart.text).toContain("[Agent switched from plan to orchestrator. Follow orchestrator agent instructions.]");
            expect(textPart.text).toContain(`Plan file path: ${planFilePath}`);
    ...
    it("should NOT include plan content when transitioning from exec to plan", () => {
        ...
        const textPart = transitionMessage.parts[0];
        if (textPart.type === "text") {
            expect(textPart.text).toBe("[Agent switched from exec to plan. Follow plan agent instructions.]");
    ...
    it("should NOT include plan content when no plan content provided", () => {
        ...
        const textPart = transitionMessage.parts[0];
        if (textPart.type === "text") {
            expect(textPart.text).toBe("[Agent switched from plan to exec. Follow exec agent instructions.]");
            expect(textPart.text).not.toContain("<plan>");
        }
    });
...
describe("injectFileChangeNotifications", () => {
    ...
    it("should append synthetic user message with file change notification", () => {
        ...
        const text = syntheticMsg.parts[0].text;
...
describe("injectPostCompactionAttachments", () => {
    it("inserts after the compaction summary and enforces a size budget", () => {
        ...
        const attachments = [
            {
                ...
                files: [
                    {
                        path: "src/huge.ts",
                        diff: "x".repeat(MAX_POST_COMPACTION_INJECTION_CHARS + 10000),
                        truncated: false,
                    },
                ],
            },
        ...
        const text = injected.parts[0].text;
...
//# sourceMappingURL=modelMessageTransform.test.js.map