...
import { createFreshRetryState, createManualRetryState, createFailedRetryState, calculateBackoffDelay, INITIAL_DELAY, } from "./retryState";
describe("retryState utilities", () => {
    ...
    describe("createFailedRetryState", () => {
        it("increments attempt counter and stores error", () => {
            const error = { type: "unknown", raw: "Test error" };
    ...
    describe("backoff progression scenario", () => {
        it("maintains exponential backoff through manual retries", () => {
            ...
            let state = createFailedRetryState(0, { type: "unknown", raw: "Error" });
            state = createFailedRetryState(state.attempt, { type: "unknown", raw: "Error" });
            state = createFailedRetryState(state.attempt, { type: "unknown", raw: "Error" });
            expect(state.attempt).toBe(3);
...
//# sourceMappingURL=retryState.test.js.map