 ...
 */
export function filterByReadState(hunks, isRead, showRead) {
    if (showRead)
        return hunks;
 ...
 */
export function filterBySearch(hunks, searchTerm, useRegex = false, matchCase = false) {
    if (!searchTerm.trim())
        return hunks;
    if (useRegex) {
        try {
        ...
        if (matchCase) {
            ...
            return hunks.filter((hunk) => {
                return (hunk.filePath.toLowerCase().includes(searchLower) ||
 ...
 */
export function applyFrontendFilters(hunks, filters) {
    let result = hunks;
...
//# sourceMappingURL=filterHunks.js.map