 ...
 */
export async function readFileLines(api, workspaceId, filePath, startLine, endLine, gitRef) {
    if (!api || startLine < 1 || endLine < startLine)
        return null;
    const script = gitRef
 ...
 */
export function getOldFileRef(diffBase, _includeUncommitted) {
    if (diffBase === "--staged" || diffBase === "HEAD")
        return "HEAD";
 ...
 */
export function formatAsContextLines(lines) {
