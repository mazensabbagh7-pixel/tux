import { isTabType } from "@/browser/types/rightSidebar";
...
export function isRightSidebarLayoutState(value) {
    if (!value || typeof value !== "object")
        return false;
    const v = value;
    if (v.version !== 1)
...
export function getDefaultRightSidebarLayoutState(activeTab) {
    // Default tabs exclude terminal - users add terminals via the "+" button
    const baseTabs = ["costs", "review", "explorer"];
 ...
 */
function injectTabIntoLayout(node, tab) {
    if (node.type === "tabset") {
...
export function removeTabEverywhere(state, tab) {
...
function updateNode(node, tabsetId, updater) {
    if (node.type === "tabset") {
        if (node.id !== tabsetId)
            return node;
        return updater(node);
...
export function setFocusedTabset(state, tabsetId) {
    if (state.focusedTabsetId === tabsetId)
        return state;
    return { ...state, focusedTabsetId: tabsetId };
}
export function selectTabInTabset(state, tabsetId, tab) {
...
export function reorderTabInTabset(state, tabsetId, fromIndex, toIndex) {
...
export function selectTabInFocusedTabset(state, tab) {
...
export function splitFocusedTabset(state, direction) {
    const focused = findTabset(state.root, state.focusedTabsetId);
    if (focused?.type !== "tabset") {
        return state;
    }
    ...
    const fallbackTab = focused.activeTab === "terminal"
        ? "costs"
...
export function updateSplitSizes(state, splitId, sizes) {
    const update = (node) => {
        if (node.type === "split") {
            if (node.id === splitId) {
                return { ...node, sizes };
 ...
 */
export function collectAllTabsWithTabset(node) {
    if (node.type === "tabset") {
 ...
 */
export function selectTabByIndex(state, index) {
...
export function getFocusedActiveTab(state, fallback) {
    const focused = findTabset(state.root, state.focusedTabsetId);
    if (focused?.type === "tabset")
        return focused.activeTab;
    return fallback;
}
export function addToolToFocusedTabset(state, tab) {
 ...
 */
export function addTabToFocusedTabset(state, tab, 
 ...
 */
export function selectOrAddTab(state, tab) {
 ...
 */
export function moveTabToTabset(state, tab, sourceTabsetId, targetTabsetId) {
    ...
    let newFocusedId = targetTabsetId;
...
function getFallbackTabForEmptyTabset(movedTab) {
 ...
 */
export function dockTabToEdge(state, tab, sourceTabsetId, targetTabsetId, edge) {
    const source = findTabset(state.root, sourceTabsetId);
    const target = findTabset(state.root, targetTabsetId);
    if (source?.type !== "tabset" || target?.type !== "tabset") {
        return state;
    ...
    const splitDirection = edge === "top" || edge === "bottom" ? "horizontal" : "vertical";
        ...
        if (node.type === "tabset") {
            if (node.id === targetTabsetId) {
                ...
                if (sourceTabsetId === targetTabsetId) {
                    ...
                    const nextActiveTab = node.activeTab === tab || !nextTabs.includes(node.activeTab)
                        ? nextTabs[0]
                        : node.activeTab;
 ...
 */
export function closeSplit(state, splitId, keepChildIndex) {
    ...
    // Ensure focusedTabsetId is still valid
    let newFocusedId = state.focusedTabsetId;
    if (findTabset(newRoot, newFocusedId) === null) {
...
//# sourceMappingURL=rightSidebarLayout.js.map