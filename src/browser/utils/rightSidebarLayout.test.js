...
import { addTabToFocusedTabset, addToolToFocusedTabset, closeSplit, dockTabToEdge, getDefaultRightSidebarLayoutState, moveTabToTabset, reorderTabInTabset, selectTabInFocusedTabset, splitFocusedTabset, } from "./rightSidebarLayout";
test("selectTabInFocusedTabset adds missing tool and makes it active", () => {
    ...
    if (s.root.type !== "tabset")
        throw new Error("expected tabset");
...
test("splitFocusedTabset moves active tab when possible (no empty tabsets)", () => {
    ...
    if (left.type !== "tabset" || right.type !== "tabset")
        throw new Error("expected tabsets");
...
test("dockTabToEdge splits a tabset and moves the dragged tab into the new pane", () => {
    ...
    if (top.type !== "tabset" || bottom.type !== "tabset")
        throw new Error("expected tabsets");
...
test("closeSplit keeps the specified child", () => {
    ...
    if (s2.root.type !== "tabset")
        throw new Error("expected tabset");
    expect(s2.root.id).toBe("tabset-2");
    expect(s2.root.tabs).toEqual(["review"]);
});
//# sourceMappingURL=rightSidebarLayout.test.js.map