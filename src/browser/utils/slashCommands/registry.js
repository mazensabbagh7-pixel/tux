 ...
 */
function parseMultilineCommand(rawInput) {
    const hasMultiline = rawInput.includes("\n");
    const lines = rawInput.split("\n");
    const firstLine = lines[0];
    const remainingLines = lines.slice(1).join("\n").trim();
    // Tokenize first line only (preserving quotes)
    const tokens = (firstLine.match(/(?:[^\s"]+|"[^"]*")+/g) ?? []).map((token) => token.replace(/^"(.*)"$/, "$1"));
    return {
        firstLine,
        tokens,
...
const truncateCommandDefinition = {
    key: "truncate",
...
const compactCommandDefinition = {
    key: "compact",
    description: "Compact conversation history using AI summarization. Use -t <tokens> to set max output tokens, -m <model> to set compaction model. Add continue message on lines after the command.",
    handler: ({ rawInput }) => {
        ...
        const unknownFlags = firstLineTokens.filter((token) => token.startsWith("-") && token !== "-t" && token !== "-c" && token !== "-m");
        ...
        if (parsed.t !== undefined) {
            const tokens = parseInt(parsed.t, 10);
...
const modelCommandDefinition = {
    ...
    suggestions: ({ stage, partialToken }) => {
        ...
        if (stage === 1) {
            const abbreviationSuggestions = Object.entries(MODEL_ABBREVIATIONS).map(([abbrev, fullModel]) => ({
                key: abbrev,
...
const planOpenCommandDefinition = {
    key: "open",
    description: "Open plan in external editor",
    appendSpace: false,
    handler: () => ({ type: "plan-open" }),
};
const planCommandDefinition = {
    key: "plan",
...
const newCommandDefinition = {
    key: "new",
    description: "Create new workspace with optional trunk branch and runtime. Use -t <branch> to specify trunk, -r <runtime> for remote execution (e.g., 'ssh hostname' or 'ssh user@host'). Add start message on lines after the command.",
    handler: ({ rawInput }) => {
        const { tokens: firstLineTokens, message: remainingLines, hasMultiline, } = parseMultilineCommand(rawInput);
        // Parse flags from first line using minimist
        const parsed = minimist(firstLineTokens, {
        ...
        const unknownFlags = firstLineTokens.filter((token) => token.startsWith("-") && token !== "-t" && token !== "-r");
        if (unknownFlags.length > 0) {
            return {
                type: "new",
...
export const SLASH_COMMAND_DEFINITION_MAP = new Map(SLASH_COMMAND_DEFINITIONS.map((definition) => [definition.key, definition]));
