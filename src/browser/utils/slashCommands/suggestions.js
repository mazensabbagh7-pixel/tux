...
function filterAndMapSuggestions(definitions, partial, build, filter) {
...
function buildTopLevelSuggestions(partial, context) {
    const isCreation = context.variant === "creation";
    const commandSuggestions = filterAndMapSuggestions(COMMAND_DEFINITIONS, partial, (definition) => {
    ...
    const skillDefinitions = (context.agentSkills ?? [])
    ...
    const modelAliasDefinitions = Object.entries(MODEL_ABBREVIATIONS).map(([alias, modelId]) => ({
        key: alias,
        description: `Send with ${formatModelDisplayName(modelId.split(":")[1] ?? modelId)} (one message, +level for thinking)`,
        appendSpace: true,
    }));
    const modelAliasSuggestions = filterAndMapSuggestions(modelAliasDefinitions, partial, (definition) => ({
...
export function getSlashCommandSuggestions(input, context = {}) {
    if (!input.startsWith("/")) {
        return [];
    }
