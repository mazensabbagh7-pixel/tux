...
export function getTokenCountPromise(client, model, text) {
...
export async function countTokensBatchRenderer(client, model, texts) {
    ...
    try {
        const rawBatchResult = await client.tokenizer.countTokensBatch({
            model: trimmedModel,
            texts: missingTexts,
        });
...
export function clearRendererTokenizerCache() {
    tokenCache.clear();
    keyOrder.length = 0;
}
