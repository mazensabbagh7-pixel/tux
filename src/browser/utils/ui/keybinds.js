 ...
 */
export function matchesKeybind(event, keybind) {
...
export function allowsEscapeToInterruptStream(target) {
    if (!target) {
        return false;
    }
 ...
 */
export function isTerminalFocused(target) {
    if (!target) {
        return false;
    }
    // Check if HTMLElement exists (not available in non-DOM test environments)
 ...
 */
export function formatNumberedKeybind(index) {
    if (index < 0 || index > 8)
        return "";
    const num = index + 1;
 ...
 */
export function matchNumberedKeybind(event) {
