...
function flattenWorkspaceTree(workspaces) {
...
export function computeWorkspaceDepthMap(workspaces) {
    const byId = new Map();
    for (const workspace of workspaces) {
        byId.set(workspace.id, workspace);
    }
    ...
    const computeDepth = (workspaceId) => {
 ...
 */
export const AGE_THRESHOLDS_DAYS = [1, 7, 30];
 ...
 */
export function buildSortedWorkspacesByProject(projects, workspaceMetadata, workspaceRecency) {
 ...
 */
export function formatDaysThreshold(days) {
    return days === 1 ? "1 day" : `${days} days`;
}
/**
 * Build the storage key for a tier's expanded state.
 */
export function getTierKey(projectPath, tierIndex) {
 ...
 */
export function findNextNonEmptyTier(buckets, startIndex) {
 ...
 */
export function partitionWorkspacesByAge(workspaces, workspaceRecency) {
    if (workspaces.length === 0) {
    ...
    const recent = [];
    const buckets = AGE_THRESHOLDS_DAYS.map(() => []);
    for (const workspace of workspaces) {
 ...
 */
export function partitionWorkspacesBySection(workspaces, sections) {
    ...
    const resolveSection = (workspace) => {
 ...
 */
export function getSectionExpandedKey(projectPath, sectionId) {
 ...
 */
export function getSectionTierKey(projectPath, sectionId, tierIndex) {
    return `section:${projectPath}:${sectionId}:tier:${tierIndex}`;
}
//# sourceMappingURL=workspaceFiltering.js.map