...
import { partitionWorkspacesByAge, formatDaysThreshold, AGE_THRESHOLDS_DAYS, buildSortedWorkspacesByProject, partitionWorkspacesBySection, sortSectionsByLinkedList, } from "./workspaceFiltering";
import { DEFAULT_RUNTIME_CONFIG } from "@/common/constants/workspace";
describe("partitionWorkspacesByAge", () => {
    const now = Date.now();
    const ONE_DAY_MS = 24 * 60 * 60 * 1000;
    const createWorkspace = (id) => ({
        id,
        name: `workspace-${id}`,
    ...
    const getAllOld = (buckets) => buckets.flat();
...
describe("buildSortedWorkspacesByProject", () => {
    const createWorkspace = (id, projectPath, isInitializing, parentWorkspaceId) => ({
        id,
        name: `workspace-${id}`,
    ...
    it("should handle multiple concurrent pending workspaces", () => {
        const projects = new Map([["/project/a", { workspaces: [] }]]);
        const metadata = new Map([
            ["pending1", createWorkspace("pending1", "/project/a", true)],
...
describe("sortSectionsByLinkedList", () => {
    ...
    it("should handle single section", () => {
        const sections = [{ id: "only", name: "Only", nextId: null }];
        const sorted = sortSectionsByLinkedList(sections);
...
describe("partitionWorkspacesBySection", () => {
    const createWorkspace = (id, sectionId, parentWorkspaceId) => ({
        id,
        name: `workspace-${id}`,
        projectName: "test-project",
        projectPath: "/test/project",
    ...
    it("should partition workspaces by section", () => {
        ...
        expect(result.unsectioned.map((w) => w.id)).toEqual(["ws3"]);
        expect(result.bySectionId.get("section-a")?.map((w) => w.id)).toEqual(["ws1"]);
        expect(result.bySectionId.get("section-b")?.map((w) => w.id)).toEqual(["ws2"]);
    });
    it("should keep child workspaces directly after their parent within a section", () => {
        ...
        const sections = [{ id: "section-a", name: "A" }];
        const result = partitionWorkspacesBySection(workspaces, sections);
        // Child should be directly after parent
        expect(result.bySectionId.get("section-a")?.map((w) => w.id)).toEqual(["parent", "child"]);
    });
    ...
    it("should handle nested children inheriting section from root parent", () => {
        ...
        expect(result.bySectionId.get("section-a")?.map((w) => w.id)).toEqual(["root", "child1", "grandchild"]);
        expect(result.unsectioned).toHaveLength(0);
    });
});
//# sourceMappingURL=workspaceFiltering.test.js.map