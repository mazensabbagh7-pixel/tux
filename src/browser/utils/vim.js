...
function assertHistorySnapshot(snapshot, label) {
    ...
    assert(snapshot.cursor <= maxCursor, `${label}.cursor out of bounds. cursor=${snapshot.cursor} max=${maxCursor} text.length=${snapshot.text.length}`);
}
function assertVimState(state) {
    assert(state.mode === "insert" ||
        state.mode === "normal" ||
        state.mode === "visual" ||
        state.mode === "visualLine", "Unexpected Vim mode");
    ...
    const maxCursor = state.mode === "insert" ? state.text.length : Math.max(0, state.text.length - 1);
    assert(state.cursor <= maxCursor, `Vim cursor out of bounds for mode=${state.mode}. cursor=${state.cursor} max=${maxCursor} text.length=${state.text.length}`);
    ...
    if (state.visualAnchor != null) {
        ...
        assert(state.visualAnchor <= maxCursor, `Vim visualAnchor out of bounds for mode=${state.mode}. visualAnchor=${state.visualAnchor} max=${maxCursor} text.length=${state.text.length}`);
    }
    ...
    if (state.lastFind != null) {
        assert(state.lastFind.variant === "f" ||
            state.lastFind.variant === "F" ||
            state.lastFind.variant === "t" ||
            state.lastFind.variant === "T", "Unexpected Vim lastFind variant");
    ...
    if (state.pending) {
        ...
        assert(pending.kind === "op" || pending.kind === "g" || pending.kind === "find", `Unexpected Vim pending kind: ${String(pending.kind)}`);
        ...
        if (pending.kind === "op") {
            assert(pending.op === "d" || pending.op === "c" || pending.op === "y", "Unexpected Vim pending operator");
        ...
        if (pending.kind === "find") {
            assert(pending.variant === "f" ||
                pending.variant === "F" ||
                pending.variant === "t" ||
                pending.variant === "T", "Unexpected Vim pending find variant");
            if (pending.op != null) {
                assert(pending.op === "d" || pending.op === "c" || pending.op === "y", "Unexpected Vim pending find operator");
            }
        }
    }
    ...
    assert(state.undoStack.length <= VIM_HISTORY_STACK_LIMIT, `Vim undoStack too large: ${state.undoStack.length} > ${VIM_HISTORY_STACK_LIMIT}`);
    assert(state.redoStack.length <= VIM_HISTORY_STACK_LIMIT, `Vim redoStack too large: ${state.redoStack.length} > ${VIM_HISTORY_STACK_LIMIT}`);
    ...
    if (state.mode === "insert") {
        ...
        assert(state.insertStartSnapshot == null, "Vim insertStartSnapshot must be null outside insert mode");
    }
    if (state.lastEdit != null) {
        ...
        switch (lastEdit.kind) {
            ...
            case "~":
                ...
                break;
            case "paste":
                assert(lastEdit.variant === "p" || lastEdit.variant === "P", "Unexpected Vim lastEdit paste variant");
                assertCount(lastEdit.count);
                break;
            case "opMotion":
                assert(lastEdit.op === "d" || lastEdit.op === "c", "Unexpected Vim lastEdit operator");
                assert(lastEdit.motion === "w" ||
                    lastEdit.motion === "W" ||
                    lastEdit.motion === "b" ||
                    lastEdit.motion === "B" ||
                    lastEdit.motion === "e" ||
                    ...
                    lastEdit.motion === "0" ||
                    lastEdit.motion === "_" ||
                    lastEdit.motion === "line", "Unexpected Vim lastEdit motion");
                assertCount(lastEdit.count);
                break;
            case "opTextObject":
                assert(lastEdit.op === "d" || lastEdit.op === "c", "Unexpected Vim lastEdit operator");
            ...
            case "opVisual":
                assert(lastEdit.op === "d" || lastEdit.op === "c", "Unexpected Vim lastEdit operator");
                assert(lastEdit.rangeKind === "char" || lastEdit.rangeKind === "line", "Unexpected Vim lastEdit visual range kind");
                assertCount(lastEdit.count);
                break;
            default:
                assert(false, `Unexpected Vim lastEdit kind: ${String(lastEdit.kind)}`);
        }
    }
}
/**
 ...
 */
export function getLinesInfo(text) {
 ...
 */
export function getRowCol(text, idx) {
    const { starts } = getLinesInfo(text);
    let row = 0;
    while (row + 1 < starts.length && starts[row + 1] <= idx)
        row++;
 ...
 */
export function getLineBounds(text, cursor) {
...
function clampSelectionIndex(text, idx) {
    return Math.max(0, Math.min(idx, text.length));
}
function getCharwiseVisualRange(text, anchor, cursor) {
...
function getLinewiseVisualRange(text, anchor, cursor) {
...
export function getVisualRange(state) {
 ...
 */
export function moveToFirstNonWhitespace(text, cursor) {
...
function isFindForward(variant) {
    return variant === "f" || variant === "t";
}
function findCharIndexInLine(text, cursor, targetChar, direction, count) {
    ...
    let searchFrom = cursor - 1;
    if (searchFrom < lineStart)
        return null;
    let match = -1;
    for (let i = 0; i < safeCount; i++) {
        match = text.lastIndexOf(targetChar, searchFrom);
        if (match === -1 || match < lineStart)
            return null;
        searchFrom = match - 1;
    }
    return match;
}
function getFindMotionDestination(text, cursor, variant, targetChar, count) {
    const { lineStart, lineEnd } = getLineBounds(text, cursor);
    const direction = isFindForward(variant) ? "forward" : "backward";
 ...
 */
export function moveVertical(text, cursor, delta, desiredColumn) {
 ...
 */
export function moveWordForward(text, cursor) {
    const n = text.length;
    if (n === 0)
        return 0;
    let i = Math.max(0, Math.min(cursor, n - 1));
    const isWord = (ch) => /[A-Za-z0-9_]/.test(ch);
    const advancePastWord = (idx) => {
        let j = idx;
        while (j < n && isWord(text[j]))
            j++;
        return j;
    };
    const advanceToWord = (idx) => {
        let j = idx;
        while (j < n && !isWord(text[j]))
            j++;
        return j;
 ...
 */
export function moveWordEnd(text, cursor) {
    const n = text.length;
    if (n === 0)
        return 0;
    if (cursor >= n - 1)
    ...
    if (!isWord(text[clamp])) {
        let i = clamp;
        while (i < n && !isWord(text[i]))
            i++;
        if (i >= n)
            return Math.max(0, n - 1);
        while (i < n - 1 && isWord(text[i + 1]))
            i++;
        return i;
    }
    let endOfCurrent = clamp;
    while (endOfCurrent < n - 1 && isWord(text[endOfCurrent + 1]))
        endOfCurrent++;
    ...
    if (j >= n)
        return Math.max(0, n - 1);
    let endOfNext = j;
    while (endOfNext < n - 1 && isWord(text[endOfNext + 1]))
        endOfNext++;
    return endOfNext;
}
/**
 ...
 * In normal mode, cursor should never go past the last character.
 */
export function moveWordBackward(text, cursor) {
    let i = cursor - 1;
 ...
 */
export function wordBoundsAt(text, idx) {
    const n = text.length;
    ...
    let a = i;
    while (a > 0 && isWord(text[a - 1]))
        a--;
    let b = i + 1;
    while (b < n && isWord(text[b]))
        b++;
 ...
 */
export function deleteRange(text, from, to, yank, yankBuffer) {
 ...
 */
export function deleteCharUnderCursor(text, cursor, yankBuffer) {
    if (cursor >= text.length)
        return { text, cursor, yankBuffer };
    return deleteRange(text, cursor, cursor + 1, true, yankBuffer);
}
/**
 * Delete entire line (like 'dd').
 */
export function deleteLine(text, cursor, _yankBuffer) {
    const { lineStart, lineEnd } = getLineBounds(text, cursor);
 ...
 */
export function yankLine(text, cursor) {
    const { lineStart, lineEnd } = getLineBounds(text, cursor);
    const isLastLine = lineEnd === text.length;
    const to = isLastLine ? lineEnd : lineEnd + 1;
 ...
 */
export function pasteAfter(text, cursor, yankBuffer) {
    if (!yankBuffer)
        return { text, cursor };
 ...
 */
export function pasteBefore(text, cursor, yankBuffer) {
    if (!yankBuffer)
        return { text, cursor };
    const newText = text.slice(0, cursor) + yankBuffer + text.slice(cursor);
 ...
 */
export function getInsertCursorPos(text, cursor, mode) {
    const { lineStart, lineEnd } = getLineBounds(text, cursor);
    switch (mode) {
        case "i":
            return { cursor, text };
 ...
 */
export function changeRange(text, from, to, _yankBuffer) {
    return deleteRange(text, from, to, true, _yankBuffer);
}
/**
 * Handle change entire line (cc).
 */
export function changeLine(text, cursor, yankBuffer) {
    const { lineStart, lineEnd } = getLineBounds(text, cursor);
    return changeRange(text, lineStart, lineEnd, yankBuffer);
}
function makeHistorySnapshot(text, cursor) {
    return { text, cursor: clampCursorForMode(text, cursor, "normal") };
}
function pushHistoryStack(stack, snapshot) {
    const next = [...stack, snapshot];
    if (next.length <= VIM_HISTORY_STACK_LIMIT)
        return next;
...
function normalizeHistoryNavigationState(state) {
    return {
        ...state,
        mode: "normal",
...
function applyLastEditOnce(state, lastEdit) {
    switch (lastEdit.kind) {
        case "x":
        case "~":
        case "paste": {
            const res = tryHandleEdit(state, lastEdit.kind === "paste" ? lastEdit.variant : lastEdit.kind, lastEdit.count);
            if (!res)
                return state;
...
function applyDotRepeat(state, repeatCount) {
    if (state.lastEdit == null) {
        return completeOperation(state, {});
    }
 ...
 */
export function handleKeyPress(state, key, modifiers) {
    ...
    const finalResult = { ...result, newState: nextState };
 ...
 */
function handleInsertModeKey(state, key, modifiers) {
 ...
 */
function handleNormalModeKey(state, key, modifiers) {
    const now = Date.now();
    let nextState = state;
    ...
    if (nextState.pending?.kind !== "find" &&
        !modifiers.ctrl &&
        !modifiers.meta &&
        !modifiers.alt &&
        key.length === 1 &&
 ...
 */
function handleVisualModeKey(state, key, modifiers) {
    const now = Date.now();
    let nextState = state;
 ...
 * Handle pending command sequences (operators, multi-key prefixes, etc.).
 */
function handlePending(state, pending, key, modifiers, now) {
    switch (pending.kind) {
        case "op":
        ...
        default:
            assert(false, `Unexpected Vim pending kind: ${String(pending.kind)}`);
            return null;
    }
}
/**
 ...
 */
function handlePendingOperator(state, pending, key, _modifiers, now) {
    ...
    if (args.length === 0) {
        ...
        if (key === "w") {
            ...
            const isWordChar = state.cursor < state.text.length && /[A-Za-z0-9_]/.test(state.text[state.cursor]);
            const motion = pending.op === "c" && isWordChar ? "e" : "w";
            return {
                handled: true,
 ...
 */
function handlePendingG(state, pending, key) {
...
function handlePendingFind(state, pending, key, modifiers) {
    ...
    const dest = getFindMotionDestination(state.text, state.cursor, pending.variant, targetChar, safeCount);
...
function clampCursorForMode(text, cursor, mode) {
 ...
 */
function completeOperation(state, updates) {
 ...
 */
function handleKey(state, updates) {
    return {
        handled: true,
        newState: { ...state, ...updates },
    };
}
function handleCommand(state, updates) {
    return handleKey(state, { ...updates, count: null });
}
function exitVisualMode(state) {
...
function applyVisualOperator(state, op) {
    const range = getVisualRange(state);
 ...
 */
function getMotionRange(text, cursor, motion, count) {
    switch (motion) {
        case "w": {
            let to = cursor;
...
function getLinewiseRange(text, cursor, count) {
    const { row } = getRowCol(text, cursor);
    const { lines, starts } = getLinesInfo(text);
    const targetRow = Math.max(0, Math.min(lines.length - 1, row + count - 1));
 ...
 */
function applyOperatorMotion(state, op, motion, count) {
    const { text, cursor, yankBuffer } = state;
    const safeCount = Math.max(1, Math.min(10000, count));
    ...
    if (!range)
        return state; // Shouldn't happen, but type safety
...
function applyOperatorFind(state, op, dest, variant, targetChar) {
    const { text, cursor, yankBuffer } = state;
    assert(targetChar.length === 1, "Find target must be a single character");
    const lastFind = { variant, char: targetChar };
...
function isVimTextObject(textObj) {
...
function getLineLocalInnerWordBoundsAt(text, idx) {
    const n = text.length;
    if (n === 0)
        return null;
    if (idx < 0)
    ...
    if (bounds.start < lineStart || bounds.start >= lineEnd)
        return null;
...
function getLineLocalAWordBoundsAt(text, idx) {
    ...
    let end = inner.end;
    while (end < lineEnd && /\s/.test(text[end]))
        end++;
    if (end > inner.end) {
...
function findQuotePairInLine(line, cursorCol, quoteChar) {
    let open = null;
    let best = null;
...
function findBracketPairInLine(line, cursorCol, openChar, closeChar) {
    const stack = [];
    let best = null;
    for (let i = 0; i < line.length; i++) {
        const ch = line[i];
...
function getLineLocalQuoteTextObjectRange(text, cursor, quoteChar, includeDelimiters) {
    if (text.length === 0)
        return null;
...
function getLineLocalBracketTextObjectRange(text, cursor, openChar, closeChar, includeDelimiters) {
    if (text.length === 0)
        return null;
    if (cursor < 0 || cursor >= text.length)
        return null;
...
function getTextObjectRange(text, cursor, textObj, count) {
    switch (textObj) {
        case "iw": {
 ...
 */
function applyOperatorTextObject(state, op, textObj, count) {
    const { text, cursor, yankBuffer } = state;
    const safeCount = Math.max(1, Math.min(10000, count));
...
function isFindVariant(key) {
    return key === "f" || key === "F" || key === "t" || key === "T";
}
function invertFindVariant(variant) {
    switch (variant) {
        case "f":
            return "F";
        case "F":
 ...
 */
function tryEnterInsertMode(state, key) {
    if (!isInsertKey(key))
        return null;
 ...
 */
function tryHandlePrefix(state, key, now) {
 ...
 */
function tryHandleNavigation(state, key, count) {
    ...
    switch (key) {
        ...
        case "G": {
            const { row } = getRowCol(text, cursor);
            const { lines } = getLinesInfo(text);
            const targetRow = state.count == null ? lines.length - 1 : Math.max(0, Math.min(lines.length - 1, count - 1));
            const delta = targetRow - row;
        ...
        case ";": {
            ...
            const dest = getFindMotionDestination(text, cursor, state.lastFind.variant, state.lastFind.char, count);
            if (dest == null) {
                return handleCommand(state, { desiredColumn: null });
            }
            return handleCommand(state, {
 ...
 */
function tryHandleEdit(state, key, count) {
    const { text, cursor, yankBuffer } = state;
    const safeCount = Math.max(1, Math.min(10000, count));
    switch (key) {
        case "x": {
        ...
        case "~": {
            ...
            for (let i = 0; i < safeCount; i++) {
                ...
                if (nextCursor >= nextText.length - 1)
                    break;
                nextCursor++;
            }
            return handleCommand(state, {
 ...
 */
function tryHandleOperator(state, key, now) {
 ...
 * - "g", "5g"
 */
export function formatPendingCommand(pending, count) {
