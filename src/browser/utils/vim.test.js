 ...
 */
function executeVimCommands(initial, keys) {
    let state = { ...initial };
    const applyBrowserKey = (current, key, ctrl) => {
        if (ctrl)
            return current;
...
describe("Vim Command Integration Tests", () => {
    ...
    describe("Mode Transitions", () => {
        test("ESC enters normal mode from insert", () => {
            const state = executeVimCommands({ ...initialState, text: "hello", cursor: 5, mode: "insert" }, ["Escape"]);
            expect(state.mode).toBe("normal");
            expect(state.cursor).toBe(4); // Clamps to last char
        });
        test("i enters insert mode at cursor", () => {
            const state = executeVimCommands({ ...initialState, text: "hello", cursor: 2, mode: "normal" }, ["i"]);
            expect(state.mode).toBe("insert");
            expect(state.cursor).toBe(2);
        });
        test("a enters insert mode after cursor", () => {
            const state = executeVimCommands({ ...initialState, text: "hello", cursor: 2, mode: "normal" }, ["a"]);
            expect(state.mode).toBe("insert");
            expect(state.cursor).toBe(3);
        });
        test("o opens line below", () => {
            const state = executeVimCommands({ ...initialState, text: "hello\nworld", cursor: 2, mode: "normal" }, ["o"]);
            expect(state.mode).toBe("insert");
    ...
    describe("Visual mode", () => {
        test("v enters visual mode and Escape exits to normal at selection start", () => {
            const afterV = executeVimCommands({ ...initialState, text: "hello world", cursor: 0, mode: "normal" }, ["v"]);
        ...
        test("motions extend the selection (v + w)", () => {
            const state = executeVimCommands({ ...initialState, text: "hello world", cursor: 0, mode: "normal" }, ["v", "w"]);
        ...
        test("visual operators act on the selection (d/c/y)", () => {
            const initial = { ...initialState, text: "hello world", cursor: 0, mode: "normal" };
        ...
        test("V enters visual line mode and d deletes whole lines", () => {
            const state = executeVimCommands({ ...initialState, text: "one\ntwo\nthree", cursor: 0, mode: "normal" }, ["V", "j", "d"]);
            expect(state.mode).toBe("normal");
        ...
        test("V enters visual line mode and c changes whole lines", () => {
            const state = executeVimCommands({ ...initialState, text: "one\ntwo\nthree", cursor: 0, mode: "normal" }, ["V", "j", "c"]);
            expect(state.mode).toBe("insert");
            expect(state.visualAnchor).toBeNull();
    ...
    describe("Navigation", () => {
        test("w moves to next word", () => {
            const state = executeVimCommands({ ...initialState, text: "hello world foo", cursor: 0, mode: "normal" }, ["w"]);
            expect(state.cursor).toBe(6);
        });
        test("b moves to previous word", () => {
            const state = executeVimCommands({ ...initialState, text: "hello world foo", cursor: 12, mode: "normal" }, ["b"]);
            expect(state.cursor).toBe(6);
        });
        test("$ moves to end of line", () => {
            const state = executeVimCommands({ ...initialState, text: "hello world", cursor: 0, mode: "normal" }, ["$"]);
            expect(state.cursor).toBe(10); // On last char, not past it
        });
        test("0 moves to start of line", () => {
            const state = executeVimCommands({ ...initialState, text: "hello world", cursor: 10, mode: "normal" }, ["0"]);
            expect(state.cursor).toBe(0);
        });
    });
    describe("Navigation", () => {
    ...
    describe("Find/till (f/F/t/T)", () => {
        test("fx moves to next match on the line", () => {
            const state = executeVimCommands({ ...initialState, text: "abcxdefx", cursor: 0, mode: "normal" }, ["f", "x"]);
            expect(state.cursor).toBe(3);
            expect(state.lastFind).toEqual({ variant: "f", char: "x" });
        });
        test("tx moves to just before next match", () => {
            const state = executeVimCommands({ ...initialState, text: "abcxdefx", cursor: 0, mode: "normal" }, ["t", "x"]);
            expect(state.cursor).toBe(2);
            expect(state.lastFind).toEqual({ variant: "t", char: "x" });
        });
        test("Tx moves to just after previous match", () => {
            const state = executeVimCommands({ ...initialState, text: "abcxdefx", cursor: 7, mode: "normal" }, ["T", "x"]);
            expect(state.cursor).toBe(4);
            expect(state.lastFind).toEqual({ variant: "T", char: "x" });
        });
        test("F, finds punctuation backwards", () => {
            const state = executeVimCommands({ ...initialState, text: "a,b,c", cursor: 4, mode: "normal" }, ["F", ","]);
            expect(state.cursor).toBe(3);
            expect(state.lastFind).toEqual({ variant: "F", char: "," });
        });
        test("3fx uses count to find the third match", () => {
            const state = executeVimCommands({ ...initialState, text: "axxx", cursor: 0, mode: "normal" }, ["3", "f", "x"]);
            expect(state.cursor).toBe(3);
        });
    });
    describe("Find repeat (; and ,)", () => {
        test("; repeats last find in the same direction", () => {
            const state = executeVimCommands({ ...initialState, text: "axxx", cursor: 0, mode: "normal" }, ["f", "x", ";"]);
            expect(state.cursor).toBe(2);
            expect(state.lastFind).toEqual({ variant: "f", char: "x" });
        });
    ...
    describe("WORD Motions (W/B/E)", () => {
        test("W moves to next WORD (whitespace-separated)", () => {
            const state = executeVimCommands({ ...initialState, text: "asd-f asdf asdf", cursor: 0, mode: "normal" }, ["W"]);
            expect(state.cursor).toBe(6);
        });
        test("B moves to previous WORD", () => {
            const state = executeVimCommands({ ...initialState, text: "asd-f asdf asdf", cursor: 11, mode: "normal" }, ["B"]);
            expect(state.cursor).toBe(6);
        });
        test("E moves to end of current WORD", () => {
            const state = executeVimCommands({ ...initialState, text: "asd-f asdf asdf", cursor: 0, mode: "normal" }, ["E"]);
            expect(state.cursor).toBe(4);
        });
        test("E at end of WORD moves to end of next WORD", () => {
            const state = executeVimCommands({ ...initialState, text: "asd-f asdf asdf", cursor: 4, mode: "normal" }, ["E"]);
            expect(state.cursor).toBe(9);
        });
    });
    describe("Simple Edits", () => {
        test("x deletes character under cursor", () => {
            const state = executeVimCommands({ ...initialState, text: "hello", cursor: 1, mode: "normal" }, ["x"]);
            expect(state.text).toBe("hllo");
            expect(state.cursor).toBe(1);
            expect(state.yankBuffer).toBe("e");
        });
        test("p pastes after cursor", () => {
            const state = executeVimCommands({ ...initialState, text: "hello", cursor: 2, mode: "normal", yankBuffer: "XX" }, ["p"]);
        ...
        test("s substitutes character under cursor", () => {
            const state = executeVimCommands({ ...initialState, text: "hello", cursor: 1, mode: "normal" }, ["s"]);
            expect(state.text).toBe("hllo");
            expect(state.cursor).toBe(1);
            expect(state.mode).toBe("insert");
            expect(state.yankBuffer).toBe("e");
        ...
        test("~ toggles case of character under cursor", () => {
            const state = executeVimCommands({ ...initialState, text: "HeLLo", cursor: 0, mode: "normal" }, ["~"]);
            expect(state.text).toBe("heLLo");
            expect(state.cursor).toBe(1);
        });
        ...
        test("~ on non-letter does nothing but advances cursor", () => {
            const state = executeVimCommands({ ...initialState, text: "a 1 b", cursor: 1, mode: "normal" }, ["~"]);
            expect(state.text).toBe("a 1 b");
            expect(state.cursor).toBe(2);
        });
        test("~ at end of text does not advance cursor", () => {
            const state = executeVimCommands({ ...initialState, text: "hello", cursor: 4, mode: "normal" }, ["~"]);
            expect(state.text).toBe("hellO");
            expect(state.cursor).toBe(4);
        });
    });
    ...
    describe("Operator + Motion: Delete", () => {
        test("d$ deletes to end of line", () => {
            const state = executeVimCommands({ ...initialState, text: "hello world", cursor: 6, mode: "normal" }, ["d", "$"]);
            expect(state.text).toBe("hello ");
        ...
        test("d0 deletes to beginning of line", () => {
            const state = executeVimCommands({ ...initialState, text: "hello world", cursor: 6, mode: "normal" }, ["d", "0"]);
            expect(state.text).toBe("world");
            expect(state.yankBuffer).toBe("hello ");
        });
        test("dw deletes to next word", () => {
            const state = executeVimCommands({ ...initialState, text: "hello world foo", cursor: 0, mode: "normal" }, ["d", "w"]);
            expect(state.text).toBe("world foo");
            expect(state.yankBuffer).toBe("hello ");
        });
        test("dfx deletes through next matching character", () => {
            const state = executeVimCommands({ ...initialState, text: "abcxdef", cursor: 0, mode: "normal" }, ["d", "f", "x"]);
        ...
        test("dFx deletes backward through previous matching character (includes cursor)", () => {
            const state = executeVimCommands({ ...initialState, text: "abcxdef", cursor: 6, mode: "normal" }, ["d", "F", "x"]);
        ...
        test("dTx deletes backward till after previous matching character (includes cursor)", () => {
            const state = executeVimCommands({ ...initialState, text: "abcxdef", cursor: 6, mode: "normal" }, ["d", "T", "x"]);
        ...
        test("dW deletes to next WORD (whitespace-separated)", () => {
            const state = executeVimCommands({ ...initialState, text: "foo-bar baz", cursor: 0, mode: "normal" }, ["d", "W"]);
            expect(state.text).toBe("baz");
            expect(state.yankBuffer).toBe("foo-bar ");
        });
        test("db deletes to previous word", () => {
            const state = executeVimCommands({ ...initialState, text: "hello world foo", cursor: 12, mode: "normal" }, ["d", "b"]);
            expect(state.text).toBe("hello foo");
        });
    });
    describe("Operator + Motion: Change", () => {
        test("c$ changes to end of line", () => {
            const state = executeVimCommands({ ...initialState, text: "hello world", cursor: 6, mode: "normal" }, ["c", "$"]);
            expect(state.text).toBe("hello ");
            expect(state.mode).toBe("insert");
            expect(state.cursor).toBe(6);
        });
        ...
        test("c0 changes to beginning of line", () => {
            const state = executeVimCommands({ ...initialState, text: "hello world", cursor: 6, mode: "normal" }, ["c", "0"]);
            expect(state.text).toBe("world");
            expect(state.mode).toBe("insert");
        });
        test("ctx changes until next matching character", () => {
            const state = executeVimCommands({ ...initialState, text: "abcxdef", cursor: 0, mode: "normal" }, ["c", "t", "x"]);
        ...
        test("cFx changes backward through previous matching character (includes cursor)", () => {
            const state = executeVimCommands({ ...initialState, text: "abcxdef", cursor: 6, mode: "normal" }, ["c", "F", "x"]);
            expect(state.text).toBe("abc");
            expect(state.cursor).toBe(3);
            expect(state.mode).toBe("insert");
            expect(state.yankBuffer).toBe("xdef");
        ...
        test("cTx changes backward till after previous matching character (includes cursor)", () => {
            const state = executeVimCommands({ ...initialState, text: "abcxdef", cursor: 6, mode: "normal" }, ["c", "T", "x"]);
            expect(state.text).toBe("abcx");
            expect(state.cursor).toBe(4);
            expect(state.mode).toBe("insert");
            expect(state.yankBuffer).toBe("def");
        ...
        test("cw changes to end of word (like ce)", () => {
            const state = executeVimCommands({ ...initialState, text: "hello world", cursor: 0, mode: "normal" }, ["c", "w"]);
            expect(state.text).toBe(" world");
            expect(state.mode).toBe("insert");
        });
        test("cw differs from dw (cw like ce)", () => {
            const dwState = executeVimCommands({ ...initialState, text: "hello world", cursor: 0, mode: "normal" }, ["d", "w"]);
            expect(dwState.text).toBe("world");
            const cwState = executeVimCommands({ ...initialState, text: "hello world", cursor: 0, mode: "normal" }, ["c", "w"]);
            expect(cwState.text).toBe(" world");
        });
        test("cW changes to end of WORD when on WORD char (like cE)", () => {
            const state = executeVimCommands({ ...initialState, text: "foo-bar baz", cursor: 0, mode: "normal" }, ["c", "W"]);
            expect(state.text).toBe(" baz");
            expect(state.yankBuffer).toBe("foo-bar");
            expect(state.mode).toBe("insert");
        });
        test("cE changes to end of WORD", () => {
            const state = executeVimCommands({ ...initialState, text: "foo-bar baz", cursor: 0, mode: "normal" }, ["c", "E"]);
            expect(state.text).toBe(" baz");
            expect(state.mode).toBe("insert");
        });
    });
    describe("Operator + Motion: Yank", () => {
        test("y$ yanks to end of line", () => {
            const state = executeVimCommands({ ...initialState, text: "hello world", cursor: 6, mode: "normal" }, ["y", "$"]);
            expect(state.text).toBe("hello world"); // Text unchanged
            expect(state.yankBuffer).toBe("world");
            expect(state.mode).toBe("normal");
        });
        test("y0 yanks to beginning of line", () => {
            const state = executeVimCommands({ ...initialState, text: "hello world", cursor: 6, mode: "normal" }, ["y", "0"]);
            expect(state.text).toBe("hello world");
            expect(state.yankBuffer).toBe("hello ");
        });
        test("yw yanks to next word", () => {
            const state = executeVimCommands({ ...initialState, text: "hello world", cursor: 0, mode: "normal" }, ["y", "w"]);
            expect(state.text).toBe("hello world");
            expect(state.yankBuffer).toBe("hello ");
        });
    });
    describe("Operator + Text Objects", () => {
        test("daw deletes a word including trailing whitespace", () => {
            const state = executeVimCommands({ ...initialState, text: "hello world foo", cursor: 0, mode: "normal" }, ["d", "a", "w"]);
            expect(state.text).toBe("world foo");
            expect(state.cursor).toBe(0);
            expect(state.mode).toBe("normal");
            expect(state.yankBuffer).toBe("hello ");
        ...
        test("caw changes a word including trailing whitespace", () => {
            const state = executeVimCommands({ ...initialState, text: "one two three", cursor: 4, mode: "normal" }, ["c", "a", "w"]);
        ...
        test('di" is a no-op when no quotes are found on the line', () => {
            const state = executeVimCommands({ ...initialState, text: "hello world", cursor: 0, mode: "normal", yankBuffer: "prev" }, ["d", "i", '"']);
            expect(state.text).toBe("hello world");
            expect(state.mode).toBe("normal");
    ...
    describe("Complex Workflows", () => {
        test("ESC then d$ deletes from insert cursor to end", () => {
            const state = executeVimCommands({ ...initialState, text: "hello world", cursor: 6, mode: "insert" }, ["Escape", "d", "$"]);
        ...
        test("navigate with w, then delete with dw", () => {
            const state = executeVimCommands({ ...initialState, text: "one two three", cursor: 0, mode: "normal" }, ["w", "d", "w"]);
            expect(state.text).toBe("one three");
        });
        test("yank line, navigate, paste", () => {
            const state = executeVimCommands({ ...initialState, text: "first\nsecond\nthird", cursor: 0, mode: "normal" }, ["y", "y", "j", "j", "p"]);
        ...
        test("delete word, move, paste", () => {
            const state = executeVimCommands({ ...initialState, text: "hello world foo", cursor: 0, mode: "normal" }, ["d", "w", "w", "p"]);
            expect(state.yankBuffer).toBe("hello ");
    ...
    describe("Edge Cases", () => {
        ...
        test("w at end of text", () => {
            const state = executeVimCommands({ ...initialState, text: "hello", cursor: 4, mode: "normal" }, ["w"]);
            expect(state.cursor).toBe(4); // Clamps to last char
        });
        test("d$ at end of line deletes last char", () => {
            const state = executeVimCommands({ ...initialState, text: "hello", cursor: 4, mode: "normal" }, ["d", "$"]);
    ...
    describe("Reported Issues", () => {
        test("issue #1: ciw should delete inner word correctly", () => {
            ...
            const state = executeVimCommands({ ...initialState, text: "hello world foo", cursor: 6, mode: "normal" }, ["c", "i", "w"]);
    ...
    describe("e/E motion", () => {
        test("e moves to end of current word", () => {
            const state = executeVimCommands({ ...initialState, text: "hello world", cursor: 1, mode: "normal" }, ["e"]);
            expect(state.cursor).toBe(4);
        });
        test("de deletes to end of word", () => {
            const state = executeVimCommands({ ...initialState, text: "hello world", cursor: 1, mode: "normal" }, ["d", "e"]);
            expect(state.text).toBe("h world");
            expect(state.yankBuffer).toBe("ello");
        });
        test("ce changes to end of word", () => {
            const state = executeVimCommands({ ...initialState, text: "hello world", cursor: 1, mode: "normal" }, ["c", "e"]);
            expect(state.text).toBe("h world");
            expect(state.mode).toBe("insert");
        });
        test("e at end of word moves to end of next word", () => {
            // Bug: when cursor is at end of word, 'e' should move to end of next word
            const state = executeVimCommands({ ...initialState, text: "hello world foo", cursor: 4, mode: "normal" }, // cursor on 'o' (end of "hello")
            ["e"]);
        ...
        test("e at end of word with punctuation moves correctly", () => {
            const state = executeVimCommands({ ...initialState, text: "hello, world", cursor: 4, mode: "normal" }, // cursor on 'o' (end of "hello")
            ["e"]);
    ...
    describe("Count Prefixes", () => {
        test("3w moves forward three words", () => {
            const state = executeVimCommands({ ...initialState, text: "one two three four", cursor: 0, mode: "normal" }, ["3", "w"]);
            expect(state.cursor).toBe(14);
        });
        test("20l moves right 20 characters (0 appends to count)", () => {
            const state = executeVimCommands({ ...initialState, text: "abcdefghijklmnopqrstuvwxyz", cursor: 0, mode: "normal" }, ["2", "0", "l"]);
            expect(state.cursor).toBe(20);
        });
        test("5x deletes five characters", () => {
            const state = executeVimCommands({ ...initialState, text: "hello world", cursor: 0, mode: "normal" }, ["5", "x"]);
            expect(state.text).toBe(" world");
            expect(state.cursor).toBe(0);
            expect(state.yankBuffer).toBe("hello");
        });
        test("2dd deletes two lines", () => {
            const state = executeVimCommands({ ...initialState, text: "one\ntwo\nthree\nfour", cursor: 0, mode: "normal" }, ["2", "d", "d"]);
            expect(state.text).toBe("three\nfour");
            expect(state.cursor).toBe(0);
            expect(state.yankBuffer).toBe("one\ntwo\n");
        });
        test("2cc changes two lines into a single replacement line", () => {
            const state = executeVimCommands({ ...initialState, text: "one\ntwo\nthree", cursor: 0, mode: "normal" }, ["2", "c", "c"]);
            expect(state.text).toBe("\nthree");
            expect(state.cursor).toBe(0);
            expect(state.mode).toBe("insert");
            expect(state.yankBuffer).toBe("one\ntwo");
        ...
        test("2c_ changes two lines into a single replacement line", () => {
            const state = executeVimCommands({ ...initialState, text: "one\ntwo\nthree", cursor: 0, mode: "normal" }, ["2", "c", "_"]);
            expect(state.text).toBe("\nthree");
            expect(state.cursor).toBe(0);
            expect(state.mode).toBe("insert");
            expect(state.yankBuffer).toBe("one\ntwo");
        ...
        test("d3w deletes three words", () => {
            const state = executeVimCommands({ ...initialState, text: "one two three four", cursor: 0, mode: "normal" }, ["d", "3", "w"]);
    ...
    describe("Undo / Redo / Dot repeat", () => {
        test("u and Ctrl-r undo/redo normal-mode edits (x)", () => {
            const initial = { ...initialState, text: "abc", cursor: 1, mode: "normal" };
        ...
        test("insert-mode typing is grouped into a single undo step", () => {
            const initial = { ...initialState, text: "hello", cursor: 0, mode: "normal" };
        ...
        test("dot repeats last single-key edit (x)", () => {
            const initial = { ...initialState, text: "abcd", cursor: 0, mode: "normal" };
            const state = executeVimCommands(initial, ["x", "."]);
            expect(state.text).toBe("cd");
            expect(state.cursor).toBe(0);
        });
        test("dot repeats last visual delete (v...d)", () => {
            const initial = { ...initialState, text: "abcdefghi", cursor: 0, mode: "normal" };
    ...
    describe("_ motion (first non-whitespace character)", () => {
        test("_ moves to first non-whitespace character", () => {
            const state = executeVimCommands({ ...initialState, text: "  hello world", cursor: 10, mode: "normal" }, ["_"]);
