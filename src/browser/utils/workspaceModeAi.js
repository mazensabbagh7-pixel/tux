...
export function resolveWorkspaceAiSettingsForAgent(args) {
    ...
    const fallbackAgentId = activeDescriptor?.base ?? (normalizedAgentId === "plan" ? "plan" : "exec");
    const fallbackIds = fallbackAgentId && fallbackAgentId !== normalizedAgentId
    ...
    const inheritedConfiguredDefaults = hasWorkspaceOverrideForAgent || configuredDefaults !== undefined
        ? undefined
        : fallbackIds
            .slice(1)
    ...
    const descriptorDefaults = fallbackIds
        ...
        .find((entry) => entry !== undefined);
    const configuredModelDefault = configuredDefaults?.modelString ?? inheritedConfiguredDefaults?.modelString;
    const configuredThinkingDefault = configuredDefaults?.thinkingLevel ?? inheritedConfiguredDefaults?.thinkingLevel;
    ...
    const candidateModel = configuredModelDefault ??
        fallbackIds
    ...
    const resolvedModel = typeof candidateModel === "string" && candidateModel.trim().length > 0
        ? candidateModel
        : args.fallbackModel;
    const candidateThinking = configuredThinkingDefault ??
        fallbackIds
...
//# sourceMappingURL=workspaceModeAi.js.map