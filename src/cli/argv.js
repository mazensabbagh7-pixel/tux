 ...
 */
export function detectCliEnvironment(versions = process.versions, defaultApp = process.defaultApp) {
 ...
 */
export function getParseOptions(env = detectCliEnvironment()) {
    return { from: env.isPackagedElectron ? "electron" : "node" };
}
/**
 * Get the subcommand from argv (e.g., "server", "api", "run").
 */
export function getSubcommand(argv = process.argv, env = detectCliEnvironment()) {
 ...
 */
export function getArgsAfterSplice(argv = process.argv, env = detectCliEnvironment()) {
 ...
 */
export const CLI_GLOBAL_FLAGS = ["--help", "-h", "--version", "-v"];
/**
 ...
 */
export function isElectronLaunchArg(subcommand, env = detectCliEnvironment()) {
    if (!env.isElectron)
        return false;
    ...
    if (env.isPackagedElectron) {
        ...
        return Boolean(subcommand?.startsWith("-") &&
            !CLI_GLOBAL_FLAGS.includes(subcommand));
    }
 ...
 */
export function isCommandAvailable(command, env = detectCliEnvironment()) {
    if (command === "run") {
