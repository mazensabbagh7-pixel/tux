...
import { CLI_GLOBAL_FLAGS, detectCliEnvironment, getParseOptions, getSubcommand, getArgsAfterSplice, isCommandAvailable, isElectronLaunchArg, } from "./argv";
...
describe("isCommandAvailable", () => {
    ...
    test("server is available everywhere", () => {
        ...
        expect(isCommandAvailable("server", detectCliEnvironment({ electron: "33.0.0" }, true))).toBe(true);
        expect(isCommandAvailable("server", detectCliEnvironment({ electron: "33.0.0" }, undefined))).toBe(true);
    });
    test("api is available everywhere", () => {
        expect(isCommandAvailable("api", detectCliEnvironment({}, undefined))).toBe(true);
        expect(isCommandAvailable("api", detectCliEnvironment({ electron: "33.0.0" }, true))).toBe(true);
        expect(isCommandAvailable("api", detectCliEnvironment({ electron: "33.0.0" }, undefined))).toBe(true);
    });
    test("desktop is available in electron environments", () => {
        // In electron dev mode, always available
        expect(isCommandAvailable("desktop", detectCliEnvironment({ electron: "33.0.0" }, true))).toBe(true);
        // In packaged electron, always available
        expect(isCommandAvailable("desktop", detectCliEnvironment({ electron: "33.0.0" }, undefined))).toBe(true);
    });
