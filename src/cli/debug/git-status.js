...
function testGitStatus(workspaceId, workspacePath) {
    console.log("\n" + "=".repeat(80));
    ...
    try {
        ...
        const { primaryBranch, ahead, behind, dirtyCount, outgoingAdditions, outgoingDeletions, incomingAdditions, incomingDeletions, } = parsed;
        const dirty = dirtyCount > 0;
        console.log("\n--- PARSED RESULT ---");
        console.log(`✅ Success: { base: ${primaryBranch}, ahead: ${ahead}, behind: ${behind}, dirty: ${dirty}, outgoing: +${outgoingAdditions}/-${outgoingDeletions}, incoming: +${incomingAdditions}/-${incomingDeletions} }`);
...
export function gitStatusCommand(workspaceId) {
    const muxSrcDir = getMuxSrcDir();
    ...
    if (workspaceId) {
        ...
        console.log(`Testing ${toTest.length} workspaces (use "bun debug git-status <id>" for specific workspace)...\n`);
