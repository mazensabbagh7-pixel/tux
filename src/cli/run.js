...
import { isCaughtUpMessage, isReasoningDelta, isReasoningEnd, isStreamAbort, isStreamDelta, isStreamEnd, isStreamError, isStreamStart, isToolCallDelta, isToolCallEnd, isToolCallStart, isUsageDelta, } from "../common/orpc/types";
import { createDisplayUsage } from "../common/utils/tokens/displayUsage";
import { getTotalCost, formatCostWithDollar, sumUsageHistory, } from "../common/utils/tokens/usageAggregator";
import { formatToolStart, formatToolEnd, formatGenericToolStart, formatGenericToolEnd, isMultilineResultTool, } from "./toolFormatters";
...
import { DEFAULT_THINKING_LEVEL, THINKING_DISPLAY_LABELS, parseThinkingInput, } from "../common/types/thinking";
...
function parseThinkingLevel(value) {
    if (!value)
        return DEFAULT_THINKING_LEVEL; // Default for mux run
...
const VALID_EXPERIMENT_IDS = new Set(Object.values(EXPERIMENT_IDS));
...
program
    ...
    .option("-t, --thinking <level>", `thinking level: ${THINKING_LABELS_LIST}`, THINKING_DISPLAY_LABELS[DEFAULT_THINKING_LEVEL])
...
async function main() {
    ...
    try {
        ...
        const tempDir = __addDisposableResource(env_1, new DisposableTempDir("mux-run"), false);
        ...
        const model = resolveModelAlias(opts.model);
        ...
        log.info(`Runtime: ${runtimeConfig.type}${runtimeConfig.type === "ssh" ? ` (${runtimeConfig.host})` : ""}`);
        ...
        const { aiService, historyService, initStateManager, backgroundProcessManager, mcpServerManager, providerService, workspaceService, } = createCoreServices({
            config,
        ...
        const chatListener = (event) => {
            ...
            if (isStreamEnd(payload)) {
                ...
                if (payload.metadata.usage) {
                    displayUsage = createDisplayUsage(payload.metadata.usage, payload.metadata.model, payload.metadata.providerMetadata);
                }
                else {
                    // Fallback: use cumulative usage from the last usage-delta event
                    const fallback = latestUsageDelta.get(payload.messageId);
                    if (fallback) {
                        displayUsage = createDisplayUsage(fallback.usage, fallback.model, fallback.providerMetadata);
                    }
                }
                if (displayUsage) {
            ...
            if (isUsageDelta(payload)) {
                ...
                // Budget enforcement
                if (budget !== undefined) {
                    const displayUsage = createDisplayUsage(payload.cumulativeUsage, model, payload.cumulativeProviderMetadata);
