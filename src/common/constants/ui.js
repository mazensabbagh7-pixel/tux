 ...
 */
export function buildCompactionPrompt(targetWords) {
 ...
 */
export function resolveSectionColor(color) {
    if (!color) {
    ...
    const paletteMatch = SECTION_COLOR_PALETTE.find(([name]) => name.toLowerCase() === trimmed.toLowerCase());
    if (paletteMatch) {
