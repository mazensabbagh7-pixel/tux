...
describe("rebuildContinueMessage", () => {
    ...
    test("uses persisted values when available", () => {
        const result = rebuildContinueMessage({ text: "continue", model: "persisted-model", agentId: "plan" }, { model: "default", agentId: "exec" });
        expect(result?.text).toBe("continue");
        expect(result?.model).toBe("persisted-model");
        expect(result?.agentId).toBe("plan");
    });
    test("migrates legacy mode to agentId", () => {
        const result = rebuildContinueMessage({ text: "continue", mode: "plan" }, { model: "default-model", agentId: "exec" });
        expect(result?.agentId).toBe("plan");
    });
    test("prefers persisted agentId over legacy mode", () => {
        const result = rebuildContinueMessage({ text: "continue", agentId: "custom-agent", mode: "plan" }, { model: "default-model", agentId: "exec" });
        expect(result?.agentId).toBe("custom-agent");
    });
    test("uses defaults when persisted values missing", () => {
        const result = rebuildContinueMessage({ text: "continue" }, { model: "default-model", agentId: "plan" });
        expect(result?.text).toBe("continue");
    ...
    test("preserves muxMetadata from persisted data", () => {
        ...
        const result = rebuildContinueMessage({ text: "continue", muxMetadata }, { model: "m", agentId: "exec" });
        expect(result?.muxMetadata).toEqual(muxMetadata);
    });
    test("preserves reviews from persisted data", () => {
        const review = makeReview("a.ts");
        const result = rebuildContinueMessage({ text: "review this", reviews: [review] }, { model: "m", agentId: "exec" });
        expect(result?.reviews?.length).toBe(1);
