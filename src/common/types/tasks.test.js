...
import { DEFAULT_TASK_SETTINGS, SYSTEM1_BASH_OUTPUT_COMPACTION_LIMITS, TASK_SETTINGS_LIMITS, normalizeTaskSettings, } from "./tasks";
describe("normalizeTaskSettings", () => {
    ...
    test("clamps values into valid ranges", () => {
        ...
        expect(normalized.bashOutputCompactionMinLines).toBe(SYSTEM1_BASH_OUTPUT_COMPACTION_LIMITS.bashOutputCompactionMinLines.min);
        expect(normalized.bashOutputCompactionMinTotalBytes).toBe(SYSTEM1_BASH_OUTPUT_COMPACTION_LIMITS.bashOutputCompactionMinTotalBytes.max);
        expect(normalized.bashOutputCompactionMaxKeptLines).toBe(SYSTEM1_BASH_OUTPUT_COMPACTION_LIMITS.bashOutputCompactionMaxKeptLines.min);
        expect(normalized.bashOutputCompactionTimeoutMs).toBe(SYSTEM1_BASH_OUTPUT_COMPACTION_LIMITS.bashOutputCompactionTimeoutMs.min);
    });
