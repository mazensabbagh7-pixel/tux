...
import { normalizeGatewayModel, getModelName, supports1MContext, isValidModelFormat, resolveModelAlias, } from "./models";
describe("normalizeGatewayModel", () => {
    it("should convert mux-gateway:provider/model to provider:model", () => {
        expect(normalizeGatewayModel("mux-gateway:anthropic/claude-opus-4-5")).toBe("anthropic:claude-opus-4-5");
        expect(normalizeGatewayModel("mux-gateway:openai/gpt-4o")).toBe("openai:gpt-4o");
        expect(normalizeGatewayModel("mux-gateway:google/gemini-2.5-pro")).toBe("google:gemini-2.5-pro");
    });
