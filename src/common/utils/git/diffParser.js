 ...
 */
export function extractAllHunks(fileDiffs) {
 ...
 */
export function buildGitDiffCommand(diffBase, includeUncommitted, pathFilter, command) {
    const flags = command === "numstat"
        ? " -M --numstat"
        : command === "name-status"
            ? " -M --name-status"
            : " -M";
