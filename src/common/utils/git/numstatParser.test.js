...
describe("extractNewPath", () => {
    ...
    test("extracts new path from plain arrow syntax", () => {
        ...
        expect(extractNewPath("src/helpers.ts => src/helpers-renamed.ts")).toBe("src/helpers-renamed.ts");
    });
    test("extracts new path from rename syntax", () => {
        ...
        expect(extractNewPath("src/components/{ChatMetaSidebar.tsx => RightSidebar.tsx}")).toBe("src/components/RightSidebar.tsx");
    });
    ...
    test("handles complex paths", () => {
        expect(extractNewPath("very/long/path/{oldname.tsx => newname.tsx}")).toBe("very/long/path/newname.tsx");
    });
});
