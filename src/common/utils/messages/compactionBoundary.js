...
export function isDurableCompactionBoundaryMarker(message) {
 ...
 */
export function findLatestCompactionBoundaryIndex(messages) {
 ...
 */
export function sliceMessagesFromLatestCompactionBoundary(messages) {
    const boundaryIndex = findLatestCompactionBoundaryIndex(messages);
    if (boundaryIndex === -1) {
        return messages;
    }
    assert(boundaryIndex >= 0 && boundaryIndex < messages.length, "compaction boundary index must be within message history bounds");
    ...
    assert(isDurableCompactionBoundaryMarker(sliced[0]), "compaction boundary slicing must start on a durable compaction boundary message");
    return sliced;
}
