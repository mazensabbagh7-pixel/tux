 ...
 */
export function extractEditedFilePaths(messages) {
    const editedFiles = [];
    const seen = new Set();
    ...
    for (let i = messages.length - 1; i >= 0; i--) {
        ...
        for (const part of message.parts) {
            if (part.type !== "dynamic-tool")
                continue;
            if (!FILE_EDIT_TOOL_NAMES.includes(part.toolName))
                continue;
 ...
 */
export function extractEditedFileDiffs(messages) {
