...
function getPlanBodyText(startHereText) {
    ...
    const textWithoutNote = textWithoutStartHereHeading.replace(/Note: This chat already contains the full plan[^\n]*/g, "");
 ...
 */
export function isStartHerePlanSummaryMessage(message) {
    ...
    if (message.metadata?.compacted !== "user")
        return false;
    if (message.metadata?.agentId !== "plan")
        return false;
    ...
    if (!text.includes(START_HERE_PLAN_PATH_NOTE_MARKER))
        return false;
    ...
    if (planBody.length === 0)
        return false;
    if (planBody.startsWith(START_HERE_PLAN_PLACEHOLDER_PREFIX))
        return false;
    return true;
}
export function hasStartHerePlanSummary(messages) {
    for (let i = messages.length - 1; i >= 0; i--) {
        if (isStartHerePlanSummaryMessage(messages[i]))
            return true;
    }
    return false;
}
