 ...
 */
export function enforceThinkingPolicy(modelString, requested) {
    const allowed = getThinkingPolicyForModel(modelString);
    if (allowed.includes(requested)) {
        return requested;
    }
    const orderedAllowed = [...allowed].sort((left, right) => THINKING_LEVELS.indexOf(left) - THINKING_LEVELS.indexOf(right));
 ...
 */
export function resolveThinkingInput(input, modelString) {
    ...
    const sorted = [...policy].sort((a, b) => THINKING_LEVELS.indexOf(a) - THINKING_LEVELS.indexOf(b));
