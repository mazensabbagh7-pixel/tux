 ...
 */
export function calculateTokenMeterData(usage, model, use1M, verticalProportions = false) {
    if (!usage)
        return { segments: [], totalTokens: 0, totalPercentage: 0 };
    const modelStats = getModelStats(model);
    const maxTokens = use1M && supports1MContext(model) ? 1000000 : modelStats?.max_input_tokens;
...
export function getSegmentLabel(type) {
