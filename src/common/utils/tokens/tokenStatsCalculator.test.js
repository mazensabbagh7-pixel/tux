...
import { collectUniqueToolNames, countEncryptedWebSearchTokens, createDisplayUsage, extractSyncMetadata, extractToolOutputData, getConsumerInfoForToolCall, isEncryptedWebSearch, mergeResults, } from "./tokenStatsCalculator";
describe("createDisplayUsage", () => {
    ...
    test("works with non-OpenAI providers that don't have providerMetadata.openai", () => {
        ...
        const result = createDisplayUsage(usage, "anthropic:claude-sonnet-4-20250514", providerMetadata);
...
describe("getConsumerInfoForToolCall", () => {
    test("labels task tool calls as task", () => {
        expect(getConsumerInfoForToolCall("task", { subagent_type: "exec", prompt: "hi", title: "t" })).toEqual({
            consumer: "task",
