...
function sanitizeBashDisplayName(rawName) {
    ...
    if (name.length === 0)
        return DEFAULT_BASH_DISPLAY_NAME;
...
export function getDefaultBashDisplayName(script) {
    const firstNonEmptyLine = script
        .split(/\r?\n/)
        .map((line) => line.trim())
        .find((line) => line.length > 0) ?? "";
    return sanitizeBashDisplayName(firstNonEmptyLine);
}
export function resolveBashDisplayName(script, displayName) {
