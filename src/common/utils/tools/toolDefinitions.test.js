...
describe("TOOL_DEFINITIONS", () => {
    ...
    it.each(filePathAliasCases)("accepts file_path alias for $toolName and normalizes to path", ({ toolName, args }) => {
    ...
    it.each(filePathAliasCases)("prefers canonical path over file_path for $toolName", ({ toolName, args }) => {
        const parsed = TOOL_DEFINITIONS[toolName].schema.safeParse({
            ...args,
    ...
    it.each(filePathAliasCases)("rejects $toolName when path is present but invalid, even if file_path is provided", ({ toolName, args }) => {
        const parsed = TOOL_DEFINITIONS[toolName].schema.safeParse({
            ...args,
            path: 123,
    ...
    it.each(filePathAliasCases)("rejects $toolName calls missing both path and file_path", ({ toolName, args }) => {
    ...
    it("asks for clarification via ask_user_question (instead of emitting open questions)", () => {
        expect(TOOL_DEFINITIONS.ask_user_question.description).toContain("MUST be used when you need user clarification");
        expect(TOOL_DEFINITIONS.ask_user_question.description).toContain("Do not output a list of open questions");
    });
