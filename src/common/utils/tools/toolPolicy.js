...
export function applyToolPolicy(tools, policy) {
    ...
    for (const filter of policy) {
        ...
        if (matches.length > 1) {
            throw new Error(`Tool policy error: Multiple tools marked as required (${matches.join(", ")}). At most one tool can be required.`);
        }
        if (matches.length === 0)
            continue;
        if (requiredTool && requiredTool !== matches[0]) {
            throw new Error(`Tool policy error: Multiple tools marked as required (${requiredTool}, ${matches[0]}). At most one tool can be required.`);
        }
