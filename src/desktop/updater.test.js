...
describe("UpdaterService", () => {
    ...
    describe("downloadUpdate", () => {
        it("should emit phase-aware error when download fails", async () => {
            ...
            service.checkForUpdates();
            await new Promise((resolve) => setImmediate(resolve));
            mockAutoUpdater.downloadUpdate.mockImplementationOnce(() => Promise.reject(new Error("Download failed due to network interruption")));
        ...
        it("should allow download retry after download failure", async () => {
            ...
            mockAutoUpdater.downloadUpdate
                .mockImplementationOnce(() => Promise.reject(new Error("Download failed due to network interruption")))
    ...
    describe("state guards", () => {
        it("should skip check when already checking", () => {
            mockAutoUpdater.checkForUpdates.mockReturnValue(new Promise(() => {
                // Never resolves
            }));
            service.checkForUpdates();
