...
import { MUX_GATEWAY_ORIGIN, MUX_GATEWAY_SESSION_EXPIRED_MESSAGE, } from "@/common/constants/muxGatewayOAuth";
...
import { DEFAULT_LAYOUT_PRESETS_CONFIG, isLayoutPresetsConfigEmpty, normalizeLayoutPresetsConfig, } from "@/common/types/uiLayouts";
...
import { discoverAgentSkills, discoverAgentSkillsDiagnostics, readAgentSkill, } from "@/node/services/agentSkills/agentSkillsService";
...
import { readSubagentTranscriptArtifactsFile, } from "@/node/services/subagentTranscriptArtifacts";
/**
...
async function resolveAgentDiscoveryContext(context, input) {
    ...
    const runtime = createRuntime({ type: "local", srcBaseDir: context.config.srcDir }, { projectPath: input.projectPath });
...
function normalizeMuxMessageFromDisk(value) {
    ...
    return normalizeLegacyMuxMetadata(value);
}
    ...
    try {
        ...
        for (let i = 0; i < lines.length; i++) {
            try {
                ...
                log.warn(`Skipping malformed JSON at line ${i + 1} in ${params.logLabel}:`, parseError instanceof Error ? parseError.message : String(parseError), "\nLine content:", lines[i].substring(0, 100) + (lines[i].length > 100 ? "..." : ""));
            }
        }
        return messages;
    }
    ...
    return t.router({
        ...
        agents: {
            list: t
                ...
                if (input.workspaceId && metadata) {
                    try {
                        planReady = await hasNonEmptyPlanFile(runtime, metadata.name, metadata.projectName, input.workspaceId);
                    }
                    catch {
                        planReady = false;
                    }
                ...
                const resolved = await Promise.all(descriptors.map(async (descriptor) => {
                    try {
                        const resolvedFrontmatter = await resolveAgentFrontmatter(runtime, discoveryPath, descriptor.id);
        ...
        muxGateway: {
            getAccountStatus: t
                ...
                if (!response.ok) {
                    ...
                    return Err(`Mux Gateway balance request failed (HTTP ${response.status}): ${prefix || response.statusText}`);
                }
                let json;
                try {
        ...
        general: {
            ...
            subscribeLogs: t
                ...
                function shouldInclude(entryLevel, minLevel) {
                    return ((LOG_LEVEL_PRIORITY[entryLevel] ?? LOG_LEVEL_PRIORITY.debug) <=
        ...
        mcp: {
            ...
            test: t
                ...
                const secrets = secretsToRecord(projectPathProvided
        ...
        nameGeneration: {
            generate: t
                ...
                const result = await generateWorkspaceIdentity(input.message, input.candidates, context.aiService);
                if (!result.success) {
                    return result;
                }
                return {
        ...
        workspace: {
            ...
            getPlanContent: t
                ...
                const runtime = createRuntimeForWorkspace(metadata);
                const result = await readPlanFile(runtime, metadata.name, metadata.projectName, input.workspaceId);
                if (!result.exists) {
