...
const CODER_ENSURE_READY_TIMEOUT_MS = 120000;
const CODER_STATUS_POLL_INTERVAL_MS = 2000;
/**
...
export class CoderSSHRuntime extends SSHRuntime {
    ...
    async finalizeConfig(finalBranchName, config) {
        ...
        if (!coder.existingWorkspace) {
            ...
            if (!CODER_NAME_REGEX.test(workspaceName)) {
                return Err(`Workspace name "${finalBranchName}" cannot be converted to a valid Coder name. ` +
        ...
        try {
            ...
            return Err(`Failed to read Coder deployment SSH config. ` +
    ...
    async validateBeforePersist(_finalBranchName, config) {
        ...
        if (exists) {
            await this.coderService.disposeProvisioningSession(workspaceName);
            return Err(`A Coder workspace named "${workspaceName}" already exists. ` +
    ...
    async deleteWorkspace(projectPath, workspaceName, force, abortSignal) {
        ...
        if (force) {
            const deleteResult = await this.coderService.deleteWorkspaceEventually(coderWorkspaceName, {
                ...
                waitForExistenceTimeoutMs: 10000,
            });
