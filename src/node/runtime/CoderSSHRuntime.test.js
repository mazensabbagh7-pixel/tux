...
function createMockCoderService(overrides) {
    ...
    return {
        createWorkspace: mock(() => (async function* () {
        ...
        deleteWorkspaceEventually: mock(() => Promise.resolve({ success: true, data: undefined })),
        ...
        getWorkspaceStatus: mock(() => Promise.resolve({ kind: "ok", status: "running" })),
        listWorkspaces: mock(() => Promise.resolve({ ok: true, workspaces: [] })),
        waitForStartupScripts: mock(() => (async function* () {
            await Promise.resolve();
...
describe("CoderSSHRuntime.deleteWorkspace", () => {
    ...
    it("never calls coderService.deleteWorkspaceEventually when existingWorkspace=true", async () => {
        const deleteWorkspaceEventually = mock(() => Promise.resolve({ success: true, data: undefined }));
    ...
    it("calls Coder deletion (no SSH) when force=true", async () => {
        ...
        expect(deleteWorkspaceEventually).toHaveBeenCalledWith("my-ws", expect.objectContaining({ waitForExistence: true, waitForExistenceTimeoutMs: 10000 }));
    });
    it("returns combined error when SSH succeeds but Coder delete fails", async () => {
        const deleteWorkspaceEventually = mock(() => Promise.resolve({ success: false, error: "Coder API error" }));
        const coderService = createMockCoderService({ deleteWorkspaceEventually });
        const runtime = createRuntime({ existingWorkspace: false, workspaceName: "my-ws" }, coderService);
        const result = await runtime.deleteWorkspace("/project", "ws", false);
        expect(result.success).toBe(false);
    ...
    it("proceeds with SSH cleanup when status check fails with API error", async () => {
        ...
        const getWorkspaceStatus = mock(() => Promise.resolve({ kind: "error", error: "coder timed out" }));
        const deleteWorkspaceEventually = mock(() => Promise.resolve({ success: true, data: undefined }));
        const coderService = createMockCoderService({ getWorkspaceStatus, deleteWorkspaceEventually });
        const runtime = createRuntime({ existingWorkspace: false, workspaceName: "my-ws" }, coderService);
        const result = await runtime.deleteWorkspace("/project", "ws", false);
    ...
    it("deletes stopped Coder workspace without SSH cleanup", async () => {
        ...
        expect(result.success).toBe(true);
        expect(sshDeleteSpy).not.toHaveBeenCalled();
        expect(deleteWorkspaceEventually).toHaveBeenCalledWith("my-ws", expect.objectContaining({ waitForExistence: false }));
    });
    it("succeeds immediately when Coder workspace status is 'deleting'", async () => {
        const getWorkspaceStatus = mock(() => Promise.resolve({ kind: "ok", status: "deleting" }));
        const deleteWorkspaceEventually = mock(() => Promise.resolve({ success: true, data: undefined }));
        const coderService = createMockCoderService({ getWorkspaceStatus, deleteWorkspaceEventually });
        const runtime = createRuntime({ existingWorkspace: false, workspaceName: "my-ws" }, coderService);
        const result = await runtime.deleteWorkspace("/project", "ws", false);
...
describe("CoderSSHRuntime.postCreateSetup", () => {
    ...
    it("creates a new Coder workspace and prepares the directory", async () => {
        const createWorkspace = mock(() => (async function* () {
            await Promise.resolve();
            yield "build line 1";
            yield "build line 2";
        })());
        ...
        const getWorkspaceStatus = mock(() => Promise.resolve(workspaceCreated
        ...
        expect(createWorkspace).toHaveBeenCalledWith("my-ws", "my-template", undefined, undefined, undefined, provisioningSession);
    ...
    it("skips workspace creation when existingWorkspace=true and workspace is running", async () => {
        ...
        const waitForStartupScripts = mock(() => (async function* () {
            await Promise.resolve();
    ...
    it("polls until stopping workspace becomes stopped before connecting", async () => {
        ...
        spyOn(runtime, "sleep").mockResolvedValue(undefined);
        const loggedSteps = [];
        await runtime.postCreateSetup({
            initLogger: {
    ...
    it("throws when workspaceName is missing", () => {
        ...
        return expect(runtime.postCreateSetup({
            initLogger: {
                logStep: noop,
                logStdout: noop,
                logStderr: noop,
