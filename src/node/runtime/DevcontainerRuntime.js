...
import { readHostGitconfig, resolveGhToken, resolveSshAgentForwarding, } from "./credentialForwarding";
...
export class DevcontainerRuntime extends LocalBaseRuntime {
    ...
    resolveHostPathForMounted(filePath) {
        if (this.currentWorkspacePath) {
            ...
            const normalizedHostRoot = stripTrailingSlashes(this.currentWorkspacePath.replaceAll("\\", "/"));
    ...
    exec(command, options) {
        ...
        const exitCode = new Promise((resolve, reject) => {
            ...
            childProcess.on("error", (err) => {
                reject(new RuntimeError(`Failed to execute devcontainer exec: ${err.message}`, "exec", err));
            });
        });
    ...
    async resolvePath(filePath) {
        ...
        if (this.hasUnexpandedTilde(expanded)) {
            ...
            if (this.remoteHomeDir) {
                ...
                throw new RuntimeError(`Failed to resolve path ${filePath}: container home directory unavailable`, "exec");
            }
        }
