...
import { getHostGitconfigPath, hasHostGitconfig, resolveGhToken, resolveSshAgentForwarding, } from "./credentialForwarding";
 ...
 */
export function getContainerName(projectPath, workspaceName) {
...
export class DockerRuntime extends RemoteRuntime {
    ...
    spawnRemoteProcess(fullCommand, _options) {
        ...
        if (!this.containerName) {
            throw new RuntimeError("Docker runtime not initialized with container name. " +
    ...
    async setupCredentials(containerName, env) {
        ...
        if (hasHostGitconfig()) {
            await runDockerCommand(`docker cp ${getHostGitconfigPath()} ${containerName}:/root/.gitconfig`, 10000);
        }
        ...
        if (ghToken) {
            await runDockerCommand(`docker exec -e GH_TOKEN=${shescape.quote(ghToken)} ${containerName} sh -c 'command -v gh >/dev/null && gh auth setup-git || true'`, 10000);
        }
    }
    /**
    ...
    async forkWorkspace(params) {
        ...
        try {
            ...
            const cpOutResult = await runDockerCommand(`docker cp ${srcContainerName}:${containerBundlePath} ${shescape.quote(hostTempPath)}`, 300000);
            ...
            const cpInResult = await runDockerCommand(`docker cp ${shescape.quote(hostTempPath)} ${destContainerName}:${containerBundlePath}`, 300000);
            ...
            try {
                ...
                if (remotesResult.exitCode === 0) {
                    ...
                    for (const remote of remotes) {
                        ...
                        await runDockerCommand(`docker exec ${destContainerName} git -C ${CONTAINER_SRC_DIR} branch ${shescape.quote(localName)} ${shescape.quote(remote)} 2>/dev/null || true`, 10000);
                    }
                }
            }
            catch {
            ...
            try {
                ...
                if (originUrl.length > 0) {
                    await runDockerCommand(`docker exec ${destContainerName} git -C ${CONTAINER_SRC_DIR} remote set-url origin ${shescape.quote(originUrl)}`, 10000);
                }
                else {
            ...
            const checkoutResult = await runDockerCommand(`docker exec ${destContainerName} bash -c ${shescape.quote(`cd ${CONTAINER_SRC_DIR} && ${checkoutCmd}`)}`, 120000);
