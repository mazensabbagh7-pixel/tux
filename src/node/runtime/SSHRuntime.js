 ...
 */
export function computeBaseRepoPath(srcBaseDir, projectPath) {
...
export class SSHRuntime extends RemoteRuntime {
    ...
    async getBgOutputDir() {
        ...
        if (dir === "~" || dir.startsWith("~/")) {
            ...
            if (result.exitCode === 0 && result.stdout.trim()) {
                ...
                log.warn(`SSHRuntime: Failed to resolve $HOME (exitCode=${result.exitCode}). Falling back to /tmp.`);
                home = "/tmp";
            }
    ...
    async resolvePath(filePath) {
        ...
        const abortController = createAbortController(10000);
        try {
    ...
    async resolveBundleTrunkRef(baseRepoPathArg, trunkBranch, abortSignal) {
        ...
        const check = await execBuffered(this, `git -C ${baseRepoPathArg} rev-parse --verify ${shescape.quote(preferredRef)}`, { cwd: "/tmp", timeout: 10, abortSignal });
        if (check.exitCode === 0) {
            return preferredRef;
        }
        // Fallback: pick the first ref under refs/mux-bundle/ (handles main↔master mismatch).
        const listResult = await execBuffered(this, `git -C ${baseRepoPathArg} for-each-ref --format='%(refname)' ${BUNDLE_REF_PREFIX} --count=1`, { cwd: "/tmp", timeout: 10, abortSignal });
    ...
    async checkWorkspaceRepo(options) {
        ...
        let revResult;
        try {
            revResult = await execBuffered(this, `git -C ${this.quoteForRemote(workspacePath)} rev-parse --git-dir`, {
                cwd: "~",
                timeout: 10,
                abortSignal: options?.signal,
            });
    ...
    async transferBundleToRemote(projectPath, initLogger, abortSignal) {
        ...
        const gitProc = spawn("git", ["-C", projectPath, "bundle", "create", "-", "--branches", "--tags"], {
        ...
        const remoteAbortController = createAbortController(300000, abortSignal);
    ...
    async syncProjectToRemote(projectPath, _workspacePath, initLogger, abortSignal) {
        ...
        try {
            ...
            const fetchResult = await execBuffered(this, `git -C ${baseRepoPathArg} fetch ${remoteBundlePathArg} '+refs/heads/*:${BUNDLE_REF_PREFIX}*' '+refs/tags/*:refs/tags/*'`, { cwd: "/tmp", timeout: 300, abortSignal });
            ...
            if (originUrl) {
                ...
                await execBuffered(this, `git -C ${baseRepoPathArg} remote set-url origin ${shescape.quote(originUrl)} 2>/dev/null || git -C ${baseRepoPathArg} remote add origin ${shescape.quote(originUrl)}`, { cwd: "/tmp", timeout: 10, abortSignal });
            }
        ...
        try {
            ...
            try {
                ...
                if (dirCheck.exitCode === 0) {
                    const gitCheck = await execBuffered(this, `git -C ${workspacePathArg} rev-parse --is-inside-work-tree`, {
                        cwd: "/tmp",
                        timeout: 20,
                        abortSignal,
                    });
          ...
          `;
            ...
            if (isWorktree) {
                ...
                if (!PROTECTED_BRANCHES.includes(workspaceName)) {
                    await execBuffered(this, `git -C ${baseRepoPathArg} branch -D ${shescape.quote(workspaceName)} 2>/dev/null || true`, { cwd: "/tmp", timeout: 10 }).catch(() => undefined);
                }
            }
            else {
    ...
    async forkWorkspace(params) {
        ...
        try {
            ...
            const branchResult = await execBuffered(this, `git -C ${sourceWorkspacePathArg} branch --show-current`, {
                cwd: "/tmp",
                timeout: 30,
                abortSignal,
            });
            ...
            if (hasBaseRepo.exitCode === 0) {
                ...
                if (worktreeResult.exitCode === 0) {
                    ...
                    log.info(`Worktree fork failed (${(worktreeResult.stderr || worktreeResult.stdout).trim()}); falling back to full copy`);
            ...
            if (!usedWorktree) {
                ...
                const copyResult = await execBuffered(this, `cp -R -P ${sourceWorkspacePathArg} ${newWorkspacePathArg}`, { cwd: "/tmp", timeout: 300, abortSignal });
                ...
                try {
                    await execBuffered(this, `cd ${newWorkspacePathArg} && for branch in $(git for-each-ref --format='%(refname:short)' refs/remotes/origin/ | grep -v 'origin/HEAD'); do localname=\${branch#origin/}; git show-ref --verify --quiet refs/heads/$localname || git branch $localname $branch; done`, { cwd: "/tmp", timeout: 30 });
                }
                catch {
                    // Ignore - best-effort.
                }
                ...
                try {
                    const originResult = await execBuffered(this, `git -C ${sourceWorkspacePathArg} remote get-url origin 2>/dev/null || true`, { cwd: "/tmp", timeout: 10 });
                    const originUrl = originResult.stdout.trim();
                    if (originUrl.length > 0) {
                        await execBuffered(this, `git -C ${newWorkspacePathArg} remote set-url origin ${shescape.quote(originUrl)}`, { cwd: "/tmp", timeout: 10 });
                    }
                    else {
                        await execBuffered(this, `git -C ${newWorkspacePathArg} remote remove origin 2>/dev/null || true`, { cwd: "/tmp", timeout: 10 });
                    }
                }
                catch {
                    // Ignore - best-effort.
                ...
                const checkoutCmd = `git checkout ${shescape.quote(newWorkspaceName)} 2>/dev/null || ` +
