...
export function parseDevcontainerStdoutLine(line) {
...
export function formatDevcontainerUpError(result, stderrSummary) {
...
export function shouldCleanupDevcontainer(result) {
...
const MAX_STDERR_BUFFER_LENGTH = 8000; // 8KB cap for error summaries
const DEFAULT_CLEANUP_TIMEOUT_MS = 60000; // 1 minute
...
const VERSION_CHECK_TIMEOUT_MS = 10000; // 10 seconds
/**
 * Check if devcontainer CLI is installed and get version.
 */
export async function checkDevcontainerCliVersion() {
 ...
 */
export async function devcontainerExec(options) {
 ...
 */
export async function getDevcontainerContainerId(workspaceFolder, _configPath, timeoutMs = 10000) {
    ...
    return new Promise((resolve) => {
        const proc = spawn("docker", ["ps", "-q", "--filter", `label=devcontainer.local_folder=${labelValue}`], {
            stdio: ["ignore", "pipe", "pipe"],
            timeout: timeoutMs,
        });
        let stdout = "";
 ...
 */
export async function getDevcontainerContainerName(workspaceFolder, timeoutMs = 10000) {
    // The devcontainer CLI labels containers with the workspace folder path
    const labelValue = workspaceFolder;
    return new Promise((resolve) => {
        const proc = spawn("docker", ["ps", "--format", "{{.Names}}", "--filter", `label=devcontainer.local_folder=${labelValue}`], {
            stdio: ["ignore", "pipe", "pipe"],
            timeout: timeoutMs,
        });
        let stdout = "";
 ...
 */
export async function devcontainerDown(workspaceFolder, _configPath, timeoutMs = 60000) {
