...
import { formatDevcontainerUpError, parseDevcontainerStdoutLine, shouldCleanupDevcontainer, } from "./devcontainerCli";
...
describe("formatDevcontainerUpError", () => {
    it("prefers message and description", () => {
        expect(formatDevcontainerUpError({
            outcome: "error",
    ...
    it("falls back to stderr summary", () => {
        expect(formatDevcontainerUpError({ outcome: "error" }, "stderr info")).toBe("devcontainer up failed: stderr info");
    });
});
