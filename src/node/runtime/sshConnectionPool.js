...
export class SSHConnectionPool {
    ...
    async acquireConnection(config, timeoutMsOrOptions = DEFAULT_PROBE_TIMEOUT_MS) {
        ...
        while (true) {
            ...
            if (health?.status === "healthy") {
                ...
                log.debug(`SSH connection to ${config.host} health is stale (${Math.round(age / 1000)}s), re-probing`);
            }
    ...
    markFailedByKey(key, error) {
        ...
        log.warn(`SSH connection failed (${failures} consecutive). Backoff for ${backoffSecs.toFixed(1)}s. Error: ${error}`);
    }
    /**
 ...
 */
export function getControlPath(config) {
    const key = makeConnectionKey(config);
