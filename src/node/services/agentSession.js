...
import { buildStreamErrorEventData, createStreamErrorMessage, createUnknownSendMessageError, } from "@/node/services/utils/sendMessageError";
import { createUserMessageId, createFileSnapshotMessageId, createAgentSkillSnapshotMessageId, } from "@/node/services/utils/messageIds";
import { FileChangeTracker, } from "@/node/services/utils/fileChangeTracker";
import { Ok, Err } from "@/common/types/result";
import { enforceThinkingPolicy } from "@/common/utils/thinking/policy";
import { createMuxMessage, isCompactionSummaryMetadata, prepareUserMessageForSend, } from "@/common/types/message";
...
const MAX_AGENT_SKILL_SNAPSHOT_CHARS = 50000;
var TurnPhase;
(function (TurnPhase) {
    TurnPhase["IDLE"] = "idle";
    TurnPhase["PREPARING"] = "preparing";
    TurnPhase["STREAMING"] = "streaming";
    TurnPhase["COMPLETING"] = "completing";
})(TurnPhase || (TurnPhase = {}));
export class AgentSession {
    constructor(options) {
        ...
        const { workspaceId, config, historyService, aiService, initStateManager, telemetryService, backgroundProcessManager, keepBackgroundProcesses, onCompactionComplete, onPostCompactionStateChange, } = options;
    ...
    async ensureMetadata(args) {
        ...
        if (existing.success) {
            ...
            assert(expectedPath === normalizedWorkspacePath, `Existing metadata workspace path mismatch for ${this.workspaceId}: expected ${expectedPath}, got ${normalizedWorkspacePath}`);
            return;
        }
    ...
    async sendMessage(message, options, internal) {
        ...
        if (trimmedMessage.length === 0 && !hasFiles) {
            return Err(createUnknownSendMessageError("Empty message not allowed. Use interruptStream() to interrupt active streams."));
        }
        if (editMessageId) {
        ...
        const additionalParts = preservedEditFileParts && preservedEditFileParts.length > 0
            ...
            : fileParts && fileParts.length > 0
                ? fileParts.map((part, index) => {
                    assert(typeof part.url === "string", `file part [${index}] must include url string content (got ${typeof part.url}): ${JSON.stringify(part).slice(0, 200)}`);
                    assert(part.url.startsWith("data:"), `file part [${index}] url must be a data URL (got: ${part.url.slice(0, 50)}...)`);
                    assert(typeof part.mediaType === "string" && part.mediaType.trim().length > 0, `file part [${index}] must include a mediaType (got ${typeof part.mediaType}): ${JSON.stringify(part).slice(0, 200)}`);
                    if (part.filename !== undefined) {
                        assert(typeof part.filename === "string", `file part [${index}] filename must be a string if present (got ${typeof part.filename}): ${JSON.stringify(part).slice(0, 200)}`);
                    }
                    return {
                        type: "file",
                        url: part.url,
        ...
        if (!options?.model || options.model.trim().length === 0) {
            return Err(createUnknownSendMessageError("No model specified. Please select a model using /model."));
        }
        ...
        if (effectiveFileParts && effectiveFileParts.length > 0) {
            ...
            if (pdfParts.length > 0) {
                ...
                if (caps && !caps.supportsPdfInput) {
                    return Err(createUnknownSendMessageError(`Model ${options.model} does not support PDF input.`));
                }
                if (caps?.maxPdfSizeMb !== undefined) {
                    ...
                    for (const part of pdfParts) {
                        ...
                        if (bytes !== null && bytes > maxBytes) {
                            ...
                            return Err(createUnknownSendMessageError(`${label} is ${actualMb}MB, but ${options.model} allows up to ${caps.maxPdfSizeMb}MB per PDF.`));
                        }
                    }
                }
            }
        ...
        const userMessage = createMuxMessage(messageId, "user", message, {
            timestamp: Date.now(),
        ...
        try {
            ...
            return Err(createUnknownSendMessageError(error instanceof Error ? error.message : String(error)));
        }
    ...
    normalizeGatewaySendOptions(options) {
        ...
        const normalizedSystem1Model = system1Model && system1Model.length > 0 ? normalizeGatewayModel(system1Model) : undefined;
        return {
            ...options,
    ...
    async streamWithHistory(modelString, options, openaiTruncationModeOverride, disablePostCompactionAttachments) {
        ...
        if (historyResult.data.length === 0) {
            return Err(createUnknownSendMessageError("Cannot resume stream: workspace history is empty. Send a new message instead."));
        }
    ...
    async maybeHardRestartExecSubagentOnContextExceeded(data) {
        ...
        const continuationMessage = createMuxMessage(createUserMessageId(), "user", continuationNotice, {
            timestamp: Date.now(),
            synthetic: true,
            uiVisible: true,
        });
    ...
    async getPostCompactionAttachmentsIfNeeded() {
        ...
        if (pendingDiffs !== null) {
            ...
            const attachments = await AttachmentService.generatePostCompactionAttachments(metadataResult.data.name, metadataResult.data.projectName, this.workspaceId, pendingDiffs, runtime, excludedItems);
            if (todoAttachment) {
    ...
    async generatePostCompactionAttachments() {
        ...
        const runtime = createRuntimeForWorkspace(metadataResult.data);
        const attachments = await AttachmentService.generatePostCompactionAttachments(metadataResult.data.name, metadataResult.data.projectName, this.workspaceId, fileDiffs, runtime, excludedItems);
        if (todoAttachment) {
            // Insert TODO after plan (if present), otherwise first.
            const planIndex = attachments.findIndex((att) => att.type === "plan_file_reference");
            const insertIndex = planIndex === -1 ? 0 : planIndex + 1;
