...
import { AgentSkillDescriptorSchema, AgentSkillPackageSchema, SkillNameSchema, } from "@/common/orpc/schemas";
...
export function getDefaultAgentSkillsRoots(runtime, workspacePath) {
    if (!workspacePath) {
...
function getGlobalSkillRoots(roots) {
    const orderedRoots = [roots.globalRoot, roots.universalRoot].filter((root) => root != null && root.length > 0);
...
export async function discoverAgentSkills(runtime, workspacePath, options) {
    if (!workspacePath) {
    ...
    const scans = [
        ...
        ...getGlobalSkillRoots(roots).map((root) => ({ scope: "global", root })),
    ];
    for (const scan of scans) {
        ...
        const directoryNames = runtime instanceof SSHRuntime
...
export async function discoverAgentSkillsDiagnostics(runtime, workspacePath, options) {
    if (!workspacePath) {
...
export async function readAgentSkill(runtime, workspacePath, name, options) {
    if (!workspacePath) {
...
export function resolveAgentSkillFilePath(runtime, skillDir, filePath) {
    if (!filePath) {
