...
import { log } from "./log";
import { addInterruptedSentinel, filterEmptyAssistantMessages, } from "@/browser/utils/messages/modelMessageTransform";
...
import { simulateContextLimitError, simulateToolPolicyNoop, } from "./streamSimulation";
...
export class AIService extends EventEmitter {
    constructor(config, historyService, initStateManager, providerService, backgroundProcessManager, sessionUsageService, workspaceMcpOverridesService, policyService, telemetryService) {
        super();
        ...
        this.pendingStreamStarts = new Map();
        // Debug: captured LLM request payloads for last send per workspace
        this.lastLlmRequestByWorkspace = new Map();
    ...
    enableMockMode() {
        ...
        this.mockAiStreamPlayer ?? (this.mockAiStreamPlayer = new MockAiStreamPlayer({
            aiService: this,
    ...
    async getWorkspaceMetadata(workspaceId) {
        try {
            ...
            if (!metadata) {
                return Err(`Workspace metadata not found for ${workspaceId}. Workspace may not be properly initialized.`);
            }
            return Ok(metadata);
        }
        catch (error) {
    ...
    async streamMessage(opts) {
        ...
        try {
            ...
            log.debug(`[STREAM MESSAGE] workspaceId=${workspaceId} messageCount=${messages.length} lastRole=${lastMessage?.role}`);
            ...
            const deleteAbortedPlaceholder = async (messageId) => {
                ...
                if (!deleteResult.success) {
                    log.error(`Failed to delete aborted assistant placeholder (${messageId}): ${deleteResult.error}`);
                }
            };
            // Mode (plan|exec|compact) is derived from the selected agent definition.
            const effectiveMuxProviderOptions = muxProviderOptions ?? {};
            const effectiveThinkingLevel = thinkingLevel ?? THINKING_LEVEL_OFF;
            ...
            if (!readyResult.ready) {
                ...
                this.emit("error", createErrorEvent(workspaceId, {
            ...
            const mcpServers = this.mcpServerManager && workspaceId !== MUX_HELP_CHAT_WORKSPACE_ID
            ...
            const { effectiveAdditionalInstructions, planFilePath, planContentForTransition } = await buildPlanInstructions({
                runtime,
                metadata,
                workspaceId,
                workspacePath,
            ...
            const { agentSystemPrompt, systemMessage, systemMessageTokens, agentDefinitions, availableSkills, } = await buildStreamSystemContext({
                runtime,
                metadata,
                workspacePath,
                workspaceId,
            ...
            const projectSecrets = workspaceId === MUX_HELP_CHAT_WORKSPACE_ID
                ? []
            ...
            const toolInstructions = await readToolInstructions(metadata, runtime, workspacePath, modelString, agentSystemPrompt);
            ...
            const allTools = await getToolsForModel(modelString, {
                ...
                muxEnv: getMuxEnv(metadata.projectPath, getRuntimeType(metadata.runtimeConfig), metadata.name, {
                    modelString,
            ...
            const providerOptions = buildProviderOptions(modelString, effectiveThinkingLevel, providerRequestMessages, (id) => this.streamManager.isResponseIdLost(id), effectiveMuxProviderOptions, workspaceId, truncationMode);
            ...
            hasQueuedMessage, metadata.name, effectiveThinkingLevel, requestHeaders);
