 ...
 */
export class DisposableProcess {
    constructor(process) {
...
export class BashExecutionService {
     ...
     */
    executeStreaming(script, config, callbacks) {
        log.debug(`BashExecutionService: Executing streaming command in ${config.cwd}`);
        log.debug(`BashExecutionService: Script: ${script.substring(0, 100)}${script.length > 100 ? "..." : ""}`);
        ...
        child.on("close", (code, signal) => {
            log.debug(`BashExecutionService: Process exited with code ${code ?? "null"}, signal ${signal ?? "none"}`);
...
//# sourceMappingURL=bashExecutionService.js.map