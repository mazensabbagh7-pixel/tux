...
describe("CoderService", () => {
    ...
    describe("waitForStartupScripts", () => {
        ...
        it("throws when exit code is non-zero", async () => {
            ...
            expect(thrown instanceof Error ? thrown.message : String(thrown)).toBe("coder ssh --wait failed (exit 1): Connection refused");
        });
    });
    ...
    describe("createWorkspace", () => {
        ...
        it("streams stdout/stderr lines and passes expected args", async () => {
            ...
            expect(spawnSpy).toHaveBeenCalledWith("coder", ["create", "my-workspace", "-t", "my-template", "--yes"], { stdio: ["ignore", "pipe", "pipe"] });
        ...
        it("includes --preset when provided", async () => {
            ...
            expect(spawnSpy).toHaveBeenCalledWith("coder", ["create", "ws", "-t", "tmpl", "--yes", "--preset", "preset"], { stdio: ["ignore", "pipe", "pipe"] });
        });
        it("includes --parameter flags for uncovered non-ephemeral params", async () => {
            ...
            expect(spawnSpy).toHaveBeenCalledWith("coder", [
                "create",
                "ws",
                "-t",
                "tmpl",
        ...
        it("throws when exit code is non-zero", async () => {
            ...
            expect(thrown).toBeTruthy();
            expect(thrown instanceof Error ? thrown.message : String(thrown)).toContain("coder create failed (exit 42)");
        });
...
describe("validateRequiredParams", () => {
    ...
    it("lists all missing required params in error", () => {
        ...
        const covered = new Set();
        expect(() => service.validateRequiredParams(params, covered)).toThrow(/missing1.*missing2|missing2.*missing1/);
    });
});
...
describe("deleteWorkspace", () => {
    ...
    it("throws when deleteWorkspaceEventually returns an error", async () => {
        ...
        expect(error.message).toContain("workspace stuck");
    });
});
describe("deleteWorkspaceEventually", () => {
    it("polls past initial not_found when waitForExistence=true", async () => {
        ...
        serviceHack.runCoderCommand = vi.fn(() => Promise.resolve({ exitCode: 0, stdout: "", stderr: "" }));
    ...
    it("short-circuits on waitForExistenceTimeoutMs before overall timeout", async () => {
        ...
        try {
            const result = await service.deleteWorkspaceEventually("mux-my-workspace", {
                ...
                waitForExistenceTimeoutMs: 5000,
            });
            expect(result.success).toBe(true);
            expect(getWorkspaceStatusSpy).toHaveBeenCalled();
