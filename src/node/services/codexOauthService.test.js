...
function validAuth(overrides) {
    return {
        ...
        refresh: "rt_test",
        expires: Date.now() + 3600000, // 1h from now
        ...overrides,
    };
}
/** Build a CodexOauthAuth that is already expired. */
function expiredAuth(overrides) {
    return validAuth({ expires: Date.now() - 60000, ...overrides });
}
...
function createService(deps) {
    return new CodexOauthService(createMockConfig(deps), createMockProviderService(deps), createMockWindowService(deps));
}
