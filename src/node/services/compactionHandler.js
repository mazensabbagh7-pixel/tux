...
function coerceFileEditDiffs(value) {
    ...
    for (const item of value) {
        ...
        if (typeof truncated !== "boolean")
            continue;
        const clampedDiff = diff.length > MAX_FILE_CONTENT_SIZE ? diff.slice(0, MAX_FILE_CONTENT_SIZE) : diff;
        diffs.push({
            path: trimmedPath,
            diff: clampedDiff,
...
export class CompactionHandler {
    constructor(options) {
        ...
        this.processedCompactionRequestIds = new Set();
    ...
    async handleCompletion(event) {
        ...
        if (looksLikeRawJsonObject(summary)) {
            log.warn("Compaction summary is a raw JSON object - aborting compaction to prevent corrupted history", {
                workspaceId: this.workspaceId,
        ...
        const boundaryHistoryResult = await this.historyService.getHistoryFromLatestBoundary(this.workspaceId);
        ...
        const result = await this.performCompaction(summary, event.metadata, messagesForCompaction, event.messageId, isIdleCompaction, pendingFollowUp);
        if (!result.success) {
    ...
    async performCompaction(summary, metadata, messages, streamedSummaryMessageId, isIdleCompaction = false, pendingFollowUp) {
        ...
        assert(streamedSummaryMessageId.trim().length > 0, "performCompaction requires streamed summary message ID");
        ...
        const maxExistingHistorySequence = messages.reduce((maxSeq, message) => {
            ...
            if (!isNonNegativeInteger(sequence)) {
                ...
                log.warn("Ignoring malformed historySequence while deriving compaction monotonicity bound", {
                    workspaceId: this.workspaceId,
                    messageId: message.id,
        ...
        const summaryMessage = createMuxMessage(persistedStreamSummary?.id ?? createCompactionSummaryMessageId(), "assistant", summary, {
        ...
        assert(summaryMessage.metadata?.compactionBoundary === true, "Compaction summary must be marked as a compaction boundary");
        assert(summaryMessage.metadata?.compactionEpoch === nextCompactionEpoch, "Compaction summary must persist the computed compaction epoch");
        assert(summaryMessage.metadata?.providerMetadata === undefined, "Compaction summary must not persist stale providerMetadata");
        assert(summaryMessage.metadata?.contextProviderMetadata === undefined, "Compaction summary must not persist stale contextProviderMetadata");
        ...
        assert(isNonNegativeInteger(persistedSequence), "Compaction summary persistence must produce a non-negative historySequence");
        if (persistedStreamSummary) {
            assert(persistedSummaryHistorySequence !== undefined &&
            ...
            assert(persistedSequence > maxExistingHistorySequence, "Compaction summary historySequence must remain monotonic");
        }
