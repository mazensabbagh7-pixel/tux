...
const createCompactionRequest = (id = "req-1") => createMuxMessage(id, "user", "Please summarize the conversation", {
    historySequence: 0,
...
describe("CompactionHandler", () => {
    ...
    beforeEach(async () => {
        ...
        telemetryCapture = mock((_payload) => {
            void _payload;
        });
    ...
    describe("handleCompletion() - Normal Compaction Flow", () => {
        ...
        it("falls back to full-history diff extraction when boundary marker is malformed", async () => {
            const staleEditMessage = createSuccessfulFileEditMessage("assistant-stale-edit", "/tmp/stale.ts", "@@ -1 +1 @@\n-old\n+stale\n", { historySequence: 0 });
            const malformedBoundaryMissingEpoch = createMuxMessage("summary-malformed-boundary", "assistant", "Malformed summary", {
                historySequence: 1,
                compacted: "user",
                compactionBoundary: true,
        ...
        it("should join multiple text parts from event.parts", async () => {
            ...
            expect(appendedMsg.parts[0].text).toBe("Part 1 Part 2 Part 3");
        });
        it("should extract summary text from event.parts", async () => {
            ...
            await handler.handleCompletion(event);
            const appendedMsg = appendSpy.mock.calls[0][1];
            expect(appendedMsg.parts[0].text).toBe("This is the summary");
        });
        ...
        it("should append summary without clearing history", async () => {
            ...
            expect(appendedMsg.parts[0].text).toBe("Summary");
        });
        ...
        it("should ignore malformed persisted historySequence values when deriving monotonic bounds", async () => {
            const malformedNegativeSequence = createMuxMessage("assistant-malformed-negative-sequence", "assistant", "Corrupted persisted metadata", {
                historySequence: -7,
            });
            const malformedFractionalSequence = createMuxMessage("assistant-malformed-fractional-sequence", "assistant", "Corrupted persisted metadata", {
            ...
            spyOn(historyService, "getLastMessages").mockResolvedValueOnce(Ok([malformedNegativeSequence, malformedFractionalSequence, priorMessage, compactionReq]));
        ...
        it("should update streamed summaries in-place without carrying stale provider metadata", async () => {
            ...
            const streamedSummary = createMuxMessage("msg-id", "assistant", "Summary", {
                ...
                model: "claude-3-5-sonnet-20241022",
                providerMetadata: { anthropic: { cacheCreationInputTokens: 50000 } },
                contextProviderMetadata: { anthropic: { cacheReadInputTokens: 10000 } },
            });
        ...
        it("should strip stale provider metadata from emitted stream-end when reusing streamed summary ID", async () => {
            ...
            expect(streamMsg?.metadata?.customField).toBe("preserved");
        });
        it("should skip malformed compaction boundary markers when deriving next epoch", async () => {
            ...
            const malformedBoundaryMissingEpoch = createMuxMessage("summary-malformed-1", "assistant", "Malformed boundary", {
                historySequence: 2,
                compacted: "user",
                compactionBoundary: true,
            });
            const malformedBoundaryMissingCompacted = createMuxMessage("summary-malformed-2", "assistant", "Malformed boundary", {
                historySequence: 3,
                compactionBoundary: true,
                compactionEpoch: 99,
            });
            const malformedBoundaryInvalidCompacted = createMuxMessage("summary-malformed-invalid-compacted", "assistant", "Malformed boundary", {
                historySequence: 4,
                compactionBoundary: true,
                compactionEpoch: 200,
            });
            ...
            const malformedBoundaryInvalidEpoch = createMuxMessage("summary-malformed-3", "assistant", "Malformed boundary", {
                historySequence: 4,
                compacted: "user",
                compactionBoundary: true,
                compactionEpoch: 0,
    ...
    describe("Event Emission", () => {
        ...
        it("should emit summary message with proper MuxMessage structure", async () => {
            ...
            expect(summaryMsg).toMatchObject({
                id: expect.stringContaining("summary-"),
                role: "assistant",
        ...
        it("should forward stream events (stream-end, stream-abort) correctly", async () => {
            ...
            const streamMsg = getEmittedStreamEndEvent(emittedEvents);
            expect(streamMsg).toBeDefined();
            expect(streamMsg?.metadata?.customField).toBe("test");
        });
    });
