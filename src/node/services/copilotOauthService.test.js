...
function createService(deps) {
    return new CopilotOauthService(createMockProviderService(deps), createMockWindowService(deps));
}
// ---------------------------------------------------------------------------
// Tests
// ---------------------------------------------------------------------------
describe("CopilotOauthService", () => {
    ...
    describe("happy path: poll → success", () => {
        ...
        it("calls focusMainWindow after successful auth", async () => {
            ...
            if (!startResult.success)
                return;
            await service.waitForDeviceFlow(startResult.data.flowId, { timeoutMs: 10000 });
    ...
    describe("cancellation", () => {
        it("resolves waitForDeviceFlow with error when cancelled", async () => {
            ...
            const waitPromise = service.waitForDeviceFlow(flowId, { timeoutMs: 30000 });
    ...
    describe("re-entrancy guard", () => {
        it("only starts one polling loop even when waitForDeviceFlow is called twice", async () => {
            ...
            const [result1, result2] = await Promise.all([
                service.waitForDeviceFlow(flowId, { timeoutMs: 10000 }),
                service.waitForDeviceFlow(flowId, { timeoutMs: 10000 }),
            ]);
            // Both should succeed
