 ...
 */
export async function materializeFileAtMentions(messageText, options) {
...
export async function injectFileAtMentions(messages, options) {
    ...
    for (let i = 0; i < messages.length; i++) {
        ...
        if (blocks && blocks.length > 0) {
            result.push(createMuxMessage(createFileAtMentionMessageId(createdAt, i), "user", blocks.join("\n\n"), {
                timestamp: createdAt,
                synthetic: true,
            }));
        }
