...
import { getSubagentGitPatchMboxPath, upsertSubagentGitPatchArtifact, } from "@/node/services/subagentGitPatchArtifacts";
...
export class GitPatchArtifactService {
    constructor(config) {
        this.config = config;
        this.pendingJobsByTaskId = new Map();
    }
    /**
    ...
    async maybeStartGeneration(parentWorkspaceId, childWorkspaceId, onComplete) {
        assert(parentWorkspaceId.length > 0, "maybeStartGeneration: parentWorkspaceId must be non-empty");
        ...
        const childAgentIdRaw = coerceNonEmptyString(childEntry?.workspace.agentId ?? childEntry?.workspace.agentType);
        ...
        if (!shouldGeneratePatch) {
            ...
            if (parsedChildAgentId.success) {
                ...
                if (agentDiscoveryEntry && agentWorkspacePath && runtimeConfig) {
                    ...
                    const workspaceName = coerceNonEmptyString(agentDiscoveryWs?.name) ?? coerceNonEmptyString(fallbackName);
                    if (workspaceName) {
                        ...
                        try {
                            const agentDefinition = await readAgentDefinition(runtime, agentWorkspacePath, agentId);
        ...
        const baseCommitSha = coerceNonEmptyString(childEntry?.workspace.taskBaseCommitSha) ?? undefined;
    ...
    async generate(parentWorkspaceId, childWorkspaceId, onComplete) {
        ...
        try {
            ...
            if (!baseCommitSha) {
                const trunkBranch = coerceNonEmptyString(ws.taskTrunkBranch) ??
                ...
                const mergeBaseResult = await execBuffered(runtime, `git merge-base ${shellQuote(trunkBranch)} HEAD`, { cwd: workspacePath, timeout: 30 });
            ...
            const countResult = await execBuffered(runtime, `git rev-list --count ${baseCommitSha}..${headCommitSha}`, { cwd: workspacePath, timeout: 30 });
