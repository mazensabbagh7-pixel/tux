...
import { isCompactionSummaryMetadata, } from "@/common/types/message";
...
function getCompactionMetadataToPreserve(workspaceId, existingMessage, incomingMessage) {
    ...
    if (isCompactionSummaryMetadata(existingMetadata.muxMetadata) &&
...
export class HistoryService {
    constructor(config) {
        this.CHAT_FILE = "chat.jsonl";
        this.PARTIAL_FILE = "partial.json";
    ...
    async readHistoryFromOffset(workspaceId, byteOffset) {
        ...
        try {
            ...
            for (const line of lines) {
                try {
                    messages.push(normalizeLegacyMuxMetadata(JSON.parse(line)));
                }
                catch {
            ...
            while (readEnd > 0 && collected.length < n) {
                ...
                for (let nl = newlinePositions.length - 1; nl >= 0 && collected.length < n; nl--) {
                    ...
                    if (line.length === 0)
                        continue;
                    try {
                        collected.push(normalizeLegacyMuxMetadata(JSON.parse(line)));
                    }
                    catch {
        ...
        try {
            ...
            for (let i = 0; i < lines.length; i++) {
                try {
                    ...
                    log.warn(`Skipping malformed JSON at line ${i + 1} in ${workspaceId}/chat.jsonl:`, parseError instanceof Error ? parseError.message : String(parseError), "\nLine content:", lines[i].substring(0, 100) + (lines[i].length > 100 ? "..." : ""));
                }
            }
            return messages;
        }
    ...
    async iterateForward(workspaceId, visitor) {
        ...
        try {
            ...
            while (readPos < fileSize) {
                ...
                for (const line of completeText.split("\n")) {
                    ...
                    if (trimmed.length === 0)
                        continue;
                    try {
                        messages.push(normalizeLegacyMuxMetadata(JSON.parse(trimmed)));
                    }
                    catch {
                        // Skip malformed lines — same self-healing behavior as readChatHistory
                    }
            ...
            if (carryoverBytes.length > 0) {
                ...
                if (line.length > 0) {
                    try {
                        const msg = normalizeLegacyMuxMetadata(JSON.parse(line));
                        await visitor([msg]);
                    }
                    catch {
                        // Skip malformed line
     ...
     */
    async commitPartial(workspaceId) {
        try {
            ...
            const existingMessage = existingMessages.find((message) => message.metadata?.historySequence === partialSeq);
            const shouldCommit = (!existingMessage || (partial.parts?.length ?? 0) > (existingMessage.parts?.length ?? 0)) &&
    ...
    async getNextHistorySequence(workspaceId) {
        ...
        if (lastResult.success && lastResult.data.length > 0) {
            ...
            if (scanResult.success) {
                ...
                assert(isNonNegativeInteger(nextSeqNum), "next history sequence counter must be a non-negative integer");
                this.sequenceCounters.set(workspaceId, nextSeqNum);
                return nextSeqNum;
            }
        }
    ...
    async _appendToHistoryUnlocked(workspaceId, message) {
        try {
            ...
            log.debug(`[HISTORY APPEND] workspaceId=${workspaceId} role=${message.role} id=${message.id}`);
            ...
            if (!message.metadata) {
                ...
                assert(isNonNegativeInteger(nextSeqNum), "getNextHistorySequence must return a non-negative integer");
                message.metadata = {
                ...
                if (existingSeqNum !== undefined) {
                    assert(isNonNegativeInteger(existingSeqNum), "appendToHistory requires historySequence to be a non-negative integer when provided");
                    // Already has history sequence, update counter if needed
                    const currentCounter = this.sequenceCounters.get(workspaceId) ?? 0;
                    assert(isNonNegativeInteger(currentCounter), "history sequence counter must remain a non-negative integer");
            ...
            log.debug(`[HISTORY APPEND] Assigned historySequence=${message.metadata.historySequence ?? "unknown"} role=${message.role}`);
    ...
    async updateHistory(workspaceId, message) {
        return this.fileLocks.withLock(workspaceId, async () => {
            try {
                ...
                assert(isNonNegativeInteger(targetSequence), "updateHistory requires historySequence to be a non-negative integer");
        ...
        return this.fileLocks.withLock(workspaceId, async () => {
            try {
                ...
                const maxSeq = filteredMessages.reduce((max, msg) => {
                    ...
                    if (!isNonNegativeInteger(seq)) {
                        log.warn("Ignoring malformed persisted historySequence while updating sequence counter after delete", {
                            workspaceId,
                            messageId: msg.id,
                            historySequence: seq,
                        });
                ...
                assert(isNonNegativeInteger(nextSeq), "next history sequence counter after delete must be a non-negative integer");
        ...
        return this.fileLocks.withLock(workspaceId, async () => {
            try {
                ...
                const maxTruncatedSeq = truncatedMessages.reduce((max, msg) => {
                    ...
                    if (!isNonNegativeInteger(seq)) {
                        log.warn("Ignoring malformed persisted historySequence while updating sequence counter after truncation", {
                            workspaceId,
                            messageId: msg.id,
                            historySequence: seq,
                        });
                ...
                assert(isNonNegativeInteger(nextSeq), "next history sequence counter after truncation must be a non-negative integer");
                this.sequenceCounters.set(workspaceId, nextSeq);
                return Ok(undefined);
            }
            catch (error) {
        ...
        return this.fileLocks.withLock(workspaceId, async () => {
            try {
                ...
                const maxRemainingSeq = remainingMessages.reduce((max, msg) => {
                    ...
                    if (!isNonNegativeInteger(seq)) {
                        log.warn("Ignoring malformed persisted historySequence while updating sequence counter after truncateHistory", {
                            workspaceId,
                            messageId: msg.id,
                            historySequence: seq,
                        });
                ...
                assert(isNonNegativeInteger(nextSeq), "next history sequence counter after truncateHistory must be a non-negative integer");
                this.sequenceCounters.set(workspaceId, nextSeq);
                return Ok(deletedSequences);
            }
            catch (error) {
    ...
    async migrateWorkspaceId(oldWorkspaceId, newWorkspaceId) {
        return this.fileLocks.withLock(newWorkspaceId, async () => {
            try {
                ...
                this.sequenceCounters.set(newWorkspaceId, oldCounter);
                this.sequenceCounters.delete(oldWorkspaceId);
                log.debug(`Migrated ${messages.length} messages from ${oldWorkspaceId} to ${newWorkspaceId}`);
                return Ok(undefined);
            }
            catch (error) {
                const message = error instanceof Error ? error.message : String(error);
...
HistoryService.REVERSE_READ_CHUNK_SIZE = 256 * 1024;
/** String-search needle for compaction boundary lines. */
HistoryService.BOUNDARY_NEEDLE = '"compactionBoundary":true';
