...
describe("HistoryService", () => {
    ...
    describe("updateHistory", () => {
        ...
        it("preserves durable compaction metadata across late in-place rewrites", async () => {
            ...
            const lateRewrite = createMuxMessage("summary-msg", "assistant", "Tool execution skipped because the requested tool is disabled by policy.", {
                historySequence: sequence,
                model: "openai:gpt-5",
            });
        ...
        it("self-heals by not preserving malformed compaction boundary metadata", async () => {
            ...
            const malformedCompactionSummary = createMuxMessage("summary-msg", "assistant", "Compacted summary", {
                historySequence: sequence,
                compacted: "user",
                compactionBoundary: true,
                compactionEpoch: 0,
    ...
    describe("iterateFullHistory", () => {
        ...
        it("should iterate forward in chronological order", async () => {
            const msgs = Array.from({ length: 5 }, (_, i) => createMuxMessage(`msg-${i}`, "user", `Message ${i}`));
        ...
        it("should support early exit by returning false", async () => {
            const msgs = Array.from({ length: 10 }, (_, i) => createMuxMessage(`msg-${i}`, "user", `Message ${i}`));
            for (const msg of msgs) {
                await service.appendToHistory(wsId, msg);
            }
            let found;
