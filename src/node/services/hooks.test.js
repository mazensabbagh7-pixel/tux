...
import { getHookPath, getToolEnvPath, getPreHookPath, getPostHookPath, runWithHook, runPreHook, runPostHook, } from "./hooks";
...
describe("hooks", () => {
    ...
    describe("runPostHook", () => {
        test("succeeds when hook exits 0", async () => {
            ...
            await fs.writeFile(hookPath, "#!/bin/bash\nexit 0");
            await fs.chmod(hookPath, 0o755);
            const result = await runPostHook(runtime, hookPath, {
                tool: "test_tool",
                toolInput: '{"arg": "value"}',
                workspaceId: "test-workspace",
                projectDir: tempDir,
...
`);
            ...
            const { result, hook } = await runWithHook(runtime, hookPath, {
                tool: "test_tool",
                toolInput: '{"arg": "value"}',
                workspaceId: "test-workspace",
                projectDir: tempDir,
...
`);
            await fs.chmod(hookPath, 0o755);
            const { hook } = await runWithHook(runtime, hookPath, {
                tool: "bash",
...
`);
            ...
            expect(runWithHook(runtime, hookPath, {
                tool: "test",
                toolInput: "{}",
                workspaceId: "test",
                projectDir: tempDir,
...
`);
            ...
            const { result, hook } = await runWithHook(spacedRuntime, hookPath, {
                tool: "test",
                toolInput: "{}",
                workspaceId: "test",
