...
export class InitStateManager extends EventEmitter {
    constructor(config) {
        ...
        this.store = new EventStore(config, "init-status.json", (state) => this.serializeInitEvents(state), (event) => this.emit(event.type, event), "InitStateManager");
    }
    /**
    ...
    serializeInitEvents(state) {
        ...
        if (lines.length > INIT_HOOK_MAX_LINES) {
            ...
            log.info(`[InitStateManager] Truncated ${excessLines} lines from old persisted data for ${workspaceId}`);
        }
    ...
    async endInit(workspaceId, exitCode) {
        ...
        log.info(`Init hook ${state.status} for workspace ${workspaceId} (exit code ${exitCode}, duration ${endTime - state.startTime}ms)`);
        // Emit init-end event
        this.emit("init-end", {
            type: "init-end",
            workspaceId,
        ...
        try {
            ...
            const timeoutPromise = new Promise((resolve) => {
                timeoutId = setTimeout(() => {
                    log.error(`Init timeout for ${workspaceId} after 5 minutes - tools will proceed anyway. ` +
