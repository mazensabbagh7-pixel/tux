...
describe("MCPServerManager", () => {
    ...
    beforeEach(() => {
        ...
        manager = new MCPServerManager(configService);
        access = manager;
    });
    afterEach(() => {
        manager.dispose();
    ...
    test("cleanupIdleServers does not stop idle servers when workspace is leased", () => {
        ...
        entry.lastActivity = Date.now() - 11 * 60000;
        access.cleanupIdleServers();
    ...
    test("getToolsForWorkspace defers restarts while leased and applies them on next request", async () => {
        ...
        const startServersMock = mock(() => Promise.resolve(new Map([
            [
                "server",
                {
                    name: "server",
    ...
    test("getToolsForWorkspace does not return tools from newly-disabled servers while leased", async () => {
        ...
        const dummyToolA = { execute: mock(() => Promise.resolve({ ok: true })) };
        const dummyToolB = { execute: mock(() => Promise.resolve({ ok: true })) };
        const startServersMock = mock(() => Promise.resolve(new Map([
            [
                "serverA",
                {
