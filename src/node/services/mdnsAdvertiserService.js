...
export function buildMuxMdnsServiceOptions(options) {
    const bindHost = options.bindHost.trim();
    assert(bindHost, "bindHost is required");
    assert(Number.isInteger(options.port) && options.port >= 1 && options.port <= 65535, "invalid port");
...
export class MdnsAdvertiserService {
    ...
    start(serviceOptions) {
        ...
        return this.enqueue(async () => {
            ...
            this.responder ?? (this.responder = getResponder());
