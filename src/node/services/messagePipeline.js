...
import { transformModelMessages, validateAnthropicCompliance, injectAgentTransition, injectFileChangeNotifications, injectPostCompactionAttachments, } from "@/browser/utils/messages/modelMessageTransform";
 ...
 */
export async function prepareMessagesForProvider(opts) {
    ...
    const messagesWithAgentContext = injectAgentTransition(messagesWithSentinel, effectiveAgentId, toolNamesForSentinel, planContentForTransition, planContentForTransition ? planFilePath : undefined);
    // Inject file change notifications as user messages (preserves system message cache)
    const messagesWithFileChanges = injectFileChangeNotifications(messagesWithAgentContext, changedFileAttachments);
    // Inject post-compaction attachments (plan file, edited files) after compaction summary
    const messagesWithPostCompaction = injectPostCompactionAttachments(messagesWithFileChanges, postCompactionAttachments);
    ...
    const rawModelMessages = await convertToModelMessages(messagesWithToolMediaExtracted, {
 ...
 */
export function sanitizeAssistantModelMessages(messages, workspaceId) {
    ...
    if (result.length < messages.length) {
        log.debug(`Self-healing: Filtered ${messages.length - result.length} empty ModelMessage(s)${workspaceId ? ` [${workspaceId}]` : ""}`);
    }
    return result;
}
