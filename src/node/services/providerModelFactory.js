...
import { PROVIDER_REGISTRY, PROVIDER_DEFINITIONS, MUX_GATEWAY_SUPPORTED_PROVIDERS, } from "@/common/constants/providers";
import { CODEX_ENDPOINT, isCodexOauthAllowedModelId, isCodexOauthRequiredModelId, } from "@/common/constants/codexOAuth";
import { parseCodexOauthAuth } from "@/node/utils/codexOauthAuth";
import { isProviderDisabledInConfig } from "@/common/utils/providers/isProviderDisabled";
import { normalizeGatewayModel } from "@/common/utils/ai/models";
import { MUX_APP_ATTRIBUTION_TITLE, MUX_APP_ATTRIBUTION_URL } from "@/constants/appAttribution";
import { resolveProviderCredentials } from "@/node/utils/providerRequirements";
import { normalizeGatewayStreamUsage, normalizeGatewayGenerateResult, } from "@/node/utils/gatewayStreamNormalization";
 ...
 */
export function buildAnthropicHeaders(existingHeaders, use1MContext) {
    if (!use1MContext) {
 ...
 * Exported for testing.
 */
export function buildAppAttributionHeaders(existingHeaders) {
 ...
 */
export async function preloadAISDKProviders() {
...
export function modelCostsIncluded(model) {
    ...
    async createModel(modelString, muxProviderOptions) {
        try {
            ...
            if (providerName !== "mux-gateway" &&
                isProviderDisabledInConfig(providerConfig)) {
            ...
            if (providerName === "mux-gateway") {
                ...
                model.doStream = async (options) => {
                    ...
                    return {
                        ...
                        stream: result.stream.pipeThrough(normalizeGatewayStreamUsage()),
                    };
                };
                const originalDoGenerate = model.doGenerate.bind(model);
