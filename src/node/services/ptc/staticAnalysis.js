...
function detectUnavailableGlobals(code, sourceFile) {
    const errors = [];
    const seen = new Set();
    const parsedSourceFile = sourceFile ?? ts.createSourceFile("code.ts", code, ts.ScriptTarget.ES2020, true);
 ...
 */
export async function analyzeCode(code, muxTypes) {
    const errors = [];
    // 1. Syntax validation
 ...
 */
export function disposeAnalysisContext() {
    if (cachedContext) {
