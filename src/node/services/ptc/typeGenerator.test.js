...
describe("generateMuxTypes", () => {
    test("generates interface from tool input schema", async () => {
        const fileReadTool = createMockTool(z.object({
            filePath: z.string(),
    ...
    test("generates optional properties for fields with .default()", async () => {
        const tool = createMockTool(z.object({
    ...
    test("generates discriminated union result types with success: true/false", async () => {
        const bashTool = createMockTool(z.object({
            script: z.string(),
    ...
    test("handles MCP tools with MCPCallToolResult", async () => {
        const mcpTool = createMockTool(z.object({
        ...
        expect(types).toContain("function mcp__github__create_issue(args: McpGithubCreateIssueArgs): MCPCallToolResult");
    ...
    test("handles tools without known result type (returns unknown)", async () => {
        const customTool = createMockTool(z.object({
            input: z.string(),
        }));
