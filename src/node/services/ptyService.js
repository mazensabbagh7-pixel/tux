...
export class PTYService {
     ...
     */
    async createSession(params, runtime, workspacePath, onData, onExit, runtimeConfig, options) {
        ...
        if (runtime instanceof SSHRuntime) {
            ...
            log.info(`[PTY] Devcontainer terminal for ${sessionId}: devcontainer ${devcontainerArgs.join(" ")}`);
            ...
            log.info(`Spawning PTY: ${shell.command}${printableArgs}, cwd: ${workspacePath}, size: ${params.cols}x${params.rows}`);
        ...
        log.info(`Creating terminal session ${sessionId} for workspace ${params.workspaceId} (${runtimeLabel})`);
    ...
    resize(params) {
        ...
        log.debug(`Resized terminal ${params.sessionId} (${session.runtimeLabel}) to ${params.cols}x${params.rows}`);
    }
    /**
