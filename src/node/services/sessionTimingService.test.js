...
describe("SessionTimingService", () => {
    ...
    it("persists aborted stream stats to session-timing.json", async () => {
        ...
        const service = new SessionTimingService(config, telemetry);
    ...
    it("does not double-count overlapping tool calls", async () => {
        ...
        expect(snapshot.lastRequest?.toolExecutionMs).toBeLessThanOrEqual(snapshot.lastRequest?.totalDurationMs ?? 0);
