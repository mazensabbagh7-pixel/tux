...
import { parsePublicKey, verifySignature } from "@coder/mux-md-client";
...
describe("SigningService", () => {
    ...
    const testDir = join(tmpdir(), `signing-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
    ...
    describe("with Ed25519 key", () => {
        it("should load key and return capabilities", () => withoutSshAgent(async () => {
        ...
        it("should sign messages", () => withoutSshAgent(async () => {
            const service = new SigningService([ed25519KeyPath]);
        ...
        it("should return consistent public key across multiple calls", () => withoutSshAgent(async () => {
            const service = new SigningService([ed25519KeyPath]);
    ...
    describe("with no key", () => {
        it("should return null publicKey when no key exists", () => withoutSshAgent(async () => {
        ...
        it("should throw when signing without a key", () => withoutSshAgent(async () => {
            const service = new SigningService(["/nonexistent/path/key"]);
            let threw = false;
            try {
    ...
    describe("key path priority", () => {
        it("should use first available key in path order", () => withoutSshAgent(async () => {
        ...
        it("should skip missing paths and use next available", () => withoutSshAgent(async () => {
    ...
    describe("with encrypted key", () => {
        it("should detect encrypted key and return hasEncryptedKey=true", () => withoutSshAgent(async () => {
        ...
        it("should skip encrypted key and use unencrypted fallback", () => withoutSshAgent(async () => {
        ...
        it("should reset hasEncryptedKey on cache clear", () => withoutSshAgent(async () => {
            const service = new SigningService([encryptedKeyPath]);
            const caps1 = await service.getCapabilities();
