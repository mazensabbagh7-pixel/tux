...
import { streamText, stepCountIs, LoadAPIKeyError, APICallError, RetryError, } from "ai";
import { Ok, Err } from "@/common/types/result";
import { log } from "./log";
import { coerceStreamErrorTypeForMessage, createErrorEvent, stripNoisyErrorPrefix, } from "@/node/services/utils/sendMessageError";
...
export function stripEncryptedContent(output) {
...
export class StreamManager extends EventEmitter {
    constructor(historyService, sessionUsageService) {
        super();
        this.workspaceStreams = new Map();
        this.streamLocks = new Map();
        this.PARTIAL_WRITE_THROTTLE_MS = 500;
        // Token tracker for live streaming statistics
        this.tokenTracker = new StreamingTokenTracker();
    ...
    async recordSessionUsage(workspaceId, model, usage, providerMetadata, logMessage, logLevel, streamInfo) {
        ...
        try {
            await this.sessionUsageService.recordUsage(workspaceId, normalizeGatewayModel(model), messageUsage);
        }
        catch (error) {
            (logLevel === "error" ? workspaceLog.error : workspaceLog.warn)(logMessage, { error });
        }
    }
    buildStreamRequestConfig(model, modelString, messages, system, tools, providerOptions, maxOutputTokens, toolPolicy, hasQueuedMessage, headers) {
    ...
    createStopWhenCondition(request) {
        ...
        const isSuccessfulAgentReportOutput = (value) => {
            return (typeof value === "object" &&
                value !== null &&
                "success" in value &&
                value.success === true);
        };
        const hasSuccessfulAgentReportResult = ({ steps }) => {
            const lastStep = steps[steps.length - 1];
            return (lastStep?.toolResults?.some((toolResult) => toolResult.toolName === "agent_report" &&
                isSuccessfulAgentReportOutput(toolResult.output)) ?? false);
        };
        return [
            stepCountIs(100000),
     ...
     */
    createStreamAtomically(workspaceId, streamToken, runtimeTempDir, runtime, messages, model, modelString, abortController, system, historySequence, messageId, tools, initialMetadata, providerOptions, maxOutputTokens, toolPolicy, hasQueuedMessage, workspaceName, thinkingLevel, headers) {
        // abortController is created and linked to the caller-provided abortSignal in startStream().
        const stepTracker = {};
        const request = this.buildStreamRequestConfig(model, modelString, messages, system, tools, providerOptions, maxOutputTokens, toolPolicy, hasQueuedMessage, headers);
                ...
                try {
                    for await (const part of streamInfo.streamResult.fullStream) {
                        ...
                        switch (part.type) {
                            ...
                            case "tool-call": {
                                ...
                                const inputText = JSON.stringify(part.input);
                                log.debug(`[StreamManager] tool-call: toolName=${part.toolName}, input length=${inputText.length}`);
                            ...
                            case "tool-result": {
                                ...
                                const strippedOutput = stripInternalToolResultFields(stripEncryptedContent(toolResultPart.output));
                            ...
                            case "finish-step": {
                                ...
                                streamInfo.cumulativeProviderMetadata = accumulateProviderMetadata(streamInfo.cumulativeProviderMetadata, finishStepPart.providerMetadata);
                    ...
                    if (!streamInfo.abortController.signal.aborted) {
                        ...
                        if (streamInfo.parts && streamInfo.parts.length > 0) {
                            ...
                            const updateResult = await this.historyService.updateHistory(workspaceId, finalAssistantMessage);
    ...
    buildStreamErrorPayload(streamInfo, error) {
        ...
        let errorMessage = stripNoisyErrorPrefix(error instanceof Error ? error.message : String(error));
    ...
    async persistStreamError(workspaceId, streamInfo, payload) {
        ...
        // Emit error event.
        this.emit("error", createErrorEvent(workspaceId, payload));
    }
    ...
    categorizeError(error) {
        ...
        if (APICallError.isInstance(error)) {
            ...
            const isContextExceeded = msgLower.includes("prompt is too long") ||
     ...
     */
    async startStream(workspaceId, messages, model, modelString, historySequence, system, runtime, messageId, abortSignal, tools, initialMetadata, providerOptions, maxOutputTokens, toolPolicy, providedStreamToken, hasQueuedMessage, workspaceName, thinkingLevel, headers) {
        ...
        try {
            ...
            try {
                ...
                log.debug(`[STREAM START] workspaceId=${workspaceId} historySequence=${historySequence} model=${modelString}`);
                ...
                try {
                    ...
                    const streamInfo = this.createStreamAtomically(typedWorkspaceId, streamToken, runtimeTempDir, runtime, messages, model, modelString, streamAbortController, system, historySequence, messageId, tools, initialMetadata, providerOptions, maxOutputTokens, toolPolicy, hasQueuedMessage, workspaceName, thinkingLevel, headers);
