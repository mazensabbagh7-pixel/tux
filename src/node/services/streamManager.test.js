...
describe("StreamManager - createTempDirForStream", () => {
    test("creates ~/.mux-tmp/<token> under the runtime's home", async () => {
        ...
        try {
            const home = __addDisposableResource(env_1, new DisposableTempDir("stream-home"), false);
...
describe("StreamManager - Concurrent Stream Prevention", () => {
    ...
    describeIntegration("with real API", () => {
        test("should prevent concurrent streams for the same workspace", async () => {
            ...
            // Start first stream
            const result1 = await streamManager.startStream(workspaceId, [{ role: "user", content: "Say hello and nothing else" }], model, KNOWN_MODELS.SONNET.id, 1, "You are a helpful assistant", runtime, "test-msg-1", undefined, {});
            expect(result1.success).toBe(true);
            // Wait for first stream to actually start
            await new Promise((resolve) => setTimeout(resolve, 200));
            // Start second stream - should cancel first
            const result2 = await streamManager.startStream(workspaceId, [{ role: "user", content: "Say goodbye and nothing else" }], model, KNOWN_MODELS.SONNET.id, 2, "You are a helpful assistant", runtime, "test-msg-2", undefined, {});
    ...
    test("should serialize multiple rapid startStream calls", async () => {
        ...
        const promises = [
            streamManager.startStream(workspaceId, [{ role: "user", content: "test 1" }], model, KNOWN_MODELS.SONNET.id, 1, "system", runtime, "test-msg-1", undefined, {}),
            streamManager.startStream(workspaceId, [{ role: "user", content: "test 2" }], model, KNOWN_MODELS.SONNET.id, 2, "system", runtime, "test-msg-2", undefined, {}),
            streamManager.startStream(workspaceId, [{ role: "user", content: "test 3" }], model, KNOWN_MODELS.SONNET.id, 3, "system", runtime, "test-msg-3", undefined, {}),
        ];
    ...
    test("should honor abortSignal before atomic stream creation", async () => {
        ...
        const anthropic = createAnthropic({ apiKey: "dummy-key" });
        const model = anthropic("claude-sonnet-4-5");
        const startPromise = streamManager.startStream(workspaceId, [{ role: "user", content: "test" }], model, KNOWN_MODELS.SONNET.id, 1, "system", runtime, "test-msg-abort", abortController.signal, {});
...
describe("StreamManager - previousResponseId recovery", () => {
    ...
    test("retryStreamWithoutPreviousResponseId retries at step boundary with existing parts", async () => {
        ...
        expect(streamInfo.request.messages).toBe(stepMessages);
