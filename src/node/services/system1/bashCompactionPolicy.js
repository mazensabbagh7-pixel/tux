...
export function isBashOutputAlreadyTargeted(script) {
...
export function decideBashOutputCompaction(params) {
    assert(params, "params is required");
    assert(typeof params.toolName === "string" && params.toolName.length > 0, "toolName must be a non-empty string");
    assert(typeof params.script === "string", "script must be a string");
    assert(typeof params.planFilePath === "string" || typeof params.planFilePath === "undefined", "planFilePath must be a string if provided");
    assert(Number.isInteger(params.totalLines) && params.totalLines >= 0, "totalLines must be a non-negative integer");
    assert(Number.isInteger(params.totalBytes) && params.totalBytes >= 0, "totalBytes must be a non-negative integer");
    assert(Number.isInteger(params.minLines) && params.minLines >= 0, "minLines must be >= 0");
    assert(Number.isInteger(params.minTotalBytes) && params.minTotalBytes >= 0, "minTotalBytes must be >= 0");
    ...
    if (params.toolName === "bash") {
        ...
        const defaultMinLines = SYSTEM1_BASH_OUTPUT_COMPACTION_LIMITS.bashOutputCompactionMinLines.default;
        const defaultMinTotalBytes = SYSTEM1_BASH_OUTPUT_COMPACTION_LIMITS.bashOutputCompactionMinTotalBytes.default;
        const defaultMaxKeptLines = SYSTEM1_BASH_OUTPUT_COMPACTION_LIMITS.bashOutputCompactionMaxKeptLines.default;
        ...
        if (isDefaultCompactionConfig) {
            if (isConflictMarkerSearch) {
                ...
                effectiveMaxKeptLines = Math.min(BASH_HARD_MAX_LINES, Math.max(params.maxKeptLines, EXPLORATION_BOOST_MAX_KEPT_LINES));
            }
        }
    }
    assert(Number.isInteger(effectiveMaxKeptLines) && effectiveMaxKeptLines > 0, "effectiveMaxKeptLines must be a positive integer");
    return {
        shouldCompact: true,
        triggeredByLines,
        triggeredByBytes,
