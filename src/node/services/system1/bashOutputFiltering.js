...
export function formatSystem1BashFilterNotice(params) {
    assert(Number.isInteger(params.keptLines) && params.keptLines >= 0, "keptLines must be a non-negative integer");
    ...
    assert(typeof params.trigger === "string" && params.trigger.length > 0, "trigger must be a string");
...
export function splitBashOutputLines(output) {
    if (output.length === 0) {
        return [];
    }
...
export function formatNumberedLinesForSystem1(lines) {
    return lines.map((line, index) => `${String(index + 1).padStart(4, "0")}| ${line}`).join("\n");
}
const GIT_CONFLICT_MARKER_REGEX = /(<{7}|={7}|>{7}|\|{7})/;
const HEURISTIC_IMPORTANT_LINE_REGEX = /(^|\b)(error|failed|failure|fatal|panic|exception|traceback|warning|assertion failed|npm err!|err!|exited with code|exit code)(\b|$)/i;
...
function normalizeKeepRanges(ranges, maxLine) {
...
//# sourceMappingURL=bashOutputFiltering.js.map