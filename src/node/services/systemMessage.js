...
import { extractModelSection, extractToolSection, stripScopedInstructionSections, } from "@/node/utils/main/markdown";
...
function buildEnvironmentContext(workspacePath, runtimeType) {
    ...
    const isRemote = runtimeType === RUNTIME_MODE.SSH ||
 ...
 */
export function extractToolInstructions(globalInstructions, contextInstructions, modelString, options) {
 ...
 * @returns Map of tool names to their additional instructions
 */
export async function readToolInstructions(metadata, runtime, workspacePath, modelString, agentInstructions) {
 ...
 */
export async function buildSystemMessage(metadata, runtime, workspacePath, additionalSystemInstructions, modelString, mcpServers, options) {
    if (!metadata)
    ...
    const modelContent = modelString
        ? searchInstructionSources({ agent: agentPrompt, context: contextInstructions, global: globalInstructions }, (src) => extractModelSection(src, modelString))
        : null;
