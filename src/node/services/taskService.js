...
import { discoverAgentDefinitions, readAgentDefinition, resolveAgentFrontmatter, } from "@/node/services/agentDefinitions/agentDefinitionsService";
...
import { coerceNonEmptyString, tryReadGitHeadCommitSha, findWorkspaceEntry, } from "@/node/services/taskUtils";
...
import { AgentReportToolArgsSchema, TaskToolResultSchema, TaskToolArgsSchema, } from "@/common/utils/tools/toolDefinitions";
...
import { readSubagentReportArtifact, readSubagentReportArtifactsFile, upsertSubagentReportArtifact, } from "@/node/services/subagentReportArtifacts";
...
function isTypedWorkspaceEvent(value, type) {
...
export class TaskService {
    constructor(config, historyService, aiService, workspaceService, initStateManager) {
        this.config = config;
        ...
        this.workspaceEventLocks = new MutexMap();
        this.mutex = new AsyncMutex();
        this.pendingWaitersByTaskId = new Map();
        this.pendingStartWaitersByTaskId = new Map();
        ...
        this.foregroundAwaitCountByWorkspaceId = new Map();
        ...
        this.completedReportsByTaskId = new Map();
        this.remindedAwaitingReport = new Set();
        /** Tracks consecutive auto-resumes per workspace. Reset when a user message is sent. */
        this.consecutiveAutoResumes = new Map();
     ...
     */
    async resolveParentAutoResumeOptions(parentWorkspaceId, parentEntry, fallbackModel, hint) {
    ...
    async initialize() {
        ...
        for (const task of reportedTasks) {
            if (!task.parentWorkspaceId)
                continue;
            try {
                await this.gitPatchArtifactService.maybeStartGeneration(task.parentWorkspaceId, task.id, (wsId) => this.requestReportedTaskCleanupRecheck(wsId));
            }
            catch (error) {
    ...
    async create(args) {
        ...
        try {
            ...
            if (requestedDepth > taskSettings.maxTaskNestingDepth) {
                return Err(`Task.create: maxTaskNestingDepth exceeded (requestedDepth=${requestedDepth}, max=${taskSettings.maxTaskNestingDepth})`);
            }
            ...
            if (!nameValidation.valid) {
                return Err(`Task.create: generated workspace name invalid (${nameValidation.error ?? "unknown error"})`);
            }
            ...
            const getRunnableHint = async () => {
                try {
                    ...
                    const runnableIds = (await Promise.all(allAgents.map(async (agent) => {
                        try {
                            const frontmatter = await resolveAgentFrontmatter(runtime, parentWorkspacePath, agent.id);
            ...
            const { forkedRuntimeConfig } = await applyForkRuntimeUpdates(this.config, parentWorkspaceId, parentRuntimeConfig, forkResult);
            ...
            if (!createResult.success || !createResult.workspacePath) {
                initLogger.logComplete(-1);
                return Err(`Task.create: failed to create agent workspace (${createResult.error ?? "unknown error"})`);
            }
            ...
            runBackgroundInit(runtimeForTaskWorkspace, {
                projectPath: parentMeta.projectPath,
                branchName: workspaceName,
                trunkBranch,
                workspacePath,
    ...
    async terminateDescendantAgentTask(ancestorWorkspaceId, taskId) {
        assert(ancestorWorkspaceId.length > 0, "terminateDescendantAgentTask: ancestorWorkspaceId must be non-empty");
    ...
    startForegroundAwait(workspaceId) {
        ...
        assert(Number.isInteger(current) && current >= 0, "startForegroundAwait: expected non-negative integer counter");
        this.foregroundAwaitCountByWorkspaceId.set(workspaceId, current + 1);
        return () => {
            const current = this.foregroundAwaitCountByWorkspaceId.get(workspaceId) ?? 0;
            assert(Number.isInteger(current) && current > 0, "startForegroundAwait cleanup: expected positive integer counter");
            if (current <= 1) {
    ...
    hasActiveDescendantAgentTasksForWorkspace(workspaceId) {
        assert(workspaceId.length > 0, "hasActiveDescendantAgentTasksForWorkspace: workspaceId must be non-empty");
        const cfg = this.config.loadConfigOrDefault();
    ...
    listDescendantAgentTasks(workspaceId, options) {
        ...
        while (stack.length > 0) {
            ...
            if (!entry)
                continue;
            assert(entry.parentWorkspaceId, `listDescendantAgentTasks: task ${next.taskId} is missing parentWorkspaceId`);
    ...
    async filterDescendantAgentTaskIds(ancestorWorkspaceId, taskIds) {
        assert(ancestorWorkspaceId.length > 0, "filterDescendantAgentTaskIds: ancestorWorkspaceId required");
    ...
    listDescendantAgentTaskIdsFromIndex(index, workspaceId) {
        assert(workspaceId.length > 0, "listDescendantAgentTaskIdsFromIndex: workspaceId must be non-empty");
        const result = [];
        const stack = [...(index.childrenByParent.get(workspaceId) ?? [])];
        while (stack.length > 0) {
            const next = stack.pop();
     ...
     * of its status.
     */
    hasChildAgentTasks(index, workspaceId) {
    ...
    async maybeStartQueuedTasks() {
        ...
        try {
            ...
            const taskSettingsAtStart = configAtStart.taskSettings ?? DEFAULT_TASK_SETTINGS;
            ...
            for (const taskId of queuedTaskIds) {
                const config = this.config.loadConfigOrDefault();
                const taskSettings = config.taskSettings ?? DEFAULT_TASK_SETTINGS;
                assert(Number.isFinite(taskSettings.maxParallelAgentTasks) &&
                ...
                const latestTaskSettings = latestConfig.taskSettings ?? DEFAULT_TASK_SETTINGS;
                ...
                if (!workspaceExists) {
                    ...
                    const { forkedRuntimeConfig: resolvedForkedRuntimeConfig } = await applyForkRuntimeUpdates(this.config, parentId, parentRuntimeConfig, forkResult);
                ...
                if (shouldRunInit) {
                    ...
                    if (agentIdRaw) {
                        ...
                        if (parsedAgentId.success) {
                            ...
                            try {
                                const frontmatter = await resolveAgentFrontmatter(runtime, parentWorkspacePath, parsedAgentId.data);
                                skipInitHook = frontmatter.subagent?.skip_init_hook === true;
                            }
                            catch (error) {
    ...
    async handleStreamEnd(event) {
        ...
        if (!entry.workspace.parentWorkspaceId) {
            ...
            const resumeOptions = await this.resolveParentAutoResumeOptions(workspaceId, entry, defaultModel, event.metadata);
    ...
    async finalizeTerminationPhaseForReportedTask(workspaceId) {
        assert(workspaceId.length > 0, "finalizeTerminationPhaseForReportedTask: workspaceId must be non-empty");
        await this.cleanupReportedLeafTask(workspaceId);
    }
    async maybeStartPatchGenerationForReportedTask(workspaceId) {
        assert(workspaceId.length > 0, "maybeStartPatchGenerationForReportedTask: workspaceId must be non-empty");
        const cfg = this.config.loadConfigOrDefault();
        ...
        try {
            await this.gitPatchArtifactService.maybeStartGeneration(parentWorkspaceId, workspaceId, (wsId) => this.requestReportedTaskCleanupRecheck(wsId));
        }
        catch (error) {
    ...
    requestReportedTaskCleanupRecheck(workspaceId) {
        assert(workspaceId.length > 0, "requestReportedTaskCleanupRecheck: workspaceId must be non-empty");
    ...
    async finalizeAgentTaskReport(childWorkspaceId, childEntry, reportArgs) {
        assert(childWorkspaceId.length > 0, "finalizeAgentTaskReport: childWorkspaceId must be non-empty");
        assert(typeof reportArgs.reportMarkdown === "string" && reportArgs.reportMarkdown.length > 0, "finalizeAgentTaskReport: reportMarkdown must be non-empty");
        ...
        if (!hasActiveDescendants && !this.aiService.isStreaming(parentWorkspaceId)) {
            const resumeOptions = await this.resolveParentAutoResumeOptions(parentWorkspaceId, parentEntry, latestChildEntry?.workspace.taskModelString ?? defaultModel);
    ...
    async deliverReportToParent(parentWorkspaceId, childWorkspaceId, childEntry, report) {
        assert(childWorkspaceId.length > 0, "deliverReportToParent: childWorkspaceId must be non-empty");
    ...
    async tryFinalizePendingTaskToolCallInPartial(workspaceId, output) {
        ...
        const pendingParts = partial.parts.filter((p) => isDynamicToolPart(p) && p.toolName === "task" && p.state === "input-available");
    ...
    async canCleanupReportedTask(workspaceId) {
