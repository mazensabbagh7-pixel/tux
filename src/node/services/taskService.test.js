...
import { getSubagentGitPatchMboxPath, readSubagentGitPatchArtifact, } from "@/node/services/subagentGitPatchArtifacts";
...
function createWorkspaceServiceMocks(overrides) {
    const sendMessage = overrides?.sendMessage ?? mock(() => Promise.resolve(Ok(undefined)));
    const resumeStream = overrides?.resumeStream ?? mock(() => Promise.resolve(Ok(undefined)));
    const remove = overrides?.remove ?? mock(() => Promise.resolve(Ok(undefined)));
...
describe("TaskService", () => {
    ...
    test("queues tasks when maxParallelAgentTasks is reached and starts them when a slot frees", async () => {
        ...
        expect(sendMessage).toHaveBeenCalledWith(queued.data.taskId, "task 2", expect.anything(), expect.objectContaining({ allowQueuedAgentTask: true }));
    ...
    test("does not count foreground-awaiting tasks towards maxParallelAgentTasks", async () => {
        ...
        const { aiService } = createAIServiceMocks(config, {
            isStreaming: mock((workspaceId) => workspaceId === streamingWorkspaceId),
        });
        const runtimeConfig = { type: "worktree", srcBaseDir: config.srcDir };
        const runtime = createRuntime(runtimeConfig, { projectPath });
        const initLogger = createNullInitLogger();
        ...
        expect(sendMessage).toHaveBeenCalledWith(childTask.data.taskId, "child task", expect.anything(), expect.objectContaining({ allowQueuedAgentTask: true }));
    ...
    test("persists forked runtime config updates when dequeuing tasks", async () => {
        ...
        let forkCallCount = 0;
        const forkSpy = spyOn(WorktreeRuntime.prototype, "forkWorkspace").mockImplementation(async function (params) {
    ...
    test("does not start queued tasks while a reported task is still streaming", async () => {
        ...
        const { aiService } = createAIServiceMocks(config, {
            isStreaming: mock((workspaceId) => workspaceId === reportedTaskId),
        });
        const { workspaceService, sendMessage } = createWorkspaceServiceMocks();
        const { taskService } = createTaskServiceHarness(config, { aiService, workspaceService });
        await taskService.initialize();
    ...
    test("agentAiDefaults outrank workspace aiSettingsByAgent for same agent", async () => {
        ...
        if (!created.success)
            return;
        expect(sendMessage).toHaveBeenCalledWith(created.data.taskId, "run task with same-agent conflicts", {
            model: "anthropic:claude-haiku-4-5",
            agentId: "explore",
            thinkingLevel: "off",
            experiments: undefined,
    ...
    test("auto-resumes a parent workspace until background tasks finish", async () => {
        ...
        expect(sendMessage).toHaveBeenCalledWith(rootWorkspaceId, expect.stringContaining(childTaskId), expect.objectContaining({
    ...
    test("auto-resume preserves parent agentId from history when stream-end metadata omits agentId", async () => {
        ...
        const appendResult = await historyService.appendToHistory(rootWorkspaceId, createMuxMessage("assistant-root-history", "assistant", "Parent is currently running in plan mode.", { timestamp: Date.now(), agentId: "plan" }));
    ...
    test("tasks-completed auto-resume preserves parent agentId from history", async () => {
        ...
        const appendResult = await historyService.appendToHistory(parentWorkspaceId, createMuxMessage("assistant-parent-history", "assistant", "Parent is currently running in plan mode.", { timestamp: Date.now(), agentId: "plan" }));
        expect(appendResult.success).toBe(true);
        const internal = taskService;
        await internal.handleStreamEnd({
            type: "stream-end",
        ...
        expect(sendMessage).toHaveBeenCalledTimes(1);
        expect(sendMessage).toHaveBeenCalledWith(parentWorkspaceId, expect.stringContaining("sub-agent task(s) have completed"), expect.objectContaining({
            agentId: "plan",
        }), expect.objectContaining({ skipAutoResumeReset: true, synthetic: true }));
    });
    test("terminateDescendantAgentTask stops stream, removes workspace, and rejects waiters", async () => {
        ...
        const { taskService } = createTaskServiceHarness(config, { aiService, workspaceService });
        const waiter = taskService.waitForAgentReport(taskId, { timeoutMs: 10000 });
        ...
        expect(stopStream).toHaveBeenCalledWith(taskId, expect.objectContaining({ abandonPartial: true }));
    ...
    test("initialize resumes awaiting_report tasks after restart", async () => {
        ...
        const { taskService } = createTaskServiceHarness(config, { aiService, workspaceService });
        await taskService.initialize();
        expect(sendMessage).toHaveBeenCalledWith(childId, expect.stringContaining("awaiting its final agent_report"), expect.objectContaining({
    ...
    test("agent_report posts report to parent, finalizes pending task tool output, and triggers cleanup", async () => {
        ...
        const parentPartial = createMuxMessage("assistant-parent-partial", "assistant", "Waiting on subagent…", { timestamp: Date.now() }, [
            {
                type: "dynamic-tool",
        ...
        const childPartial = createMuxMessage("assistant-child-partial", "assistant", "", { timestamp: Date.now(), historySequence: childHistorySequence }, [
            {
                type: "dynamic-tool",
                toolCallId: "agent-report-call-1",
                toolName: "agent_report",
        ...
        expect(emit).toHaveBeenCalledWith("metadata", expect.objectContaining({ workspaceId: childId }));
        expect(remove).toHaveBeenCalled();
        expect(sendMessage).toHaveBeenCalledWith(parentId, expect.stringContaining("sub-agent task(s) have completed"), expect.any(Object), expect.objectContaining({ skipAutoResumeReset: true, synthetic: true }));
    ...
    test("agent_report generates git format-patch artifact for exec tasks and defers cleanup", async () => {
        ...
        const writeParentPartial = await partialService.writePartial(parentId, parentPartial);
        expect(writeParentPartial.success).toBe(true);
        const childPartial = createMuxMessage("assistant-child-partial", "assistant", "", { timestamp: Date.now(), historySequence: 0 }, [
            {
                type: "dynamic-tool",
                toolCallId: "agent-report-call-1",
                toolName: "agent_report",
        ...
        expect(artifactAfterStreamEnd?.status === "pending" || artifactAfterStreamEnd?.status === "ready").toBe(true);
    ...
    test("agent_report updates queued/running task tool output in parent history", async () => {
        ...
        const parentHistoryMessage = createMuxMessage("assistant-parent-history", "assistant", "Spawned subagent…", { timestamp: Date.now() }, [
            {
                type: "dynamic-tool",
                toolCallId: "task-call-1",
                toolName: "task",
        ...
        expect(remove).toHaveBeenCalled();
        expect(sendMessageMock).toHaveBeenCalledWith(parentId, expect.stringContaining("sub-agent task(s) have completed"), expect.any(Object), expect.objectContaining({ skipAutoResumeReset: true, synthetic: true }));
    });
    ...
    test("missing agent_report triggers one reminder, then posts fallback output and cleans up", async () => {
        ...
        const writeParentPartial = await partialService.writePartial(parentId, parentPartial);
        expect(writeParentPartial.success).toBe(true);
        const assistantOutput = createMuxMessage("assistant-child-output", "assistant", "Final output without agent_report", { timestamp: Date.now() });
    ...
    test("sibling reported task blocks parent deletion during cleanup", async () => {
        ...
        const remove = mock(async (workspaceId) => {
        ...
        const removedWorkspaceIds = remove.mock.calls.map((call) => call[0]);
