...
export class TerminalService {
    constructor(config, ptyService) {
        // Event emitters for each session
        this.outputEmitters = new Map();
        this.exitEmitters = new Map();
        ...
        this.headlessTerminals = new Map();
        this.serializeAddons = new Map();
        this.headlessOnDataDisposables = new Map();
        this.config = config;
    ...
    async create(params) {
        try {
            ...
            if (!workspaceMetadata.projectPath) {
                ...
                throw new Error(`Workspace "${workspaceMetadata.name}" (${params.workspaceId}) is missing projectPath. ` +
            ...
            const workspacePath = runtime.getWorkspacePath(workspaceMetadata.projectPath, workspaceMetadata.name);
            ...
            const secrets = shouldInjectLocalEnv && workspaceMetadata.id !== MUX_HELP_CHAT_WORKSPACE_ID
            ...
            // 5. Create session
            const session = await this.ptyService.createSession(params, runtime, workspacePath, onData, onExit, workspaceMetadata.runtimeConfig, { env: terminalEnv });
            ...
            const disposeHeadlessOnData = headless.onData((data) => {
                if (!data) {
                    return;
                }
                try {
    ...
    async openWindow(workspaceId, sessionId) {
        try {
            const allMetadata = await this.config.getAllWorkspaceMetadata();
            ...
            if (isDesktop) {
                log.info(`Opening terminal window for workspace: ${workspaceId}${sessionId ? ` (session: ${sessionId})` : ""}`);
                await this.terminalWindowManager.openTerminalWindow(workspaceId, sessionId);
            }
            else {
                log.info(`Browser mode: terminal UI handled by browser for ${isSSH ? "SSH" : "local"} workspace: ${workspaceId}`);
            }
        }
        catch (err) {
     ...
     */
    async openNativeWithCommand(command, workspacePath) {
        await this.openNativeTerminal({
            type: "local",
     ...
     */
    async openNativeTerminal(config) {
    ...
    async openNativeTerminalMacOS(config, sshArgs, logPrefix) {
        const isSSH = config.type === "ssh";
    ...
    openNativeTerminalWindows(config, sshArgs, logPrefix) {
        const isSSH = config.type === "ssh";
        const command = config.command;
        const workspacePath = config.type === "local" ? config.workspacePath : config.remotePath;
        // Windows
    ...
    async openNativeTerminalLinux(config, sshArgs, logPrefix) {
        const isSSH = config.type === "ssh";
        const command = config.command;
        const workspacePath = config.type === "local" ? config.workspacePath : config.remotePath;
        ...
        if (availableTerminal) {
            ...
            log.info(`Opening ${logPrefix}: ${availableTerminal.cmd} ${availableTerminal.args.join(" ")}${cwdInfo}`);
     ...
     */
    async findAvailableTerminal(terminals) {
    ...
    onExit(sessionId, callback) {
        ...
        const handler = (code) => callback(code);
