...
describe("TokenizerService", () => {
    ...
    describe("calculateStats", () => {
        test("delegates to underlying function and persists token stats cache", async () => {
            ...
            const persistSpy = spyOn(sessionUsageService, "setTokenStatsCache").mockResolvedValue(undefined);
            ...
            expect(persistSpy).toHaveBeenCalledWith("test-workspace", expect.objectContaining({
                version: 1,
                computedAt: 1234,
                model: "gpt-4",
                tokenizerName: "cl100k",
        ...
        test("throws on invalid messages", () => {
            // @ts-expect-error testing runtime validation
            expect(service.calculateStats("test-workspace", null, "gpt-4")).rejects.toThrow("requires an array");
        });
