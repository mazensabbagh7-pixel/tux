...
describe("agent_skill_read_file (Chat with Mux sandbox)", () => {
    it("allows reading built-in skill files", async () => {
        ...
        try {
            const tempDir = __addDisposableResource(env_1, new TestTempDir("test-agent-skill-read-file-mux-chat"), false);
    ...
    it("rejects project/global skill file reads on disk", async () => {
        ...
        try {
            const tempDir = __addDisposableResource(env_2, new TestTempDir("test-agent-skill-read-file-mux-chat-reject"), false);
