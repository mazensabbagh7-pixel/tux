...
import { BASH_DEFAULT_TIMEOUT_SECS, BASH_HARD_MAX_LINES, BASH_MAX_LINE_BYTES, BASH_MAX_TOTAL_BYTES, BASH_MAX_FILE_BYTES, BASH_TRUNCATE_MAX_TOTAL_BYTES, BASH_TRUNCATE_MAX_FILE_BYTES, } from "@/common/constants/toolLimits";
 ...
 */
export const createBashTool = (config) => {
    ...
    const maxTotalBytes = overflowPolicy === "truncate" ? BASH_TRUNCATE_MAX_TOTAL_BYTES : BASH_MAX_TOTAL_BYTES;
    const maxFileBytes = overflowPolicy === "truncate" ? BASH_TRUNCATE_MAX_FILE_BYTES : BASH_MAX_FILE_BYTES;
    ...
    return tool({
        ...
        execute: async ({ script, timeout_secs, run_in_background, display_name }, { abortSignal, toolCallId }) => {
            ...
            const shouldRewriteNullRedirects = process.platform === "win32" && config.runtime instanceof LocalBaseRuntime;
...
${scriptWithEnv}`;
            ...
            const MAX_LIVE_EVENT_CHARS = 32768;
                ...
                if (shouldBackground) {
                    ...
                    if (didExit) {
                        ...
                        if (config.backgroundProcessManager && config.workspaceId) {
                            ...
                            const migrateResult = await migrateToBackground(migrationStream, {
                                cwd: config.cwd,
                                workspaceId: config.workspaceId,
                                processId,
                                script,
