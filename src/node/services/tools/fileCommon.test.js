...
import { validatePlanModeAccess, validatePathInCwd, validateFileSize, validateNoRedundantPrefix, MAX_FILE_SIZE, } from "./fileCommon";
...
describe("fileCommon", () => {
    ...
    describe("validateNoRedundantPrefix", () => {
        ...
        it("should work with cwd that has trailing slash", () => {
            const cwdWithSlash = "/workspace/project/";
            const result = validateNoRedundantPrefix("/workspace/project/src/file.ts", cwdWithSlash, runtime);
            expect(result).not.toBeNull();
            expect(result?.correctedPath).toBe("src/file.ts");
            expect(result?.warning).toContain("auto-corrected");
        });
        it("should handle nested paths correctly", () => {
            const result = validateNoRedundantPrefix("/workspace/project/src/components/Button/index.ts", cwd, runtime);
            expect(result).not.toBeNull();
        ...
        it("should work with SSH runtime", () => {
            ...
            const result = validateNoRedundantPrefix("/home/user/mux/project/branch/src/file.ts", sshCwd, sshRuntime);
            expect(result).not.toBeNull();
            expect(result?.correctedPath).toBe("src/file.ts");
            expect(result?.warning).toContain("auto-corrected");
