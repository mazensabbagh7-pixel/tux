import { tool } from "ai";
import { EDIT_FAILED_NOTE_PREFIX, FILE_EDIT_DIFF_OMITTED_MESSAGE, NOTE_READ_FILE_RETRY, } from "@/common/types/tools";
...
export const createFileEditInsertTool = (config) => {
    return tool({
        description: TOOL_DEFINITIONS.file_edit_insert.description,
        inputSchema: TOOL_DEFINITIONS.file_edit_insert.schema,
        execute: async ({ path, content, insert_before, insert_after }, { abortSignal }) => {
            try {
                const { correctedPath, warning: pathWarning } = validateAndCorrectPath(path, config.cwd, config.runtime);
                path = correctedPath;
