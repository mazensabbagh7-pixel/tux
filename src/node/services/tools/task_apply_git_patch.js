...
import { tool } from "ai";
import { TaskApplyGitPatchToolResultSchema, TOOL_DEFINITIONS, } from "@/common/utils/tools/toolDefinitions";
import { shellQuote } from "@/common/utils/shell";
import { execBuffered } from "@/node/utils/runtime/helpers";
import { getSubagentGitPatchMboxPath, markSubagentGitPatchArtifactApplied, readSubagentGitPatchArtifact, } from "@/node/services/subagentGitPatchArtifacts";
...
export const createTaskApplyGitPatchTool = (config) => {
    ...
    function inferMuxRootFromWorkspaceSessionDir(workspaceSessionDir) {
        assert(workspaceSessionDir.length > 0, "inferMuxRootFromWorkspaceSessionDir: workspaceSessionDir must be non-empty");
    ...
    async function tryGetConflictPaths(params) {
        ...
        try {
            const diffResult = await execBuffered(config.runtime, "git diff --name-only --diff-filter=U", {
                cwd: params.cwd,
                timeout: 30,
            });
    ...
    async function findGitPatchArtifactInWorkspaceOrAncestors(params) {
        assert(params.workspaceId.length > 0, "findGitPatchArtifactInWorkspaceOrAncestors: workspaceId must be non-empty");
        assert(params.workspaceSessionDir.length > 0, "findGitPatchArtifactInWorkspaceOrAncestors: workspaceSessionDir must be non-empty");
        assert(params.childTaskId.length > 0, "findGitPatchArtifactInWorkspaceOrAncestors: childTaskId must be non-empty");
        ...
        if (!muxRootDir) {
            log.debug("task_apply_git_patch: workspaceSessionDir not under sessions/; skipping ancestor lookup", {
    ...
    return tool({
        ...
        execute: async (args, { abortSignal }) => {
            ...
            if (dryRun) {
                ...
                const addResult = await execBuffered(config.runtime, `git worktree add --detach ${shellQuote(dryRunWorktreePath)} HEAD`, { cwd: config.cwd, timeout: 60 });
                ...
                try {
                    ...
                    try {
                        const removeResult = await execBuffered(config.runtime, `git worktree remove --force ${shellQuote(dryRunWorktreePath)}`, { cwd: config.cwd, timeout: 60 });
