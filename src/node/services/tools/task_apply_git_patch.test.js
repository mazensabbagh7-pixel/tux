...
import { getSubagentGitPatchMboxPath, readSubagentGitPatchArtifact, upsertSubagentGitPatchArtifact, } from "@/node/services/subagentGitPatchArtifacts";
...
describe("task_apply_git_patch tool", () => {
    ...
    it("applies a ready patch artifact via git am and marks it applied", async () => {
        ...
        expect(execSync("git log -1 --pretty=%s", { cwd: targetRepo, encoding: "utf-8" }).trim()).toBe("child change");
    ...
    it("supports dry_run without changing the repo or marking applied", async () => {
        ...
        expect(execSync("git log -1 --pretty=%s", { cwd: targetRepo, encoding: "utf-8" }).trim()).toBe("base");
        const artifact = await readSubagentGitPatchArtifact(sessionDir, childTaskId);
    ...
    it("returns structured conflict diagnostics on dry_run failure", async () => {
        ...
        expect(execSync("git log -1 --pretty=%s", { cwd: targetRepo, encoding: "utf-8" }).trim()).toBe("target change");
        const artifact = await readSubagentGitPatchArtifact(sessionDir, childTaskId);
        expect(artifact?.appliedAtMs).toBeUndefined();
    }, 20000);
