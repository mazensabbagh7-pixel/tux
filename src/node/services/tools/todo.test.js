...
describe("Todo Storage", () => {
    ...
    describe("setTodosForSessionDir", () => {
        ...
        it("should reject when exceeding MAX_TODOS limit", async () => {
            ...
            await expect(setTodosForSessionDir(workspaceId, workspaceSessionDir, tooManyTodos)).rejects.toThrow(/Too many TODOs \(8\/7\)/i);
            await expect(setTodosForSessionDir(workspaceId, workspaceSessionDir, tooManyTodos)).rejects.toThrow(/Keep high precision at the center/i);
        });
        ...
        it("should reject multiple in_progress tasks", async () => {
            ...
            await expect(setTodosForSessionDir(workspaceId, workspaceSessionDir, invalidTodos)).rejects.toThrow(/only one task can be marked as in_progress/i);
        ...
        it("should reject when in_progress tasks appear after pending", async () => {
            ...
            await expect(setTodosForSessionDir(workspaceId, workspaceSessionDir, invalidTodos)).rejects.toThrow(/in-progress tasks must appear before pending tasks/i);
        });
        it("should reject when completed tasks appear after in_progress", async () => {
            ...
            await expect(setTodosForSessionDir(workspaceId, workspaceSessionDir, invalidTodos)).rejects.toThrow(/completed tasks must appear before in-progress or pending tasks/i);
        });
