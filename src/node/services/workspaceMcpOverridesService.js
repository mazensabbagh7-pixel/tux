...
export class WorkspaceMcpOverridesService {
    ...
    async getRuntimeAndWorkspacePath(workspaceId) {
        ...
        assert(typeof workspacePath === "string" && workspacePath.length > 0, "workspacePath is required");
    ...
    async removeOverridesFile(runtime, workspacePath) {
        ...
        await execBuffered(runtime, `rm -f "${MCP_OVERRIDES_DIR}/${MCP_OVERRIDES_JSONC}" "${MCP_OVERRIDES_DIR}/${MCP_OVERRIDES_JSON}"`, {
            cwd: workspacePath,
            timeout: 10,
        });
    }
