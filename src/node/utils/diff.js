 ...
 */
function buildHunks(oldLines, newLines, lcs, contextSize) {
    const hunks = [];
    ...
    while (oldIdx < oldLines.length || newIdx < newLines.length) {
        ...
        if (currentHunk && match?.[0] === oldIdx && match[1] === newIdx) {
            ...
            if (nextNonMatch === null || nextNonMatch > contextSize * 2) {
                ...
                let contextAdded = 0;
                while (contextAdded < contextSize &&
 ...
 */
function findNextNonMatch(oldIdx, newIdx, lcs, lcsIdx, oldLines, newLines) {
    let distance = 0;
    let oi = oldIdx;
    let ni = newIdx;
    let li = lcsIdx;
