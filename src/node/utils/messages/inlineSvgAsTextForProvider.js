...
function decodeSvgDataUrlToUtf8(svgDataUrl, maxBytes, maxChars) {
    ...
    if (isBase64) {
        ...
        if (estimatedBytes > maxBytes) {
            throw new Error(`SVG attachment is too large to inline as text (${estimatedBytes} bytes > ${maxBytes} bytes).`);
        }
        const buf = Buffer.from(payload, "base64");
        if (buf.length > maxBytes) {
            throw new Error(`SVG attachment is too large to inline as text (${buf.length} bytes > ${maxBytes} bytes).`);
        }
        const svgText = buf.toString("utf8");
        if (svgText.length > maxChars) {
            throw new Error(`SVG attachment is too long to inline as text (${svgText.length} chars > ${maxChars} chars).`);
        }
        return svgText;
    }
    let decoded;
    ...
    if (decoded.length > maxChars) {
        throw new Error(`SVG attachment is too long to inline as text (${decoded.length} chars > ${maxChars} chars).`);
    }
    const byteLength = Buffer.byteLength(decoded, "utf8");
    if (byteLength > maxBytes) {
        throw new Error(`SVG attachment is too large to inline as text (${byteLength} bytes > ${maxBytes} bytes).`);
    }
    return decoded;
}
/**
 ...
 */
export function inlineSvgAsTextForProvider(messages, options) {
    ...
    const result = messages.map((msg) => {
        ...
        const hasSvg = msg.parts.some((part) => part.type === "file" && normalizeMediaType(part.mediaType) === SVG_MEDIA_TYPE);
        if (!hasSvg) {
            return msg;
        }
        didChange = true;
        ...
        for (const part of msg.parts) {
            if (part.type === "file" && normalizeMediaType(part.mediaType) === SVG_MEDIA_TYPE) {
                try {
                    ...
                    const textPart = {
                        type: "text",
                        text: `[SVG attachment converted to text (providers generally don't accept ${SVG_MEDIA_TYPE} as an image input).]\n\n` +
                            `\`\`\`svg\n${svgText}\n\`\`\``,
                    };
                    newParts.push(textPart);
                }
                catch (error) {
                    const errorMessage = error instanceof Error ? error.message : "Failed to decode SVG attachment.";
                    const textPart = {
                        type: "text",
...
//# sourceMappingURL=inlineSvgAsTextForProvider.js.map