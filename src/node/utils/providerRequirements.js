 ...
 */
export function resolveProviderCredentials(provider, config, env = process.env) {
 ...
 */
export function checkProviderConfigured(provider, config, env = process.env) {
 ...
 */
export function hasAnyConfiguredProvider(providers) {
    if (!providers)
        return false;
    for (const [providerKey, rawConfig] of Object.entries(providers)) {
        ...
        if (providerKey === "openai" &&
            parseCodexOauthAuth(rawConfig.codexOauth) !== null) {
            return true;
        }
