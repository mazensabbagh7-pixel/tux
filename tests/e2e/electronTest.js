...
import { prepareDemoProject } from "./utils/demoProject";
import { createWorkspaceUI } from "./utils/ui";
...
function assertDistBundleReady() {
    if (!shouldLoadDist) {
        return;
    }
    for (const filePath of REQUIRED_DIST_FILES) {
        if (!fs.existsSync(filePath)) {
            throw new Error(`Missing build artifact at ${filePath}. Run "make build" before executing dist-mode e2e tests.`);
        }
    }
}
...
export function parseEnvVarFromPsCommand(command, name) {
    if (!name) {
    ...
    const needle = `${name}=`;
    for (let index = command.indexOf(needle); index !== -1; index = command.indexOf(needle, index + needle.length)) {
...
function readEnvVarFromProcess(pid, name) {
...
export const electronTest = base.extend({
    ...
    app: async ({ workspace }, use, testInfo) => {
        ...
        if (shouldStartDevServer) {
            ...
            devServerExitPromise = new Promise((resolve) => {
                const handleExit = () => {
        ...
        try {
            ...
            if (electronMuxRoot !== configRoot) {
                throw new Error(`E2E harness bug: Electron process MUX_ROOT mismatch. Expected ${configRoot}, got ${electronMuxRoot ?? "<unset>"}`);
            }
            try {
                ...
                if (recordVideoDir) {
                    try {
                        ...
                        if (electronApp && videoFiles.length) {
                            ...
                            const baseName = sanitizeForPath(testInfo.testId || testInfo.title || "mux-e2e-video");
...
//# sourceMappingURL=electronTest.js.map