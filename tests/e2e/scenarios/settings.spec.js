...
test.describe("Settings", () => {
    test("opens settings via gear button", async ({ ui, page }) => {
        ...
        await expect(page.getByTestId("settings-button")).toHaveAttribute("aria-label", "Close settings");
    ...
    test("shows all provider names correctly", async ({ ui, page }) => {
        ...
        await expect(page.getByRole("button", { name: /OpenAI/i }).filter({ has: page.getByText("OpenAI icon") })).toBeVisible();
