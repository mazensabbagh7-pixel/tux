...
function assertHistoryLines(lines) {
    if (lines === undefined) {
        return;
    }
...
export function prepareDemoProject(rootDir, options = {}) {
    ...
    fs.writeFileSync(path.join(workspaceSessionDir, "metadata.json"), JSON.stringify(metadata, null, 2));
