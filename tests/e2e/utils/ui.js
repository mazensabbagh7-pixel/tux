...
export function createWorkspaceUI(page, context) {
    const projects = {
    ...
    const chat = {
         ...
         */
        async setThinkingLevel(targetLevel) {
        ...
        async expectTranscriptContains(text) {
            await expect(transcriptLocator(page)).toContainText(text, { timeout: 45000 });
        },
        async expectActionButtonVisible(label) {
        ...
        async expectStatusMessageContains(text) {
            const toastSelector = `[role="status"]:has-text("${text}")`;
            await page.waitForSelector(toastSelector, { state: "visible", timeout: 30000 });
        },
        /**
         ...
         */
        async sendCommandAndExpectStatus(command, expectedStatus) {
        ...
        async captureStreamTimeline(action, options) {
            const timeoutMs = options?.timeoutMs ?? 30000;
            const workspaceId = context.workspaceId;
            await page.evaluate((id) => {
                const win = window;
                const store = win.__muxStreamCapture ??
                    (win.__muxStreamCapture = Object.create(null));
                ...
                void (async () => {
                    try {
                        ...
                        const iterator = await window.__ORPC_CLIENT__.workspace.onChat({ workspaceId: id }, { signal });
            ...
            try {
                ...
                await page.waitForFunction((id) => {
                    ...
                    return capture.events.some((event) => event.type === "stream-end" || event.type === "stream-error");
                }, workspaceId, { timeout: timeoutMs });
            }
            catch (error) {
                actionError = error;
            }
    ...
    const metaSidebar = {
        ...
        async expectTerminalError(expectedText) {
            const errorElement = page.locator(".terminal-view").getByText(/Terminal Error:/);
         ...
         */
        async expectTerminalOutput(_expectedText, _timeoutMs = 10000) {
    ...
    const settings = {
        ...
        async expandProvider(providerName) {
     ...
     */
    async function dragElement(source, target, options) {
        ...
        await page.evaluate(async ([src, tgt, pos]) => {
        ...
        }, [sourceHandle, targetHandle, targetPosition]);
