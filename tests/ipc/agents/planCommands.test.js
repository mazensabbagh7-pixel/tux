...
describeIntegration("Plan Commands Integration", () => {
    ...
    describe("replaceChatHistory", () => {
        it("should delete plan file when deletePlanFile is true", async () => {
            ...
            try {
                ...
                const summaryMessage = createMuxMessage(`start-here-test-${Date.now()}`, "assistant", "summary", { timestamp: Date.now(), compacted: true });
        ...
        it("should preserve agentId metadata in summary message", async () => {
            ...
            const workspaceId = createResult.metadata.id;
            try {
                const summaryMessage = createMuxMessage(`start-here-test-${Date.now()}`, "assistant", "summary", { timestamp: Date.now(), compacted: true, agentId: "plan" });
                const replaceResult = await env.orpc.workspace.replaceChatHistory({
                    workspaceId,
                    summaryMessage,
                });
                ...
                const historyEntry = JSON.parse(firstLine);
    ...
    it("should preserve existing history and append a durable boundary in append-compaction-boundary mode", async () => {
        ...
        const workspaceId = createResult.metadata.id;
        try {
            const legacySummary = createMuxMessage(`legacy-summary-${Date.now()}`, "assistant", "legacy summary", {
                timestamp: Date.now(),
                compacted: true,
            });
            ...
            const malformedBoundaryMessage = createMuxMessage(malformedBoundaryId, "assistant", "malformed boundary", {
                timestamp: Date.now(),
                compacted: false,
            ...
            const chatHistoryPath = path.join(env.config.getSessionDir(workspaceId), "chat.jsonl");
            await fs.appendFile(chatHistoryPath, JSON.stringify({ ...malformedBoundaryMessage, workspaceId }) + "\n");
            const appendSummary = createMuxMessage(`append-summary-${Date.now()}`, "assistant", "append summary", {
                timestamp: Date.now(),
                compacted: "user",
                agentId: "plan",
            });
...
//# sourceMappingURL=planCommands.test.js.map