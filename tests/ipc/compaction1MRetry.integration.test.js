...
function buildFillerText(charCount) {
...
describeIntegration("compaction 1M context retry", () => {
    ...
    test("should auto-retry compaction with 1M context when exceeding 200k default limit", async () => {
        ...
        try {
            ...
            for (let i = 0; i < pairsNeeded; i++) {
                const userMsg = createMuxMessage(`filler-user-${i}`, "user", buildFillerText(charsPerMessage), {});
                const assistantMsg = createMuxMessage(`filler-asst-${i}`, "assistant", buildFillerText(charsPerMessage), {});
            ...
            if (terminalEvent?.type === "stream-error") {
                ...
                throw new Error(`Compaction failed (expected 1M retry to succeed): ` +
...
//# sourceMappingURL=compaction1MRetry.integration.test.js.map