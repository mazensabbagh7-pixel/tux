...
import { shouldRunIntegrationTests, cleanupTestEnvironment, createTestEnvironment, setupWorkspace, validateApiKeys, } from "../setup";
import { createTempGitRepo, cleanupTempGitRepo, resolveOrpcClient, sendMessageWithModel, createStreamCollector, assertStreamSuccess, extractTextFromEvents, HAIKU_MODEL, } from "../helpers";
...
const TEST_SCREENSHOT_MCP_SERVER_PATH = path.join(__dirname, "..", "fixtures", "mcp-screenshot-server.js");
...
async function waitForToolCallEnd(collector, toolName, timeoutMs) {
 ...
 */
function assertValidScreenshotResult(result, allowedMediaTypes) {
...
describeIntegration("MCP server integration with model", () => {
    ...
    test.each(imageFormatCases)("MCP $name image content is correctly transformed to AI SDK format", async ({ name, prompt, mediaTypePattern }) => {
        const { env, workspaceId, tempGitRepo: _tempGitRepo, cleanup, } = await setupWorkspace("anthropic", `mcp-chrome-${name.toLowerCase()}`);
        const client = resolveOrpcClient(env);
        ...
        try {
            ...
            const screenshotResult = await waitForToolCallEnd(collector, "chrome_take_screenshot", 60000);
            // Debug: log tool calls if screenshot not found
            if (!screenshotResult) {
                collector.logEventDiagnostics(`[MCP ${name} Test] Missing chrome_take_screenshot tool-call-end`);
                const toolCallEnds = collector
                    .getEvents()
                    .filter((e) => e.type === "tool-call-end");
    ...
    test.concurrent("MCP tools are available to the model", async () => {
        console.log("[MCP Test] Setting up workspace...");
        // Setup workspace with Anthropic provider
        const { env, workspaceId, tempGitRepo, cleanup } = await setupWorkspace("anthropic", "mcp-memory");
        const client = resolveOrpcClient(env);
        ...
        try {
            ...
            const result = await sendMessageWithModel(env, workspaceId, 'Use the create_entities tool from MCP to create an entity with name "TestEntity" and entityType "test" and observations ["integration test"]. Then confirm you did it.', HAIKU_MODEL);
            ...
            const toolCallStarts = events.filter((e) => e.type === "tool-call-start");
            console.log("[MCP Test] Tool calls:", toolCallStarts.map((e) => e.toolName));
            ...
            const mcpToolCall = toolCallStarts.find((e) => e.toolName === "memory_server_create_entities");
    ...
    }, 90000); // MCP server startup + tool call can take time
});
//# sourceMappingURL=mcpConfig.test.js.map