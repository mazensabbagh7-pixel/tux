...
describeIntegration("model_not_found error handling", () => {
    test.concurrent("should classify Anthropic 404 as model_not_found (not retryable)", async () => {
        ...
        try {
            ...
            void sendMessageWithModel(env, workspaceId, "Hello", modelString("anthropic", "invalid-model-that-does-not-exist-xyz123"));
    ...
    test.concurrent("should classify OpenAI 400 model_not_found as model_not_found (not retryable)", async () => {
        ...
        try {
            ...
            void sendMessageWithModel(env, workspaceId, "Hello", modelString("openai", "gpt-nonexistent-model-xyz123"));
            // Wait for error event
            await collector.waitForEvent("stream-error", 10000);
            const events = collector.getEvents();
            const errorEvent = events.find((e) => "type" in e && e.type === "stream-error");
...
//# sourceMappingURL=modelNotFound.test.js.map