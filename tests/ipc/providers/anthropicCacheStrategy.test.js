...
describeIntegration("Anthropic cache strategy integration", () => {
    test("should apply cache control to messages, system prompt, and tools for Anthropic models", async () => {
        ...
        try {
            ...
            const firstUsage = firstEndEvent?.metadata?.usage;
            const firstProviderMetadata = firstEndEvent?.metadata?.providerMetadata?.anthropic;
            ...
            const hasCacheCreation = firstProviderMetadata?.cacheCreationInputTokens !== undefined &&
            ...
            if (hasCacheCreation) {
                ...
                console.log(`✓ Cache creation working: ${firstProviderMetadata.cacheCreationInputTokens} tokens cached`);
            }
                ...
                throw new Error("Expected cache creation but got 0 tokens. Cache control may not be working.");
            }
            else {
...
//# sourceMappingURL=anthropicCacheStrategy.test.js.map