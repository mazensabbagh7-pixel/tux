...
import { sendMessageWithModel, createStreamCollector, assertStreamSuccess, extractTextFromEvents, modelString, configureTestRetries, } from "../helpers";
...
describeOllama("Ollama integration", () => {
    ...
    test("should successfully send message to Ollama and receive response", async () => {
        ...
        try {
            ...
            const result = await sendMessageWithModel(env, workspaceId, "Say 'hello' and nothing else", modelString("ollama", OLLAMA_MODEL));
    ...
    test("should successfully call tools with Ollama", async () => {
        ...
        try {
            ...
            const result = await sendMessageWithModel(env, workspaceId, 'Use bash to run: date. Set display_name="current-time" and timeout_secs=30. Do not spawn a sub-agent.', modelString("ollama", OLLAMA_MODEL), {
            ...
            const toolCallEnds = events.filter((e) => e.type === "tool-call-end" && e.toolName === "bash");
            const bashOutput = toolCallEnds
                .map((e) => e.result?.output)
    ...
    test("should handle file operations with Ollama", async () => {
        ...
        try {
            ...
            const result = await sendMessageWithModel(env, workspaceId, "Read the README.md file and tell me what the first heading says.", modelString("ollama", OLLAMA_MODEL));
            expect(result.success).toBe(true);
            // Wait for stream to complete
            await collector.waitForEvent("stream-end", 60000);
            assertStreamSuccess(collector);
    ...
    test("should handle errors gracefully when Ollama is not running", async () => {
        ...
        try {
            ...
            const result = await sendMessageWithModel(env, workspaceId, "This should fail", modelString("ollama", OLLAMA_MODEL), {
                providerOptions: {
                    ollama: {},
                },
            });
