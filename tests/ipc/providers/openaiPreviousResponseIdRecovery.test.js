...
import { sendMessageWithModel, createStreamCollector, modelString, configureTestRetries, } from "../helpers";
...
const DISABLE_TOOLS = [{ regex_match: ".*", action: "disable" }];
function createInvalidResponseId() {
    return `resp_${randomBytes(12).toString("hex")}`;
}
describeIntegration("OpenAI previousResponseId recovery", () => {
    configureTestRetries(3);
    test.concurrent("recovers from invalid previousResponseId on the first request", async () => {
        const { env, workspaceId, cleanup } = await setupWorkspace("openai");
        try {
            const invalidResponseId = createInvalidResponseId();
            const summaryMessage = createMuxMessage(`summary-${Date.now()}`, "assistant", "Summary placeholder for previousResponseId recovery.", {
                timestamp: Date.now(),
                model: OPENAI_MODEL,
                mode: "exec",
                providerMetadata: {
            ...
            const result = await sendMessageWithModel(env, workspaceId, "Respond with DONE.", OPENAI_MODEL, {
...
//# sourceMappingURL=openaiPreviousResponseIdRecovery.test.js.map