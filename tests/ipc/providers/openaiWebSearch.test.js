...
import { sendMessageWithModel, createStreamCollector, assertStreamSuccess, modelString, configureTestRetries, } from "../helpers";
...
describeIntegration("OpenAI web_search integration tests", () => {
    ...
    test.concurrent("should handle reasoning + web_search without itemId errors", async () => {
        ...
        try {
            ...
            const result = await sendMessageWithModel(env, workspaceId, "Use web search to find the current weather in San Francisco. " +
                "Then tell me if it's a good day for a picnic.", modelString("openai", "gpt-5.2"), {
            ...
            const hasWebSearchCall = events.some((e) => "type" in e &&
...
//# sourceMappingURL=openaiWebSearch.test.js.map