...
import { createTempGitRepo, cleanupTempGitRepo, generateBranchName, createWorkspaceWithInit, sendMessageAndWait, configureTestRetries, HAIKU_MODEL, } from "../helpers";
...
const TASK_TERMINATE_ONLY = [
    { regex_match: ".*", action: "disable" },
    { regex_match: "task_terminate", action: "require" },
];
const TASK_AWAIT_ONLY = [
    { regex_match: ".*", action: "disable" },
 ...
 */
function extractBashTaskId(events) {
    for (const event of events) {
        if (!("type" in event) || event.type !== "tool-call-end")
            continue;
 ...
 */
function collectTaskAwaitOutputs(events) {
    const outputs = [];
    for (const event of events) {
        if (!("type" in event) || event.type !== "tool-call-end")
            continue;
        if (!("toolName" in event) || event.toolName !== "task_await")
            continue;
 ...
 */
function extractTerminatedTaskIds(events) {
    for (const event of events) {
        if (!("type" in event) || event.type !== "tool-call-end")
            continue;
        if (!("toolName" in event) || event.toolName !== "task_terminate")
            continue;
        const results = event.result?.results;
        if (!Array.isArray(results))
            return [];
        ...
        for (const result of results) {
            if (result.status !== "terminated")
                continue;
            if (!Array.isArray(result.terminatedTaskIds))
                continue;
...
describeIntegration("Background Bash Execution", () => {
    test.concurrent("should start a background process and list it", async () => {
        ...
        try {
            ...
            const { workspaceId, cleanup } = await createWorkspaceWithInit(env, tempGitRepo, branchName, undefined, // local runtime
            true // waitForInit
            );
            try {
                // Start a background bash process via bash(run_in_background=true)
                const startEvents = await sendMessageAndWait(env, workspaceId, 'Use the bash tool with args: { script: "true && sleep 30", timeout_secs: 60, run_in_background: true, display_name: "bg-basic" }. Do not spawn a sub-agent.', HAIKU_MODEL, BASH_ONLY, 30000);
                ...
                const terminateEvents = await sendMessageAndWait(env, workspaceId, `Use task_terminate with task_ids: ["${taskId}"] to terminate the task.`, HAIKU_MODEL, TASK_TERMINATE_ONLY, 20000);
    ...
    test.concurrent("should terminate a background process", async () => {
        const env = await createTestEnvironment();
        const tempGitRepo = await createTempGitRepo();
        try {
            // Setup provider
            ...
            try {
                ...
                const startEvents = await sendMessageAndWait(env, workspaceId, 'Use the bash tool with args: { script: "true && sleep 300", timeout_secs: 600, run_in_background: true, display_name: "bg-terminate" }. Do not spawn a sub-agent.', HAIKU_MODEL, BASH_ONLY, 30000);
                const taskId = extractBashTaskId(startEvents);
                expect(taskId).not.toBeNull();
                // Terminate the task
                const terminateEvents = await sendMessageAndWait(env, workspaceId, `Use task_terminate with task_ids: ["${taskId}"] to terminate the task.`, HAIKU_MODEL, TASK_TERMINATE_ONLY, 20000);
    ...
    test.concurrent("should capture output from background process", async () => {
        const env = await createTestEnvironment();
        const tempGitRepo = await createTempGitRepo();
        try {
            // Setup provider
            ...
            try {
                ...
                const startEvents = await sendMessageAndWait(env, workspaceId, `Use the bash tool with args: { script: "echo \"${marker}\" && sleep 1", timeout_secs: 30, run_in_background: true, display_name: "bg-output" }. Do not spawn a sub-agent.`, HAIKU_MODEL, BASH_ONLY, 30000);
                const taskId = extractBashTaskId(startEvents);
                expect(taskId).not.toBeNull();
                // Wait for the process to complete and retrieve its output
                const awaitEvents = await sendMessageAndWait(env, workspaceId, `Use task_await with task_ids: ["${taskId}"] and timeout_secs: 10 to retrieve output.`, HAIKU_MODEL, TASK_AWAIT_ONLY, 20000);
...
//# sourceMappingURL=backgroundBash.test.js.map