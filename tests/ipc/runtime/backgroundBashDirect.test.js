...
describe("Background Bash Direct Integration", () => {
    ...
    it("should retrieve output after tools are recreated (multi-message flow)", async () => {
        ...
        const outputResult = (await bashOutput2.execute({ process_id: processId, timeout_secs: 0 }, { toolCallId: "read", messages: [] }));
    ...
    it("should support incremental reads", async () => {
        ...
        const spawnResult = await manager.spawn(runtime, workspaceId, `echo "${marker1}"; sleep ${markerDelaySecs}; echo "${marker2}"`, { cwd: workspacePath, displayName: testId });
        expect(spawnResult.success).toBe(true);
        if (!spawnResult.success)
            return;
    ...
    it("should isolate processes by workspace", async () => {
        ...
        try {
            ...
            if (!spawn1.success || !spawn2.success)
                return;
            await new Promise((resolve) => setTimeout(resolve, 200));
            // Cleanup workspace 2
...
describe("Background Bash Output Capture", () => {
    ...
    it("should capture output when script fails mid-execution", async () => {
        ...
        const spawnResult = await manager.spawn(runtime, workspaceId, `echo "${marker1}"; echo "${marker2}" >&2; false; echo "NEVER_SEEN"`, { cwd: workspacePath, displayName: testId });
        expect(spawnResult.success).toBe(true);
        if (!spawnResult.success)
            return;
        await new Promise((resolve) => setTimeout(resolve, 300));
    ...
    it("should handle long-running script that outputs to both streams", async () => {
        ...
        const spawnResult = await manager.spawn(runtime, workspaceId, `for i in 1 2 3; do echo "${outMarker}_$i"; echo "${errMarker}_$i" >&2; done`, { cwd: workspacePath, displayName: testId });
        expect(spawnResult.success).toBe(true);
        if (!spawnResult.success)
            return;
...
describe("Foreground to Background Migration", () => {
    ...
    it("should migrate foreground bash to background and continue running", async () => {
        ...
        const outputResult = (await bashOutput2.execute({ process_id: testId, timeout_secs: 0 }, { toolCallId: "output_read", messages: [] }));
        expect(outputResult.success).toBe(true);
...
//# sourceMappingURL=backgroundBashDirect.test.js.map