...
import { createTempGitRepo, cleanupTempGitRepo, createWorkspace, waitFor, waitForInitComplete, resolveOrpcClient, configureTestRetries, } from "../helpers";
async function executeBashUntilReady(client, workspaceId, script, timeoutMs = 5000) {
    let lastResult = null;
...
function expectWorkspaceCreationSuccess(result) {
...
const TEST_TIMEOUT_MS = process.platform === "win32" ? 60000 : 15000;
...
describeIntegration("executeBash", () => {
    test.concurrent("should execute bash command in workspace context", async () => {
    ...
    test.concurrent("should execute git status in workspace context", async () => {
        const env = await createTestEnvironment();
        const tempGitRepo = await createTempGitRepo();
        try {
            // Create a workspace
    ...
    test.concurrent("should handle command failure with exit code", async () => {
        const env = await createTestEnvironment();
        const tempGitRepo = await createTempGitRepo();
        try {
            // Create a workspace
    ...
    test.concurrent("should respect timeout option", async () => {
        const env = await createTestEnvironment();
        const tempGitRepo = await createTempGitRepo();
        try {
            // Create a workspace
    ...
    test.concurrent("should handle large output without truncation (IPC uses truncate policy with 10K line limit)", async () => {
        const env = await createTestEnvironment();
        const tempGitRepo = await createTempGitRepo();
        try {
            // Create a workspace
    ...
    test.concurrent("should fail gracefully with invalid workspace ID", async () => {
        const env = await createTestEnvironment();
        try {
    ...
    test.concurrent("should inject secrets as environment variables", async () => {
        const env = await createTestEnvironment();
        const tempGitRepo = await createTempGitRepo();
        try {
            // Create a workspace
    ...
    test.concurrent("should set GIT_TERMINAL_PROMPT=0 to prevent credential prompts", async () => {
        const env = await createTestEnvironment();
        const tempGitRepo = await createTempGitRepo();
        try {
            // Create a workspace
            ...
            const gitEnvResult = await executeBashUntilReady(client, workspaceId, 'echo "GIT_TERMINAL_PROMPT=$GIT_TERMINAL_PROMPT"', 10000);
            expect(gitEnvResult.success).toBe(true);
            if (!gitEnvResult.success)
                return;
            ...
            const invalidFetchResult = await client.workspace.executeBash({
                workspaceId,
                script: "git fetch https://invalid-remote-that-does-not-exist-12345.com/repo.git 2>&1 || true",
                options: { timeout_secs: GIT_FETCH_TIMEOUT_SECS },
            });
            expect(invalidFetchResult.success).toBe(true);
            if (!invalidFetchResult.success)
                return;
...
//# sourceMappingURL=executeBash.test.js.map