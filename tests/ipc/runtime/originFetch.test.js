 ...
 */
async function createRepoWithAheadOrigin() {
    // Create local repo
    const repoPath = await fs.mkdtemp(path.join(os.tmpdir(), "mux-test-repo-"));
    await execAsync(`git init`, { cwd: repoPath });
    await execAsync(`git config user.email "test@example.com" && git config user.name "Test User" && git config commit.gpgsign false`, { cwd: repoPath });
    ...
    await execAsync(`git config user.email "test@example.com" && git config user.name "Test User" && git config commit.gpgsign false`, { cwd: tempClone });
    await execAsync(`echo "${ORIGIN_COMMIT_MARKER}" > origin-only.txt && git add . && git commit -m "Origin-only commit"`, { cwd: tempClone });
 ...
 */
async function cleanupTestRepos(repoPath, originPath) {
    await Promise.all([
...
describeIntegration("Origin fetch ordering during workspace creation", () => {
    ...
    describe("Runtime: worktree", () => {
        test.concurrent("new branch starts from origin/trunk, not stale local trunk", async () => {
            const env = await createTestEnvironment();
            const { repoPath, originPath, originCommitHash, trunkBranch } = await createRepoWithAheadOrigin();
            try {
        ...
        test.concurrent("existing branch fast-forwards to origin after checkout", async () => {
            const env = await createTestEnvironment();
            const { repoPath, originPath, originCommitHash, trunkBranch } = await createRepoWithAheadOrigin();
            try {
        ...
        test.concurrent("gracefully falls back to local trunk when origin is unreachable", async () => {
            const env = await createTestEnvironment();
            // Create a repo with a fake origin URL that doesn't exist
            const repoPath = await fs.mkdtemp(path.join(os.tmpdir(), "mux-test-repo-"));
            await execAsync(`git init`, { cwd: repoPath });
            await execAsync(`git config user.email "test@example.com" && git config user.name "Test User" && git config commit.gpgsign false`, { cwd: repoPath });
            await execAsync(`echo "initial" > README.md && git add . && git commit -m "Initial commit"`, { cwd: repoPath });
        ...
        test.concurrent("preserves local trunk when local is ahead of origin (unpushed work)", async () => {
            const env = await createTestEnvironment();
            ...
            await execAsync(`echo "${localOnlyMarker}" > local-only.txt && git add . && git commit -m "Local unpushed commit"`, { cwd: repoPath });
    ...
    describe("Runtime: ssh", () => {
        ...
        test.concurrent("creates workspace from bundled local state when origin is inaccessible from remote", async () => {
            const env = await createTestEnvironment();
        ...
        test.concurrent("gracefully handles unreachable origin URL", async () => {
            const env = await createTestEnvironment();
...
//# sourceMappingURL=originFetch.test.js.map