...
import { createTempGitRepo, cleanupTempGitRepo, generateBranchName, createWorkspaceWithInit, sendMessageAndWait, extractTextFromEvents, HAIKU_MODEL, TEST_TIMEOUT_LOCAL_MS, TEST_TIMEOUT_SSH_MS, getTestRunner, } from "../helpers";
...
const BASH_ONLY = [{ regex_match: "bash", action: "require" }];
/**
 * Collect tool outputs from stream events
 */
function collectToolOutputs(events, toolName) {
    return events
 ...
 */
function getToolDuration(events, toolName) {
    const startEvent = events.find((e) => "type" in e && e.type === "tool-call-start" && "toolName" in e && e.toolName === toolName);
    if (!startEvent?.toolCallId || !startEvent.timestamp) {
        return -1;
    }
    const endEvent = events.find((e) => "type" in e &&
...
describeIntegration("Runtime Bash Execution", () => {
    ...
    describe.each([{ type: "local" }, { type: "ssh" }])("Runtime: $type", ({ type }) => {
        ...
        runTest("should execute simple bash command", async () => {
            ...
            try {
                ...
                try {
                    ...
                    const events = await sendMessageAndWait(env, workspaceId, 'Use the bash tool with args: { script: "echo Hello World", timeout_secs: 30, run_in_background: false, display_name: "echo-hello" }. Do not spawn a sub-agent.', HAIKU_MODEL, BASH_ONLY);
                    ...
                    const toolCallStarts = events.filter((e) => "type" in e && e.type === "tool-call-start");
                    const bashCall = toolCallStarts.find((e) => {
                        if (!("toolName" in e) || e.toolName !== "bash")
                            return false;
                        const args = e.args;
        ...
        runTest("should handle bash command with environment variables", async () => {
            const env = await createTestEnvironment();
            const tempGitRepo = await createTempGitRepo();
            try {
                // Setup provider
                ...
                try {
                    ...
                    const events = await sendMessageAndWait(env, workspaceId, 'Use the bash tool with args: { script: "export TEST_VAR=test123 && echo Value:$TEST_VAR", timeout_secs: 30, run_in_background: false, display_name: "env-var" }. Do not spawn a sub-agent.', HAIKU_MODEL, BASH_ONLY);
                    // Extract response text
                    const responseText = extractTextFromEvents(events);
        ...
        runStdinTest("should not hang on commands that read stdin without input", async () => {
            const env = await createTestEnvironment();
            const tempGitRepo = await createTempGitRepo();
            try {
                // Setup provider
                ...
                try {
                    ...
                    const events = await sendMessageAndWait(env, workspaceId, 'Use the bash tool with args: { script: "echo testdata > /tmp/test.txt && cat /tmp/test.txt | grep test", timeout_secs: 30, run_in_background: false, display_name: "stdin-grep" }. Do not spawn a sub-agent.', HAIKU_MODEL, BASH_ONLY, 30000 // Relaxed timeout for CI stability (was 10s)
                    );
                    ...
                    // Verify bash was called
                    const toolCallStarts = events.filter((e) => "type" in e && e.type === "tool-call-start");
                    const bashCalls = toolCallStarts.filter((e) => "toolName" in e && e.toolName === "bash");
        ...
        runTest("should not hang on grep | head pattern over SSH", async () => {
            const env = await createTestEnvironment();
            const tempGitRepo = await createTempGitRepo();
            try {
                // Setup provider
...
//# sourceMappingURL=runtimeExecuteBash.test.js.map