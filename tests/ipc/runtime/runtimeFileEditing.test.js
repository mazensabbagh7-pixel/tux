 ...
 */
import { createTestEnvironment, cleanupTestEnvironment, shouldRunIntegrationTests, validateApiKeys, getApiKey, setupProviders, } from "../setup";
import { createTempGitRepo, cleanupTempGitRepo, generateBranchName, createWorkspaceWithInit, sendMessageAndWait, extractTextFromEvents, configureTestRetries, HAIKU_MODEL, TEST_TIMEOUT_LOCAL_MS, TEST_TIMEOUT_SSH_MS, STREAM_TIMEOUT_LOCAL_MS, STREAM_TIMEOUT_SSH_MS, getTestRunner, } from "../helpers";
...
describeIntegration("Runtime File Editing Tools", () => {
    ...
    describe.each([{ type: "local" }, { type: "ssh" }])("Runtime: $type", ({ type }) => {
        ...
        const getRuntimeConfig = (branchName) => {
        ...
        runTest("should read file content with file_read tool", async () => {
            ...
            try {
                ...
                const { workspaceId, cleanup } = await createWorkspaceWithInit(env, tempGitRepo, branchName, runtimeConfig, true, // waitForInit
                type === "ssh");
                try {
                    ...
                    const createEvents = await sendMessageAndWait(env, workspaceId, `Create a file called ${testFileName} with the content: "Hello from mux file tools!"`, HAIKU_MODEL, FILE_TOOLS_ONLY, streamTimeout);
                    // Verify file was created successfully
                    const createStreamEnd = createEvents.find((e) => "type" in e && e.type === "stream-end");
                    expect(createStreamEnd).toBeDefined();
                    expect(createStreamEnd.error).toBeUndefined();
                    // Now ask AI to read the file (explicitly request file_read tool)
                    const readEvents = await sendMessageAndWait(env, workspaceId, `Use the file_read tool to read ${testFileName} and tell me what it contains.`, HAIKU_MODEL, FILE_TOOLS_ONLY, streamTimeout);
                    // Verify stream completed successfully
                    const streamEnd = readEvents.find((e) => "type" in e && e.type === "stream-end");
                    expect(streamEnd).toBeDefined();
                    expect(streamEnd.error).toBeUndefined();
                    // Verify file_read tool was called
                    const toolCalls = readEvents.filter((e) => "type" in e && e.type === "tool-call-start");
        ...
        runTest("should replace text with file_edit_replace_string tool", async () => {
            const env = await createTestEnvironment();
            const tempGitRepo = await createTempGitRepo();
            try {
                // Setup provider
                ...
                try {
                    ...
                    const streamTimeout = type === "ssh" ? STREAM_TIMEOUT_SSH_MS : STREAM_TIMEOUT_LOCAL_MS;
                    const createEvents = await sendMessageAndWait(env, workspaceId, `Create a file called ${testFileName} with the content: "The quick brown fox jumps over the lazy dog."`, HAIKU_MODEL, FILE_TOOLS_ONLY, streamTimeout);
                    // Verify file was created successfully
                    const createStreamEnd = createEvents.find((e) => "type" in e && e.type === "stream-end");
                    expect(createStreamEnd).toBeDefined();
                    expect(createStreamEnd.error).toBeUndefined();
                    // Ask AI to replace text (explicitly request file_edit_replace_string tool)
                    const replaceEvents = await sendMessageAndWait(env, workspaceId, `Use the file_edit_replace_string tool to replace "brown fox" with "red panda" in ${testFileName}.`, HAIKU_MODEL, FILE_TOOLS_ONLY, streamTimeout);
                    // Verify stream completed successfully
                    const streamEnd = replaceEvents.find((e) => "type" in e && e.type === "stream-end");
                    expect(streamEnd).toBeDefined();
                    expect(streamEnd.error).toBeUndefined();
                    // Verify file_edit_replace_string tool was called
                    const toolCalls = replaceEvents.filter((e) => "type" in e && e.type === "tool-call-start");
                    ...
                    expect(responseText.toLowerCase().includes("replace") ||
                        responseText.toLowerCase().includes("changed") ||
                        responseText.toLowerCase().includes("updated")).toBe(true);
                }
                finally {
                    await cleanup();
                }
        ...
        runTest("should insert text with file_edit_insert tool", async () => {
            const env = await createTestEnvironment();
            const tempGitRepo = await createTempGitRepo();
            try {
                // Setup provider
                ...
                try {
                    ...
                    const streamTimeout = type === "ssh" ? STREAM_TIMEOUT_SSH_MS : STREAM_TIMEOUT_LOCAL_MS;
                    const createEvents = await sendMessageAndWait(env, workspaceId, `Create a file called ${testFileName} with two lines: "Line 1" and "Line 3".`, HAIKU_MODEL, FILE_TOOLS_ONLY, streamTimeout);
                    // Verify file was created successfully
                    const createStreamEnd = createEvents.find((e) => "type" in e && e.type === "stream-end");
                    expect(createStreamEnd).toBeDefined();
                    expect(createStreamEnd.error).toBeUndefined();
                    // Ask AI to insert text (explicitly request file_edit tool usage)
                    const insertEvents = await sendMessageAndWait(env, workspaceId, `Use the file_edit_insert (preferred) or file_edit_replace_string tool to insert "Line 2" between Line 1 and Line 3 in ${testFileName}.`, HAIKU_MODEL, FILE_TOOLS_ONLY, streamTimeout);
                    // Verify stream completed successfully
                    ...
                    const toolCalls = insertEvents.filter((e) => "type" in e && e.type === "tool-call-start");
                    const editCall = toolCalls.find((e) => e.toolName === "file_edit_insert" || e.toolName === "file_edit_replace_string");
                    expect(editCall).toBeDefined();
                    // Verify the insertion was successful
                    const responseText = extractTextFromEvents(insertEvents);
                    expect(responseText.toLowerCase().includes("insert") ||
        ...
        runTest("should handle relative paths correctly when editing files", async () => {
            const env = await createTestEnvironment();
            const tempGitRepo = await createTempGitRepo();
            try {
                // Setup provider
                ...
                const runtimeConfig = getRuntimeConfig(branchName);
                const { workspaceId, cleanup } = await createWorkspaceWithInit(env, tempGitRepo, branchName, runtimeConfig, true, type === "ssh");
                try {
                    const streamTimeout = type === "ssh" ? STREAM_TIMEOUT_SSH_MS : STREAM_TIMEOUT_LOCAL_MS;
                    // Create a file using AI with a relative path
                    const relativeTestFile = "subdir/relative_test.txt";
                    const createEvents = await sendMessageAndWait(env, workspaceId, `Create a file at path "${relativeTestFile}" with content: "Original content"`, HAIKU_MODEL, FILE_TOOLS_ONLY, streamTimeout);
                    // Verify file was created successfully
                    const createStreamEnd = createEvents.find((e) => "type" in e && e.type === "stream-end");
                    expect(createStreamEnd).toBeDefined();
                    expect(createStreamEnd.error).toBeUndefined();
                    // Now edit the file using a relative path
                    const editEvents = await sendMessageAndWait(env, workspaceId, `Replace the text in ${relativeTestFile}: change "Original" to "Modified"`, HAIKU_MODEL, FILE_TOOLS_ONLY, streamTimeout);
                    // Verify edit was successful
                    const editStreamEnd = editEvents.find((e) => "type" in e && e.type === "stream-end");
                    expect(editStreamEnd).toBeDefined();
                    expect(editStreamEnd.error).toBeUndefined();
                    // Verify file_edit_replace_string tool was called
                    const toolCalls = editEvents.filter((e) => "type" in e && e.type === "tool-call-start");
                    const editCall = toolCalls.find((e) => e.toolName === "file_edit_replace_string");
                    expect(editCall).toBeDefined();
                    // Read the file to verify the edit was applied
                    const readEvents = await sendMessageAndWait(env, workspaceId, `Read the file ${relativeTestFile} and tell me its content`, HAIKU_MODEL, FILE_TOOLS_ONLY, streamTimeout);
                    const responseText = extractTextFromEvents(readEvents);
...
//# sourceMappingURL=runtimeFileEditing.test.js.map