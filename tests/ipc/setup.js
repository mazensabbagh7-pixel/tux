...
import { generateBranchName, createWorkspace, createWorkspaceWithInit, resolveOrpcClient, createTempGitRepo, cleanupTempGitRepo, } from "./helpers";
 ...
 */
export async function cleanupTestEnvironment(env) {
 ...
 */
export async function setupProviders(source, providers) {
    ...
    for (const [providerName, providerConfig] of Object.entries(providers)) {
        for (const [key, value] of Object.entries(providerConfig)) {
            ...
            if (!result.success) {
                throw new Error(`Failed to set provider config for ${providerName}.${key}: ${result.error}`);
            }
        }
    }
}
 ...
 */
export async function preloadTestModules() {
 ...
 */
export async function setupWorkspace(provider, branchPrefix, options) {
    ...
    try {
        if (waitForInit) {
            const initResult = await createWorkspaceWithInit(env, tempGitRepo, branchName, runtimeConfig, true, isSSH);
            workspaceId = initResult.workspaceId;
            workspacePath = initResult.workspacePath;
        }
        else {
            const createResult = await createWorkspace(env, tempGitRepo, branchName, undefined, runtimeConfig);
 ...
 */
export async function setupWorkspaceWithoutProvider(branchPrefix) {
