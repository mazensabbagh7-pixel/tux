...
export class StreamCollector {
     ...
     */
    async waitForSubscription(timeoutMs = 5000) {
        if (!this.started) {
    ...
    async collectLoop() {
        try {
            ...
            const iterator = await this.client.workspace.onChat({ workspaceId: this.workspaceId }, { signal: this.abortController.signal });
    ...
    checkWaiters(message) {
        const msgType = "type" in message ? message.type : null;
        if (!msgType)
            return;
        const satisfiedIndices = [];
    ...
    async waitForEvent(eventType, timeoutMs = 30000) {
        ...
        const existing = this.events.find((e) => "type" in e && e.type === eventType);
        if (existing) {
            return existing;
        }
        // Wait for the event
     ...
     */
    async waitForEventN(eventType, n, timeoutMs = 30000) {
        if (!this.started) {
            throw new Error("StreamCollector not started. Call start() first.");
        }
        if (n < 1) {
        ...
        const countExisting = () => this.events.filter((e) => "type" in e && e.type === eventType).length;
        // If we already have enough events, return the Nth one
        const existing = countExisting();
        if (existing >= n) {
            const matches = this.events.filter((e) => "type" in e && e.type === eventType);
            return matches[n - 1];
        }
        ...
        return new Promise((resolve) => {
            ...
            const check = () => {
                ...
                if (Date.now() - startTime >= timeoutMs) {
                    this.logEventDiagnostics(`waitForEventN timeout: Expected ${n}x "${eventType}", got ${matches.length}`);
                    resolve(null);
                    return;
                }
                setTimeout(check, 50);
    ...
    logEventDiagnostics(context) {
        ...
        this.events.forEach((event, idx) => {
            ...
            const type = "type" in event ? event.type : "no-type";
        ...
        const eventTypeCounts = this.events.reduce((acc, e) => {
            const type = "type" in e ? e.type : "unknown";
 ...
 */
export function createStreamCollector(client, workspaceId) {
 ...
 */
export function assertStreamSuccess(collector) {
    ...
    if (!collector.hasStreamEnd()) {
        ...
        if (errorEvent && "error" in errorEvent) {
            ...
            throw new Error(`Stream did not complete successfully. Got stream-error: ${errorEvent.error}\n` +
    ...
    if (collector.hasError()) {
        ...
        throw new Error(`Stream completed but also has error event: ${errorMsg}\n` +
            `See detailed event diagnostics above.`);
    }
    ...
    if (!finalMessage) {
        ...
        throw new Error(`Stream completed but final message is missing.\n` + `See detailed event diagnostics above.`);
    }
}
/**
 ...
 * });
 */
export async function withStreamCollection(client, workspaceId, fn) {
 ...
 */
export async function waitForStreamSuccess(client, workspaceId, timeoutMs = 30000) {
    const collector = createStreamCollector(client, workspaceId);
    collector.start();
...
//# sourceMappingURL=streamCollector.js.map