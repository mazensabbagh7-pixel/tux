...
describeIntegration("empty assistant message self-healing", () => {
    test.concurrent("should handle corrupted history with empty assistant parts array", async () => {
        ...
        try {
            ...
            collector.start();
            const sendResult = await sendMessageWithModel(env, workspaceId, "This should work despite corrupted history", HAIKU_MODEL);
    ...
    test.concurrent("should handle corrupted history with incomplete tool-only assistant message", async () => {
        const { env, workspaceId, cleanup } = await setupWorkspace("anthropic");
        try {
            const historyService = new HistoryService(env.config);
                    ...
                    parts: [
                        {
                            ...
                            toolName: "bash",
                            toolCallId: "call-123",
                            state: "input-available", // Incomplete - will be dropped by SDK
            ...
            const collector = createStreamCollector(env.orpc, workspaceId);
            collector.start();
            const sendResult = await sendMessageWithModel(env, workspaceId, "This should work despite corrupted tool history", HAIKU_MODEL);
            expect(sendResult.success).toBe(true);
            const streamEnd = await collector.waitForEvent("stream-end", 30000);
            expect(streamEnd).toBeDefined();
            collector.stop();
...
//# sourceMappingURL=emptyAssistantMessage.test.js.map