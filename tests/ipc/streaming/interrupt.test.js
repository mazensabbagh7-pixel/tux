 ...
 */
import { shouldRunIntegrationTests, validateApiKeys, createTestEnvironment, cleanupTestEnvironment, setupProviders, getApiKey, } from "../setup";
import { createTempGitRepo, cleanupTempGitRepo, createWorkspace, generateBranchName, sendMessageWithModel, waitFor, HAIKU_MODEL, } from "../helpers";
...
async function addInitHook(repoPath, scriptBody) {
...
describeIntegration("interruptStream during startup", () => {
    test("should emit stream-abort without stream-start when interrupted before stream-start", async () => {
        ...
        await addInitHook(repoPath, ['echo "Starting init hook..."', "sleep 6", 'echo "Init hook done"', "exit 0"].join("\n"));
        ...
        try {
            ...
            const sendPromise = sendMessageWithModel(env, workspaceId, "Say 'hello' and nothing else", HAIKU_MODEL);
