...
import { createTempGitRepo, cleanupTempGitRepo, createWorkspace, generateBranchName, sendMessageWithModel, waitFor, HAIKU_MODEL, createStreamCollector, } from "../helpers";
...
describe("Queued messages during stream completion", () => {
    ...
    test("isBusy returns true during COMPLETING phase", async () => {
        ...
        const compactionHandler = session.compactionHandler;
        const enteredCompletion = createDeferred();
        ...
        try {
            ...
            const firstSendResult = await sendMessageWithModel(env, workspaceId, "First message", HAIKU_MODEL);
    ...
    test("queues message sent during COMPLETING and auto-sends after completion finishes", async () => {
        ...
        try {
            ...
            const secondSendResult = await sendMessageWithModel(env, workspaceId, "Second message", HAIKU_MODEL);
    ...
    test("edit waits for completion cleanup before truncating history", async () => {
        ...
        const historyService = env.services.workspaceService
            .historyService;
        const truncateSpy = jest.spyOn(historyService, "truncateAfterMessage");
        try {
            await collector.waitForSubscription(5000);
            const firstMessageText = "First message";
            const firstSendResult = await sendMessageWithModel(env, workspaceId, firstMessageText, HAIKU_MODEL);
            expect(firstSendResult.success).toBe(true);
            ...
            // Hold the session in COMPLETING.
            await enteredCompletion.promise;
            const editSendPromise = sendMessageWithModel(env, workspaceId, "Edited message", HAIKU_MODEL, {
    ...
    test("should let edit truncate before queued message runs", async () => {
        ...
        try {
            ...
            const queuedSendResult = await sendMessageWithModel(env, workspaceId, queuedText, HAIKU_MODEL);
...
//# sourceMappingURL=queuedMessages.completing.test.js.map