...
describe("Queued messages during stream start", () => {
    ...
    test("queues follow-up message sent before stream-start and auto-sends after first stream", async () => {
        ...
        try {
            ...
            const streamStarts = timestampedEvents.filter((entry) => "type" in entry.event && entry.event.type === "stream-start");
            const streamEnds = timestampedEvents.filter((entry) => "type" in entry.event && entry.event.type === "stream-end");
...
//# sourceMappingURL=queuedMessages.starting.test.js.map