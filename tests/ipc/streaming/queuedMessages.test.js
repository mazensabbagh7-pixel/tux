...
import { sendMessageWithModel, sendMessage, createStreamCollector, waitFor, TEST_IMAGES, modelString, resolveOrpcClient, configureTestRetries, } from "../helpers";
...
async function getQueuedMessages(collector, options = {}) {
...
async function waitForQueuedMessageEvent(collector, timeoutMs = 5000) {
...
async function waitForRestoreToInputEvent(collector, timeoutMs = 5000) {
...
describeIntegration("Queued messages", () => {
    ...
    test.concurrent("should queue message during streaming and auto-send on stream end", async () => {
        const { env, workspaceId, cleanup } = await setupWorkspace("anthropic");
        try {
            // Start initial stream
            void sendMessageWithModel(env, workspaceId, "Say 'FIRST' and nothing else", modelString("anthropic", "claude-sonnet-4-5"));
            ...
            const queueResult = await sendMessageWithModel(env, workspaceId, "Say 'SECOND' and nothing else", modelString("anthropic", "claude-sonnet-4-5"));
    ...
    test.concurrent("should restore queued message to input on stream abort", async () => {
        const { env, workspaceId, cleanup } = await setupWorkspace("anthropic");
        try {
            // Start a stream
            void sendMessageWithModel(env, workspaceId, "Count to 10 slowly", modelString("anthropic", "claude-sonnet-4-5"));
            const collector = createStreamCollector(env.orpc, workspaceId);
            collector.start();
            await collector.waitForEvent("stream-start", 5000);
            // Queue a message
            await sendMessageWithModel(env, workspaceId, "This message should be restored", modelString("anthropic", "claude-sonnet-4-5"));
    ...
    test.concurrent("should send queued message immediately when sendQueuedImmediately is true", async () => {
        const { env, workspaceId, cleanup } = await setupWorkspace("anthropic");
        try {
            ...
            await collector.waitForEvent("stream-start", 5000);
            // Queue a message
            await sendMessageWithModel(env, workspaceId, "This message should be sent immediately", modelString("anthropic", "claude-sonnet-4-5"));
            // Verify message was queued
            const queued = await getQueuedMessages(collector);
    ...
    test.concurrent("should combine multiple queued messages with newline separator", async () => {
        const { env, workspaceId, cleanup } = await setupWorkspace("anthropic");
        try {
            // Start collector first and wait for subscription to avoid replay/live
            // event duplication of the user message (same race as resumeStream fix).
    ...
    test.concurrent("should auto-send queued message with images", async () => {
        const { env, workspaceId, cleanup } = await setupWorkspace("anthropic");
        try {
            // Start a stream
            void sendMessageWithModel(env, workspaceId, "Say 'FIRST' and nothing else", modelString("anthropic", "claude-sonnet-4-5"));
    ...
    test.concurrent("should handle image-only queued message", async () => {
        const { env, workspaceId, cleanup } = await setupWorkspace("anthropic");
        try {
            // Start a stream
            void sendMessageWithModel(env, workspaceId, "Say 'FIRST' and nothing else", modelString("anthropic", "claude-sonnet-4-5"));
    ...
    test.concurrent("should preserve latest options when queueing", async () => {
        const { env, workspaceId, cleanup } = await setupWorkspace("anthropic");
        try {
            // Start a stream
            void sendMessageWithModel(env, workspaceId, "Say 'FIRST' and nothing else", modelString("anthropic", "claude-sonnet-4-5"));
    ...
    test.concurrent("should preserve compaction metadata when queueing", async () => {
        const { env, workspaceId, cleanup } = await setupWorkspace("anthropic");
        try {
            // Start a stream
            void sendMessageWithModel(env, workspaceId, "Say 'FIRST' and nothing else", modelString("anthropic", "claude-sonnet-4-5"));
...
//# sourceMappingURL=queuedMessages.test.js.map