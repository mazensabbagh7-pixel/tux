...
import { sendMessageWithModel, createStreamCollector, modelString, resolveOrpcClient, configureTestRetries, } from "../helpers";
...
describeIntegration("resumeStream", () => {
    ...
    test.concurrent("should resume interrupted stream without new user message", async () => {
        ...
        try {
            ...
            void sendMessageWithModel(env, workspaceId, `Use bash to run: for i in {1..10}; do sleep 0.5; done && echo '${expectedWord}'. Set display_name="resume-test" and timeout_secs=120. Do not spawn a sub-agent.`, modelString("anthropic", "claude-sonnet-4-5"), {
    ...
    test.concurrent("should resume from single assistant message (post-compaction scenario)", async () => {
        const { env, workspaceId, cleanup } = await setupWorkspace("anthropic");
        ...
        try {
            ...
            const summaryMessage = createMuxMessage("compaction-summary-msg", "assistant", `I previously helped with a task. The conversation has been compacted for token efficiency. My next message will contain the word ${verificationWord} to confirm continuation works correctly.`, {
                compacted: true,
            });
