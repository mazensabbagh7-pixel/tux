...
import { createSharedRepo, cleanupSharedRepo, withSharedWorkspace, configureTestRetries, } from "../sendMessageTestHelpers";
...
describeIntegration("sendMessage basic integration tests", () => {
    ...
    describe.each(PROVIDER_CONFIGS)("%s provider tests", (provider, model) => {
        test.concurrent("should successfully send message and receive response", async () => {
            await withSharedWorkspace(provider, async ({ env, workspaceId, collector }) => {
                ...
                const result = await sendMessageWithModel(env, workspaceId, "Say 'hello' and nothing else", modelString(provider, model));
        ...
        test.concurrent("should interrupt streaming with interruptStream()", async () => {
            await withSharedWorkspace(provider, async ({ env, workspaceId, collector }) => {
                // Start a message that would generate a long response
                const longMessage = "Write a very long essay about the history of computing, at least 1000 words.";
        ...
        test.concurrent("should track usage tokens", async () => {
            await withSharedWorkspace(provider, async ({ env, workspaceId, collector }) => {
                // Send a message
                const result = await sendMessageWithModel(env, workspaceId, "Say 'test' and nothing else", modelString(provider, model));
                expect(result.success).toBe(true);
                // Wait for stream to complete
                await collector.waitForEvent("stream-end", 15000);
                // Check for usage-delta events
                const events = collector.getEvents();
                const usageEvents = events.filter((e) => "type" in e && e.type === "usage-delta");
                // Should have at least one usage event
                expect(usageEvents.length).toBeGreaterThan(0);
            });
        }, 20000);
        test.concurrent("should handle multiple sequential messages", async () => {
            await withSharedWorkspace(provider, async ({ env, workspaceId, collector }) => {
                // Send first message
                const result1 = await sendMessageWithModel(env, workspaceId, "Say 'one'", modelString(provider, model));
                ...
                collector.clear();
                // Send second message
                const result2 = await sendMessageWithModel(env, workspaceId, "Say 'two'", modelString(provider, model));
    ...
    test.concurrent("should work with both providers in same workspace", async () => {
        await withSharedWorkspace("openai", async ({ env, workspaceId, collector }) => {
            // Send with OpenAI
            const openaiResult = await sendMessageWithModel(env, workspaceId, "Say 'openai'", modelString("openai", KNOWN_MODELS.GPT.providerModelId));
            ...
            collector.clear();
            // Send with Anthropic
            const anthropicResult = await sendMessageWithModel(env, workspaceId, "Say 'anthropic'", modelString("anthropic", KNOWN_MODELS.HAIKU.providerModelId));
            expect(anthropicResult.success).toBe(true);
            await collector.waitForEvent("stream-end", 15000);
        });
    }, 40000);
});
//# sourceMappingURL=sendMessage.basic.test.js.map