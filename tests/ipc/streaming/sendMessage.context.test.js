...
import { createSharedRepo, cleanupSharedRepo, withSharedWorkspace, configureTestRetries, getSharedRepoPath, } from "../sendMessageTestHelpers";
...
describeIntegration("sendMessage context handling tests", () => {
    ...
    describe.each(PROVIDER_CONFIGS)("%s conversation continuity", (provider, model) => {
        test.concurrent("should maintain conversation context across messages", async () => {
            await withSharedWorkspace(provider, async ({ env, workspaceId, collector }) => {
                ...
                const result1 = await sendMessageWithModel(env, workspaceId, "My name is TestUser. Remember this.", modelString(provider, model), { toolPolicy: [{ regex_match: ".*", action: "disable" }] });
                ...
                const result2 = await sendMessageWithModel(env, workspaceId, "What is my name?", modelString(provider, model), { toolPolicy: [{ regex_match: ".*", action: "disable" }] });
                ...
                const textFromContent = typeof finalMessage?.content === "string" ? finalMessage.content : "";
    ...
    describe("message editing", () => {
        test.concurrent("should support editing a previous message", async () => {
            await withSharedWorkspace("openai", async ({ env, workspaceId, collector }) => {
                // Send initial message
                const result1 = await sendMessageWithModel(env, workspaceId, "Say 'original'", modelString("openai", KNOWN_MODELS.GPT.providerModelId));
                expect(result1.success).toBe(true);
                await collector.waitForEvent("stream-end", 15000);
                // Get the message ID from the stream events
                const events = collector.getEvents();
                const streamStart = events.find((e) => "type" in e && e.type === "stream-start");
    ...
    describe("mode-specific behavior", () => {
        test.concurrent("should respect additionalSystemInstructions", async () => {
            await withSharedWorkspace("openai", async ({ env, workspaceId, collector }) => {
                const result = await sendMessageWithModel(env, workspaceId, "What is 2+2?", modelString("openai", KNOWN_MODELS.GPT.providerModelId), {
                    // Disable tools so the model returns assistant text deterministically in CI.
                    toolPolicy: [{ regex_match: ".*", action: "disable" }],
                    additionalSystemInstructions: "Answer the user's question. Include the word BANANA in your response.",
                });
    ...
    describe("tool calls", () => {
        test.concurrent("should execute bash tool when requested", async () => {
            await withSharedWorkspace("anthropic", async ({ env, workspaceId, collector }) => {
                ...
                try {
                    ...
                    const result = await sendMessageWithModel(env, workspaceId, `Use bash to run: cat ${testFilePath}. Set display_name="read-file" and timeout_secs=30. Do not spawn a sub-agent.`, modelString("anthropic", KNOWN_MODELS.HAIKU.providerModelId), {
        ...
        test.concurrent("should respect tool policy 'none'", async () => {
            await withSharedWorkspace("anthropic", async ({ env, workspaceId, collector }) => {
                // Ask for something that would normally use tools
                // Policy to disable all tools: match any tool name and disable
                const result = await sendMessageWithModel(env, workspaceId, "Run the command 'echo test' using bash.", modelString("anthropic", KNOWN_MODELS.HAIKU.providerModelId), {
                    toolPolicy: [{ regex_match: ".*", action: "disable" }],
                });
                expect(result.success).toBe(true);
                await collector.waitForEvent("stream-end", 15000);
    ...
    describe("history truncation", () => {
        test.concurrent("should handle truncateHistory", async () => {
            await withSharedWorkspace("openai", async ({ env, workspaceId, collector }) => {
                // Send a few messages to build history
                for (let i = 0; i < 3; i++) {
                    await sendMessageWithModel(env, workspaceId, `Message ${i + 1}`, modelString("openai", KNOWN_MODELS.GPT.providerModelId));
                    await collector.waitForEvent("stream-end", 15000);
                    collector.clear();
                }
                ...
                const result = await sendMessageWithModel(env, workspaceId, "After truncation", modelString("openai", KNOWN_MODELS.GPT.providerModelId));
                expect(result.success).toBe(true);
                await collector.waitForEvent("stream-end", 15000);
            });
        }, 60000);
...
//# sourceMappingURL=sendMessage.context.test.js.map