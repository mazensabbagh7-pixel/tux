 ...
 * - Stream errors
 */
import { shouldRunIntegrationTests, validateApiKeys, createTestEnvironment, cleanupTestEnvironment, } from "../setup";
...
describeIntegration("sendMessage error handling tests", () => {
    ...
    describe("validation errors", () => {
        test.concurrent("should reject empty message", async () => {
            await withSharedWorkspace("openai", async ({ env, workspaceId }) => {
                const result = await sendMessageWithModel(env, workspaceId, "", // Empty message
                modelString("openai", KNOWN_MODELS.GPT.providerModelId));
                // Should fail with validation error
                expect(result.success).toBe(false);
            });
        }, 15000);
        test.concurrent("should reject whitespace-only message", async () => {
            await withSharedWorkspace("openai", async ({ env, workspaceId }) => {
                const result = await sendMessageWithModel(env, workspaceId, "   \n\t  ", // Whitespace only
                modelString("openai", KNOWN_MODELS.GPT.providerModelId));
                // Should fail with validation error
                expect(result.success).toBe(false);
            });
    ...
    describe("model errors", () => {
        test.concurrent("should fail with invalid model format (model validation happens before message is persisted)", async () => {
            await withSharedWorkspace("openai", async ({ env, workspaceId }) => {
        ...
        test.concurrent("should fail with non-existent model", async () => {
        ...
        test.concurrent("should fail with non-existent provider", async () => {
            await withSharedWorkspace("openai", async ({ env, workspaceId }) => {
                const result = await sendMessage(env, workspaceId, "Hello", {
    ...
    describe("stream error recovery", () => {
        test.concurrent("should handle stream interruption gracefully", async () => {
            await withSharedWorkspace("openai", async ({ env, workspaceId, collector }) => {
                // Start a long-running request
                void sendMessageWithModel(env, workspaceId, "Write a 500 word essay about technology.", modelString("openai", KNOWN_MODELS.GPT.providerModelId));
                ...
                collector.clear();
                const result = await sendMessageWithModel(env, workspaceId, "Say 'recovered'", modelString("openai", KNOWN_MODELS.GPT.providerModelId));
                expect(result.success).toBe(true);
    ...
    describe("concurrent message handling", () => {
        test.concurrent("should queue messages sent while streaming", async () => {
            await withSharedWorkspace("openai", async ({ env, workspaceId, collector }) => {
                // Start first message
                void sendMessageWithModel(env, workspaceId, "Count from 1 to 10 slowly.", modelString("openai", KNOWN_MODELS.GPT.providerModelId));
                // Wait for stream to start
                await collector.waitForEvent("stream-start", 10000);
                // Send second message while first is streaming (should be queued)
                const result2 = await sendMessageWithModel(env, workspaceId, "Say 'queued'", modelString("openai", KNOWN_MODELS.GPT.providerModelId));
...
//# sourceMappingURL=sendMessage.errors.test.js.map