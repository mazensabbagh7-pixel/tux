...
describeIntegration("sendMessage heavy/load tests", () => {
    ...
    describe("OpenAI context limit error (forced)", () => {
        ...
        test.concurrent("should emit context_exceeded when forceContextLimitError is set", async () => {
            await withSharedWorkspace(provider, async ({ env, workspaceId, collector }) => {
                const result = await sendMessageWithModel(env, workspaceId, "Trigger a forced context limit error", modelString(provider, model), {
                    providerOptions: {
                        openai: {
    ...
    describe("context limit handling", () => {
        test.concurrent("should handle very long single messages", async () => {
            await withSharedWorkspace("openai", async ({ env, workspaceId, collector }) => {
                // Send a very long message
                const longContent = "This is a test message. ".repeat(1000);
                const result = await sendMessageWithModel(env, workspaceId, longContent, modelString("openai", KNOWN_MODELS.GPT.providerModelId));
                expect(result.success).toBe(true);
...
//# sourceMappingURL=sendMessage.heavy.test.js.map