...
describeIntegration("sendMessage image handling tests", () => {
    ...
    describe.each(PROVIDER_CONFIGS)("%s image support", (provider, model) => {
        test.concurrent("should send images to AI model and get response", async () => {
            ...
            await withSharedWorkspace(provider, async ({ env, workspaceId, collector }) => {
                ...
                const result = await sendMessage(env, workspaceId, "This is a small solid-color image. What color is it? Answer with just the color name.", {
        ...
        test.concurrent("should handle multiple images in single message", async () => {
    ...
    describe("image conversation context", () => {
        test.concurrent("should maintain image context across messages", async () => {
            await withSharedWorkspace("openai", async ({ env, workspaceId, collector }) => {
                ...
                const result2 = await sendMessage(env, workspaceId, "What color was the image I showed you?", {
                    model: modelString("openai", OPENAI_VISION_MODEL),
                });
                ...
                const imageMsg = messages.find((msg) => msg.role === "user" &&
                    msg.parts.some((part) => part.type === "text" && part.text === "Remember this image"));
                expect(imageMsg).toBeTruthy();
                if (!imageMsg)
                    return;
                const imagePart = imageMsg.parts.find((part) => part.type === "file" && part.url === RED_PIXEL.url);
                expect(imagePart).toBeTruthy();
            });
        }, 60000);
    });
});
//# sourceMappingURL=sendMessage.images.test.js.map