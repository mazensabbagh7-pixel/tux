...
describeIntegration("Anthropic reasoning parameter tests", () => {
    ...
    test.concurrent("Sonnet 4.5 with thinking (budgetTokens)", async () => {
    ...
    test.concurrent("Opus 4.6 with thinking (effort + adaptive)", async () => {
        await withSharedWorkspace("anthropic", async ({ env, workspaceId, collector }) => {
    ...
    test.concurrent("should receive reasoning events when thinking enabled", async () => {
        await withSharedWorkspace("anthropic", async ({ env, workspaceId, collector }) => {
            ...
            const reasoningStarts = events.filter((e) => "type" in e && e.type === "reasoning-start");
            const reasoningDeltas = events.filter((e) => "type" in e && e.type === "reasoning-delta");
...
//# sourceMappingURL=sendMessage.reasoning.test.js.map