...
import { sendMessageWithModel, createStreamCollector, readChatHistory, modelString, resolveOrpcClient, configureTestRetries, } from "../helpers";
 ...
 */
function extractMarkers(nonce, text) {
 ...
 */
function getMaxMarker(nonce, text) {
 ...
 */
function truncateToLastCompleteMarker(text, nonce) {
 ...
 */
async function resumeAndWaitForSuccess(workspaceId, client, model, timeoutMs = 15000, options) {
 ...
 * Uses StreamCollector for ORPC-native event handling
 */
async function collectStreamUntil(collector, predicate, timeoutMs = 15000) {
    ...
    while (Date.now() - startTime < timeoutMs) {
        ...
        if (newDeltas.length > 0) {
            ...
            if (allDeltas.length % 20 === 0) {
                console.log(`[collectStreamUntil] Processed ${allDeltas.length} deltas, buffer length: ${buffer.length}`);
            }
            // Check predicate after processing new deltas
            if (predicate(buffer)) {
                console.log(`[collectStreamUntil] Predicate satisfied after ${allDeltas.length} deltas, buffer length: ${buffer.length}`);
                return buffer;
            }
        }
...
describeIntegration("Stream Error Recovery (No Amnesia)", () => {
    ...
    test.concurrent("should preserve exact prefix and continue from exact point after stream error", async () => {
...
IMPORTANT: Do not add any other text. Start immediately with ${nonce}-1: one. If interrupted, resume from where you stopped without repeating any lines.`;
            ...
            collector.start();
            const sendResult = await sendMessageWithModel(env, workspaceId, prompt, modelString(PROVIDER, MODEL), { toolPolicy: [{ regex_match: ".*", action: "disable" }] });
            expect(sendResult.success).toBe(true);
            // Collect stream deltas until we have at least STABLE_PREFIX_THRESHOLD complete markers
            const preErrorBuffer = await collectStreamUntil(collector, (buf) => getMaxMarker(nonce, buf) >= STABLE_PREFIX_THRESHOLD, 15000);
            ...
            const searchWindow = normalizedFinal.substring(normalizedPrefix.length, normalizedPrefix.length + 2000);
...
//# sourceMappingURL=streamErrorRecovery.test.js.map