...
import { THINKING_LEVELS } from "../../../src/common/types/thinking";
import { applySystem1KeepRangesToOutput, formatNumberedLinesForSystem1, splitBashOutputLines, } from "../../../src/node/services/system1/bashOutputFiltering";
...
import { cleanupTestEnvironment, createTestEnvironment, preloadTestModules, setupProviders, shouldRunIntegrationTests, } from "../setup";
...
function pickThinkingLevels(levels) {
...
const requestedModels = (process.env.SYSTEM1_BASH_COMPACTION_TEST_MODELS
...
const configuredModels = requestedModels.filter((modelString) => {
    ...
    if (!apiKey) {
        ...
        console.warn(`Skipping System1 bash compaction integration test for ${modelString}: missing API key env vars for provider ${parsed.provider}`);
        return false;
    }
    return true;
});
...
if (shouldRunIntegrationTests() && !shouldRunSuite) {
    // eslint-disable-next-line no-console
    console.warn("Skipping System1 bash compaction integration tests: no configured models (missing API keys)");
}
...
describeIntegration("System1 bash output compaction (keep_ranges matrix)", () => {
    ...
    for (const modelString of configuredModels) {
        test(`should generate keep_ranges for ${modelString}`, async () => {
            ...
            for (const thinkingLevel of thinkingLevels) {
                const providerOptions = buildProviderOptions(modelString, thinkingLevel, undefined, undefined, undefined, "system1-test");
...
//# sourceMappingURL=system1BashCompaction.matrix.test.js.map