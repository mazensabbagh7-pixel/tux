...
import { sendMessageWithModel, createStreamCollector, assertStreamSuccess, resolveOrpcClient, modelString, } from "../helpers";
...
describeIntegration("truncateHistory", () => {
    test.concurrent("should truncate 50% of chat history and verify context is updated", async () => {
        ...
        try {
            ...
            const result = await sendMessageWithModel(env, workspaceId, "What was the word I asked you to remember at the beginning? Reply with just the word or 'I don't know'.");
            expect(result.success).toBe(true);
            // Wait for response
    ...
    test.concurrent("should truncate 100% of chat history and verify context is cleared", async () => {
        ...
        try {
            ...
            const result = await sendMessageWithModel(env, workspaceId, "What was the word I asked you to remember? Reply with just the word or 'I don't know'.");
            expect(result.success).toBe(true);
            // Wait for response
            await collector.waitForEvent("stream-end", 20000);
            assertStreamSuccess(collector);
    ...
    test.concurrent("should block truncate during active stream and require Esc first", async () => {
        ...
        try {
            ...
            void sendMessageWithModel(env, workspaceId, 'Use bash to run: for i in {1..60}; do sleep 0.5; done && echo done. Set display_name="truncate-stream" and timeout_secs=120. Do not spawn a sub-agent.', modelString("anthropic", "claude-sonnet-4-5"), {
