...
import { sendMessageWithModel, createStreamCollector, modelString, assertStreamSuccess, configureTestRetries, } from "../helpers";
...
describeIntegration("usage-delta events", () => {
    ...
    test.concurrent("should emit usage-delta events during multi-step tool call streams", async () => {
        const { env, workspaceId, cleanup } = await setupWorkspace("anthropic");
        const collector = createStreamCollector(env.orpc, workspaceId);
        collector.start();
        try {
            // Ask the model to read a file - guaranteed to trigger tool use
            const result = await sendMessageWithModel(env, workspaceId, "Use the file_read tool to read README.md. Only read the first 5 lines.", modelString("anthropic", KNOWN_MODELS.SONNET.providerModelId));
...
//# sourceMappingURL=usageDelta.test.js.map