...
import { createTempGitRepo, cleanupTempGitRepo, createWorkspace, resolveOrpcClient, } from "../helpers";
...
describeIntegration("terminal PTY", () => {
    test.concurrent("should create terminal session, send command, receive output, and close", async () => {
    ...
    test.concurrent("should handle exit event when terminal closes", async () => {
        const env = await createTestEnvironment();
        const tempGitRepo = await createTempGitRepo();
        try {
            // Create a workspace
    ...
    test.concurrent("should handle terminal resize", async () => {
        const env = await createTestEnvironment();
        const tempGitRepo = await createTempGitRepo();
        try {
            // Create a workspace
    ...
    test.concurrent("attach should return screenState first, then live output", async () => {
        const env = await createTestEnvironment();
        const tempGitRepo = await createTempGitRepo();
        try {
    ...
    test.concurrent("attach should preserve output order - no race conditions", async () => {
        const env = await createTestEnvironment();
        const tempGitRepo = await createTempGitRepo();
        try {
            ...
            await Promise.race([
                attachPromise,
                new Promise((_, reject) => setTimeout(() => reject(new Error("Timeout in order test")), 10000)),
            ]);
    ...
    test.concurrent("reattach via attach should restore terminal state with escape sequences", async () => {
        const env = await createTestEnvironment();
        const tempGitRepo = await createTempGitRepo();
        try {
            ...
            const outputPromise = (async () => {
                ...
                for await (const chunk of iterator) {
                    outputChunks.push(chunk);
                    if (outputChunks.join("").includes("REATTACH_TEST"))
                        break;
                }
            })();
            client.terminal.sendInput({
            ...
            await Promise.race([
                outputPromise,
                new Promise((_, reject) => setTimeout(() => reject(new Error("Timeout waiting for output")), 5000)),
            ]);
            ...
            await Promise.race([
                attachPromise,
                new Promise((_, reject) => setTimeout(() => reject(new Error("Timeout in attach")), 5000)),
            ]);
    ...
    test.concurrent("listSessions should return active session IDs for a workspace", async () => {
        const env = await createTestEnvironment();
        const tempGitRepo = await createTempGitRepo();
        try {
