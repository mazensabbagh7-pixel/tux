...
import { createTempGitRepo, cleanupTempGitRepo, generateBranchName, createStreamCollector, getTestRunner, } from "../helpers";
...
function expectExecuteBashSuccess(result, context) {
 ...
 */
async function setupInitEventCapture(env, workspaceId) {
 ...
 */
async function createInitHook(repoPath, hookContent) {
 ...
 */
async function createWorkspaceWithCleanup(env, projectPath, branchName, trunkBranch, runtimeConfig) {
...
describeIntegration("WORKSPACE_CREATE with both runtimes", () => {
    ...
    describe.each([{ type: "local" }, { type: "ssh" }])("Runtime: $type", ({ type }) => {
        ...
        describe("Branch handling", () => {
            runTest("creates new branch from trunk when branch doesn't exist", async () => {
                ...
                try {
                    ...
                    const { result, cleanup } = await createWorkspaceWithCleanup(env, tempGitRepo, branchName, trunkBranch, runtimeConfig);
                    expect(result.success).toBe(true);
                    if (!result.success) {
                        throw new Error(`Failed to create workspace for new branch '${branchName}': ${result.error}`);
                    }
            ...
            runTest("checks out existing branch when branch already exists", async () => {
                const env = await createTestEnvironment();
                const tempGitRepo = await createTempGitRepo();
                try {
                    ...
                    expect(result.success).toBe(true);
                    if (!result.success) {
                        throw new Error(`Failed to check out existing branch '${branchName}': ${result.error}`);
                    }
                    expect(result.metadata.id).toBeDefined();
                    await cleanup();
                }
            ...
            runTest("creates new branch from specified trunk branch, not from default branch", async () => {
                const env = await createTestEnvironment();
                const tempGitRepo = await createTempGitRepo();
                try {
                    // Create a custom trunk branch with a unique commit
                    const customTrunkBranch = "custom-trunk";
                    await execAsync(`git checkout -b ${customTrunkBranch} && echo "custom-trunk-content" > trunk-file.txt && git add . && git commit -m "Custom trunk commit"`, { cwd: tempGitRepo });
                    // Create a different branch (which will become the default if we checkout to it)
                    const otherBranch = "other-branch";
                    await execAsync(`git checkout -b ${otherBranch} && echo "other-content" > other-file.txt && git add . && git commit -m "Other branch commit"`, { cwd: tempGitRepo });
                    ...
                    const { result, cleanup } = await createWorkspaceWithCleanup(env, tempGitRepo, newBranchName, customTrunkBranch, // Specify custom trunk branch
                    runtimeConfig);
                    expect(result.success).toBe(true);
                    if (!result.success) {
                        throw new Error(`Failed to create workspace from custom trunk '${customTrunkBranch}': ${result.error}`);
                    }
                    ...
                    const trunkFileData = expectExecuteBashSuccess(checkTrunkFileResult, "custom trunk: trunk-file");
                    ...
                    const otherFileData = expectExecuteBashSuccess(checkOtherFileResult, "custom trunk: other-file");
        ...
        describe("Init hook execution", () => {
            runTest("executes .mux/init hook when present and streams logs", async () => {
                const env = await createTestEnvironment();
                const tempGitRepo = await createTempGitRepo();
                try {
...
`);
            ...
            runTest("handles init hook failure gracefully", async () => {
                const env = await createTestEnvironment();
                const tempGitRepo = await createTempGitRepo();
                try {
...
`);
                    ...
                    try {
                        ...
                        const endEvents = filterEventsByType(initEvents, EVENT_TYPE_INIT_END);
                        expect(endEvents.length).toBe(1);
                        const endEventData = endEvents[0];
            ...
            runTest("completes successfully when no init hook present", async () => {
                const env = await createTestEnvironment();
                const tempGitRepo = await createTempGitRepo();
                try {
            ...
            if (type === "ssh") {
                runTest("streams sync progress to init events (SSH only)", async () => {
                    const env = await createTestEnvironment();
                    const tempGitRepo = await createTempGitRepo();
                    try {
                        ...
                        try {
                            ...
                            expect(outputLines.some((line) => line.includes("Syncing project files"))).toBe(true);
                            // SSH init creates a worktree (or checks out a branch on legacy workspaces)
                            expect(outputLines.some((line) => line.includes("Checking out branch") ||
                                line.includes("Creating worktree for branch"))).toBe(true);
                ...
                runTest("resolves tilde paths in srcBaseDir to absolute paths (SSH only)", async () => {
                    if (!sshConfig) {
                    ...
                    try {
                        ...
                        const { result, cleanup } = await createWorkspaceWithCleanup(env, tempGitRepo, branchName, trunkBranch, tildeRuntimeConfig);
                        ...
                        const projectWorkspaces = projectsConfig.projects.get(tempGitRepo)?.workspaces ?? [];
                ...
                runTest("resolves bare tilde in srcBaseDir to home directory (SSH only)", async () => {
                    if (!sshConfig) {
                        throw new Error("SSH server is required for SSH integration tests");
                    }
                    const env = await createTestEnvironment();
                ...
                runTest("can execute commands in workspace immediately after creation (SSH only)", async () => {
                    const env = await createTestEnvironment();
                    const tempGitRepo = await createTempGitRepo();
                    try {
        ...
        describe("Validation", () => {
            runTest("rejects invalid workspace names", async () => {
                const env = await createTestEnvironment();
                const tempGitRepo = await createTempGitRepo();
                try {
                    const invalidCases = [
                    ...
                    for (const { name, expectedErrorFragment } of invalidCases) {
                        ...
                        const { result } = await createWorkspaceWithCleanup(env, tempGitRepo, name, trunkBranch, runtimeConfig);
    ...
    describe("SSH-specific behavior", () => {
        test.concurrent("forwards origin remote instead of bundle path", async () => {
            if (!sshConfig) {
                throw new Error("SSH server is required for SSH integration tests");
            }
            const env = await createTestEnvironment();
            ...
            try {
                ...
                try {
                    ...
                    const originStream = await runtime.exec(checkOriginCmd, {
                        cwd: "~",
                        timeout: 10,
                    });
