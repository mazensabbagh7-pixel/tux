...
import { createTestEnvironment, cleanupTestEnvironment, setupProviders, preloadTestModules, shouldRunIntegrationTests, } from "../setup";
import { createTempGitRepo, cleanupTempGitRepo, generateBranchName, createStreamCollector, HAIKU_MODEL, STREAM_TIMEOUT_LOCAL_MS, } from "../helpers";
...
describeIntegration("File Change Notification Integration", () => {
    ...
    it("should inject file change notification when tracked plan file is modified externally", async () => {
        ...
        try {
            ...
            const debugFiles = await readdir(debugObjDir).catch(() => []);
            ...
            if (redactedFile) {
                ...
                const fileChangeMessage = messages.find((m) => m.role === "user" &&
...
//# sourceMappingURL=fileChangeNotification.test.js.map