import { shouldRunIntegrationTests, createTestEnvironment, cleanupTestEnvironment, setupWorkspace, validateApiKeys, } from "../setup";
import { createTempGitRepo, cleanupTempGitRepo, sendMessageWithModel, createStreamCollector, assertStreamSuccess, configureTestRetries, waitFor, modelString, resolveOrpcClient, createWorkspaceWithInit, TEST_TIMEOUT_SSH_MS, } from "../helpers";
...
describeIntegration("Workspace fork", () => {
    ...
    describe.each([{ type: "local" }, { type: "ssh" }])("Runtime: $type", ({ type }) => {
        ...
        const getTimeout = (baseMs) => isSSH ? Math.max(baseMs, TEST_TIMEOUT_SSH_MS) : baseMs;
        const runtimeTest = isSSH ? test : test.concurrent;
        runtimeTest("should fail to fork workspace with invalid name", async () => {
            const env = await createTestEnvironment();
            const tempGitRepo = await createTempGitRepo();
            try {
                const runtimeConfig = getRuntimeConfig();
                const { workspaceId: sourceWorkspaceId } = await createWorkspaceWithInit(env, tempGitRepo, "source-workspace", runtimeConfig, isSSH, isSSH);
        ...
        runtimeTest("should fork workspace and send message successfully", async () => {
            const runtimeConfig = getRuntimeConfig();
            const { env, workspaceId: sourceWorkspaceId, cleanup, } = await setupWorkspace("anthropic", undefined, {
                runtimeConfig,
                waitForInit: isSSH,
                isSSH,
            });
            try {
                ...
                const sendResult = await sendMessageWithModel(env, forkedWorkspaceId, "What is 2+2? Answer with just the number.", modelString("anthropic", "claude-sonnet-4-5"));
        ...
        runtimeTest("should preserve chat history when forking workspace", async () => {
            const runtimeConfig = getRuntimeConfig();
            const { env, workspaceId: sourceWorkspaceId, cleanup, } = await setupWorkspace("anthropic", undefined, {
                runtimeConfig,
                waitForInit: isSSH,
        ...
        runtimeTest("should create independent workspaces that can send messages concurrently", async () => {
            const runtimeConfig = getRuntimeConfig();
            const { env, workspaceId: sourceWorkspaceId, cleanup, } = await setupWorkspace("anthropic", undefined, {
                runtimeConfig,
                waitForInit: isSSH,
            ...
            try {
                ...
                const [sourceResult, forkedResult] = await Promise.all([
                    sendMessageWithModel(env, sourceWorkspaceId, "What is 5+5? Answer with just the number.", modelString("anthropic", "claude-sonnet-4-5")),
                    sendMessageWithModel(env, forkedWorkspaceId, "What is 3+3? Answer with just the number.", modelString("anthropic", "claude-sonnet-4-5")),
                ]);
        ...
        runtimeTest("should preserve partial streaming response when forking mid-stream", async () => {
            const runtimeConfig = getRuntimeConfig();
            const { env, workspaceId: sourceWorkspaceId, cleanup, } = await setupWorkspace("anthropic", undefined, {
                runtimeConfig,
                waitForInit: isSSH,
            ...
            try {
                ...
                const sendResult = await sendMessageWithModel(env, sourceWorkspaceId, 'Count from 1 to 25, one number per line, and include the word "alpha" after each number.', modelString("anthropic", "claude-sonnet-4-5"));
                expect(sendResult.success).toBe(true);
                // Wait for stream to start
                const streamStartEvent = await sourceCollector.waitForEvent("stream-start", getTimeout(15000));
                expect(streamStartEvent).not.toBeNull();
                const deltaEvent = await sourceCollector.waitForEvent("stream-delta", getTimeout(15000));
                ...
                const forkedSendResult = await sendMessageWithModel(env, forkedWorkspaceId, "What is 7+3? Answer with just the number.", modelString("anthropic", "claude-sonnet-4-5"));
        ...
        runtimeTest("should make forked workspace available in workspace list", async () => {
            const env = await createTestEnvironment();
            const tempGitRepo = await createTempGitRepo();
            try {
                const runtimeConfig = getRuntimeConfig();
    ...
    test.concurrent("should fork local (project-dir) workspace successfully", async () => {
        const env = await createTestEnvironment();
        const tempGitRepo = await createTempGitRepo();
        try {
            // Create source workspace with LocalRuntime (project-dir mode)
            // This means type: "local" without srcBaseDir
            const localRuntimeConfig = { type: "local" };
            const client = resolveOrpcClient(env);
            ...
            expect(forkResult.metadata.namedWorkspacePath).toBe(createResult.metadata.namedWorkspacePath);
    ...
    test.concurrent("should preserve chat history when forking local (project-dir) workspace", async () => {
        const env = await createTestEnvironment();
        const tempGitRepo = await createTempGitRepo();
        try {
            // Create source workspace with LocalRuntime (project-dir mode)
            ...
            const historyMessages = [
                ...
                createMuxMessage("msg-2", "assistant", `I will remember the local word "${uniqueWord}".`, {}),
            ];
            for (const msg of historyMessages) {
                const result = await historyService.appendToHistory(sourceWorkspaceId, msg);
                expect(result.success).toBe(true);
    ...
    test.concurrent("should persist local runtimeConfig through config reload after fork", async () => {
        const env = await createTestEnvironment();
        const tempGitRepo = await createTempGitRepo();
        try {
            // Create source workspace with LocalRuntime (project-dir mode)
            ...
            const workspaces = await client.workspace.list();
            const forkedWorkspace = workspaces.find((w) => w.id === forkedWorkspaceId);
