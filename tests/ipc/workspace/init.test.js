import { shouldRunIntegrationTests, createTestEnvironment, cleanupTestEnvironment, validateApiKeys, getApiKey, setupProviders, } from "../setup";
import { generateBranchName, createWorkspace, waitForInitComplete, waitForInitEnd, collectInitEvents, resolveOrpcClient, } from "../helpers";
 ...
 */
async function cleanupTempGitRepo(repoPath) {
    const maxRetries = 3;
    let lastError;
...
describeIntegration("Workspace init hook", () => {
    test.concurrent("should stream init hook output and allow workspace usage on hook success", async () => {
        ...
        try {
            ...
            const outputEvents = initEvents.filter((e) => isInitOutput(e) && !e.isError);
            const errorEvents = initEvents.filter((e) => isInitOutput(e) && e.isError === true);
    ...
    test.concurrent("should stream init hook output and allow workspace usage on hook failure", async () => {
        const env = await createTestEnvironment();
        const tempGitRepo = await createTempGitRepoWithInitHook({
            exitCode: 1,
        ...
        try {
            ...
            const failureErrorEvents = initEvents.filter((e) => isInitOutput(e) && e.isError === true);
    ...
    test.concurrent("should not emit meta events when no init hook exists", async () => {
        const env = await createTestEnvironment();
        ...
        try {
            ...
            await execAsync(`git init`, { cwd: tempDir });
            await execAsync(`git config user.email "test@example.com" && git config user.name "Test User"`, { cwd: tempDir });
            await execAsync(`echo "test" > README.md && git add . && git commit -m "Initial commit"`, {
                cwd: tempDir,
            });
    ...
    test.concurrent("should persist init state to disk for replay across page reloads", async () => {
        const env = await createTestEnvironment();
        ...
        try {
            ...
            expect(status.lines).toEqual(expect.arrayContaining([
...
describeIntegration("Init Queue - Runtime Matrix", () => {
    ...
    describe.each([{ type: "local" }, { type: "ssh" }])("Runtime: $type", ({ type }) => {
        ...
        test("file_read should wait for init hook before executing (even when init fails)", async () => {
            if (!shouldRunSSH()) {
...
`,
            ...
            try {
                ...
                const client = resolveOrpcClient(env);
                // Create workspace to trigger init hook
                const result = await createWorkspace(env, repoPath, branchName, undefined, runtimeConfig);
                expect(result.success).toBe(true);
                const workspaceId = result.success ? result.metadata.id : null;
                expect(workspaceId).toBeTruthy();
