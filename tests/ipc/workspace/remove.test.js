...
import { createTestEnvironment, cleanupTestEnvironment, shouldRunIntegrationTests, } from "../setup";
import { createTempGitRepo, cleanupTempGitRepo, generateBranchName, addSubmodule, waitForFileNotExists, createStreamCollector, waitFor, createWorkspaceWithInit, TEST_TIMEOUT_LOCAL_MS, TEST_TIMEOUT_SSH_MS, getTestRunner, } from "../helpers";
import { isDockerAvailable, startSSHServer, stopSSHServer, } from "../../runtime/test-fixtures/ssh-fixture";
 ...
 */
async function executeBash(env, workspaceId, command) {
    const result = await env.orpc.workspace.executeBash({ workspaceId, script: command });
    if (!result.success || !result.data) {
        const errorMessage = "error" in result ? result.error : "unknown error";
        throw new Error(`Bash execution failed for "${command}" (workspace=${workspaceId}): ${errorMessage}`);
    }
 ...
 */
async function workspaceExists(env, workspaceId) {
    try {
 ...
 */
async function makeWorkspaceDirty(env, workspaceId) {
    // Modify an existing tracked file (README.md exists in test repos)
    // This ensures git will detect uncommitted changes
    await executeBash(env, workspaceId, 'echo "test modification to make workspace dirty" >> README.md');
}
/**
 ...
 */
async function createTempGitRepoWithInitHook(customScript) {
    ...
    try {
        ...
        const commitProc = __addDisposableResource(env_1, execAsync(`git -C "${repoPath}" add -A && git -C "${repoPath}" -c commit.gpgsign=false commit -m "Add init hook"`), false);
...
async function executeSSH(command) {
    ...
    try {
        ...
        const proc = __addDisposableResource(env_2, execAsync(`ssh -i "${sshConfig.privateKeyPath}" -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -o LogLevel=ERROR -o ConnectTimeout=2 -p ${sshConfig.port} testuser@localhost '${command}'`), false);
...
async function clearMarkerFile(runtimeType, markerPath) {
    if (runtimeType === "ssh") {
...
async function markerFileExists(runtimeType, markerPath) {
    if (runtimeType === "ssh") {
...
describeIntegration("Workspace deletion integration tests", () => {
    beforeAll(async () => {
        ...
        if (!(await isDockerAvailable())) {
            throw new Error("Docker is required for SSH runtime tests. Please install Docker or skip tests by unsetting TEST_INTEGRATION.");
        }
    ...
    describe.each([{ type: "local" }, { type: "ssh" }])("Runtime: $type", ({ type }) => {
        ...
        const getRuntimeConfig = (_branchName) => {
        ...
        runTest("should successfully delete workspace", async () => {
            const env = await createTestEnvironment();
            const tempGitRepo = await createTempGitRepo();
            try {
                const branchName = generateBranchName("delete-test");
                const runtimeConfig = getRuntimeConfig(branchName);
                const { workspaceId, workspacePath } = await createWorkspaceWithInit(env, tempGitRepo, branchName, runtimeConfig, true, // waitForInit
                type === "ssh");
                ...
                if (type === "local") {
                    ...
                    try {
                        const branchProc = __addDisposableResource(env_3, execAsync(`git -C "${tempGitRepo}" branch --list "${branchName}"`), false);
        ...
        runTest("force remove during init should delete workspace and cancel init", async () => {
            const env = await createTestEnvironment();
            const branchName = generateBranchName("remove-during-init");
            const markerPath = type === "ssh"
                ? `/home/testuser/mux-init-cancel-${branchName}`
                : path.join(os.tmpdir(), `mux-init-cancel-${branchName}`);
            const sleepSeconds = 5;
            const tempGitRepo = await createTempGitRepoWithInitHook(`echo HOOK_STARTED\nsleep ${sleepSeconds}\necho done > \"${markerPath}\"`);
            try {
                await clearMarkerFile(type, markerPath);
                const runtimeConfig = getRuntimeConfig(branchName);
                const { workspaceId } = await createWorkspaceWithInit(env, tempGitRepo, branchName, runtimeConfig, false, // waitForInit
                type === "ssh");
                const collector = createStreamCollector(env.orpc, workspaceId);
                collector.start();
                try {
                    await collector.waitForSubscription(type === "ssh" ? 20000 : 10000);
                    const sawHookStarted = await waitFor(() => collector.getEvents().some((event) => {
                    ...
                    if (type === "local") {
                        ...
                        try {
                            const branchProc = __addDisposableResource(env_4, execAsync(`git -C "${tempGitRepo}" branch --list "${branchName}"`), false);
                            const { stdout } = await branchProc.result;
                            expect(stdout.trim()).toBe("");
                        }
                        catch (e_4) {
        ...
        runTest("should handle deletion of non-existent workspace gracefully", async () => {
            const env = await createTestEnvironment();
            try {
        ...
        runTest("should handle deletion when directory is already deleted", async () => {
            const env = await createTestEnvironment();
            const tempGitRepo = await createTempGitRepo();
            try {
                const branchName = generateBranchName("already-deleted");
                const runtimeConfig = getRuntimeConfig(branchName);
                const { workspaceId, workspacePath: _workspacePath } = await createWorkspaceWithInit(env, tempGitRepo, branchName, runtimeConfig, true, // waitForInit
                type === "ssh");
        ...
        runTest("should fail to delete dirty workspace without force flag", async () => {
            const env = await createTestEnvironment();
            const tempGitRepo = await createTempGitRepo();
            try {
                const branchName = generateBranchName("delete-dirty");
                const runtimeConfig = getRuntimeConfig(branchName);
                const { workspaceId } = await createWorkspaceWithInit(env, tempGitRepo, branchName, runtimeConfig, true, // waitForInit
                type === "ssh");
                ...
                expect(deleteResult.error).toMatch(/uncommitted changes|worktree contains modified|contains modified or untracked files/i);
        ...
        runTest("should delete dirty workspace with force flag", async () => {
            const env = await createTestEnvironment();
            const tempGitRepo = await createTempGitRepo();
            try {
        ...
        if (type === "local") {
            runTest("should successfully delete clean workspace with submodule", async () => {
                const env = await createTestEnvironment();
                const tempGitRepo = await createTempGitRepo();
                try {
                    ...
                    try {
                        ...
                        const { workspaceId, workspacePath } = await createWorkspaceWithInit(env, tempGitRepo, branchName, undefined, true, // waitForInit
                        false // not SSH
                        );
                        // Initialize submodule in the worktree
                        const initProc = __addDisposableResource(env_5, execAsync(`cd "${workspacePath}" && git submodule update --init`), false);
            ...
            runTest("should fail to delete dirty workspace with submodule, succeed with force", async () => {
                const env = await createTestEnvironment();
                const tempGitRepo = await createTempGitRepo();
                try {
                    ...
                    try {
                        ...
                        // Initialize submodule in the worktree
                        const initProc = __addDisposableResource(env_6, execAsync(`cd "${workspacePath}" && git submodule update --init`), false);
                        await initProc.result;
                        // Make worktree dirty
    ...
    describe("Local project-dir runtime tests", () => {
        const getLocalProjectDirConfig = () => {
            // Local project-dir: type "local" without srcBaseDir
            return { type: "local" };
        };
        test.concurrent("should delete only the specified workspace, not all workspaces with same path", async () => {
            const env = await createTestEnvironment();
            const tempGitRepo = await createTempGitRepo();
            try {
                ...
                const { workspaceId: ws1Id } = await createWorkspaceWithInit(env, tempGitRepo, "local-ws-1", runtimeConfig, true, // waitForInit
                false // not SSH
                );
                const { workspaceId: ws2Id } = await createWorkspaceWithInit(env, tempGitRepo, "local-ws-2", runtimeConfig, true, false);
                const { workspaceId: ws3Id } = await createWorkspaceWithInit(env, tempGitRepo, "local-ws-3", runtimeConfig, true, false);
        ...
        test.concurrent("should not delete project directory when deleting local workspace", async () => {
            const env = await createTestEnvironment();
            const tempGitRepo = await createTempGitRepo();
            try {
                const runtimeConfig = getLocalProjectDirConfig();
                const { workspaceId } = await createWorkspaceWithInit(env, tempGitRepo, "local-ws-test", runtimeConfig, true, false);
    ...
    describe("SSH-only tests", () => {
        ...
        runSshTest("should fail to delete SSH workspace with unpushed refs without force flag", async () => {
            const env = await createTestEnvironment();
            const tempGitRepo = await createTempGitRepo();
            try {
                ...
                await executeBash(env, workspaceId, "git remote set-url origin https://github.com/fake/repo.git 2>/dev/null || git remote add origin https://github.com/fake/repo.git");
        ...
        runSshTest("should delete SSH workspace with unpushed refs when force flag is set", async () => {
            const env = await createTestEnvironment();
            const tempGitRepo = await createTempGitRepo();
            try {
        ...
        runSshTest.skip("should include commit list in error for unpushed refs", async () => {
            const env = await createTestEnvironment();
            const tempGitRepo = await createTempGitRepo();
            try {
        ...
        runSshTest.skip("should allow deletion of squash-merged branches without force flag", async () => {
            const env = await createTestEnvironment();
            const tempGitRepo = await createTempGitRepo();
            try {
                ...
                await executeBash(env, workspaceId, `git remote get-url origin >/dev/null 2>&1 && git remote set-url origin "${originPath}" || git remote add origin "${originPath}"`);
                ...
                await executeBash(env, workspaceId, `git clone "${originPath}" "${tempCheckoutPath}" && ` +
                    `cd "${tempCheckoutPath}" && ` +
                ...
                const logResult = await executeBash(env, workspaceId, "git log --branches --not --remotes --oneline");
                ...
                try {
                    ...
                    try {
                        const cleanupProc = __addDisposableResource(env_7, execAsync(`rm -rf "${originPath}"`), false);
        ...
        runSshTest("should block deletion when branch has genuinely unmerged content", async () => {
            const env = await createTestEnvironment();
            const tempGitRepo = await createTempGitRepo();
            try {
                ...
                try {
                    ...
                    try {
                        const cleanupProc = __addDisposableResource(env_8, execAsync(`rm -rf "${originPath}"`), false);
                        await cleanupProc.result;
                    }
                    catch (e_8) {
                        env_8.error = e_8;
