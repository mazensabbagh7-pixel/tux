...
import { createTempGitRepo, cleanupTempGitRepo, generateBranchName, createWorkspaceWithInit, TEST_TIMEOUT_SSH_MS, } from "../helpers";
...
describeIntegration("WORKSPACE_RENAME with both runtimes", () => {
    ...
    describe.each([{ type: "local" }, { type: "ssh" }])("Runtime: $type", ({ type }) => {
        ...
        runTest("should successfully rename workspace and update all paths", async () => {
            ...
            try {
                ...
                const { workspaceId, workspacePath, cleanup } = await createWorkspaceWithInit(env, tempGitRepo, branchName, runtimeConfig, true, // waitForInit
                type === "ssh");
        ...
        runTest("should fail to rename if new name conflicts with existing workspace", async () => {
            const env = await createTestEnvironment();
            const tempGitRepo = await createTempGitRepo();
            try {
                ...
                const { workspaceId: firstWorkspaceId, cleanup: firstCleanup } = await createWorkspaceWithInit(env, tempGitRepo, branchName, runtimeConfig, true, // waitForInit
                type === "ssh");
                // Create second workspace
                const { cleanup: secondCleanup } = await createWorkspaceWithInit(env, tempGitRepo, secondBranchName, runtimeConfig, true, // waitForInit
                type === "ssh");
