 ...
 */
export async function stopSSHServer(config) {
    try {
 ...
 */
async function waitForSSH(host, port, privateKeyPath, maxAttempts = 30) {
    for (let i = 0; i < maxAttempts; i++) {
        try {
    ...
    return new Promise((resolve, reject) => {
        ...
        child.on("close", (code) => {
            ...
            if (code === 0) {
                ...
                reject(new Error(`Command failed with exit code ${String(code)}: ${command} ${args.join(" ")}\nstderr: ${stderr}`));
            }
        });
...
//# sourceMappingURL=ssh-fixture.js.map