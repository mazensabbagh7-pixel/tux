...
async function openModelSelector(container) {
    window.dispatchEvent(new CustomEvent(CUSTOM_EVENTS.OPEN_MODEL_SELECTOR));
    return await waitFor(() => {
        const input = container.querySelector('input[placeholder="Search [provider:model-name]"]');
        if (!input) {
...
async function selectModel(container, workspaceId, model) {
    ...
    await waitFor(() => {
        ...
        if (!displayedModel.includes(expectedDisplayName)) {
            throw new Error(`Waiting for UI to show "${expectedDisplayName}", currently shows "${displayedModel}"`);
        }
    }, { timeout: 3000 });
...
async function setThinkingToXHigh(container) {
    ...
    await waitFor(async () => {
        ...
        const rightPaddle = group.querySelector('button[data-thinking-paddle="right"]');
        if (!rightPaddle) {
...
async function expectThinkingLabel(container, expected) {
    await waitFor(() => {
        const group = container.querySelector('[data-component="ThinkingSliderGroup"]');
...
//# sourceMappingURL=thinkingPersistence.test.js.map