...
import { cleanupSharedRepo, createSharedRepo, withSharedWorkspace, } from "../../ipc/sendMessageTestHelpers";
...
function isLikelyGitLockError(message) {
...
async function executeWorkspaceBashOrThrow(params) {
    ...
    for (let attempt = 1; attempt <= retries; attempt++) {
        ...
        throw new Error(`executeBash failed (exit ${toolResult.exitCode}): ${toolResult.error}\n${toolResult.output ?? ""}`);
    }
    throw new Error(lastError ?? "executeBash failed");
}
function getHunk(container) {
    return container.querySelector("[data-hunk-id]");
}
async function refreshReviewAndWaitForHunk(view) {
    ...
    await waitFor(() => {
        ...
        if (!hunk)
            throw new Error("No hunk found");
        return hunk;
    }, { timeout: 60000 });
    await waitForRefreshButtonIdle(refreshButton);
}
async function waitForButtonToDisappear(container, ariaLabel, timeoutMs = 10000) {
    await waitFor(() => {
        const btn = container.querySelector(`button[aria-label="${ariaLabel}"]`);
        if (btn)
            throw new Error(`Button still visible: ${ariaLabel}`);
    }, { timeout: timeoutMs });
}
async function waitForNotLoading(container, timeoutMs = 15000) {
    await waitFor(() => {
...
async function withReviewPanel(params, fn) {
...
async function waitForButton(container, ariaLabel, timeoutMs = 10000) {
    return waitFor(() => {
        const btn = container.querySelector(`button[aria-label="${ariaLabel}"]`);
        if (!btn)
 ...
 */
async function setupReviewPanelWithDiff(view, metadata, workspaceId, orpc) {
...
git diff HEAD -- bof-test.ts | grep -q "Modified line 1"`,
                ...
                expect(view.container.querySelector('button[aria-label="Show more context above"]')).toBeNull();
                // Expand-down button should still exist
                expect(view.container.querySelector('button[aria-label="Show more context below"]')).not.toBeNull();
            });
        });
    }, 180000);
...
git diff HEAD -- eof-test.ts | grep -q "MODIFIED"`,
                ...
                fireEvent.click(expandDownButton);
                await waitForNotLoading(view.container, 30000);
                await waitForButtonToDisappear(view.container, "Show more context below", 30000);
                await waitForButton(view.container, "Collapse context below", 30000);
            });
        });
    }, 180000);
...
git diff HEAD -- tiny-file.ts | grep -q "MODIFIED"`,
                ...
                const expandDownButton = await waitForButton(view.container, "Show more context below");
                fireEvent.click(expandDownButton);
                await waitForNotLoading(view.container, 15000);
                await waitForButtonToDisappear(view.container, "Show more context below", 15000);
                // Wait a bit and verify button doesn't flash back
                await new Promise((r) => setTimeout(r, 1000));
                // Button should STILL be hidden (no flash back)
                expect(view.container.querySelector('button[aria-label="Show more context below"]')).toBeNull();
            });
        });
    }, 180000);
    test("multiple expand clicks accumulate context", async () => {
        await withSharedWorkspace("anthropic", async ({ env, workspaceId, metadata }) => {
            await withReviewPanel({ apiClient: env.orpc, metadata }, async (view) => {
                ...
                await waitForNotLoading(container, 15000);
                await waitForButtonToDisappear(container, "Show more context above", 5000);
                await waitForButton(container, "Collapse context above", 5000);
            });
        });
    }, 180000);
    test("per-side collapse button hides expanded context", async () => {
        await withSharedWorkspace("anthropic", async ({ env, workspaceId, metadata }) => {
            await withReviewPanel({ apiClient: env.orpc, metadata }, async (view) => {
                ...
                const collapseButton = await waitForButton(container, "Collapse context above", 15000);
                fireEvent.click(collapseButton);
                await waitForButtonToDisappear(container, "Collapse context above", 10000);
                expect(getHunk(container)).not.toBeNull();
            });
        });
    }, 180000);
    ...
    test.skip("expansion state persists across tab switches", async () => {
        await withSharedWorkspace("anthropic", async ({ env, workspaceId, metadata }) => {
            await withReviewPanel({ apiClient: env.orpc, metadata }, async (view) => {
                ...
                const expandUpButton = await waitForButton(container, "Show more context above");
                fireEvent.click(expandUpButton);
                await waitForNotLoading(container, 10000);
...
//# sourceMappingURL=readMore.test.js.map