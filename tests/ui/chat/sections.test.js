 ...
 */
function findWorkspaceRow(container, workspaceId) {
 ...
 */
function findWorkspaceRowByTitle(container, title) {
    const rows = container.querySelectorAll("[data-workspace-id][data-workspace-path]");
 ...
 */
function getSectionIdsInOrder(container) {
 ...
 */
async function createSectionViaAPI(env, projectPath, sectionName) {
...
describeIntegration("Workspace Sections", () => {
    ...
    test("section renders with drop zones after creation", async () => {
        ...
        if (!wsResult.success)
            throw new Error(`Failed to create workspace: ${wsResult.error}`);
        ...
        try {
            ...
            const sectionDragWrapper = view.container.querySelector(`[data-section-drag-id="${sectionId}"]`);
    ...
    test("workspace created with sectionId is assigned to that section", async () => {
        ...
        if (!setupWs.success)
            throw new Error(`Setup failed: ${setupWs.error}`);
    ...
    test("clicking section add button sets pending section for creation", async () => {
        ...
        try {
            ...
            const addButton = sectionHeader.querySelector('button[aria-label="New workspace in section"]');
            ...
            const clearButton = sectionSelector.querySelector('button[aria-label="Clear section selection"]');
        ...
        try {
            ...
            try {
                ...
                const textarea = await waitFor(() => {
                    ...
                    const textareas = Array.from(view.container.querySelectorAll('textarea[aria-label="Message Claude"]'));
    ...
    test("reorderSections API updates section order", async () => {
        ...
        if (!sectionA.success)
            throw new Error(`Failed to create section: ${sectionA.error}`);
        const sectionB = await env.orpc.projects.sections.create({
            projectPath,
            name: "Section B",
        });
        if (!sectionB.success)
            throw new Error(`Failed to create section: ${sectionB.error}`);
        const sectionC = await env.orpc.projects.sections.create({
            projectPath,
            name: "Section C",
        });
        if (!sectionC.success)
            throw new Error(`Failed to create section: ${sectionC.error}`);
        try {
    ...
    test("sections render in linked-list order from config", async () => {
        ...
        if (!sectionFirst.success)
            throw new Error(`Failed to create section: ${sectionFirst.error}`);
        const sectionSecond = await env.orpc.projects.sections.create({
            projectPath,
            name: "Second Section",
        });
        if (!sectionSecond.success)
            throw new Error(`Failed to create section: ${sectionSecond.error}`);
        const cleanupDom = installDom();
        expandProjects([projectPath]);
        const view = renderApp({ apiClient: env.orpc, metadata: setupWs.metadata });
        try {
