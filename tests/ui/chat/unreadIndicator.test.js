 ...
 */
function getWorkspaceUnreadState(workspaceId) {
    const state = workspaceStore.getWorkspaceSidebarState(workspaceId);
    return {
        recencyTimestamp: state.recencyTimestamp,
        isUnread: (lastReadTimestamp) => state.recencyTimestamp !== null && state.recencyTimestamp > lastReadTimestamp,
    };
}
/**
 * Get the lastReadTimestamp from persisted state.
 */
function getLastReadTimestamp(workspaceId) {
    return readPersistedState(getWorkspaceLastReadKey(workspaceId), 0);
}
/**
 * Find the workspace element in the sidebar and check if it shows the unread indicator.
 */
function getWorkspaceUnreadIndicator(container, workspaceId) {
    const workspaceEl = container.querySelector(`[data-workspace-id="${workspaceId}"]`);
    if (!workspaceEl)
        return null;
    // The unread bar is a span with the unread styling
    const unreadBar = workspaceEl.querySelector('span[class*="bg-muted-foreground"]');
    return {
        element: workspaceEl,
...
describe("Unread indicator (mock AI router)", () => {
    ...
    describe("basic unread tracking", () => {
        ...
        test("workspace is not unread when first opened", async () => {
            ...
            const { recencyTimestamp: _recencyTimestamp, isUnread } = getWorkspaceUnreadState(app.workspaceId);
...
//# sourceMappingURL=unreadIndicator.test.js.map