...
function getBackgroundProcessManager(env) {
    return env.services.backgroundProcessManager;
}
async function waitForForegroundToolCallId(env, workspaceId, toolCallId) {
    const controller = new AbortController();
    let iterator = null;
    try {
        const subscribedIterator = await env.orpc.workspace.backgroundBashes.subscribe({ workspaceId }, { signal: controller.signal });
...
describe("Compaction UI (mock AI router)", () => {
    ...
    test("auto-compacts after context_exceeded and resumes", async () => {
        ...
        try {
            ...
            await app.chat.expectTranscriptContains("Mock compaction summary:", 60000);
            await app.chat.expectTranscriptContains(`Continue with: ${triggerMessage}`, 60000);
            await app.chat.expectTranscriptContains(`Mock response: ${triggerMessage}`, 60000);
    ...
    test("force compaction triggers during streaming and resumes with Continue", async () => {
        ...
        try {
            ...
            await app.chat.send(triggerMessage);
            await app.chat.expectTranscriptContains("Mock compaction summary:", 60000);
            await app.chat.expectTranscriptContains("Mock response: Continue", 60000);
            // Compaction transcript now renders a single top boundary row.
            await app.chat.expectTranscriptContains("Compaction boundary", 60000);
            // Force compaction is append-only: keep the triggering history around the boundary rows.
            await app.chat.expectTranscriptContains(triggerMessage, 60000);
        }
        finally {
            await app.dispose();
        }
    ...
    test("/compact command sends any foreground bash to background", async () => {
        ...
        try {
            ...
            const registration = manager.registerForegroundProcess(app.workspaceId, toolCallId, "echo foreground bash for compact", "foreground bash for compact", () => {
                backgrounded = true;
                unregister?.();
            });
    ...
    test("force compaction sends any foreground bash to background", async () => {
        ...
        try {
            ...
            let backgrounded = false;
            const registration = manager.registerForegroundProcess(app.workspaceId, toolCallId, "echo foreground bash", "foreground bash", () => {
                backgrounded = true;
                unregister?.();
            });
            unregister = registration.unregister;
...
describe("Compaction notification behavior (mock AI router)", () => {
    const notifications = [];
    let originalWindowNotification;
    beforeAll(async () => {
        await preloadTestModules();
        originalWindowNotification = globalThis.Notification;
    });
    beforeEach(() => {
        ...
        globalThis.Notification = mockWithPermission;
    });
    afterEach(() => {
        if (originalWindowNotification !== undefined) {
            globalThis.Notification = originalWindowNotification;
        }
        else {
    ...
    async function setupNotificationTest(seedMessage) {
        ...
        const originalActiveElement = Object.getOwnPropertyDescriptor(Object.getPrototypeOf(globalThis.document), "activeElement");
        ...
        const notifyButton = app.view.container.querySelector('[data-testid="notify-on-response-button"]');
        if (!notifyButton)
        ...
        await app.chat.send(seedMessage);
        await app.chat.expectTranscriptContains(`Mock response: ${seedMessage}`);
        await waitFor(() => expect(notifications.length).toBeGreaterThanOrEqual(1), { timeout: 5000 });
    ...
    async function waitForNewNotifications(countAfterSeed) {
    ...
    test("compaction with continue message should fire only ONE notification (for continue response)", async () => {
        const { app, countAfterSeed, cleanup } = await setupNotificationTest("Seed for notification test");
    ...
    test("compaction without continue message should fire notification with 'Compaction complete'", async () => {
        const { app, countAfterSeed, cleanup } = await setupNotificationTest("Seed for standalone compaction");
        try {
            await app.chat.send("/compact -t 500");
            await app.chat.expectTranscriptContains("Mock compaction summary:");
            const { newCount, last } = await waitForNewNotifications(countAfterSeed);
    ...
    test.skip("force compaction with auto-continue should fire only ONE notification", async () => {
        const { app, countAfterSeed, cleanup } = await setupNotificationTest("Seed for force compaction");
        try {
