...
describeIntegration("Git Init Banner (UI)", () => {
    ...
    test("clicking git init button runs git init and reloads branches", async () => {
        ...
        try {
            ...
            await waitFor(() => {
                if (!gitInitCalled)
                    throw new Error("git init not called");
                if (!branchesRefreshed)
                    throw new Error("branches not refreshed");
            }, { timeout: 5000 });
            ...
            await waitFor(() => {
                ...
                if (bannerAfter)
                    throw new Error("Banner should be gone after git init");
            }, { timeout: 5000 });
        }
        finally {
            await cleanupView(view, cleanupDom);
...
//# sourceMappingURL=initBanner.test.js.map