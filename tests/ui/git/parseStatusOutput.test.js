...
/**
 * Unit tests for parseGitStatusScriptOutput — the parser for the bash script
...
function buildOutput(opts) {
    const { headBranch = "feature-x", primaryBranch = "main", ahead = 0, behind = 0, dirty = 0, outAdd = 0, outDel = 0, inAdd = 0, inDel = 0, } = opts;
    return [
        "---HEAD_BRANCH---",
        headBranch,
        "---PRIMARY---",
...
describe("parseGitStatusScriptOutput", () => {
    ...
    it("handles branch names with slashes", () => {
        const result = parseGitStatusScriptOutput(buildOutput({ headBranch: "user/feature/deep-nested" }));
        expect(result).not.toBeNull();
        expect(result.headBranch).toBe("user/feature/deep-nested");
    });
});
//# sourceMappingURL=parseStatusOutput.test.js.map