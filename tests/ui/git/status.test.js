...
import { cleanupSharedRepo, createSharedRepo, getSharedRepoPath, withSharedWorkspace, } from "../../ipc/sendMessageTestHelpers";
import { addFakeOrigin } from "../../ipc/helpers";
import { installDom } from "../dom";
import { renderReviewPanel } from "../renderReviewPanel";
import { cleanupView, getGitStatusFromElement, setupWorkspaceView, waitForAheadStatus, waitForBranchStatus, waitForIdleGitStatus, waitForGitStatusElement, waitForDirtyStatus, waitForCleanStatus, } from "../helpers";
 ...
 */
function triggerGitStatusRefresh(workspaceId) {
...
describeIntegration("GitStatus (UI + ORPC)", () => {
    ...
    test("initial git status shows clean state for fresh workspace", async () => {
        await withSharedWorkspace("anthropic", async ({ env, workspaceId, metadata }) => {
            ...
            try {
                ...
                const statusElement = await waitForGitStatusElement(view.container, workspaceId, 30000);
    ...
    test("git status updates on window focus after file change", async () => {
        await withSharedWorkspace("anthropic", async ({ env, workspaceId, metadata }) => {
            ...
            try {
                await setupWorkspaceView(view, metadata, workspaceId);
                await waitForCleanStatus(view.container, workspaceId, 30000);
                ...
                const dirtyStatus = await waitForDirtyStatus(view.container, workspaceId, 30000);
        ...
        await withSharedWorkspace("anthropic", async ({ env, workspaceId, metadata }) => {
            ...
            try {
                ...
                if (!bashRes.success)
                    return;
                triggerGitStatusRefresh(workspaceId);
                const status = await waitForCleanStatus(view.container, workspaceId, 30000);
                expect(status.dirty).toBe(false);
            }
            finally {
                await cleanupView(view, cleanupDom);
    ...
    test("git status reflects ahead count", async () => {
        await withSharedWorkspace("anthropic", async ({ env, workspaceId, metadata }) => {
            ...
            try {
                ...
                await waitForIdleGitStatus(workspaceId, (s) => s.ahead === 0 && !s.dirty, "ahead === 0, clean, idle", 10000);
                ...
                invalidateGitStatus(workspaceId);
                await waitForAheadStatus(view.container, workspaceId, 2, 30000);
            }
            finally {
                await cleanupView(view, cleanupDom);
            }
    ...
    test("git status detects branch change after external checkout", async () => {
        await withSharedWorkspace("anthropic", async ({ env, workspaceId, metadata }) => {
            ...
            try {
                ...
                await waitForBranchStatus(view.container, workspaceId, newBranch, 30000);
