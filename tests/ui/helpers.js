 ...
 */
export async function waitForToolCallEnd(collector, toolName, timeoutMs = 10000) {
 ...
 */
export function getRefreshIconClass(refreshButton) {
 ...
 */
export async function waitForRefreshButtonIdle(refreshButton, timeoutMs = 60000) {
    await waitFor(() => {
 ...
 */
export async function assertRefreshButtonHasLastRefreshInfo(refreshButton, expectedTrigger, timeoutMs = 5000) {
    await waitFor(() => {
 ...
 */
export function simulateFileModifyingToolEnd(workspaceId) {
 ...
 */
export async function setupWorkspaceView(view, metadata, workspaceId) {
    await view.waitForReady();
    // Expand project tree
    const projectRow = await waitFor(() => {
        const el = view.container.querySelector(`[data-project-path="${metadata.projectPath}"]`);
        if (!el)
            throw new Error("Project not found in sidebar");
        return el;
    }, { timeout: 10000 });
    ...
    const workspaceElement = await waitFor(() => {
        ...
        if (!el)
            throw new Error("Workspace not found in sidebar");
 ...
 */
export async function openProjectCreationView(view, projectPath) {
    await view.waitForReady();
    const projectRow = await waitFor(() => {
        const el = view.container.querySelector(`[data-project-path="${projectPath}"][aria-controls]`);
        if (!el)
            throw new Error("Project not found in sidebar");
        return el;
    }, { timeout: 10000 });
 ...
 */
export async function addProjectViaUI(view, projectPath) {
    await view.waitForReady();
    const existingProjectPaths = new Set(Array.from(view.container.querySelectorAll("[data-project-path]"))
    ...
    const pathInput = await dialogCanvas.findByRole("textbox", {}, { timeout: 10000 });
    const user = userEvent.setup({ document: view.container.ownerDocument });
    await user.clear(pathInput);
    await user.type(pathInput, projectPath);
    const submitButton = await dialogCanvas.findByRole("button", { name: /add project/i }, { timeout: 10000 });
...
export function getWorkspaceDraftIds(projectPath) {
    const parsedDrafts = readPersistedState(WORKSPACE_DRAFTS_BY_PROJECT_KEY, {});
    const draftsForProject = parsedDrafts[projectPath] ?? [];
    return draftsForProject.map((draft) => draft.draftId);
}
export async function waitForLatestDraftId(projectPath, timeoutMs = 5000) {
    return waitFor(() => {
 ...
 */
export async function cleanupView(view, cleanupDom) {
    view.unmount();
    cleanup();
 ...
 */
export function getGitStatusFromElement(element) {
    const statusAttr = element.getAttribute("data-git-status");
    if (!statusAttr)
        return null;
    try {
        return JSON.parse(statusAttr);
    }
    catch {
        return null;
    }
 ...
 */
export async function waitForGitStatusElement(container, workspaceId, timeoutMs = 30000) {
    return waitFor(() => {
        const el = container.querySelector(`[data-workspace-id="${workspaceId}"][data-git-status]`);
        if (!el)
            throw new Error("Git status element not found");
        return el;
    }, { timeout: timeoutMs });
}
/**
 ...
 */
async function waitForGitStatus(container, workspaceId, predicate, description, timeoutMs) {
    let lastStatus = null;
    await waitFor(() => {
        const el = container.querySelector(`[data-workspace-id="${workspaceId}"][data-git-status]`);
        if (!el)
            throw new Error("Git status element not found");
        lastStatus = getGitStatusFromElement(el);
        if (!lastStatus)
            throw new Error("Could not parse git status");
 ...
 */
export function waitForDirtyStatus(container, workspaceId, timeoutMs = 60000) {
 ...
 */
export function waitForCleanStatus(container, workspaceId, timeoutMs = 60000) {
 ...
 */
export function waitForAheadStatus(container, workspaceId, minAhead, timeoutMs = 60000) {
    return waitForGitStatus(container, workspaceId, (s) => (s.ahead ?? 0) >= minAhead, `ahead >= ${minAhead}`, timeoutMs);
}
/**
 * Wait for git status to report a specific branch name.
 */
export function waitForBranchStatus(container, workspaceId, expectedBranch, timeoutMs = 60000) {
    return waitForGitStatus(container, workspaceId, (s) => s.branch === expectedBranch, `branch === "${expectedBranch}"`, timeoutMs);
}
/**
 ...
 */
export function waitForIdleGitStatus(workspaceId, predicate, description, timeoutMs = 60000) {
    ...
    return waitFor(() => {
        ...
        if (!status)
            throw new Error("Git status not yet available");
        if (!predicate(status)) {
