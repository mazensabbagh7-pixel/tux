...
import { RIGHT_SIDEBAR_COLLAPSED_KEY, RIGHT_SIDEBAR_TAB_KEY, RIGHT_SIDEBAR_WIDTH_KEY, getRightSidebarLayoutKey, getTerminalTitlesKey, } from "@/common/constants/storage";
...
describeIntegration("RightSidebar (UI)", () => {
    ...
    test("tab switching updates active tab and persists selection", async () => {
        ...
        try {
            ...
            const sidebar = await waitFor(() => {
                const el = view.container.querySelector('[role="complementary"][aria-label="Workspace insights"]');
                if (!el)
                    throw new Error("RightSidebar not found");
                return el;
            }, { timeout: 10000 });
            ...
            const costsTab = await waitFor(() => {
                ...
                if (!tab)
                    throw new Error("Costs tab not found");
                return tab;
            }, { timeout: 5000 });
            ...
            await waitFor(() => {
                ...
                if (!reviewPanel)
                    throw new Error("Review panel should be visible after tab click");
            });
            ...
            const newTerminalButton = await waitFor(() => {
                ...
                if (!btn)
                    throw new Error("New terminal button not found");
                return btn;
            }, { timeout: 5000 });
            ...
            await waitFor(() => {
                ...
                if (!terminalPanel)
                    throw new Error("Terminal panel should be visible after tab click");
            });
        }
        finally {
    ...
    test("stats tab appears by default and does not steal focus", async () => {
        ...
        try {
            ...
            await waitFor(() => {
                ...
                if (!statsTab)
                    throw new Error("Stats tab not found");
                expect(costsTab.getAttribute("aria-selected")).toBe("true");
    ...
    test("sidebar collapse and expand via button", async () => {
        ...
        try {
            ...
            await waitFor(() => {
                ...
                if (!tablist)
                    throw new Error("Tablist should be visible when expanded");
            });
            // Find and click collapse button
            const collapseButton = await waitFor(() => {
                // The collapse button has aria-label containing "collapse" or "expand"
                const btn = sidebar.querySelector('button[aria-label*="ollapse"]');
                if (!btn)
            ...
            await waitFor(() => {
                const tablist = sidebar.querySelector('[role="tablist"]');
                if (tablist)
                    throw new Error("Tablist should be hidden when collapsed");
            });
            // Re-query sidebar and find expand button (sidebar reference may be stale after collapse)
            const collapsedSidebar = view.container.querySelector('[role="complementary"][aria-label="Workspace insights"]');
            ...
            await waitFor(() => {
                const tablist = sidebar.querySelector('[role="tablist"]');
                if (!tablist)
                    throw new Error("Tablist should be visible after expand");
            });
        }
        finally {
            await cleanupView(view, cleanupDom);
    ...
    test("tab selection persists across workspace navigation", async () => {
        ...
        try {
            ...
            const projectRow = view.container.querySelector(`[data-project-path="${metadata.projectPath}"]`);
            if (projectRow) {
            ...
            await waitFor(() => {
                const sidebar2 = view.container.querySelector('[role="complementary"][aria-label="Workspace insights"]');
                if (!sidebar2)
                    throw new Error("Sidebar not found after navigation");
                const reviewTab = sidebar2.querySelector('[role="tab"][aria-controls*="review"]');
                if (!reviewTab)
                    throw new Error("Review tab not found after navigation");
                if (reviewTab.getAttribute("aria-selected") !== "true") {
    ...
    test("correct tab content is displayed for each tab", async () => {
        ...
        try {
            ...
            await waitFor(() => {
                ...
                if (!costsPanel)
                    throw new Error("Costs panel not found");
            });
    ...
    test("sidebar width persists consistently across all tabs", async () => {
        ...
        try {
            ...
            await waitFor(() => {
                ...
                if (width < 400)
                    throw new Error(`Expected width >= 400, got ${width}`);
            });
    ...
    test("resizing works the same regardless of which tab is active", async () => {
        ...
        try {
            ...
            await waitFor(() => {
                const width = getSidebarWidth();
                if (width === widthAfterReviewResize)
                    throw new Error("Width should have changed");
            });
    ...
    test("Cmd+T opens terminal and selects its tab", async () => {
        ...
        try {
            ...
            const terminalTab = await waitFor(() => {
                const tab = sidebar.querySelector('[role="tab"][aria-controls*="terminal:"]');
                if (!tab)
                    throw new Error("Terminal tab not found after Cmd+T");
                return tab;
            }, { timeout: 10000 });
            await waitFor(() => {
                expect(terminalTab.getAttribute("aria-selected")).toBe("true");
            ...
            const terminalPanel = await waitFor(() => {
                ...
                if (!panel)
                    throw new Error("Terminal panel not visible");
                return panel;
            }, { timeout: 5000 });
...
//# sourceMappingURL=rightSidebar.test.js.map