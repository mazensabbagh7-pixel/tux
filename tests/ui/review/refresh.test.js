...
import { cleanupView, setupWorkspaceView, waitForToolCallEnd, waitForRefreshButtonIdle, assertRefreshButtonHasLastRefreshInfo, simulateFileModifyingToolEnd, } from "../helpers";
...
async function setupReviewPanel(view, metadata, workspaceId) {
    ...
    await view.findAllByText(/No changes found/i, {}, { timeout: 60000 });
    ...
    test("manual refresh updates diff and sets lastRefreshInfo", async () => {
        await withSharedWorkspace("anthropic", async ({ env, workspaceId, metadata }) => {
            ...
            try {
                ...
                await view.findByText(new RegExp(MANUAL_MARKER), {}, { timeout: 60000 });
    ...
    test("simulated file-modifying tool triggers scheduled refresh", async () => {
        await withSharedWorkspace("anthropic", async ({ env, workspaceId, metadata }) => {
            ...
            try {
                ...
                await view.findByText(new RegExp(SIMULATED_MARKER), {}, { timeout: 60000 });
    ...
    test("multiple simulated tool completions are rate-limited with trailing debounce", async () => {
        await withSharedWorkspace("anthropic", async ({ env, workspaceId, metadata }) => {
            ...
            try {
                ...
                await view.findByText(new RegExp(MARKER_1), {}, { timeout: 60000 });
                await view.findByText(new RegExp(MARKER_2), {}, { timeout: 5000 });
                await view.findByText(new RegExp(MARKER_3), {}, { timeout: 5000 });
    ...
    test("refresh runs while panel focused", async () => {
        await withSharedWorkspace("anthropic", async ({ env, workspaceId, metadata }) => {
            ...
            try {
                ...
                await view.findByText(new RegExp(FOCUS_MARKER), {}, { timeout: 60000 });
                // Verify lastRefreshInfo
                await waitForRefreshButtonIdle(refreshButton);
                await assertRefreshButtonHasLastRefreshInfo(refreshButton, "scheduled");
            }
    ...
    test("tool-call-end triggers scheduled refresh", async () => {
        await withSharedWorkspace("anthropic", async ({ env, workspaceId, collector, metadata }) => {
            ...
            try {
                ...
                const FORCE_BASH = [{ regex_match: "bash", action: "require" }];
                const autoRes = await sendMessageWithModel(env, workspaceId, 'Use bash to run: echo ping. Set display_name="ping" and timeout_secs=30. Do not modify files.', HAIKU_MODEL, {
                    agentId: "exec",
                    thinkingLevel: "off",
                ...
                const matches = await view.findAllByText(new RegExp(AUTO_MARKER), {}, { timeout: 60000 });
    ...
    test("refresh button is disabled while composing review note", async () => {
        await withSharedWorkspace("anthropic", async ({ env, workspaceId, metadata }) => {
            ...
            try {
                ...
                await view.findByText(new RegExp(INITIAL_MARKER), {}, { timeout: 30000 });
                ...
                const diffIndicator = await waitFor(() => {
                    ...
                    if (indicators.length === 0)
                        throw new Error("No diff indicators found");
                    const addIndicator = Array.from(indicators).find((el) => el.textContent?.trim().startsWith("+"));
                    if (!addIndicator)
                        throw new Error("No add line indicator found");
                    return addIndicator;
                }, { timeout: 10000 });
                ...
                const textarea = (await view.findByPlaceholderText(/Add a review note/i, {}, { timeout: 10000 }));
                ...
                const submitButton = await view.findByLabelText(/Submit review note/i, {}, { timeout: 5000 });
                ...
                await waitFor(() => {
                    ...
                    if (stillThere)
                        throw new Error("Review note input still visible after submit");
                });
                // Ensure the review was persisted before asserting UI updates.
                await waitFor(() => {
                    const persisted = readPersistedState(getReviewsKey(workspaceId), null);
                    if (!persisted)
                        throw new Error("Review not persisted");
                ...
                await waitFor(() => {
                    const inlineNotes = Array.from(view.container.querySelectorAll("[data-inline-review-note]"));
                    if (inlineNotes.length === 0)
                        throw new Error("No inline review notes rendered");
