...
function findRuntimeButton(container, label) {
    ...
    test("selecting Docker shows image input", async () => {
        ...
        try {
            ...
            await waitFor(() => {
                ...
                if (!btn)
                    throw new Error("Docker button not found");
            ...
            const imageInput = await waitFor(() => {
                const input = view.container.querySelector('input[placeholder="node:20"]');
                if (!input) {
            ...
            await waitFor(() => {
                ...
                if (!input)
                    throw new Error("SSH host input not found");
            }, { timeout: 2000 });
    ...
    test("Docker runtime is disabled for non-git projects", async () => {
        ...
        try {
            ...
            await waitFor(() => {
                ...
                if (!localButton)
                    throw new Error("Local button not found");
