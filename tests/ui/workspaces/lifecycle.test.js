    ...
    test("workspace selection persists after clicking workspace in sidebar", async () => {
        ...
        await withSharedWorkspace("anthropic", async ({ env, workspaceId, metadata }) => {
            ...
            try {
                ...
                await waitFor(() => {
                    const messageArea = view.container.querySelector('[role="log"], [data-testid="chat-input"], textarea');
                    if (!messageArea) {
    ...
    test("archiving the active workspace navigates to the next sibling workspace", async () => {
        ...
        try {
            ...
            await waitFor(() => {
                ...
                if (!el)
                    throw new Error("Second workspace not in sidebar");
            }, { timeout: 5000 });
            // Archive the first workspace via sidebar menu
            const menuButton = await waitFor(() => {
                const btn = view.container.querySelector(`[aria-label="Workspace actions for ${firstDisplayTitle}"]`);
                if (!btn)
                    throw new Error("Workspace actions menu button not found");
                return btn;
            }, { timeout: 5000 });
            ...
            const archiveButton = await waitFor(() => {
                ...
                if (!archiveBtn)
                    throw new Error("Archive button not found in menu");
                return archiveBtn;
            }, { timeout: 5000 });
            ...
            await waitFor(() => {
                ...
                if (wsEl)
                    throw new Error("Archived workspace still in sidebar");
            }, { timeout: 5000 });
            ...
            await waitFor(() => {
                ...
                if (!window.location.pathname.includes(secondWorkspace.id)) {
                    throw new Error(`Expected to navigate to second workspace (${secondWorkspace.id}), ` +
    ...
    test("archiving the only workspace in a project falls back to project page", async () => {
        ...
        await withSharedWorkspace("anthropic", async ({ env, workspaceId, metadata }) => {
            ...
            try {
                ...
                await waitFor(() => {
                    const wsView = view.container.querySelector('[role="log"], [data-testid="chat-input"], textarea');
                    if (!wsView)
                        throw new Error("Not in workspace view");
                }, { timeout: 5000 });
                // Open the workspace actions menu (hamburger) and click archive
                const menuButton = await waitFor(() => {
                    const btn = view.container.querySelector(`[aria-label="Workspace actions for ${displayTitle}"]`);
                    if (!btn)
                        throw new Error("Workspace actions menu button not found");
                    return btn;
                }, { timeout: 5000 });
                ...
                await waitFor(() => {
                    ...
                    if (wsEl)
                        throw new Error("Workspace still in sidebar");
                }, { timeout: 5000 });
                ...
                await waitFor(() => {
                    ...
                    const projectSelected = view.container.querySelector(`[data-project-path="${projectPath}"]`);
...
describeIntegration("Workspace Archive List Reactivity (UI)", () => {
    ...
    test("newly archived workspace appears immediately in archive list after archiving from workspace view", async () => {
        ...
        try {
            ...
            const menuButton = await waitFor(() => {
                const btn = view.container.querySelector(`[aria-label="Workspace actions for ${secondDisplayTitle}"]`);
                if (!btn)
                    throw new Error("Workspace actions menu button not found for second workspace");
                return btn;
            }, { timeout: 5000 });
            fireEvent.click(menuButton);
            // Find and click the archive button inside the menu
            ...
            const expandArchivedButton = await waitFor(() => {
                ...
                const collapse = view.container.querySelector('[aria-label="Collapse archived workspaces"]');
                if (!expand && !collapse) {
                    throw new Error("Archived workspaces toggle not found - navigation may not have completed");
                }
                return expand;
            }, { timeout: 10000 });
            ...
            await waitFor(() => {
                const deleteBtn = view.container.querySelector(`[aria-label="Delete workspace ${secondDisplayTitle}"]`);
                if (!deleteBtn) {
            ...
            const stillInSidebar = view.container.querySelector(`[data-workspace-id="${secondWorkspace.id}"]`);
    ...
    test("clicking delete on archived workspace stays on project page", async () => {
        ...
        try {
            ...
            const deleteButton = await waitFor(() => {
                const btn = view.container.querySelector(`[aria-label="Delete workspace ${displayTitle}"]`);
                if (!btn)
                    throw new Error("Delete button not found in archived list");
                return btn;
            }, { timeout: 5000 });
            // Click delete
            ...
            await waitFor(() => {
                const btn = view.container.querySelector(`[aria-label="Delete workspace ${displayTitle}"]`);
                if (btn)
                    throw new Error("Delete button still present");
            }, { timeout: 5000 });
            ...
            const projectStillVisible = view.container.querySelector(`[data-project-path="${projectPath}"]`);
    ...
    test("clicking delete on archived workspace stays on project page (explicit navigation)", async () => {
        ...
        try {
            ...
            await waitFor(() => {
                const btn = view.container.querySelector(`[aria-label="Delete workspace ${displayTitle}"]`);
                if (btn)
                    throw new Error("Delete button still present - deletion not complete");
            }, { timeout: 5000 });
