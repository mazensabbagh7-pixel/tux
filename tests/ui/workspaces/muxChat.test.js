...
describe("Chat with Mux system workspace (UI)", () => {
    ...
    test("Mux logo navigates back to Chat with Mux", async () => {
        ...
        try {
            ...
            const logoButton = view.container.querySelector('button[aria-label="Open Chat with Mux"]');
            if (!logoButton) {
    ...
    test("Chat with Mux is permanent (no Archive button; Ctrl+N does nothing)", async () => {
        ...
        try {
            ...
            await waitFor(() => {
                expect(view.container.querySelector(`[data-project-path="${systemProjectPath}"]`)).toBeNull();
            }, { timeout: 10000 });
            ...
            expect(view.container.querySelector(`[data-workspace-id="${MUX_HELP_CHAT_WORKSPACE_ID}"]`)).toBeNull();
...
//# sourceMappingURL=muxChat.test.js.map